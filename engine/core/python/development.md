...
with MONOLITH(monolith_home="/absolute/path/to/test-monolith-home") as harness:
    print(harness.run("say hi").final_response)
```

Alternatively export a non-empty `MONOLITH_HOME`. The SDK rejects a launch that would silently use `~/.monolith`.

## Run against Node source

Repository contributors can select either development route; both execute the normal `monolith --profile sdk` launcher:

- Set `MONOLITH_RUNTIME_MODE=node` to use the built Node carrier on system Node `>=22.19`. The build script refreshes this carrier, but distributions never include or auto-select it.
- Set `monolith_bin` to the absolute built `apps/cli/lib/bin.js` path to exercise the checkout's CLI directly. Supply an explicit `monolith_home`, plus `profile` and ordered `patches` as needed.
...
The root `package.json` version is authoritative for both Python distributions. The staging script injects that version into both wheels and pins the SDK to the same `monolith-runtime-bin` version.
...
python scripts/build-python-release.py --package runtime --platform macos-arm64 --runtime-exe dist-exe/monolith-sdk-runtime-macos-arm64 --output-dir dist-python
pip install \
  "dist-python/monolith_sdk-$version-py3-none-any.whl" \
  "dist-python/monolith_runtime_bin-$version-py3-none-macosx_14_0_arm64.whl"
```
