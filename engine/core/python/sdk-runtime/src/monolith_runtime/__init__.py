"""Locate and execute the bundled monolith CLI shipped with the Python SDK runtime.
...
- **exe (production)**: single-file Node executables named
  ``monolith-sdk-runtime-<platform>-<arch>`` for Linux/macOS and an
...
- **node (dev-only)**: the full deploy closure under ``runtime/node/``
  ...
  runtime/node/node_modules/@monolith/cli/lib/bin.js`` on a
...
Both carriers execute the same monolith command grammar. The Python SDK selects the
``sdk`` profile and requires an explicit MONOLITH home; the installed ``monolith``
console command requires ``MONOLITH_HOME`` for the same reason.
"""
...
PACKAGE_METADATA_FILENAME = "monolith-runtime.json"

RUNTIME_MODE_ENV_VAR = "MONOLITH_RUNTIME_MODE"
...
_EXE_ACQUISITION_HINT = (
    ...
    "monolith checkout, or install the matching `monolith-runtime-bin` platform "
...
def bundled_package_dir() -> Path:
    ...
    if not metadata.is_file():
        raise FileNotFoundError(f"monolith-runtime-bin is missing {metadata}")
    return root
...
def bundled_runtime_path() -> Path:
    ...
    path = bundled_package_dir() / "runtime" / f"monolith-sdk-runtime-{tag}{extension}"
    if not path.is_file():
        raise FileNotFoundError(
            f"monolith-runtime-bin is missing the runtime executable at {path}. "
            + _EXE_ACQUISITION_HINT
        )
    ripgrep = (
    ...
    if not ripgrep.is_file():
        raise FileNotFoundError(
            f"monolith-runtime-bin is missing the ripgrep sidecar at {ripgrep}. "
            + _EXE_ACQUISITION_HINT
        )
    if tag.startswith("macos-"):
        ...
        if not helper.is_file():
            raise FileNotFoundError(
                f"monolith-runtime-bin is missing the node-pty spawn helper at {helper}. "
                + _EXE_ACQUISITION_HINT
            )
    return path
...
def resolve_bundled_launch_args(mode: str | None = None) -> tuple[str, ...]:
    ...
    ``MONOLITH_RUNTIME_MODE`` environment variable (``exe`` | ``node``), then
    ...
    raise ValueError(
        f"unsupported MONOLITH runtime mode {selected!r}: expected 'exe' or 'node' "
...
def _current_platform_tag() -> str:
    ...
    if (
        ...
        raise FileNotFoundError(
            "no bundled MONOLITH SDK runtime exists for this platform "
...
def _node_launch_args() -> tuple[str, str]:
    ...
    if not bin_js.is_file():
        raise FileNotFoundError(
            ...
            f"(no {bin_js}); run `scripts/build-exe-for-python-sdk.ts` in a monolith "
...
def main() -> None:
    """Execute the bundled monolith CLI with an explicitly selected MONOLITH home."""
    if not os.environ.get("MONOLITH_HOME", "").strip():
        print(
            "monolith: the Python runtime command requires an explicit MONOLITH_HOME; "
            "it never uses ~/.monolith implicitly",
            file=sys.stderr,
        )
        raise SystemExit(2)
