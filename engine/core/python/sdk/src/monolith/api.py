...
class MONOLITH:
    """Reusable synchronous SDK for running MONOLITH agent turns.
    ...
    def __init__(
        ...
        **kwargs: object,
        ...
        if config is not None and kwargs:
            raise TypeError("pass either MONOLITHConfig or keyword options, not both")
        self.config = config or MONOLITHConfig(**kwargs)
        ...
        self._client = HarnessClient(
            HarnessConfig(
                monolith_bin=self.config.monolith_bin,
                profile=self.config.profile,
                patches=self.config.patches,
                monolith_home=self.config.monolith_home,
                cwd=runtime_cwd,
                env=env,
...
class Session:
    def __init__(self, harness: MONOLITH, session_id: str) -> None:
