...
import monolith_runtime as runtime
import pytest

from monolith_runtime import (
...
def test_runtime_requires_spawn_helper_only_on_macos(
    ...
    runtime_dir.mkdir()
    linux = runtime_dir / "monolith-sdk-runtime-linux-x64"
    linux.touch()
    Path(f"{linux}-rg").touch()
    macos = runtime_dir / "monolith-sdk-runtime-macos-arm64"
    macos.touch()
...
def test_windows_runtime_uses_exe_payload_and_exe_sidecar(
    ...
    runtime_dir = tmp_path / "runtime"
    runtime_dir.mkdir()
    executable = runtime_dir / "monolith-sdk-runtime-win-x64.exe"
    executable.touch()
    (runtime_dir / "monolith-sdk-runtime-win-x64-rg.exe").touch()
    monkeypatch.setattr(runtime, "bundled_package_dir", lambda: tmp_path)
...
def test_runtime_requires_ripgrep_sidecar(
    ...
    runtime_dir = tmp_path / "runtime"
    runtime_dir.mkdir()
    (runtime_dir / "monolith-sdk-runtime-linux-x64").touch()
    monkeypatch.setattr(runtime, "bundled_package_dir", lambda: tmp_path)
    monkeypatch.setattr(runtime, "_current_platform_tag", lambda: "linux-x64")
...
def test_node_mode_runs_the_deployed_monolith_cli(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    bin_js = tmp_path / "runtime" / "node" / "node_modules" / "@monolith" / "monolith" / "lib" / "bin.js"
...
def test_python_monolith_command_requires_explicit_home(
    ...
    assert "explicit MONOLITH_HOME" in capsys.readouterr().err


def test_python_monolith_command_executes_the_bundled_cli(
    monkeypatch: pytest.MonkeyPatch
) -> None:
    called: dict[str, object] = {}
    monkeypatch.setenv("MONOLITH_HOME", "/explicit/home")
    monkeypatch.setattr(runtime, "resolve_bundled_launch_args", lambda: ("/runtime",))
    monkeypatch.setattr(runtime.sys, "argv", ["monolith", "plugin", "--profile", "sdk", "list"])

    def execvpe(file: str, args: tuple[str, ...], env: dict[str, str]) -> None:
        called.update(file=file, args=args, home=env.get("MONOLITH_HOME"))
