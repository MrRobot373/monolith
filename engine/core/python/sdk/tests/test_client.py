...
from monolith import MONOLITH, HarnessClient, HarnessConfig, Notification, RunResult, SdkProtocolError
from monolith.errors import JsonRpcError
...
json.dump({
    ...
    "MONOLITH_CWD": os.environ.get("MONOLITH_CWD"),
    "MONOLITH_SESSION_ROOT": os.environ.get("MONOLITH_SESSION_ROOT"),
    "MONOLITH_CORDIS_CONFIG": os.environ.get("MONOLITH_CORDIS_CONFIG"),
}, open(env_dump, "w"))
...
""".strip()
    ...
    assert dumped_env["MONOLITH_CWD"] is None
    assert dumped_env["MONOLITH_SESSION_ROOT"] is None
    assert dumped_env["MONOLITH_CORDIS_CONFIG"] is None
...
for line in sys.stdin:
    ...
    if msg.get("method") == "initialize":
        json.dump({"process": os.getcwd(), "environment": os.environ.get("MONOLITH_CWD"), "wire": msg["params"]["cwd"]}, open(os.environ["CAPTURE"], "w"))
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-runtime"}}}), flush=True)
...
""".strip()
    ...
    with MONOLITH(_launch_args=(sys.executable, str(script)), cwd=str(tmp_path)) as harness:
...
for line in sys.stdin:
    ...
    method = msg.get("method")
    if method == "initialize":
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-monolith"}}}), flush=True)
    elif method == "session/prompt":
        params = msg.get("params") or {}
...
""".strip()
    ...
    with HarnessClient(_launch_args=(sys.executable, str(script))) as client:
        ...
        assert init.serverInfo.name == "fake-monolith"
...
def test_public_signatures_omit_unsupported_wire_parameters() -> None:
    from monolith import MONOLITHConfig, Session
    ...
    assert "profile" not in inspect.signature(MONOLITH.run).parameters
    assert "profile" not in inspect.signature(Session.run).parameters
    assert "system_prompt" not in MONOLITHConfig.__dataclass_fields__
    assert "max_tokens" in MONOLITHConfig.__dataclass_fields__
    assert "reasoning_effort" in MONOLITHConfig.__dataclass_fields__
    ...
    assert {"monolith_bin", "profile", "patches", "monolith_home"} <= set(
        MONOLITHConfig.__dataclass_fields__
    )
    assert {"monolith_bin", "profile", "patches", "monolith_home"} <= set(
        HarnessConfig.__dataclass_fields__
    )
    assert "initialize_timeout_seconds" in MONOLITHConfig.__dataclass_fields__
    assert "initialize_timeout_seconds" in HarnessConfig.__dataclass_fields__
    assert MONOLITHConfig().initialize_timeout_seconds == 30.0
    ...
    for removed in ("cordis", "session_root", "runtime_bin", "bridge_bin", "launch_args_override"):
        assert removed not in MONOLITHConfig.__dataclass_fields__
...
def _install_fake_bundled_monolith(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Install a fake runtime package that records monolith argv and serves lifecycle calls."""
    runtime = tmp_path / "monolith.py"
    runtime.write_text(
        """
import json
import os
...
json.dump({
    "argv": sys.argv[1:],
    "MONOLITH_HOME": os.environ.get("MONOLITH_HOME"),
    "MONOLITH_CORDIS_CONFIG": os.environ.get("MONOLITH_CORDIS_CONFIG"),
...
""".strip()
    ...
    module_dir = tmp_path / "monolith_runtime"
    module_dir.mkdir()
...
""".strip()
    ...
    monkeypatch.delitem(sys.modules, "monolith_runtime", raising=False)


def test_client_default_launch_uses_bundled_monolith_sdk_profile_and_explicit_home(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    env_dump = tmp_path / "env.json"
    home = tmp_path / "home"
    ...
    patch.write_text("[]\n")
    _install_fake_bundled_monolith(tmp_path, monkeypatch)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("MONOLITH_HOME", str(tmp_path / "ambient-home"))
    monkeypatch.delenv("MONOLITH_CORDIS_CONFIG", raising=False)

    with HarnessClient(HarnessConfig(
        profile="sdk",
        patches=("sdk.patch.yml",),
        monolith_home=str(home),
        env={"ENV_DUMP": str(env_dump), "MONOLITH_HOME": str(tmp_path / "env-home")},
    )) as client:
    ...
    assert json.loads(env_dump.read_text()) == {
        ...
        "MONOLITH_HOME": str(home),
        "MONOLITH_CORDIS_CONFIG": None,
    }


def test_client_accepts_explicit_environment_monolith_home(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    env_dump = tmp_path / "env.json"
    ...
    with HarnessClient(
        HarnessConfig(profile="custom", env={"ENV_DUMP": str(env_dump), "MONOLITH_HOME": str(home)})
    ) as client:
...
def test_client_rejects_an_implicit_default_monolith_home(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _install_fake_bundled_monolith(tmp_path, monkeypatch)
    monkeypatch.delenv("MONOLITH_HOME", raising=False)

    with pytest.raises(ValueError, match="explicit monolith_home or non-empty MONOLITH_HOME"):
...
def test_client_reports_missing_bundled_runtime_dependency(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="Install monolith-runtime-bin"):
        HarnessClient(HarnessConfig(monolith_home="/explicit/home")).start()
