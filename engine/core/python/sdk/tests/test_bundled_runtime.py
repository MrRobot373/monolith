"""Keyless boot tests for the production exe and development monolith carrier.
...
from monolith import MONOLITH, HarnessClient, HarnessConfig
from monolith.errors import JsonRpcError, TransportClosedError
from monolith_runtime import RUNTIME_MODE_ENV_VAR, resolve_bundled_launch_args
...
def _client(tmp_path: Path, mode: str, monkeypatch: pytest.MonkeyPatch, *patches: Path) -> HarnessClient:
    ...
    return HarnessClient(
        HarnessConfig(
            monolith_home=str(tmp_path / "home"),
...
def test_bundled_runtime_boots_the_sdk_profile(
    ...
    assert init.serverInfo.name == "monolith-sdk-runtime"
    profile = json.loads((tmp_path / "home" / "profiles" / "sdk" / "package.json").read_text())
    assert profile["monolith"]["profile"]["bundles"] == [
        "@monolith/base",
        "@monolith/sdk-app",
    ]
...
def test_python_sdk_applies_an_ordered_profile_patch(
    ...
    harness = MONOLITH(
        ...
        monolith_home=str(tmp_path / "home"),
        patches=(str(patch),),
        env={"MONOLITH_PERMISSION_MODE": "danger-full-access"},
...
def test_bundled_runtime_surfaces_unbundled_plugin_failure(
    ...
    patch.write_text(json.dumps([{
        "insert": [{"id": "missing", "name": "@monolith/does-not-exist"}],
    }]))
    ...
    assert "@monolith/does-not-exist" in str(excinfo.value)
