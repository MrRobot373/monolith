"""Drive the repo-source monolith SDK profile through the SDK and a keyless mock SSE server.
...
def run_smoke(repo_root: Path, keep_sessions: bool) -> None:
    monolith_home = Path(tempfile.mkdtemp(prefix="monolith-sdk-smoke-home-"))
    session_root = monolith_home / "sessions"
    ...
    print(f"monolith_home={monolith_home}")
    ...
    if keep_sessions:
        print(f"kept_monolith_home={monolith_home}")
    else:
        shutil.rmtree(monolith_home)
        print("removed temporary monolith home")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Path to the monolith checkout.",
    )
