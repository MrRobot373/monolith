...
Python subprocess SDK for driving MONOLITH over newline-delimited JSON-RPC on stdio. Install `monolith-sdk`; it installs the exact same-version `monolith-runtime-bin` wheel for the current platform.
...
The Python SDK has no separate application entrypoint. It launches the bundled `monolith` CLI with `--profile sdk`; the selected profile owns the JSON-RPC server, agent composition, credentials, persistence, tools, and shutdown behavior.

Every launch requires an explicit MONOLITH home. Pass `monolith_home` or provide a non-empty `MONOLITH_HOME` in the child environment. The SDK deliberately never discovers `~/.monolith`.

```py
from monolith import MONOLITH

with MONOLITH(
    monolith_home="/absolute/path/to/isolated-monolith-home",
...
`MONOLITH` starts lazily and reuses its runtime until `close()` or context-manager exit. The initial profile handshake has an independent 30-second default bound through `initialize_timeout_seconds`; ordinary turns remain unbounded unless `request_timeout_seconds` is set. A timeout names the selected profile and includes retained runtime diagnostics. `cwd` is the agent workspace; `runtime_cwd` independently selects the subprocess working directory. Both become absolute before launch. `provider`, `model`, optional `reasoning_effort`, and optional positive `max_tokens` are sent during JSON-RPC initialization. `base_url` and `api_key` explicitly override `DEEPSEEK_BASE_URL` and `DEEPSEEK_API_KEY` in the child environment.

## Customize plugins

Persistent customization belongs to a `monolith` profile. Initialize the shipped SDK profile and install an external bundle with the runtime wheel's `monolith` command:

```sh
export MONOLITH_HOME=/absolute/path/to/isolated-monolith-home
monolith --profile sdk --dump-default-config >/dev/null
monolith plugin --profile sdk add file:/absolute/path/to/my-plugin-bundle
```

The `file:` form installs the local bundle into the profile package tree, where its peer imports reach the bundled installation fallback. The profile manifest records installed dependencies and ordered bundle layers; its `$MONOLITH_HOME/profiles/sdk/cordis.patch.yml` is the persistent user patch. `monolith plugin` needs `pnpm` only when managing external packages. Running the SDK does not require system Node.js.
...
`profile` may select another existing profile, but that composition must retain `@monolith/sdk-app` or another `@monolith/sdk-jsonrpc-server` row. Misconfiguration fails during CLI boot or SDK initialization; there is no complete-config fallback. `monolith_bin` may select another `monolith` executable while preserving the same profile grammar. Arbitrary argv replacement remains an internal fake-runtime test adapter, not public API.
