...
The public package family belongs to the `@monolith` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@monolith/node-addon-landlock-run
@monolith/node-addon-landlock-run-<platform>
```
