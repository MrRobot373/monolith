/** Recorded-session replay through the shipped headless `monolith` profile. */
...
import { resolvePwshPath } from '@monolith/pwsh-local'
import { parseSessionLog } from '@monolith/llm-replay'

const repoRoot = fileURLToPath(new URL('../../', import.meta.url))
const snapshotsRoot = fileURLToPath(new URL('./', import.meta.url))
const monolithBin = join(repoRoot, 'apps/cli/src/bin.ts')
...
function snapshotMode(value: string | undefined): SnapshotMode {
  switch (value) {
    ...
    default: throw new Error(`unknown MONOLITH_SNAPSHOT mode: ${value}`)
  }
}

const mode = snapshotMode(process.env.MONOLITH_SNAPSHOT)
const RUNTIME_WORKSPACE_ENTRIES = ['.agents', '.monolith', '.snapshot-patches'] as const
...
async function persistedSessions(cwd: string): Promise<SessionLog[]> {
  const root = join(cwd, '.monolith', 'sessions')
...
function stderrFromSession(log: string): string {
  ...
  return `${output}monolith: ${error.code}: ${error.message}\n`
}
...
const workspaceSetups: Record<string, (cwd: string) => Promise<void>> = {
  async 'editing-cordis-skill'(cwd) {
    const target = join(cwd, '.monolith', 'skills', 'editing-cordis-compositions', 'SKILL.md')
...
describe('headless recorded-session snapshots', () => {
  ...
  for (const scenario of scenarios) {
    ...
    scenarioTest(`${mode}s ${scenario.name} through monolith --profile headless`, async () => {
      ...
      try {
        result = await runLoaderSmoke({
          ...
          expectedExitCode: turnReasonFromSession(primaryFixture)?.kind === 'completed'
            ...
            : 1,
          env: {
            MONOLITH_SNAPSHOT: replaying ? 'replay' : 'record',
            MONOLITH_SNAPSHOT_PROVIDER: model.provider,
            MONOLITH_SNAPSHOT_MODEL: model.model,
            MONOLITH_SNAPSHOT_SPILL_ROOT: spillRoot,
            MONOLITH_SNAPSHOT_FILE: join(scenario.dir, 'session.jsonl'),
            ...(replaying && fixtureFiles.length > 1
              ? { MONOLITH_SNAPSHOT_CHILD_FILES: fixtureFiles.slice(1).map(file => join(scenario.dir, file)).join(delimiter) }
              : {}),
            ...
            ...(scenario.manifest.permission === undefined
              ? {}
              : { MONOLITH_PERMISSION_MODE: scenario.manifest.permission }),
