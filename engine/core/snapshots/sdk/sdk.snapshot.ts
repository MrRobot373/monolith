 ...
 * the real `monolith --profile sdk` runtime through
 * `@monolith/sdk-client`, drives one turn over stdio JSON-RPC,
 * and pins the SDK `RunResult`, the complete notification stream, and the
 * persisted session logs. Replay serves recorded model
 * responses via `llm-replay` (`cordis.snapshot.yml`); `MONOLITH_SNAPSHOT=record`
 * re-records against the live API; `MONOLITH_SNAPSHOT=refresh` replays committed
...
const mode = process.env.MONOLITH_SNAPSHOT ?? 'replay'
...
const RUNTIME_WORKSPACE_ENTRIES = [
  ...
  '.child-monolith',
  '.monolith',
  '.monolith-sdk-background-release',
  '.replay-fixtures',
  '.snapshot-patches',
] as const
const monolithSdkDiagnosticChildPatch = fileURLToPath(new URL(
  './subagent-monolith-sdk-diagnostic/child.cordis.yml',
  import.meta.url,
))
const monolithSdkChildConfig = fileURLToPath(new URL(
  '../../packages/subagent/subagent-monolith-sdk/tests/fixtures/loader/child.patch.yml',
  import.meta.url,
))
...
interface SdkAssertions {
  ...
  /** A separate MONOLITH SDK child whose persisted session joins the evidence. */
  monolithSdkChild?: {
...
const SDK_ASSERTIONS: Readonly<Record<string, SdkAssertions>> = {
  'subagent-monolith-sdk-diagnostic': {
    environment: { MONOLITH_TEST_CHILD_PATCH: monolithSdkDiagnosticChildPatch },
  },
  'persistent-tools': {
    environment: { MONOLITH_SYSTEM_PROMPT: MINIMAL_SYSTEM_PROMPT },
    ...
    runtimeContext: {
      includes: ['Current MONOLITH file policy: danger-full-access', 'Approval prompts are disabled in this session'],
      excludes: ['workspace-write'],
    },
  },
  'subagent-monolith-sdk-dynamic-route': {
    environment: { MONOLITH_TEST_PARENT_PROVIDER: 'deepseek-official' },
    monolithSdkChild: {
      config: monolithSdkChildConfig,
      agentConfig: {
        provider: 'mock',
        model: 'mock-routed',
        reasoningEffort: 'max',
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@monolith/system-prompt') return []
...
/** One SDK-controlled recorded scenario against a fresh `monolith --profile sdk` subprocess. */
async function runScenario(scenario: CorpusScenario): Promise<{
  ...
  const monolithHome = join(cwd, '.monolith')
  const sessionsRoot = join(monolithHome, 'sessions')
  ...
  if (assertions.monolithSdkChild !== undefined) {
    const childHome = join(cwd, '.child-monolith')
    const childPatch = materializeProfilePatch(assertions.monolithSdkChild.config, cwd, patchRoot, patches.length)
    await mkdir(childHome, { recursive: true })
    childSessionsRoot = join(childHome, 'sessions')
    childEnvironment = {
      MONOLITH_TEST_CHILD_PATCHES: JSON.stringify([childPatch]),
      MONOLITH_TEST_CHILD_HOME: childHome,
    }
  }
  ...
  const env: Record<string, string> = {
    ...
    MONOLITH_SNAPSHOT: mode,
    MONOLITH_SNAPSHOT_PROVIDER: route.provider,
    MONOLITH_SNAPSHOT_MODEL: route.model,
    MONOLITH_TELEMETRY_DISABLED: '1',
    MONOLITH_AGENTS_HOME: join(cwd, '.agents'),
    NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
    ...parentFixture === undefined ? {} : {
      MONOLITH_SNAPSHOT_FILE: parentFixture,
      ...childFixtures.length > 0 ? { MONOLITH_SNAPSHOT_CHILD_FILES: childFixtures.join(delimiter) } : {},
    },
    ...!recording && scenario.manifest.replay?.override === true
      ? { MONOLITH_SNAPSHOT_OVERRIDE: join(scenario.dir, 'replay.override.json') }
      : {},
...
function orderLogs(logs: PersistedLog[], expectedCount: number, separateMonolithSdkChild: boolean): PersistedLog[] {
  if (separateMonolithSdkChild) {
...
async function verifyHeaders(
  ...
  ordered: readonly PersistedLog[],
  ctx: NormalizeContext,
  monolithSdkChildConfig?: Readonly<Record<string, unknown>>,
): Promise<void> {
  ...
  for (const [logIndex, log] of ordered.entries()) {
    ...
    for (const [index, header] of headers.entries()) {
      ...
      const configured = logIndex === 1 && monolithSdkChildConfig !== undefined
        ? { ...base as JsonObject, config: monolithSdkChildConfig }
        : base
...
describe('TypeScript SDK snapshots over the jsonrpc runtime', () => {
  for (const scenario of sdkScenarios) {
    ...
    scenarioTest(`${mode}s ${scenario.name} through monolith --profile sdk`, async () => {
      ...
      const ordered = orderLogs(
        ...
        assertions.monolithSdkChild !== undefined,
      )
      ...
      await verifyHeaders(scenario, ordered, actualContext, assertions.monolithSdkChild?.agentConfig)
      ...
      if (ordered.length > 1 && assertions.monolithSdkChild === undefined) {
