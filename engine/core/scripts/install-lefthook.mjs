...
const HOOKS_DIRECTORY = 'monolith-hooks'
const OWNERSHIP_MARKER = '.monolith-lefthook-owned'
const OWNERSHIP_MARKER_VERSION = 1
const OWNERSHIP_MARKER_OWNER = 'monolith worktree-local lefthook hooks'
const INSTALL_LOCK = 'monolith-lefthook-install.lock'
const INSTALL_LOCK_TIMEOUT_MS = 30_000
const INSTALL_LOCK_INITIALIZATION_TIMEOUT_MS = 5_000
const INSTALL_LOCK_POLL_MS = 50
const ALLOW_HOOKS_PATH_OVERRIDE = 'MONOLITH_LEFTHOOK_ALLOW_HOOKS_PATH_OVERRIDE'
const REPOSITORY_EXTENSION_PATTERN = '^extensions\\.'
const PAIRING_MERGE_DRIVER_CONFIG = [
  ['merge.monolith-translation-pairing.name', 'MONOLITH bilingual pairing records'],
  [
    'merge.monolith-translation-pairing.driver',
  ...
  while (true) {
    try {
      ...
      try {
        ...
        const writeDelay = Number(process.env.MONOLITH_TEST_LEFTHOOK_LOCK_WRITE_DELAY_MS ?? 0)
        if (writeDelay > 0) {
