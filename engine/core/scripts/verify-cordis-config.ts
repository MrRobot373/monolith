 ...
 * silently changes composition. Shipped and test-only monolith overlays resolve
...
// These overlays are consumed by the built monolith app, so their bare specifiers
...
const CHOOSER_PACKAGE = '@monolith/host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@monolith/host-directory-picker-native',
  '@monolith/host-directory-picker-browse',
  '@monolith/client-ui-directory-picker-browse',
  '@monolith/client-ui-directory-picker-native',
]
const errors: string[] = []
 ...
 * `monolith.client` block, and the node half of a surface plugin is an empty
 ...
 * `monolith.client` declaration disagree.
 */
function validateClientHalvesDeclared(): string[] {
  return globSync('packages/client/*/package.json', { cwd: root }).flatMap((manifestPath) => {
    const manifest = readManifest(manifestPath) as PackageManifest & {
      ...
      monolith?: { client?: unknown }
    }
    const shipsClient = manifest.exports !== undefined && Object.hasOwn(manifest.exports, './client')
    const declaresClient = manifest.monolith?.client !== undefined
    if (shipsClient === declaresClient) return []
    return [shipsClient
      ? `${manifestPath}: exports "./client" but declares no monolith.client, so its browser half is never served`
      : `${manifestPath}: declares monolith.client but exports no "./client" entry to serve`]
  })
}
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if (value.name !== '@monolith/cordis-plugin-include') return
...
function validateAppResolution(): string[] {
  ...
  // monolith app's own dependency surface — the profile module fallback mirrors it.
  ...
  for (const manifestPath of bundleManifests) {
    ...
    const patch = manifest.monolith?.bundle?.patch
...
export function bundleManifestPaths(repoRoot: string = root): string[] {
  return globSync('packages/*/*/package.json', { cwd: repoRoot })
    .filter(path => typeof readManifest(path, repoRoot).monolith?.bundle?.patch === 'string')
 ...
 * the tsconfig `paths` facade to a `.ts`/`.tsx` source file. The `monolith` source
