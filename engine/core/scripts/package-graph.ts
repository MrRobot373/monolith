...
export function collectPackageGraph(root: string, groupOrder: readonly string[], gate: string): PackageGraphNode[] {
  // Vendored framework packages share the workspace scope but are not nodes of
  // the product graph: they are upstream code consumed like any dependency.
  const vendored = new Set(
    globSync('vendor/*/package.json', { cwd: root })
      .map(path => path.split(sep).join('/'))
      .map(rel => (JSON.parse(readFileSync(resolve(root, rel), 'utf8')) as { name: string }).name),
  )
