...
function facts(manifest: PackageDependencyManifest): PackageDependencyFacts {
  return {
    ...
    workspaceNames: new Set([
      ...
      '@monolith/types',
      '@monolith/stale',
      '@monolith/schemastery',
    ]),
    allSourceUses: new Map([
      ['@monolith/runtime', ['packages/core/probe/src/index.ts']],
      ['@monolith/types', ['packages/core/probe/src/types.ts']],
    ]),
    hostRuntimeSourceUses: new Map([
      ['@monolith/runtime', ['packages/core/probe/src/index.ts']],
    ]),
    hostRuntimeExportUses: [{
      packageName: '@monolith/runtime',
      specifier: '@monolith/runtime',
      exportName: 'runtimeValue',
      sourcePath: 'packages/core/probe/src/index.ts',
      line: 1,
      column: 10,
      sourceLine: "import { runtimeValue } from '@monolith/runtime'",
    }],
...
describe('package dependency scope', () => {
  it('keeps the measured Host relay roster explicit', () => {
    ...
    expect(PACKAGE_DEPENDENCY_POLICY.configurationOnlyDevDependencies).toEqual({
      '@monolith/client-locale': ['@monolith/api-remotes'],
      '@monolith/client-ui-conversation': [
        '@monolith/api-remotes',
        '@monolith/client-ui-workspace',
      ],
      '@monolith/client-ui-model-selection': ['@monolith/client-ui-input-trigger'],
      '@monolith/client-ui-sidebar': ['@monolith/client-ui-workspace'],
      '@monolith/client-ui-subagent': ['@monolith/client-ui-input-trigger'],
      '@monolith/client-ui-theme': ['@monolith/api-remotes'],
      '@monolith/client-ui-tool': ['@monolith/api-remotes'],
    })
    expect(PACKAGE_DEPENDENCY_POLICY.duplicateSafePackages).toEqual([
      '@monolith/brand',
      '@monolith/typert-protocol',
      '@monolith/util-crypto',
      '@monolith/util-values',
    ])
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@monolith/deque']).toEqual(['Deque'])
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@monolith/schemastery']).toEqual(['default'])
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@monolith/session/types']).toBeUndefined()
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@monolith/typert-protocol']).toBeUndefined()
    expect(PACKAGE_DEPENDENCY_POLICY.peerRequiredHostExports['@monolith/scope']).toEqual([
      'carrierKeyOf', 'scopeOf', 'scopeTarget',
    ])
    expect(PACKAGE_DEPENDENCY_POLICY.peerRequiredHostExports['@monolith/typert-protocol']).toBeUndefined()
  })

  it('discovers the Client directory, monolith.client declarations, and configured Host packages', () => {
    const packages = [
      pkg('@f/static', 'packages/client/static/package.json'),
      pkg('@f/dynamic-client', 'packages/client/dynamic/package.json', { monolith: { client: {} } }),
      pkg('@f/dual', 'packages/api/dual/package.json', { monolith: { client: {} } }),
      ...
      pkg('@f/excluded', 'packages/api/excluded/package.json', { monolith: { client: {} } }),
  ...
  it('rejects stale, redundant, overlapping, and unknown configuration', () => {
    ...
    expect(found.violations).toEqual(expect.arrayContaining([
      ...
      expect.stringContaining('clientFaceExclude names @f/host, which declares no monolith.client entry'),
...
describe('face-aware source classification', () => {
  it('fails when a managed Host package has no Host entry', () => {
    const root = mkdtempSync(join(tmpdir(), 'monolith-package-missing-host-'))
    roots.push(root)
  ...
  it('counts Host values as dependencies and Client values as development inputs', () => {
    const root = mkdtempSync(join(tmpdir(), 'monolith-package-faces-'))
    roots.push(root)
    const subject = pkg('@f/dual', 'packages/g/dual/package.json', {
      monolith: { client: { inject: ['@f/injected'] } },
    })
    const files = {
...
describe('dependency sections', () => {
  it('does not leak repository configuration into captured dependency facts', () => {
    const manifest: PackageDependencyManifest = {
      name: '@monolith/client-locale',
      dependencies: { '@monolith/runtime': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@monolith/types': 'workspace:^' },
      peerDependencies: { [CORDIS]: 'workspace:^' },
    }
    const base = facts(manifest)
    const subject: PackageDependencyFacts = {
      ...base,
      workspaceNames: new Set([...base.workspaceNames, '@monolith/api-remotes']),
    }
  ...
  it('requires non-workspace Host runtime imports in dependencies', () => {
    const manifest: PackageDependencyManifest = {
      name: '@monolith/probe',
      dependencies: { '@monolith/runtime': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@monolith/types': 'workspace:^', external: '^1.0.0' },
      peerDependencies: { [CORDIS]: 'workspace:^' },
    }
    const subject: PackageDependencyFacts = {
      ...facts(manifest),
  ...
  it('accepts Host dependencies, development-only inputs, and shared Cordis', () => {
    const manifest: PackageDependencyManifest = {
      name: '@monolith/probe',
      dependencies: {
        '@monolith/runtime': 'workspace:^',
        '@monolith/schemastery': 'workspace:^',
        external: '^1.0.0',
      },
      devDependencies: {
        '@monolith/types': 'workspace:^',
        [CORDIS]: 'workspace:^',
      },
      peerDependencies: { [CORDIS]: 'workspace:^' },
    }
  ...
  it('lists managed Host runtime dependencies for fix review', () => {
    const subject = facts({ name: '@monolith/probe' })
    expect(formatManagedRuntimeDependencies({
      ...
      '  @monolith/probe -> @monolith/runtime: @monolith/runtime#runtimeValue',
    ])
  })
  ...
  it('reports an unapproved Host runtime export without rewriting its dependency section', () => {
    ...
    expect(safetyViolations).toEqual([
      'packages/core/probe/src/index.ts:1:10: @monolith/runtime#runtimeValue is not classified as '
      + 'safe or peer-required — import { runtimeValue } from \'@monolith/runtime\'',
    ])
    expect(fixPackageDependencies('/unused', state)).toEqual([])
    expect(manifest.dependencies).toEqual({ '@monolith/runtime': 'workspace:^' })
  })
  ...
  it('keeps an edge as a peer when one imported export requires shared identity', () => {
    ...
    const subject: PackageDependencyFacts = {
      ...facts(manifest),
      peerRequiredHostDependencies: new Set(['@monolith/runtime']),
    }
    expect(collectHostDependencyExportPolicyViolations(
      [subject],
      subject.workspaceNames,
  ...
  it('reports wrong sections, workspace ranges, and stale peer metadata', () => {
    const manifest: PackageDependencyManifest = {
      name: '@monolith/probe',
      dependencies: { '@monolith/types': 'workspace:*' },
      devDependencies: { [CORDIS]: 'workspace:^', '@monolith/runtime': 'workspace:^' },
      peerDependencies: { [CORDIS]: 'workspace:*', '@monolith/runtime': 'workspace:^' },
      peerDependenciesMeta: { '@monolith/missing': { optional: true } },
    }
    const state = {
      facts: [facts(manifest)], packages: [], policyViolations: [], workspaceNames: facts(manifest).workspaceNames,
    }
    ...
    expect(violations).toEqual(expect.arrayContaining([
      expect.stringContaining('@monolith/runtime'),
      expect.stringContaining('@monolith/types'),
      expect.stringContaining(`${CORDIS} must be matching peerDependencies + devDependencies`),
      expect.stringContaining('dependencies.@monolith/types must use workspace:^'),
      expect.stringContaining('peerDependenciesMeta.@monolith/missing has no matching'),
    ]))
  })

  it('repairs owned relationships without changing unrelated dependencies', () => {
    const root = mkdtempSync(join(tmpdir(), 'monolith-package-dependencies-'))
    roots.push(root)
    const manifestPath = 'package.json'
    const manifest: PackageDependencyManifest = {
      name: '@monolith/probe',
      dependencies: { '@monolith/schemastery': 'workspace:*', external: '^1.0.0' },
      devDependencies: { [CORDIS]: 'workspace:^', '@monolith/runtime': 'workspace:^' },
      peerDependencies: {
        [CORDIS]: 'workspace:^',
        '@monolith/runtime': 'workspace:^',
        '@monolith/stale': 'workspace:^',
      },
      peerDependenciesMeta: { '@monolith/stale': { optional: true } },
    }
