...
function checkManifest(
  ...
  if (owner.packageName === '@monolith/invariants') return
  ...
  const peerRange = manifest.peerDependencies?.['@monolith/invariants']
  if (developmentOnlyInvariant ? peerRange !== undefined : peerRange !== expectedRange) {
    addViolation(violations, owner.manifestPath, developmentOnlyInvariant
      ? '@monolith/invariants must not be a peerDependency under this package dependency policy'
      : '@monolith/invariants must be a workspace:^ peerDependency')
  }
  if (manifest.devDependencies?.['@monolith/invariants'] !== expectedRange) {
    addViolation(
      violations,
      owner.manifestPath,
      `@monolith/invariants must be a ${expectedRange} devDependency`,
    )
  }
}
...
function checkBuild(
  ...
  if (hasCompanion
    && owner.packageName !== '@monolith/invariants'
