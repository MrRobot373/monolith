...
function declaration(
  ...
  return {
    name: short.startsWith('@') ? short : '@monolith/client-' + short,
...
describe('source package uses', () => {
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      "import type { A } from '@monolith/a/subpath'",
      "declare module '@monolith/client-ui-slots' {}",
      "const load = () => import('@monolith/b/remote')",
    ...
    expect([...uses].sort()).toEqual([
      '@monolith/a',
      '@monolith/b',
      '@monolith/client-ui-slots',
      'react',
    ])
    expect([...collectRuntimeSourcePackageUses('feature.tsx', [
      "import type { A } from '@monolith/a/subpath'",
      "declare module '@monolith/client-ui-slots' {}",
      "const load = () => import('@monolith/b')",
      'export const view = <div />',
    ].join('\n'))].sort()).toEqual([
      '@monolith/b',
      'react',
...
describe('package modes', () => {
  ...
  it('requires seeded workspace packages to use staticLinked and preloads to name dynamic rows', () => {
    ...
    expect(found.join('\n')).toContain('has no dynamic monolith.client row')
  })
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@monolith/client-bootstrap/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('module requests', () => {
  it('rejects runtime requests from one client feature package to another dynamic row', () => {
    const ui = declaration('ui', {
      external: ['@monolith/client-slots/client'],
      runtimeSourceUses: {
        '@monolith/client-slots': ['packages/client/ui/src/client/index.ts'],
      },
    })
    ...
    expect(collectClientPackageViolations(facts([], { declarations: [ui, slots] }))).toEqual([
      ...
      + '"@monolith/client-slots/client"; import shared types only or call an injected Cordis service',
    ])
  })

  it('rejects stale externals and accepts a runtime import outside client feature packages', () => {
    const gateway = {
      ...declaration('@monolith/api-gateway'), manifest: 'packages/api/gateway/package.json',
    }
    const stale = { ...declaration('@monolith/api-stale', {
      external: ['@monolith/api-gateway/client'],
    }), manifest: 'packages/api/stale/package.json' }
    const live = { ...declaration('@monolith/api-live', {
      external: ['@monolith/api-gateway/client'],
      runtimeSourceUses: {
        '@monolith/api-gateway': ['packages/api/live/src/client/index.ts'],
      },
      runtimeSourceSpecifiers: {
        '@monolith/api-gateway/client': ['packages/api/live/src/client/index.ts'],
      },
    }), manifest: 'packages/api/live/package.json' }
    expect(collectClientPackageViolations(facts([], {
      declarations: [gateway, stale, live],
    }))).toEqual([
      stale.manifest + ': monolith.client.external "@monolith/api-gateway/client"'
  ...
  it('requires the exact external subpath to be imported at runtime', () => {
    ...
    const subject = { ...declaration('@monolith/api-session-controller', {
      external: ['@monolith/api-gateway/client'],
      runtimeSourceUses: {
        '@monolith/api-gateway': ['packages/api/session-controller/src/client/index.ts'],
      },
      runtimeSourceSpecifiers: {
        '@monolith/api-gateway/remote': ['packages/api/session-controller/src/client/index.ts'],
      },
    ...
    expect(collectClientPackageViolations(facts([], { declarations: [gateway, subject] }))).toEqual([
      subject.manifest + ': monolith.client.external "@monolith/api-gateway/client"'
      + ' has no runtime import or re-export in production source; remove the stale declaration',
    ])
  })
  ...
  it('rejects an explicit baseline request', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      ui.manifest + ': monolith.client.external repeats baseline module "react"; remove the explicit declaration',
    ])
  })

  it('rejects duplicates, empty values, self-requests, and missing suppliers', () => {
    const ui = declaration('ui', {
      external: ['', '@monolith/client-ui', '@monolith/missing', '@monolith/missing'],
      inject: ['', '@monolith/a', '@monolith/a'],
    })
    const found = collectClientPackageViolations(facts([], { declarations: [ui] }))
    expect(found).toHaveLength(6)
    expect(found.join('\n')).toContain('monolith.client.external contains an empty value')
    expect(found.join('\n')).toContain('monolith.client.inject contains an empty value')
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = { ...declaration('@monolith/api-a', {
      external: ['@monolith/api-b'],
      inject: ['@monolith/api-b'],
      runtimeSourceUses: { '@monolith/api-b': ['packages/api/a/src/client.ts'] },
      runtimeSourceSpecifiers: { '@monolith/api-b': ['packages/api/a/src/client.ts'] },
    }), manifest: 'packages/api/a/package.json' }
    const b = { ...declaration('@monolith/api-b', {
      external: ['@monolith/api-a'],
      inject: ['@monolith/api-a'],
      runtimeSourceUses: { '@monolith/api-a': ['packages/client/b/src/client.ts'] },
      runtimeSourceSpecifiers: { '@monolith/api-a': ['packages/client/b/src/client.ts'] },
    }), manifest: 'packages/api/b/package.json' }
    const found = collectClientPackageViolations(facts([], { declarations: [a, b] }))
    expect(found).toHaveLength(1)
    expect(found[0]).toContain('synchronous monolith.client.external cycle')
  })
})
...
describe('manifest declarations', () => {
  it('reports malformed arrays without hiding other packages', () => {
    ...
    const files: Record<string, unknown> = {
      'packages/g/a/package.json': {
        name: '@f/a', monolith: { client: { external: 'react', inject: ['@f/b', 1] } },
      },
      'packages/g/b/package.json': { name: '@f/b', monolith: { client: {} } },
    }
    ...
    expect(result.malformed).toEqual([
      'packages/g/a/package.json: @f/a monolith.client.external must be a string array',
      'packages/g/a/package.json: @f/a monolith.client.inject must be a string array',
    ])
  })
  ...
  it('fixes malformed declaration entries without changing dependency sections', () => {
    ...
    const subject = pkg('feature', {
      external: ['', 'react', '@monolith/client-feature', '@monolith/missing'],
      inject: ['', '@monolith/agent', '@monolith/agent'],
      sourceUses: {
        '@monolith/agent': ['packages/client/feature/src/index.ts'],
        '@monolith/client-ui-slots': ['packages/client/feature/src/view.tsx'],
      },
      dependencies: {
        [CORDIS]: 'workspace:^',
        '@monolith/agent': 'workspace:*',
      },
      peerDependencies: {
        '@monolith/client-ui-slots': 'workspace:^',
        '@monolith/cordis-plugin-loader': 'workspace:^',
      },
      devDependencies: {},
    })
    const slots = declaration('ui-slots', { dynamic: false })
    const manifest = {
      name: subject.name,
      monolith: { client: { external: subject.external, inject: subject.inject, platform: 'web' } },
    ...
    const fixed = JSON.parse(readFileSync(join(root, subject.manifest), 'utf8')) as {
      monolith: { client: { external: string[]; inject: string[] } }
      dependencies?: Record<string, string>
      peerDependencies: Record<string, string>
      devDependencies: Record<string, string>
    }
    expect(fixed.monolith.client).toMatchObject({
      external: ['@monolith/missing'],
      inject: ['@monolith/agent'],
    })
