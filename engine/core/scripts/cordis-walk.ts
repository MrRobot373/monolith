...
const MERGE_HEAD = /declare module ['"](?:@monolith\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@monolith/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@monolith/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@monolith/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@monolith/cordis'` Context merge
...
/**
 * Every event name a `declare module '@monolith/cordis'` Events merge declares in one
