...
import {
  ...
  REGION_BEGIN,
  REGION_END,
} from '@monolith/typert-generator'
import type { CordisCatalogPolicy } from '@monolith/typert-generator'
 ...
 * The scan that enforces this list reads EVERY `declare module '@monolith/cordis'`
...
export const SERVICE_WALK_EXEMPTIONS: Record<string, string> = {
  ...
  monolithHomePath: 'not a service: boot-provided root accessor function (typeof monolithHomePath | undefined) for Loader !!js config expressions — packages/boot/app-boot/README.md owns the boot contract',
 ...
 * scan reads EVERY `declare module '@monolith/cordis'` Events merge under
...
export const LINK_MAP: Readonly<Record<string, string>> = {
  ...
  MonolithEnvironment: 'subprocess.md',
...
export function computeOutputs(): [string, string][] {
  ...
  for (const page of pages) {
    ...
    // Only the English page is required. A locale companion receives the same
    // injected region when one is present, so a fork that carries no
    // translations regenerates cleanly instead of reporting a missing pair.
    const zhSide = page.replace(/\.md$/, '.zh.md')
    const sides = existsSync(resolve(root, `${SUBSYSTEMS_DIR}/${zhSide}`)) ? [page, zhSide] : [page]
