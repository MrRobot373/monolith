...
function createFixture(attributes = true): Fixture {
  const root = mkdtempSync(join(tmpdir(), 'monolith-translation-pairing-merge-'))
  fixtures.push(root)
  ...
  if (attributes) write(root, '.gitattributes', '*.i18n.yaml merge=monolith-translation-pairing\n')
  return fixture
}
...
// The value matches MONOLITH_COVERAGE_TEST_TIMEOUT_MS, which the Windows coverage
