...
describe('next package benchmark graph', () => {
  it('applies a source-derived candidate without changing the filesystem', () => {
    ...
    const facts: PackageDependencyFacts = {
      ...
      manifest,
      workspaceNames: new Set(['@monolith/cordis', '@f/probe', '@f/runtime', '@f/types']),
  ...
  it('finds reachable unconfigured packages with non-Cordis peers', () => {
    const index = new Map([
      ['@monolith/cli', new Map([['1.0.0', {
        name: '@monolith/cli', version: '1.0.0', dependencies: { '@f/a': '^1.0.0', '@f/b': '^1.0.0' },
      }]])],
      ['@f/a', new Map([['1.0.0', {
        name: '@f/a', version: '1.0.0', peerDependencies: { '@f/runtime': '^1.0.0' },
      }]])],
      ['@f/b', new Map([['1.0.0', {
        name: '@f/b', version: '1.0.0', peerDependencies: { '@monolith/cordis': '^4.0.0' },
      }]])],
    ...
    expect(discoverBenchmarkCandidates(
      index,
      new Map([['@monolith/cli', '1.0.0'], ['@f/a', '1.0.0'], ['@f/b', '1.0.0']]),
      release,
      new Set(),
    )).toEqual(['@f/a'])
  })
