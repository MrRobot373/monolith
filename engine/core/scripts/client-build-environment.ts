...
const CLIENT_BUILD_ENV_PREFIX = 'MONOLITH_CLIENT_'

/** Non-public selector used by build orchestration to request a named client profile. */
export const CLIENT_BUILD_PROFILE_SELECTOR = 'MONOLITH_BUILD_CLIENT_PROFILE'

/** Public client environment required by official MONOLITH artifacts. */
...
const CLIENT_COMMIT_HASH_VARIABLE = 'MONOLITH_CLIENT_COMMIT_HASH'

/** Public variable carrying the repository package version embedded in client artifacts. */
const CLIENT_VERSION_VARIABLE = 'MONOLITH_CLIENT_VERSION'

/** Repository-relative path of the complete client build record. */
export const CLIENT_BUILD_RECORD_PATH = '.monolith-build/client-build-environment.json'
...
export function repositoryClientBuildEnvironment(
  ...
  delete inherited.MONOLITH_CLIENT_COMMIT_HASH
  delete inherited.MONOLITH_CLIENT_GIT_DIRTY
  delete inherited.MONOLITH_CLIENT_VERSION
  const dirty = repositoryGitDirty(root)
  return {
    ...inherited,
    MONOLITH_CLIENT_COMMIT_HASH: repositoryCommitHash(root, environment),
    ...(dirty === true ? { MONOLITH_CLIENT_GIT_DIRTY: 'true' } : {}),
    MONOLITH_CLIENT_VERSION: repositoryVersion(root),
  }
}
...
export function officialClientBuildEnvironment(
  root: string,
  environment: NodeJS.ProcessEnv = process.env,
): Readonly<Record<`MONOLITH_CLIENT_${string}`, string>> {
  return {
    MONOLITH_CLIENT_COMMIT_HASH: repositoryCommitHash(root, environment),
    MONOLITH_CLIENT_VERSION: repositoryVersion(root),
 ...
 * @returns defined `MONOLITH_CLIENT_*` values only.
 */
...
export function resolveClientBuildEnvironment(
  ...
  if (profile === 'official') {
    ...
    return {
      MONOLITH_CLIENT_COMMIT_HASH: commitHash,
      MONOLITH_CLIENT_VERSION: version,
      ...OFFICIAL_CLIENT_BUILD_ENVIRONMENT,
    }
  }
...
export function assertClientBuildEnvironment(
  ...
  expected: Readonly<Record<`MONOLITH_CLIENT_${string}`, string>>,
): void {
