...
function fixture(packages: Readonly<Record<string, readonly string[]>>): string {
  const root = mkdtempSync(join(tmpdir(), 'monolith-package-graph-'))
  roots.push(root)
...
describe('collectPackageGraph', () => {
  ...
  it('rejects a missing in-repo peer', () => {
    ...
    expect(() => collectPackageGraph(root, ['client'], 'fixture'))
      .toThrow('fixture: @monolith/consumer references missing in-repo peer @monolith/missing')
  })
})
...
describe('renderModuleGraph', () => {
  it('renders the same peer edge in both generated languages', () => {
    const packages = [
      { short: 'provider', name: '@monolith/provider', group: 'core', rel: 'packages/core/provider', deps: [] },
      { short: 'consumer', name: '@monolith/consumer', group: 'core', rel: 'packages/core/consumer', deps: ['provider'] },
    ]
