...
describe('Vitest jsdom compatibility', () => {
  it('provides isolated browser storage instead of Node process storage', () => {
    ...
    localStorage.setItem('monolith-vitest-storage-probe', 'available')

    expect(localStorage.getItem('monolith-vitest-storage-probe')).toBe('available')
    localStorage.removeItem('monolith-vitest-storage-probe')
  })
})
