/**
 * Rescope the vendored Cordis packages into the `@monolith` scope, and undo
 ...
 * `@monolith/tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@monolith/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@monolith/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@monolith/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@monolith/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@monolith/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@monolith/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@monolith/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@monolith/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@monolith/cordis-plugin-logger-console' },
]
...
const POSTCONDITIONS: readonly PostCondition[] = [
  { file: 'vendor/cordis/package.json', text: '"name": "@monolith/cordis"', count: 1 },
  { file: 'vendor/hmr/package.json', text: '"name": "@monolith/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@monolith\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@monolith/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@monolith/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@monolith/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@monolith/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@monolith/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@monolith/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@monolith` rescope**', count: 1 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@monolith\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@monolith/cordis']
    const dev = manifest.devDependencies?.['@monolith/cordis']

    if (!peer) errors.push(\`\${label}: @monolith/cordis must be a peerDependency\`)
    if (!dev) errors.push(\`\${label}: @monolith/cordis must also be a devDependency\`)
    if (peer && dev && peer !== dev) {
      errors.push(\`\${label}: @monolith/cordis peer (\${peer}) and dev (\${dev}) ranges must match\`)`,
    expect: 1,
  },
  {
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@monolith/\')) {',
  ...
  },
  {
    ...
    replace: 'whose lazy `require(\'@monolith/cosmokit\')` can race',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@monolith/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@monolith/` as "another plugin package".
    ...
    replace: `/**
 * Vendored framework libraries: rescoped into @monolith, so the gate below
 ...
 */
const VENDORED_LIBRARY = /^@monolith\\/(cosmokit|schemastery)(\\/|$)/
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@monolith/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@monolith\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.upstream.replace('https://', '')}](\${row.upstream}) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@monolith/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
    ...
    writeFileSync(join(consumerDir, 'package.json'), JSON.stringify({ name: 'monolith-packed-consumer', private: true, type: 'module' }))
