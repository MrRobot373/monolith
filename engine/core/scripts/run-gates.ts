...
async function main(args: string[]): Promise<number> {
  ...
  const concurrencyOverride = process.env.MONOLITH_GATE_CONCURRENCY
  const maxConcurrency = concurrencyFromEnv('MONOLITH_GATE_CONCURRENCY', concurrencyDefault.workers)
  const concurrencySource = concurrencyOverride === undefined || concurrencyOverride === ''
    ? concurrencyDefault.source
    : '$MONOLITH_GATE_CONCURRENCY'
  const failFast = flagEnabled('MONOLITH_GATE_FAIL_FAST')
 ...
 * @param failFast - whether `MONOLITH_GATE_FAIL_FAST` is enabled.
    ...
    case 'check-all':
      return [
        ...
        ...docSyncLeafGates({
          ...
          docTypecheckEnv: { MONOLITH_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
...
function ciSharedStaticGates(): Gate[] {
  return [
    ...
    pnpmScript('monolith-package-licenses', 'verify-monolith-package-licenses', { label: 'MONOLITH package licenses' }),
...
function nodeCompatGates(): Gate[] {
  const typecheck = flagEnabled('MONOLITH_NODE_COMPAT_SKIP_TYPECHECK')
    ? []
...
function nodeCompatSmokeGates(options: { cliSmoke?: boolean } = {}): Gate[] {
  const gates: Gate[] = [
    ...
    pnpmExec('monolith-source-launch-smoke', [
      'vitest',
      'run',
      'apps/cli/tests/source-launch.compat.spec.ts',
    ], { label: 'monolith source-launch smoke' }),
  ...
  if (options.cliSmoke) {
    gates.push(
      pnpmExec('cli-lazy-search-startup-smoke', [
        ...
        env: { MONOLITH_REQUIRE_BUILT_CLI_SMOKE: '1' },
        needs: ['build:web'],
      }),
    )
  }
...
function ciConsumerGates(): Gate[] {
  ...
  return [
    ...
    pnpmScript('doc-typecheck', 'doc-typecheck:contracts-ready', {
      needs: validatedBuild,
      env: { MONOLITH_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
    }),
    pnpmScript('node-next-types', 'verify-node-next-types', {
      label: 'node-next types',
      needs: validatedBuild,
...
function webSnapshotGate(needs: string[], after?: string[]): Gate {
  ...
  const workerRaw = process.env.MONOLITH_WEB_SNAPSHOT_WORKERS
  if (workerRaw !== undefined && workerRaw !== '') {
    ...
    if (!Number.isSafeInteger(workers) || workers < 2 || String(workers) !== workerRaw) {
      throw new Error(`run-gates: MONOLITH_WEB_SNAPSHOT_WORKERS must be an integer greater than 1, got ${JSON.stringify(workerRaw)}.`)
    }
    return pnpmScript('web-snapshot', 'test:web:ci', {
      label: 'web browser snapshot',
      displayCommand: `MONOLITH_SNAPSHOT=replay MONOLITH_WEB_SNAPSHOT_WORKERS=${workers} pnpm run test:web:ci`,
      env: { MONOLITH_SNAPSHOT: 'replay' },
      ...order,
      streamOutput: true,
    })
  }
  return pnpmScript('web-snapshot', 'test:web:built', {
    label: 'web browser snapshot',
    displayCommand: 'MONOLITH_SNAPSHOT=replay pnpm run test:web:built',
    env: { MONOLITH_SNAPSHOT: 'replay' },
    ...order,
  })
}
...
function lintGate(options: { needs?: string[] } = {}): Gate {
  const raw = process.env.MONOLITH_OXLINT_THREADS
  const script = 'lint:contracts-ready'
  return pnpmScript('lint', script, {
    ...raw === undefined || raw === ''
      ? {}
      : { displayCommand: `MONOLITH_OXLINT_THREADS=${raw} pnpm run ${script}` },
...
//
// MONOLITH_COVERAGE_MAX_WORKERS is the ordinary lane's worker budget, so the two
// parallel gates split it instead of each claiming it whole. When
// MONOLITH_COVERAGE_PARTITIONS is set, its single-worker processes replace the
...
// total of one (the serial reference jobs) also set MONOLITH_GATE_CONCURRENCY=1,
// which keeps the gates from overlapping at all.
// MONOLITH_COVERAGE_TEST_TIMEOUT_MS raises Vitest's per-test, expect.poll, and hook
...
function coverageWorkerArgs(): { instrumented: string[]; exempt: string[] } {
  const [flag] = positiveIntArg('MONOLITH_COVERAGE_MAX_WORKERS', '--maxWorkers')
...
function printResult(result: GateResult): void {
  const verbose = process.env.MONOLITH_GATE_VERBOSE === '1'
