...
import LlmRuntime from '@monolith/llm'
import type { ToolSchema } from '@monolith/llm'
...
import SqliteSessionQueryEngine from '@monolith/session-query-sqlite'
import GoalService from '@monolith/goal'
import SystemPrompt from '@monolith/system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@monolith/tools'
import LocalBashExecutor from '@monolith/bash-local'
import * as BashEnvPlugin from '@monolith/shell-env'
import { PwshLocalExecutor } from '@monolith/pwsh-local'
import LocalSubprocessRuntime from '@monolith/subprocess-local'
import LocalFileSystem from '@monolith/fs-local'
import { AttachmentStore } from '@monolith/attachment'
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@monolith/attachment'
import UserQuestionService from '@monolith/user-questions'
import PlanModeController from '@monolith/plan-mode'
import WebRuntime from '@monolith/web'
import * as WebSearchExa from '@monolith/web-search-exa'
import * as WebFetchLocal from '@monolith/web-fetch-http'
import SubagentRuntime from '@monolith/subagent'
import type { SubagentProvider } from '@monolith/subagent'
import * as ToolSubagentControl from '@monolith/tool-subagent-control'
import * as ToolSubagentListAgents from '@monolith/tool-subagent-control/list-agents'
import SkillRegistry from '@monolith/skill'
import * as SkillFileSystem from '@monolith/skill-filesystem'
import LocalJobRegistry from '@monolith/jobs-local'
import * as ToolAskUser from '@monolith/tool-ask-user'
import * as ToolBash from '@monolith/tool-bash'
import * as ToolPwsh from '@monolith/tool-pwsh'
...
import CordisHostRunner from '@monolith/cordis-host-runner'
import * as ToolCordis from '@monolith/tool-cordis'
import * as ToolFs from '@monolith/tool-fs'
import * as ToolFsSearch from '@monolith/tool-fs-search'
import * as ToolStrReplaceEditor from '@monolith/tool-str-replace-editor'
import TerminalSessionService from '@monolith/terminal'
import * as ToolPty from '@monolith/tool-terminal'
import * as ToolGoal from '@monolith/tool-goal'
import * as ToolSchedule from '@monolith/schedule'
import Lsp from '@monolith/lsp'
import * as ToolLsp from '@monolith/tool-lsp'
import * as ToolSkill from '@monolith/tool-skill'
import * as ToolSessionQuery from '@monolith/tool-session-query'
import * as ToolTasks from '@monolith/tool-jobs'
import type TeamService from '@monolith/experimental-agent-team'
import * as ToolTeam from '@monolith/experimental-tool-agent-team'
import * as ToolTodo from '@monolith/tool-todo'
import * as ToolSubagent from '@monolith/tool-subagent'
import { registerListSubagentModels } from '../packages/subagent/tool-subagent/src/list-models.ts'
import * as ToolWeb from '@monolith/tool-web'
import VmWorkflowEngine from '@monolith/workflow-worker-thread'
import * as ToolRalph from '@monolith/tool-ralph'
import * as ToolWorkflow from '@monolith/tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@monolith/tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@monolith/tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@monolith/pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `MONOLITH_*` environment comes from `@monolith/shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@monolith/tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@monolith/cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or MONOLITH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@monolith/tool-bash-persistent',
  ...
  {
    pkg: '@monolith/tool-pwsh-persistent',
  ...
  {
    pkg: '@monolith/tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@monolith/fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. The image tool is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@monolith/tool-fs-search',
    dir: 'tool-fs-search',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@monolith/lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@monolith/tool-ralph',
    dir: 'tool-ralph',
  ...
  },
  {
    ...
    async mount(ctx) {
      ...
      await ctx.plugin(SkillFileSystem, {
        monolithHome: resolve(root, '.tmp/tool-catalog/.monolith'),
  ...
  {
    pkg: '@monolith/tool-session-query',
  ...
  {
    pkg: '@monolith/tool-subagent-control',
  ...
  {
    pkg: '@monolith/experimental-tool-agent-team',
    dir: 'tool-agent-team',
    ...
    scope: ctx => catalogChildScopes.get(ctx) as Agent,
    note:
      'All nine tools are scoped to implicit Team Leads and durable teammates. The shipped monolith-base bundle keeps the package disabled; the documented Agent Teams profile patch enables it while disabling the legacy continuable-child control names.',
  },
  {
    pkg: '@monolith/tool-todo',
    dir: 'tool-todo',
