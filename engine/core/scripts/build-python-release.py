...
SDK_DISTRIBUTION = "monolith-sdk"
RUNTIME_DISTRIBUTION = "monolith-runtime-bin"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="monolith-python-release-") as temporary:
        staging = Path(temporary) / args.package
        if args.package == "sdk":
            stage_sdk(staging, wheel_version)
            environment = None
            expected = output_dir / f"monolith_sdk-{wheel_version}-py3-none-any.whl"
        else:
            ...
            environment = {"MONOLITH_RUNTIME_PLATFORM_TAG": platform_tag}
            expected = output_dir / f"monolith_runtime_bin-{wheel_version}-py3-none-{platform_tag}.whl"
...
def stage_sdk(destination: Path, version: str) -> None:
    ...
    text, count = re.subn(
        r'"monolith-runtime-bin==[^"]+"',
        f'"monolith-runtime-bin=={version}"',
        pyproject.read_text(),
        count=1,
    )
    if count != 1:
...
def stage_runtime(destination: Path, version: str, executable: Path, executable_name: str) -> None:
    ...
    runtime_dir = destination / "src" / "monolith_runtime" / "runtime"
...
def verify_wheel(
    ...
    with zipfile.ZipFile(wheel) as archive:
        ...
        runtime_files = [
            name for name in archive.namelist() if "/runtime/monolith-sdk-runtime-" in name
        ]
        if package == "runtime":
