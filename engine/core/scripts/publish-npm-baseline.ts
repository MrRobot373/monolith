...
const RELEASE_ENTRY_PACKAGE = '@monolith/cli'
...
while time.monotonic() < deadline:
    ...
    if not termination_sent and b"monolith web: http://127.0.0.1:" in snapshot:
        ready_seen = True
...
if not ready_seen:
    sys.stderr.write("installed monolith web did not reach its ready URL\n")
    sys.exit(124)
actual_exit = os.waitstatus_to_exitcode(status)
if actual_exit != 0:
    sys.stderr.write(f"installed monolith web exited {actual_exit}, expected 0\n")
    sys.exit(125)
`
...
class DetachedWorktree {
  ...
  static create(repositoryRoot: string, commit: string, runner: CommandRunner): DetachedWorktree {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'monolith-npm-baseline-'))
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@monolith/')) {
        throw new Error(`${manifestPath} must name an @monolith package`)
      }
      if (name === '@monolith/root') {
...
/** Installs one complete bundle outside the workspace and probes the shipped monolith entry. */
class InstalledBundleSmoke {
  ...
  run(): void {
    const consumerRoot = mkdtempSync(join(tmpdir(), 'monolith-npm-consumer-'))
    try {
      ...
      writeFileSync(resolve(consumerRoot, 'package.json'), `${JSON.stringify({
        name: 'monolith-npm-baseline-consumer',
        version: '0.0.0',
        private: true,
        dependencies,
      }, null, 2)}\n`)
      ...
      const bin = resolve(consumerRoot, 'node_modules/@monolith/cli/lib/bin.js')
      assertPathWithin(consumerRoot, bin, 'installed monolith bin')
      ...
      if (version !== this.bundle.manifest.version) {
        throw new Error(
          `installed monolith --version returned ${JSON.stringify(version)}; `
          + `expected ${this.bundle.manifest.version}`,
        )
      }
      this.probeWeb(bin, consumerRoot, environment)
      console.log('publish-npm-baseline: installed monolith entry and Web startup probes passed')
    } finally {
  ...
  private probeWeb(bin: string, consumerRoot: string, environment: NodeJS.ProcessEnv): void {
    if (process.platform === 'win32') {
      throw new Error('installed monolith Web probe requires a POSIX host with python3')
    }
    const result = this.runner.result(
      'python3',
    ...
    if (result.status !== 0) {
      throw commandFailure('python3', ['installed-monolith-web-probe'], result)
    }
  }
}
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@monolith/') || name === '@monolith/root')) {
...
function installedArtifactEnvironment(consumerRoot: string): NodeJS.ProcessEnv {
  ...
  environment.MONOLITH_HOME = resolve(consumerRoot, '.monolith')
  environment.MONOLITH_AGENTS_HOME = resolve(consumerRoot, '.agents')
  environment.MONOLITH_TELEMETRY_DISABLED = '1'
