...
describe('verify-cordis-config metadata expressions', () => {
  it('accepts a disabled !!js expression', () => {
    const problems = metadataExpressionErrors(
      { id: 'tool-bash', name: '@monolith/tool-bash', disabled: { __jsExpr: "process.platform === 'win32'" } },
      '[0]',
    )
...
describe('workspace Bundle discovery and product dependency closures', () => {
  it('discovers a Bundle outside packages/bundle from its manifest declaration', () => {
    const fixture = mkdtempSync(join(tmpdir(), 'monolith-bundle-discovery-'))
    try {
      ...
      writeFileSync(join(bundleDir, 'package.json'), JSON.stringify({
        name: '@monolith/subagent-example',
  ...
  it('allows a Bundle to mount itself but rejects an undeclared plugin package', () => {
    ...
    const self = { file, name: '@monolith/subagent-example' }
    ...
    expect(bundlePluginDependencyErrors(manifestPath, manifest, [
      self,
      { file, name: '@monolith/missing-plugin' },
    ])).toEqual([
      `${file}: @monolith/missing-plugin must be declared in ${manifestPath} dependencies`,
    ])
  })
})
...
describe('package-owned Loader test dependency closures', () => {
  it('requires package test configs to declare each named plugin they load', () => {
    ...
    const manifest = {
      ...
      dependencies: {},
      devDependencies: {
        '@monolith/declared': 'workspace:^',
      },
    }
    expect(packageTestPluginDependencyErrors(manifestPath, manifest, [
      { file, name: '@monolith/owner' },
      { file, name: '@monolith/declared' },
      { file, name: '@monolith/missing' },
    ])).toEqual([
      `${file}: @monolith/missing must be declared in ${manifestPath} dependencies or devDependencies`,
    ])
  })

  it('requires executable package test fixtures to declare their bare imports', () => {
    const fixture = mkdtempSync(join(tmpdir(), 'monolith-package-test-entrypoint-'))
    try {
      ...
      writeFileSync(join(driverDir, 'driver.ts'), [
        "import '@monolith/owner'",
        "import '@monolith/declared'",
        "import '@monolith/missing'",
      ].join('\n'))
      writeFileSync(join(driverDir, 'cordis.yml'), '[]\n')
      writeFileSync(join(driverDir, 'fixture.mjs'), "import '@monolith/declared'\n")
      const unrelatedDir = join(packageDir, 'tests/fixtures/unrelated')
      mkdirSync(unrelatedDir, { recursive: true })
      writeFileSync(join(unrelatedDir, 'driver.ts'), "import '@monolith/unrelated'\n")

      expect(packageTestFixtureDependencyErrors(fixture)).toEqual([
        'packages/example/owner/tests/fixtures/loader/driver.ts: '
        + '@monolith/missing must be declared in '
  ...
  it('fails loud when package-owned Loader fixtures disappear from the scan', () => {
    const fixture = mkdtempSync(join(tmpdir(), 'monolith-empty-package-test-entrypoint-'))
    try {
      expect(packageTestFixtureDependencyErrors(fixture)).toEqual([
