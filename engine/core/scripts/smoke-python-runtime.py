...
if TYPE_CHECKING:
    from monolith import RunResult
...
PROFILE_PLUGIN_PROMPT = "Verify the Python-installed monolith profile plugin."
PROFILE_PLUGIN_TEXT = "profile plugin smoke ok"
PROFILE_PLUGIN_MARKER = "PYTHON_INSTALLED_MONOLITH_PROFILE_PLUGIN"
IS_WINDOWS = sys.platform == "win32"
MINIMAL_SHELL_TOOL = "pwsh" if IS_WINDOWS else "bash"
MINIMAL_SHELL_COMMAND = (
    "$global:monolithSdkCounter = [int]$global:monolithSdkCounter + 1; "
    'Write-Output "COUNT=$global:monolithSdkCounter CWD=$((Get-Location).Path)"; '
    "if ($global:monolithSdkCounter -eq 1) { Set-Location $env:TEMP }"
    if IS_WINDOWS
    else (
...
def write_profile_patch(
    ...
    """Write one JSON-form monolith profile patch with deterministic persistence."""
    path = root / name
...
def write_advanced_profile_patch(root: Path, name: str, sessions: Path) -> Path:
    ...
    return write_profile_patch(root, name, sessions, [
        ...
        *({"id": row_id, "disabled": True} for row_id in LEGACY_CUSTOM_DISABLED_ROWS),
        ...
        {"insert": [
            {"id": "code-runtime", "name": "@monolith/code-runtime-worker-thread"},
            {"id": "cordis-host-runner", "name": "@monolith/cordis-host-runner"},
            {"id": "cordis-tool", "name": "@monolith/tool-cordis"},
        ]},
    ])
...
def assert_installed_wheel_environment() -> Path:
    ...
    if os.environ.get("MONOLITH_RUNTIME_MODE"):
        raise AssertionError("installed-wheel smoke requires MONOLITH_RUNTIME_MODE to be unset")
    ...
    sdk_version = importlib.metadata.version("monolith-sdk")
    runtime_version = importlib.metadata.version("monolith-runtime-bin")
    if sdk_version != runtime_version:
        raise AssertionError(
            f"installed SDK/runtime versions differ: {sdk_version} != {runtime_version}"
        )
    expected_runtime_requirement = f"monolith-runtime-bin=={sdk_version}"
    requirements = importlib.metadata.requires("monolith-sdk") or []
    ...
    for name in ("monolith", "monolith_runtime"):
    ...
    runtime_module = sys.modules["monolith_runtime"]
    executable = runtime_module.bundled_runtime_path().resolve()
    runtime_package = imported["monolith_runtime"].parent
    ...
    runtime_files = importlib.metadata.files("monolith-runtime-bin") or []
...
def smoke_sdk_live() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="monolith-sdk-live-") as temporary:
        root = Path(temporary).resolve()
        monolith_home = root / "home"
        sessions = monolith_home / "sessions"
...
def smoke_sdk_default(base_url: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="monolith-sdk-default-") as temporary:
        root = Path(temporary).resolve()
        monolith_home = root / "home"
        sessions = monolith_home / "sessions"
        with MONOLITH(
...
def smoke_sdk_custom(base_url: str, executable: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="monolith-sdk-custom-") as temporary:
        root = Path(temporary).resolve()
        monolith_home = root / "home"
        sessions = monolith_home / "sessions"
...
def smoke_sdk_minimal(base_url: str, executable: Path, update_snapshots: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="monolith-sdk-minimal-") as temporary:
        root = Path(temporary).resolve()
...
def smoke_sdk_fs_search(base_url: str, executable: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="monolith-sdk-fs-search-") as temporary:
        root = Path(temporary).resolve()
...
def smoke_sdk_spawn_node(base_url: str, executable: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="monolith-sdk-spawn-node-") as temporary:
        root = Path(temporary).resolve()
        monolith_home = root / "home"
        sessions = monolith_home / "sessions"
...
def smoke_sdk_mcp(base_url: str, executable: Path | None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="monolith-sdk-mcp-") as temporary:
        root = Path(temporary).resolve()
        monolith_home = root / "home"
        sessions = monolith_home / "sessions"
        ...
        with MONOLITH(
            ...
            model="smoke-model",
            cwd=str(root),
            monolith_bin=None if executable is None else str(executable),
            monolith_home=str(monolith_home),
            patches=(str(patch),),
            env={
                "MONOLITH_PERMISSION_MODE": "danger-full-access",
...
def smoke_sdk_profile_plugin(base_url: str) -> None:
    """Install an external bundle through Python's monolith command and load it in the SDK."""
    from monolith import MONOLITH

    with tempfile.TemporaryDirectory(prefix="monolith-sdk-profile-plugin-") as temporary:
        root = Path(temporary).resolve()
        monolith_home = root / "home"
        plugin = root / "plugin"
        plugin.mkdir()
        (plugin / "package.json").write_text(json.dumps({
            "name": "monolith-python-blackbox-plugin",
            "version": "1.0.0",
            "private": True,
            "type": "module",
            "exports": "./index.js",
            "peerDependencies": {"@monolith/cordis": "*"},
        ...
        (plugin / "cordis.patch.yml").write_text(json.dumps([{
            "insert": [{"id": "python-sdk-blackbox-plugin", "name": "monolith-python-blackbox-plugin"}],
        }], indent=2))

        monolith = Path(sysconfig.get_path("scripts")) / ("monolith.exe" if IS_WINDOWS else "monolith")
        environment = {**os.environ, "MONOLITH_HOME": str(monolith_home)}
        installed = subprocess.run(
            [str(monolith), "plugin", "--profile", "sdk", "add", f"file:{plugin}"],
            cwd=root,
            env=environment,
            text=True,
            capture_output=True,
        ...
        if installed.returncode != 0:
            raise AssertionError(
                f"Python-installed monolith could not add the external profile plugin: "
                f"stdout={installed.stdout!r} stderr={installed.stderr!r}"
            )
        manifest = json.loads((monolith_home / "profiles" / "sdk" / "package.json").read_text())
        if "monolith-python-blackbox-plugin" not in manifest.get("dependencies", {}):
            raise AssertionError(f"monolith plugin did not record the external dependency: {manifest}")
        if "monolith-python-blackbox-plugin" not in manifest["monolith"]["profile"]["bundles"]:
            raise AssertionError(f"monolith plugin did not activate the external bundle: {manifest}")
        ...
        assert_zstd_session_log(monolith_home / "sessions")
...
def smoke_sdk_snapshot(base_url: str, executable: Path, update_snapshots: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="monolith-sdk-snapshot-") as temporary:
        root = Path(temporary).resolve()
        monolith_home = root / "home"
        sessions = monolith_home / "sessions"
...
def smoke_sdk_restart_snapshot(base_url: str, executable: Path, update_snapshots: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="monolith-sdk-restart-") as temporary:
        root = Path(temporary).resolve()
        monolith_home = root / "home"
        sessions = monolith_home / "sessions"
...
def smoke_direct(base_url: str, executable: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="monolith-direct-") as temporary:
        root = Path(temporary).resolve()
        monolith_home = root / "home"
        sessions = monolith_home / "sessions"
