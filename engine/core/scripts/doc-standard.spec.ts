 ...
 * stays valid, the consolidated `monolith-doc` skill carries no stale copied
...
/**
 * The kind system: each label maps to exactly one template in the monolith-doc
...
const KIND_TEMPLATES: Readonly<Record<string, string>> = {
  'package-group': '.agents/skills/monolith-doc/templates/package-group.md',
  'package-reference': '.agents/skills/monolith-doc/templates/package-reference.md',
  'package-library': '.agents/skills/monolith-doc/templates/package-library.md',
  'package-bundle': '.agents/skills/monolith-doc/templates/package-bundle.md',
}
 ...
 * bundle (`monolith.bundle.patch`). Each entry names why the package is a
...
/** Whether the package manifest declares `monolith.bundle.patch`. */
function declaresBundle(dir: string): boolean {
  const manifest = resolve(root, dir, 'package.json')
  if (!existsSync(manifest)) return false
  const metadata = JSON.parse(readFileSync(manifest, 'utf8')) as { monolith?: { bundle?: { patch?: string } } }
  return metadata.monolith?.bundle?.patch !== undefined
}
...
describe('monolith-doc skill consolidation', () => {
  it('carries no prototype-era language', () => {
    const files = [
      '.agents/skills/monolith-doc/SKILL.md',
      '.agents/skills/monolith-doc/references/metadata-links-i18n.md',
      '.agents/skills/monolith-doc/references/structure-hierarchy.md',
      '.agents/skills/monolith-doc/references/style.md',
      '.agents/skills/monolith-doc/references/review.md',
      '.agents/skills/monolith-doc/references/website-sync.md',
    ]
  ...
  it('copies no stale website sidebar or section-owner values', () => {
    const source = readFileSync(resolve(root, '.agents/skills/monolith-doc/references/website-sync.md'), 'utf8')
  ...
  it('keeps the reference example linked from the skill', () => {
    const skill = readFileSync(resolve(root, '.agents/skills/monolith-doc/SKILL.md'), 'utf8')
  ...
  it('defines controlled English as a precision-preserving review discipline', () => {
    const skill = readFileSync(resolve(root, '.agents/skills/monolith-doc/SKILL.md'), 'utf8')
    const style = readFileSync(resolve(root, '.agents/skills/monolith-doc/references/style.md'), 'utf8')
  ...
  it('maps every kind label to exactly one skill template that exists', () => {
    const templateFiles = globSync('.agents/skills/monolith-doc/templates/*.md', { cwd: root }).map(path => path.split(sep).join('/')).sort()
