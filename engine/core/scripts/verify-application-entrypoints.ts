/**
 * Enforce monolith profiles as the only supported Node application launcher.
...
interface DemoPolicy {
  readonly kind: 'monolith-direct' | 'monolith-wrapper'
...
const MANIFEST_BIN_ALLOWLIST = new Map<string, ManifestBin>([
  ['apps/cli/package.json', { monolith: 'lib/bin.js' }],
  ['packages/experimental/webworker-packer/package.json', { 'monolith-pack-vfs-image': './bin.js' }],
])
...
const EXECUTABLE_SOURCE_ALLOWLIST = new Map<string, string>([
  ['apps/cli/src/bin.ts', 'supported monolith application launcher'],
  ...
  ['packages/subagent/subagent-monolith-sdk/tests/fixtures/loader/driver.ts', 'test-only subprocess driver'],
...
/** Root demos are application wrappers and therefore must visibly select monolith. */
const ROOT_DEMO_POLICIES = new Map<string, DemoPolicy>([
  ['demo:ptc', { kind: 'monolith-wrapper', wrapper: 'scripts/demo-ptc.mjs' }],
  ['demo:inspector', { kind: 'monolith-direct' }],
])
...
function manifestBinViolations(root: string): string[] {
  ...
  for (const rawPath of manifests) {
    ...
    if (expected === undefined) {
      failures.push(`${path}: package bin bypasses the monolith launcher; applications use apps/cli profiles`)
      continue
    }
...
function referencesMonolithCli(source: string): boolean {
...
function rootDemoViolations(root: string): string[] {
  ...
  for (const [name, commandValue] of Object.entries(manifest.scripts ?? {}).sort(([left], [right]) => left.localeCompare(right))) {
    ...
    if (policy === undefined) {
      failures.push(`package.json scripts.${name}: demo launcher has no explicit monolith or in-process classification`)
      continue
    }
    if (policy.kind === 'monolith-direct') {
      if (!referencesMonolithCli(command)) failures.push(`package.json scripts.${name}: application demo must launch apps/cli/src/bin.ts`)
    ...
    if (!referencesMonolithCli(source)) failures.push(`${wrapper}: application demo wrapper must launch apps/cli/src/bin.ts`)
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (failures.length > 0) {
    ...
    console.log('verify-application-entrypoints: monolith is the only supported Node application launcher.')
  }
}
