...
function workspaceFixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'monolith-project-reference-faces-'))
  roots.push(root)
  ...
  writeJson(join(shared, 'package.json'), { name: '@monolith/shared' })
  writeJson(join(shared, 'tsconfig.json'), {
    extends: '../../../tsconfig.base.json',
    references: [],
  })
  writeJson(join(split, 'package.json'), { name: '@monolith/split' })
...
describe('Project Reference compiler faces', () => {
  ...
  it('uses the referencing project face throughout the reachable graph', () => {
    ...
    writeJson(join(hostConsumer, 'package.json'), { name: '@monolith/host-consumer' })
    ...
    writeJson(join(clientConsumer, 'package.json'), { name: '@monolith/client-consumer' })
