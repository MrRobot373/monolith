...
describe('npm resolution benchmark', () => {
  ...
  it('combines installed metadata with current publishable workspace fields', () => {
    const root = mkdtempSync(join(tmpdir(), 'monolith-npm-registry-index-'))
    roots.push(root)
    ...
    writeJson(root, 'apps/cli/package.json', {
      name: '@monolith/cli',
      version: '0.1.0',
      dependencies: { '@monolith/child': 'workspace:^', external: '^2.0.0' },
    ...
    expect(index.get('@monolith/cli')?.get('0.1.0')).toEqual({
      name: '@monolith/cli',
      version: '0.1.0',
      dependencies: { '@monolith/child': '^0.1.0', external: '^2.0.0' },
    })
  })
  ...
  it('runs npm against the local registry without requesting an archive', async () => {
    const index: RegistryIndex = new Map([[
      '@monolith/cli',
      new Map([['0.1.0', { name: '@monolith/cli', version: '0.1.0' }]]),
    ]])
  ...
  it('returns npm placement for two aliased package versions without requesting archives', async () => {
    const index: RegistryIndex = new Map([[
      '@monolith/cli',
      new Map([
        ['0.1.0', { name: '@monolith/cli', version: '0.1.0' }],
        ['0.2.0', { name: '@monolith/cli', version: '0.2.0' }],
      ]),
    ]])

    const result = await resolveNpmPackageLock(index, {
      '@monolith/cli': '0.2.0',
      'monolith-previous': 'npm:@monolith/cli@0.1.0',
    }, 10_000)

    expect(result.archiveRequests).toBe(0)
    expect(result.packageLock.packages['node_modules/@monolith/cli']?.version).toBe('0.2.0')
    expect(result.packageLock.packages['node_modules/monolith-previous']).toMatchObject({
      name: '@monolith/cli',
      version: '0.1.0',
    })
  })
  ...
  it('isolates peer resolution from inherited npm configuration', async () => {
    const root = mkdtempSync(join(tmpdir(), 'monolith-hostile-npm-config-'))
    roots.push(root)
    const userConfig = join(root, 'user.npmrc')
    writeFileSync(userConfig, '@monolith:registry=http://127.0.0.1:1/\nlegacy-peer-deps=true\nomit=peer\n')
    const previous = {
    ...
    try {
      const index: RegistryIndex = new Map([
        ['@monolith/cli', new Map([['0.1.0', {
          name: '@monolith/cli',
          version: '0.1.0',
          peerDependencies: { '@monolith/peer': '1.0.0' },
        }]])],
        ['@monolith/peer', new Map([['1.0.0', {
          name: '@monolith/peer',
          version: '1.0.0',
        }]])],
      ])

      const result = await resolveNpmPackageLock(index, { '@monolith/cli': '0.1.0' }, 10_000)

      expect(result.archiveRequests).toBe(0)
      expect(result.packageLock.packages['node_modules/@monolith/peer']?.version).toBe('1.0.0')
    } finally {
