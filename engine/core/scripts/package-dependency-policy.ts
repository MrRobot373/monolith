...
/** Packages treated as Client/Host packages without declaring `monolith.client`. */
const CLIENT_FACE_INCLUDE: readonly string[] = []

/** Packages exempted from automatic Client/Host treatment despite declaring `monolith.client`. */
...
const SAFE_HOST_DEPENDENCY_EXPORTS = {
  '@monolith/credentials': ['credentialKey'],
  '@monolith/deque': ['Deque'],
  '@monolith/llm': ['callConfigEquals'],
  '@monolith/timeout': ['MAX_TIMER_DELAY_MS'],
  '@monolith/schemastery': ['default'],
} as const satisfies HostDependencyExports

/** Runtime exports that require every consumer to resolve the provider's shared peer instance. */
const PEER_REQUIRED_HOST_EXPORTS = {
  '@monolith/scope': ['carrierKeyOf', 'scopeOf', 'scopeTarget'],
  '@monolith/session-persistence': ['SessionPersistenceNotFoundError'],
} as const satisfies HostDependencyExports
...
export function hasClientDeclaration(monolithField: unknown): boolean {
  return isRecord(monolithField) && Object.hasOwn(monolithField, 'client')
}
...
export function usesFlattenedPackageDependencies(
  ...
  const included = hasClientDeclaration(monolithField) || policy.clientFaceInclude.includes(packageName)
