...
it('samples one local environment at startup without validating watcher outputs', async () => {
  const root = await mkdtemp(join(tmpdir(), 'monolith-dev-web-environment-'))
  try {
    ...
    writeClientBuildRecord(root, {
      ...
      MONOLITH_CLIENT_COMMIT_HASH: 'fffffff',
      MONOLITH_CLIENT_TITLE: 'MONOLITH',
      MONOLITH_CLIENT_VERSION: '1.2.2',
    })
    ...
    expect(devWebBuildEnvironment(root, {
      ...
      MONOLITH_CLIENT_COMMIT_HASH: 'abc1234',
      MONOLITH_CLIENT_EXTRA: 'launch-value',
    })).toEqual({
      PATH: '/bin',
      MONOLITH_CLIENT_COMMIT_HASH: 'abc1234',
      MONOLITH_CLIENT_EXTRA: 'launch-value',
...
it('discovers monolith.client packages with sibling roles', async () => {
  const root = await mkdtemp(join(tmpdir(), 'monolith-dev-web-discovery-'))
  try {
...
it('discovers client-preset packages the shell links, excluding loader-delivered and test infrastructure', async () => {
  const root = await mkdtemp(join(tmpdir(), 'monolith-dev-web-library-'))
  try {
    ...
    await write('packages/client/delivered', { monolith: { client: { platform: 'web' } } }, clientPreset)
...
it('rebuilds a client-plugin bundle after its source changes', async () => {
  const root = await mkdtemp(join(tmpdir(), 'monolith-dev-web-watch-'))
  let bundles: TsdownBundle[] = []
  try {
    await symlink(join(import.meta.dirname, '..', 'node_modules'), join(root, 'node_modules'), 'dir')
    await writeFile(join(root, 'package.json'), JSON.stringify({ name: '@monolith-test/dev-web-watch', private: true, type: 'module' }))
