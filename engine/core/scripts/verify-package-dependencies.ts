...
export function discoverPackageDependencyScope(
  ...
  for (const name of include) {
    ...
    if (pkg !== undefined
      && (pkg.manifestPath.startsWith('packages/client/') || hasClientDeclaration(pkg.manifest.monolith))) {
  ...
  for (const name of exclude) {
    ...
    } else if (pkg !== undefined && !hasClientDeclaration(pkg.manifest.monolith)) {
      violations.push(`clientFaceExclude names ${name}, which declares no monolith.client entry`)
    }
  }
  ...
  for (const pkg of packages) {
    ...
    const clientHost = (hasClientDeclaration(pkg.manifest.monolith) || include.has(pkg.name)) && !exclude.has(pkg.name)
...
export function readPackageDependencyFacts(
  ...
  const inject = pkg.manifest.monolith?.client?.inject ?? []
...
export function expectedPackageDependencies(
  ...
  for (const name of facts.clientInject) {
    if (facts.workspaceNames.has(name)) add(name, 'devDependencies', 'monolith.client.inject')
  }
