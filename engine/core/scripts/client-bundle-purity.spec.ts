...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@monolith/client-test', ['lib/types/index.js'])
    const development = bundle({ env: {} }).find(config => config.platform === 'browser')
    const artifact = bundle({ env: { MONOLITH_BUILD_FACE: 'client' } })
...
function purityResolveId(id = REQUESTING_PACKAGE): ResolveId {
  ...
  const gate = plugins.find(p => p.name === 'monolith-client-bundle-purity')
...
function cssModulePlugin(): CssModulePlugin {
  ...
  const plugin = plugins.find(candidate => candidate.name === 'monolith-css-modules-inline')
...
function sourceMapPlugin(): SourceMapPlugin {
  ...
  const plugin = plugins.find(candidate => candidate.name === 'monolith-tsc-sourcemap')
...
describe('client bundle purity gate', () => {
  ...
  it('leaves default externals and non-scoped specifiers alone', () => {
    expect(resolveId('@monolith/client-store')).toBeNull()
    expect(resolveId('@monolith/client-ui-slots')).toBeNull()
    expect(resolveId('@monolith/client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects the retired web-react platform package', () => {
    expect(() => resolveId('@monolith/client-web-react')).toThrow(/purity/)
    expect(() => resolveId('@monolith/client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe libraries inline', () => {
    expect(resolveId('@monolith/session/surface')).toBeNull()
    expect(resolveId('@monolith/brand')).toBeNull()
    expect(resolveId('@monolith/deque')).toBeNull()
    expect(resolveId('@monolith/util-values')).toBeNull()
    expect(resolveId('@monolith/token-meter/client')).toBeNull()
    expect(() => resolveId('@monolith/token-meter')).toThrow(/purity/)
    expect(() => resolveId('@monolith/token-meter/client/internal')).toThrow(/purity/)
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@monolith/goal/remote')).toBeNull()
    expect(() => resolveId('@monolith/goal')).toThrow(/purity/)
    expect(() => resolveId('@monolith/goal/client')).toThrow(/purity/)
    expect(() => resolveId('@monolith/goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @monolith leak', () => {
    expect(() => resolveId('@monolith/agent')).toThrow(/purity/)
    expect(() => resolveId('@monolith/client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike', () => {
    expect(() => resolveId('@monolith/client-connection')).toThrow(/purity/)
    expect(() => resolveId('@monolith/client-ui-session')).toThrow(/purity/)
    expect(() => resolveId('@monolith/client-ui-layout/client')).toThrow(/purity/)
  })

  it('admits package-specific requests only for the declaring bundle', () => {
    const requesting = purityResolveId('@monolith/api-session-controller')
    expect(requesting('@monolith/api-gateway/client')).toBeNull()
    expect(() => resolveId('@monolith/api-gateway/client')).toThrow(/purity/)
  })
  ...
  it('externalizes the baseline independently of each package manifest', () => {
    ...
    const plain = clientConfigs('@monolith/client-connection')[0]?.deps as {
    ...
    expect(plain.neverBundle('@monolith/client-store')).toBe(true)
  })
})

describe('client bundle module requests', () => {
  it('requests what the declaration lists', () => {
    const requests = requestedExternals('@monolith/client-fixture', {
      external: ['react', 'react/jsx-runtime', '@monolith/client-ui-slots'],
    })

    expect([...requests].sort()).toEqual([
      '@monolith/client-ui-slots', 'react', 'react/jsx-runtime',
    ])
  })

  it('requests nothing when the declaration is absent', () => {
    expect(requestedExternals('@monolith/client-fixture', {}).size).toBe(0)
  })

  it('rejects a malformed declaration instead of reading past it', () => {
    expect(() => requestedExternals('@monolith/client-fixture', { external: 'react' }))
      .toThrow(/monolith\.client\.external must be a string array/)
  })
})
...
describe('client bundle debug artifacts', () => {
  ...
  it('chains emitted tsc maps when the production Client build consumes lib/types', async () => {
    const root = mkdtempSync(join(tmpdir(), 'monolith-client-sourcemap-'))
    try {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@monolith/client-ui-goal')
    ...
    const resolved = new URL(source, 'https://monolith.test/plugins/@monolith/client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@monolith/host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@monolith/client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://monolith.test/plugins/@monolith/client-connection/client.js.map')
