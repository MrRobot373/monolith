...
export interface CatalogEntry {
  /** npm package name, e.g. `@monolith/agent-loop`. */
  pkg: string
...
/**
 * Flatten an `exports` map to its string targets, descending through
 * conditional-export objects.
 * @param node - an `exports` value: a target string, or a nested condition map.
 * @returns every string target reachable from the node.
 */
...
export function collectConfigCatalog(scanRoot: string = root): CatalogEntry[] {
  ...
  for (const manifestRel of globSync('packages/*/*/package.json', { cwd: scanRoot }).map(path => path.split(sep).join('/')).sort()) {
    ...
    const exported = exportTargets(manifest.exports)
    if (exported.length > 0 && !exported.some(target => target.endsWith('.js'))) {
      // A data-only package — one that exports a patch or manifest and no
      // JavaScript — has no plugin to classify either.
      continue
    }
