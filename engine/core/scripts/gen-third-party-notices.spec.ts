...
describe('tierExternalDeps', () => {
  it('tiers by declaring area, not by the declaring section name', () => {
    const { manifests, names } = workspace({
      ...
      'packages/test-support/loader-smoke/package.json': { name: '@monolith/loader-smoke', dependencies: { 'smoke-helper': '^1' } },
      'packages/test-support/client-runtime/package.json': { name: '@monolith/client-test-runtime', dependencies: { 'test-lib': '^1' } },
      ...
      'packages/mcp/mcp-client/package.json': { name: '@monolith/mcp-client', dependencies: { 'protocol-sdk': '^1' }, devDependencies: { 'protocol-fixture-server': '^1' } },
      'apps/cli/package.json': { name: '@monolith/cli', dependencies: { 'cli-lib': '^1', '@monolith/mcp-client': 'workspace:^' } },
    })
  ...
  it('keeps a package runtime when any shipping area declares it, and excludes workspace links', () => {
    const { manifests, names } = workspace({
      ...
      'packages/interaction/tui/package.json': { name: '@monolith/tui', dependencies: { shared: '^1', '@monolith/cli': 'workspace:^' } },
      'apps/cli/package.json': { name: '@monolith/cli' },
    })

    expect(tierExternalDeps(manifests, names).get('shared')).toBe(true)
    expect(tierExternalDeps(manifests, names).has('@monolith/cli')).toBe(false)
  })
})

describe('virtualManifest', () => {
  it('resolves a manifest from an ordinary prefix-matching store directory', () => {
    const root = mkdtempSync(join(tmpdir(), 'monolith-notices-prefix-'))
    try {
  ...
  it('falls back to a content scan when pnpm 11 truncates the store directory name', () => {
    const root = mkdtempSync(join(tmpdir(), 'monolith-notices-truncated-'))
    try {
      const name = '@scope/pkg'
      const version = '2.0.0'
      const store = join(root, 'store')
  ...
  it('selects the requested version when the store retains historical copies', () => {
    const root = mkdtempSync(join(tmpdir(), 'monolith-notices-version-'))
    try {
      const name = '@scope/pkg'
      const store = join(root, 'store')
  ...
  it('returns undefined when neither the prefix nor the content scan finds the package', () => {
    const root = mkdtempSync(join(tmpdir(), 'monolith-notices-miss-'))
    try {
      const store = join(root, 'store')
...
describe('parseVendoredRows', () => {
  ...
  it('yields nothing when the table columns change, so the generator fails loud', () => {
    expect(parseVendoredRows('| `cordis/` | `@monolith/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\n')).toEqual([])
  })
