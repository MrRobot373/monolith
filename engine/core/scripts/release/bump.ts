 ...
 *
 * The monolith family shares one version across its publishable members, private
...
/** Release types the monolith family accepts besides an explicit version. */
const RELEASE_TYPES = ['major', 'minor', 'patch'] as const

/** The workspace root manifest, which carries the monolith family's version. */
...
/** One private monolith package whose version follows the publishable family. */
interface PrivateMonolithVersion {
...
function nextSharedVersion(current: string, request: string): string {
  if (!RELEASE_TYPES.includes(request as typeof RELEASE_TYPES[number])) {
    if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(request)) {
      throw new Error(`usage: release:monolith <major|minor|patch|x.y.z>, got ${request}`)
    }
    return request
  }
...
/**
 * Discover private package manifests that share the monolith version without joining
 * its publish set.
 * @param root - repository root.
 * @returns Private package manifests sorted by path.
 */
function privateMonolithVersions(root: string): PrivateMonolithVersion[] {
...
/**
 * Plan the monolith family's rewrite: one version for every publishable member,
 * private package, and the root.
 * @param family - the monolith family.
 * @param root - repository root.
...
export function planShared(
  ...
  for (const entry of privateMonolithVersions(root)) {
...
function main(): void {
  ...
  if (values.family === undefined) throw new Error('usage: bump.ts --family <monolith|vendor> [version]')
  ...
  if (family.id === 'monolith') {
    const request = positionals[0]
    if (request === undefined) throw new Error('usage: release:monolith <major|minor|patch|x.y.z>')
    if (values.prerelease !== undefined) {
      throw new Error('release:monolith takes the prerelease in its version argument, as in 0.0.1-rc.1')
    }
