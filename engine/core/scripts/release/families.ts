 ...
 * owns: `monolith` and `vendor`. Each family carries its own version baseline, tag
...
const WORKSPACE_ROOT_PACKAGE = '@monolith/root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@monolith/')) throw new Error(`${normalized} must name an @monolith package`)
...
class MonolithFamily extends ReleaseFamily {
  readonly id = 'monolith'
  readonly patterns = ['packages/!(experimental)/*/package.json', 'apps/*/package.json'] as const
  readonly tagPrefix = 'monolith-v'
  ...
  verifyVersions(members: readonly ReleaseMember[]): void {
    ...
    if (versions.size !== 1) {
      ...
      throw new Error(`monolith release members must share one version:\n${detail}`)
    }
  }
  ...
  readonly installedEntry = { packageName: '@monolith/cli', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@monolith/', '')}-v`
  }
...
function releaseFamilies(): readonly ReleaseFamily[] {
  return [new MonolithFamily(), new VendorFamily()]
}
