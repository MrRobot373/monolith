 ...
 * `@monolith/*` listed 49 candidate globs and `@monolith/*\/invariant`
 ...
 * @returns Every directory whose manifest names a `@monolith/` package and that carries `src`.
 */
 ...
 * `@monolith/<directory>`. Packages named after something other than
 ...
 *
 * @returns Declared names of every `@monolith/` package carrying a `src` directory.
 */
...
export function mappedSpecifiers(text: string): Set<string> {
  ...
  for (const match of text.matchAll(/^\s*"(@monolith\/[^"/]+)":/gm)) {
    const key = match[1]
...
function handWrittenSpecifiers(text: string): Set<string> {
  ...
  const keys = new Set<string>()
  for (const match of outside.matchAll(/^\s*"(@monolith\/[^"]+)":/gm)) {
    const key = match[1]
    if (key !== undefined) keys.add(key)
  }
  return keys
