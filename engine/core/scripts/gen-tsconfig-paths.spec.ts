...
describe('generated tsconfig package aliases', () => {
  it('maps each package to its own source directory', () => {
    ...
    const session = aliases.find(alias => alias.specifier === '@monolith/session')
    expect(session).toEqual({
      specifier: '@monolith/session',
    ...
    expect(aliases.some(alias => alias.specifier === '@monolith/typert-protocol')).toBe(false)
  })

  it('yields to a hand-written alias and closes without a trailing comma', () => {
    const aliases = [
      { specifier: '@monolith/a', source: './packages/g/a/src', hasInvariant: true },
      { specifier: '@monolith/b', source: './packages/g/b/src', hasInvariant: false },
    ]
    const body = renderAliases(aliases, new Set(['@monolith/a']))

    // The hand-written bare alias is skipped; its /invariant sibling is not.
    expect(body).toBe([
      '      "@monolith/a/invariant": ["./packages/g/a/src/invariant.ts"]',
      '      "@monolith/b": ["./packages/g/b/src"]',
    ].join(',\n'))
  ...
  it('names a package that no alias covers', () => {
    ...
    expect(uncoveredPackages(
      ...
      new Set(['@monolith/a', '@monolith/a/invariant']),
    )).toEqual(['@monolith/b'])

    expect(uncoveredPackages(['@monolith/a'], new Set(['@monolith/a']))).toEqual([])
  })
  ...
  it('covers every workspace package in the committed config', () => {
    ...
    expect(names).toContain('@monolith/typert-protocol')
  ...
  it('leaves no wildcard that probes every package group', () => {
    ...
    expect(config).not.toContain('"@monolith/*":')
    expect(config).not.toContain('"@monolith/*/invariant":')
  })
})
