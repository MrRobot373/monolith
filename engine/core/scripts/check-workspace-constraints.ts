...
const vendoredPackages = new Set([
  ...
  '@monolith/cordis-plugin-loader',
  '@monolith/cordis-plugin-include',
  '@monolith/cordis-plugin-group',
  '@monolith/cordis-plugin-timer',
  '@monolith/cordis-plugin-hmr',
  '@monolith/cordis-plugin-logger-console',
])
const publicLandlockPackages = new Set([
  '@monolith/node-addon-landlock-run',
  '@monolith/node-addon-landlock-run-linux-arm64',
  '@monolith/node-addon-landlock-run-linux-x64',
])
...
const publicationSourceAllowlist: Readonly<Record<string, readonly string[]>> = {
  '@monolith/node-addon-landlock-run': ['src/main.c'],
}
const repositoryUrl = 'git+https://github.com/monolith/monolith.git'
/**
 ...
 */
const publishedRepositoryUrl = 'git+https://github.com/MrRobot373/monolith.git'
...
const experimentalPackageNamePrefix = '@monolith/experimental-'
...
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  ...
  '@monolith/web-frontend': ['dist', '!dist/**/*.map', '!dist/preview.html', '!dist/preview'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  '@monolith/client-ui-primitives': ['lib/**/*.css'],
  '@monolith/client-web': ['lib/**/*.css'],
  '@monolith/client-ui-theme': ['lib/styles'],
  // The CPython side ships as source .py files, published as-is rather than built.
  '@monolith/experimental-code-runtime-python': ['py/**/*.py'],
  // The shipped preset compositions travel inside the roster package.
  '@monolith/agent-presets': ['presets'],
  // The Web Host mounts the default-off settings owner independently of each
  // Agent-scoped delegation-tool instance.
  '@monolith/tool-subagent': ['lib/model-selection-settings.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@monolith/sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  '@monolith/skill-badge': ['assets'],
  ...
  '@monolith/experimental-webworker-packer': ['bin.js', 'lib/repository-*.js'],
  '@monolith/subprocess-local': ['scripts/ensure-spawn-helper.mjs'],
}
...
export function expectedMonolithPackageFiles(manifest: PackageManifest): readonly string[] {
  const declaredPatch = manifest.monolith?.bundle?.patch
...
/**
 * Check one workspace manifest against publication and monolith-package policy.
...
export function checkWorkspaceManifest({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (isPublicLandlockPackage) {
    ...
    // them, while the monolith family stays restricted until its own sequence goes
  ...
  if (manifest.name?.startsWith('@monolith/')) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@monolith/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@monolith/')) {
    const peer = manifest.peerDependencies?.['@monolith/cordis']
    ...
    const expectedFiles = expectedMonolithPackageFiles(manifest)
...
function checkRepositoryVersion(): string[] {
  // The root carries the monolith release family's version, so a prerelease such as
  // 0.0.1-rc.1 is a valid state between `release:monolith` and its publication.
