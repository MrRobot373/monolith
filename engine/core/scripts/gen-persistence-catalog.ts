...
const SESSION_PACKAGE = '@monolith/session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@monolith/session/types'
 ...
 * top-level declaration (in `@monolith/session`) and any declaration
 * merge inside a `declare module '@monolith/session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @monolith/session/known-event-types',
    ' */',
    '',
    '/**',
