/** Application-entrypoint classification and monolith-launch enforcement. */
...
function fixture(): string {
  const root = mkdtempSync(join(tmpdir(), 'monolith-application-entrypoints-'))
  cleanups.push(root)
  return root
}
...
describe('application entrypoints', () => {
  ...
  it('rejects a package-level application bin', () => {
    ...
    expect(applicationEntrypointViolations(root)).toEqual([
      'packages/example/app/package.json: package bin bypasses the monolith launcher; applications use apps/cli profiles',
    ])
  })
  ...
  it('rejects a private Python application carrier outside monolith', () => {
    const root = fixture()
  ...
  it('rejects a new root demo until its launch role is classified', () => {
    const root = fixture()
    write(root, 'package.json', JSON.stringify({ scripts: { 'demo:new-app': 'monolith --profile new-app' } }))

    expect(applicationEntrypointViolations(root)).toEqual([
      'package.json scripts.demo:new-app: demo launcher has no explicit monolith or in-process classification',
    ])
  })
})
