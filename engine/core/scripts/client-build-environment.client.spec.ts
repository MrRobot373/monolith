...
const PROBE_NAME = 'MONOLITH_CLIENT_BUILD_TEST'
...
function buildFixture(environment: Record<string, string>): string {
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'monolith-client-build-'))
...
function repositoryFixture(version = '1.2.3-rc.4'): string {
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'monolith-client-build-repository-'))
  roots.push(fixtureRoot)
  ...
  git(fixtureRoot, ['config', 'user.name', 'MONOLITH test'])
  git(fixtureRoot, ['config', 'user.email', 'monolith-test@example.invalid'])
...
describe('client build environment', () => {
  it('requires an exact public environment for a named artifact profile', () => {
    const expected = {
      MONOLITH_CLIENT_BUILD_PROFILE: 'official',
      MONOLITH_CLIENT_COMMIT_HASH: COMMIT_HASH.slice(0, 7),
      MONOLITH_CLIENT_TITLE: 'MONOLITH',
      MONOLITH_CLIENT_VERSION: '1.2.3',
    } as const

    expect(() => { assertClientBuildEnvironment({ PATH: '/bin', ...expected }, expected) }).not.toThrow()
    expect(() => { assertClientBuildEnvironment({}, expected) }).toThrow(/MONOLITH_CLIENT_TITLE/)
    expect(() => { assertClientBuildEnvironment({ MONOLITH_CLIENT_TITLE: 'Other' }, expected) }).toThrow(/MONOLITH_CLIENT_TITLE/)
    expect(() => {
      assertClientBuildEnvironment({ ...expected, MONOLITH_CLIENT_UNDECLARED: 'value' }, expected)
    }).toThrow(/MONOLITH_CLIENT_UNDECLARED/)
  })

  it('inherits public values by default and isolates an explicit official profile', () => {
    const parent = {
      PATH: '/bin',
      MONOLITH_BUILD_CLIENT_PROFILE: 'official',
      MONOLITH_CLIENT_BUILD_PROFILE: 'local',
      MONOLITH_CLIENT_COMMIT_HASH: COMMIT_HASH.slice(0, 7),
      MONOLITH_CLIENT_GIT_DIRTY: 'true',
      MONOLITH_CLIENT_TITLE: 'Local title',
      MONOLITH_CLIENT_VERSION: '1.2.3',
      MONOLITH_CLIENT_EXTRA: 'local-extra',
    }

    expect(resolveClientBuildEnvironment({ MONOLITH_CLIENT_TITLE: 'Local title' })).toEqual({
      MONOLITH_CLIENT_TITLE: 'Local title',
    })
    ...
    expect(() => {
      resolveClientBuildEnvironment({ MONOLITH_BUILD_CLIENT_PROFILE: 'official' })
    }).toThrow(/MONOLITH_CLIENT_COMMIT_HASH/)
    expect(() => {
      resolveClientBuildEnvironment({
        MONOLITH_BUILD_CLIENT_PROFILE: 'official',
        MONOLITH_CLIENT_COMMIT_HASH: COMMIT_HASH.slice(0, 7),
      })
    }).toThrow(/MONOLITH_CLIENT_VERSION/)
    ...
    expect(repositoryCommitHash('/unused', { MONOLITH_CLIENT_COMMIT_HASH: COMMIT_HASH })).toBe(COMMIT_HASH.slice(0, 7))
  })
  ...
  it('owns repository version, commit, and dirty metadata for complete builds', () => {
    ...
    expect(repositoryClientBuildEnvironment(fixtureRoot, {
      MONOLITH_CLIENT_COMMIT_HASH: COMMIT_HASH,
      MONOLITH_CLIENT_EXTRA: 'preserved',
      MONOLITH_CLIENT_GIT_DIRTY: 'true',
      MONOLITH_CLIENT_VERSION: 'spoofed',
    })).toEqual({
      MONOLITH_CLIENT_COMMIT_HASH: COMMIT_HASH.slice(0, 7),
      MONOLITH_CLIENT_EXTRA: 'preserved',
      MONOLITH_CLIENT_VERSION: '1.2.3-rc.4',
    })
    expect(officialClientBuildEnvironment(fixtureRoot)).toEqual({
      MONOLITH_CLIENT_BUILD_PROFILE: 'official',
      MONOLITH_CLIENT_COMMIT_HASH: commit,
      MONOLITH_CLIENT_TITLE: 'MONOLITH',
      MONOLITH_CLIENT_VERSION: '1.2.3-rc.4',
    })
  ...
  it('omits dirty metadata when repository metadata is unavailable', () => {
    const fixtureRoot = mkdtempSync(join(tmpdir(), 'monolith-client-build-no-git-'))
    roots.push(fixtureRoot)
  ...
  it('defines only public client values over a non-enumerable fallback', () => {
    expect(clientBuildEnvironmentDefines({
      PATH: '/bin',
      MONOLITH_TEST_API_KEY: 'secret',
      MONOLITH_CLIENT_VARIANT: 'quoted "value"',
      MONOLITH_CLIENT_EMPTY: '',
      MONOLITH_CLIENT_UNSET: undefined,
    })).toEqual({
      'process.env': '{}',
      'process.env.MONOLITH_CLIENT_EMPTY': '""',
      'process.env.MONOLITH_CLIENT_VARIANT': '"quoted \\"value\\""',
    })
  })
  ...
  it('feeds the same build-process value to dynamic tsdown bundles and the Vite shell', async () => {
    ...
    const configs = clientBundle('@monolith/client-ui-sidebar', [
      'lib/types/index.js',
      'lib/types/invariant.js',
    ])({ env: { MONOLITH_BUILD_FACE: 'client' } })
    if (!Array.isArray(configs)) throw new TypeError('client bundle config must be an array')
    const dynamic = configs.find(config => config.name === '@monolith/client-ui-sidebar/client')
  ...
  it('binds the recorded environment to a complete set of client artifacts', () => {
    ...
    expect(() => { readClientBuildRecord(defaultBuild, officialEnvironment) }).toThrow(/MONOLITH_CLIENT_/)
  ...
  it('keeps public client values out of workflow-wide environments', () => {
    for (const name of monolithBuildWorkflows) {
      ...
      expect(JSON.stringify(document), path).not.toContain('MONOLITH_CLIENT_')
    }
  })
})
