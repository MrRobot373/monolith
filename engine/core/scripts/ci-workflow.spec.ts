...
describe('CI workflow', () => {
  ...
  it('keeps required Wine and split native Windows jobs with failover, plus a master-only standby', () => {
    ...
    for (const [jobName, job] of [['windows-build', windowsBuild], ['windows-coverage', windowsCoverage], ['windows-native-tests', windowsNativeTests], ['windows-observational', windowsObservational]] as const) {
      ...
      expect(job['runs-on'], `${jobName} runs-on must use the Windows failover switch`).toContain('MONOLITH_CI_FAILOVER_WINDOWS')
      expect(job['runs-on'], `${jobName} runs-on must not use the Linux failover switch`).not.toContain('MONOLITH_CI_FAILOVER_LINUX')
      expect(job['runs-on']).toContain('self-hosted')
      expect(job['runs-on']).toContain('monolith-win-ci')
      expect(job['runs-on']).toContain('monolith-windows-2025-16core')
    ...
    expect(windowsCoverage.env).toMatchObject({ MONOLITH_COVERAGE_PARTITIONS: '4' })
    ...
    expect(serialWindows['runs-on']).toEqual(['self-hosted', 'monolith-win-ci', 'windows'])
    ...
    expect(serialGate!.env).toMatchObject({ MONOLITH_COVERAGE_TEST_TIMEOUT_MS: '90000' })
    ...
    // and the verdict job resolve their pool through MONOLITH_CI_FAILOVER_LINUX,
    // never the Windows switch.
    for (const [jobName, job] of [['node-24', node24], ['node-24-coverage', node24Coverage], ['node-24-consumers', node24Consumers]] as const) {
      expect(typeof job['runs-on']).toBe('string')
      expect(job['runs-on'], `${jobName} runs-on must use the Linux failover switch`).toContain('MONOLITH_CI_FAILOVER_LINUX')
      expect(job['runs-on'], `${jobName} runs-on must not use the Windows failover switch`).not.toContain('MONOLITH_CI_FAILOVER_WINDOWS')
      expect(job['runs-on']).toContain('vm-backup')
    }
    expect(aggregate['runs-on']).toContain('MONOLITH_CI_FAILOVER_LINUX')
    expect(aggregate['runs-on']).not.toContain('MONOLITH_CI_FAILOVER_WINDOWS')
    ...
    for (const [jobName, job] of [['node-24', node24], ['node-24-coverage', node24Coverage], ['node-24-consumers', node24Consumers], ['node-compat', nodeCompat]] as const) {
      expect(job.env, `${jobName} must enable fail-fast`).toMatchObject({ MONOLITH_GATE_FAIL_FAST: '1' })
    }
    ...
    expect(windowsBuild.env, 'windows-build must enable fail-fast').toMatchObject({ MONOLITH_GATE_FAIL_FAST: '1' })
    expect(windowsCoverage.env, 'windows-coverage must enable fail-fast').toMatchObject({ MONOLITH_GATE_FAIL_FAST: '1' })
    ...
    expect(windowsObservational.env).not.toMatchObject({ MONOLITH_GATE_FAIL_FAST: '1' })
  })
...
describe('DeepSeek e2e workflow', () => {
  ...
  it('bounds profile subprocess fan-out to the tested e2e default', () => {
    ...
    expect(step).toMatchObject({ env: { MONOLITH_E2E_MAX_WORKERS: 4 } })
  })
})
  ...
  it('is manual-only and fails loud before running the focused live suite', () => {
    ...
    expect(e2b).toMatchObject({
      env: {
        ...
        MONOLITH_E2E_MAX_WORKERS: '1',
        MONOLITH_EXAMPLE_MODE: 'lib',
      },
    })
...
describe('Python release workflows', () => {
  it('keeps complete wheel validation separate from protected public publication', () => {
    ...
    expect(pythonCompatSteps).toContain('dist/monolith_sdk-$VERSION-py3-none-any.whl')
    expect(pythonCompatSteps).toContain('dist/monolith_runtime_bin-$VERSION-py3-none-manylinux_2_28_x86_64.whl')
  ...
  it('exposes the native wheel builder to the release caller with normalized versions', () => {
    ...
    expect(JSON.stringify(installedRealApiPosix)).toContain('-u MONOLITH_RUNTIME_MODE')
    ...
    expect(JSON.stringify(manylinuxSmoke)).toContain('-e MONOLITH_TELEMETRY_DISABLED')
  })
...
describe('Issue lifecycle workflow', () => {
  ...
  it('uses a read-only Project token only for human pull request policy metadata', () => {
    ...
    expect(tokenStep).toMatchObject({
      ...
      with: {
        'client-id': '${{ vars.MONOLITH_ISSUE_APP_CLIENT_ID }}',
        'private-key': '${{ secrets.MONOLITH_ISSUE_APP_PRIVATE_KEY }}',
        owner: 'monolith',
        repositories: 'monolith',
...
describe('npm release workflows', () => {
  ...
  it('runs dependency policy and npm layout checks in the MONOLITH release workflow', () => {
    ...
    if (!isRecord(workflow.on) || !Array.isArray(dependencies.steps)) {
      throw new TypeError('MONOLITH release workflow must define triggers and dependency steps')
    }
...
describe('Documentation site publication', () => {
  it('keeps Pages deployment dispatch-only from a monolith-v* tag', () => {
    ...
    // RELEASE_PUBLISH makes release:verify reject every ref that is not a monolith-v*
    ...
    expect(verify).toMatchObject({
      ...
      run: 'pnpm run release:verify --family monolith',
    })
