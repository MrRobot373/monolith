...
const TARGET_PACKAGE = '@monolith/cli'
...
export async function resolveNpmPackageLock(
  ...
  const consumer = mkdtempSync(join(tmpdir(), 'monolith-npm-resolution-'))
  try {
    writeFileSync(join(consumer, 'package.json'), `${JSON.stringify({
      name: 'monolith-npm-resolution-benchmark',
      version: '0.0.0',
      private: true,
      dependencies,
    }, null, 2)}\n`)
 ...
 * @param index - Package metadata exposed through the local registry.
 * @param targetVersion - Version of `@monolith/cli` to install.
 * @param timeoutMs - Hard wall-clock limit for the npm child process.
