...
describe('compaction-tool-result-pruner real Loader composition', () => {
  it('loads and resolves the flat YAML plugin shape', async () => {
    root = await mkdtemp(join(tmpdir(), 'monolith-compact-tool-result-prune-loader-'))
    ...
    context.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@monolith/session-projection') return SessionProjectionRegistry
        if (specifier === '@monolith/token-meter') return TokenMeter
        if (specifier === '@monolith/compaction-tool-result-pruner') return ToolResultPruner
