...
/**
 * A backend-generated default root name: `monolith-spill-` plus the 6-character
 * suffix `mkdtemp` appends. Discovery matches this
 * EXACT shape, not the bare prefix, so an unrelated `monolith-spill-test-*` fixture
 * or a foreign tool's differently-shaped `monolith-spill-…` directory is never
...
export interface SweepRoot {
   ...
   * pruned. Set for DISCOVERED prior-default `monolith-spill-*` roots (one per past
...
/**
 * Discover prior default spill roots: the `monolith-spill-<6 chars>` directories
 ...
 * prefix, so an unrelated `monolith-spill-test-*` fixture or a foreign
