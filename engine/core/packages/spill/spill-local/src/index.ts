 ...
 * `@monolith/spill` storage seam. Persists a tool's oversized text to a
...
export class LocalSpillStore extends SpillStore {
   ...
   * `monolith-spill-*` temp root (see {@link discoverDefaultRoots}), pruned when
  ...
  /**
   * The directory scanned for prior default `monolith-spill-*` roots — the OS tmpdir,
