...
Mount this backend in a composition that spills tool output to the local filesystem. It registers as the `ctx.spillStore` service that the `monolith-spill-policy` plugin and other callers use.
...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-spill-local) is the exhaustive source for every accepted field.
...
- [monolith-spill-policy](../spill-policy/README.md) — the policy that calls this backend when a result is too large.
