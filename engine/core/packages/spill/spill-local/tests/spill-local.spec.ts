...
import type { SaveTextSpill } from '@monolith/spill'
import LocalSpillStore, {
  ...
  sessionDir,
  sweepSpillRoots,
} from '@monolith/spill-local'
import type { SweepRoot } from '@monolith/spill-local'
...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'monolith-spill-test-'))
})
afterEach(() => {
...
describe('LocalSpillStore service', () => {
  ...
  it('defaults cleanupPeriodDays to 30', async () => {
    ...
    const emptyBase = mkdtempSync(join(tmpdir(), 'monolith-empty-'))
...
describe('startup cleanup sweep', () => {
  ...
  it('prunes an emptied DISCOVERED default root but never the active root', async () => {
    ...
    const prior = mkdtempSync(join(tmpdir(), 'monolith-spill-'))
  ...
  it('covers the configured root AND discovered default roots (real gatherRoots)', async () => {
    ...
    const fakeTmp = mkdtempSync(join(tmpdir(), 'monolith-faketmp-'))
...
describe('discoverDefaultRoots', () => {
  it('returns only real monolith-spill-* directories, excluding symlinks and non-matches', async () => {
    const base = mkdtempSync(join(tmpdir(), 'monolith-disc-'))
    try {
      // A real backend-shaped root (monolith-spill-<6>) via mkdtemp — the only match.
