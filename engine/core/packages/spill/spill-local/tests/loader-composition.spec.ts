...
import LocalSpillStore, { sessionDir } from '@monolith/spill-local'
...
describe('spill-local real Loader composition through cordis.yml', () => {
  it('loads cleanupPeriodDays and prunes only expired session contents', async () => {
    root = await mkdtemp(join(tmpdir(), 'monolith-spill-loader-'))
    ...
    context.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier !== '@monolith/spill-local') throw new Error(`unexpected Loader import: ${specifier}`)
