 ...
 *
 * `@monolith/tools`' `ToolExecution` satisfies this shape, so the policy
 * reads `exec` straight through without importing `monolith-tools` or `monolith-agent`.
 * Only the session HEADER id is read — the same identity every other subsystem
 * keys off (see `monolith-tool-bash`'s owner derivation).
 *
 * @module @monolith/spill-policy/types
 */
