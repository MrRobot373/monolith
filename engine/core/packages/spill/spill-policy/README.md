...
# @monolith/spill-policy

## Summary

`monolith-spill-policy` keeps oversized plain-text tool results out of the model's context: when a final result exceeds `maxInlineBytes`, it saves the full text through `ctx.spillStore` and replaces the model-facing result with a bounded head/tail preview plus the backend's locator and retrieval guidance, which the model can use to read or grep the spill file. It registers no service and owns no storage or preview mechanics — storage is the mounted `SpillStore` backend and previews come from `monolith-output-retention`; it only decides when to spill and composes the notice. It is opt-in and best-effort: omitted `maxInlineBytes` disables it entirely, and a spill failure leaves the original result visible. A second arm applies the same cap to the durable log copy of `run_code` sub-call results, so replay and UIs never grow unbounded either.
...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-spill-policy) is the exhaustive source for every accepted field. A negative or fractional cap fails plugin load rather than corrupting per-call behavior.
...
- [monolith-spill-local](../spill-local/README.md) — the local backend that stores the spilled text.
- [monolith-output-retention](../../util/output-retention/README.md) — the preview mechanics (`TextRetainer`) the policy composes.
