...
```yaml
- name: '@monolith/hooks-claude-code'
  config:
...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-hooks-claude-code) is the exhaustive source for every accepted field.
...
The three emit points (`SessionStart`, `SubagentStart`, `SubagentStop`) run detached — no extension point awaits them. Each run chain is tracked, and disposing the bridge aborts still-running hook processes, then drains the continuations before the dispose resolves (`createDetachedRuns` in `monolith-hook-protocol`).
...
- [Generated configuration catalog](../../../docs/config-catalog.md#monolith-hooks-claude-code) — every accepted config field and its source declaration.
