...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-hooks-codex) is the exhaustive source for every accepted field.
...
`SessionStart` is the one emit point and runs detached — no extension point awaits it. Each run chain is tracked, and disposing the bridge aborts a still-running hook process, then drains the continuation before the dispose resolves (`createDetachedRuns` in `monolith-hook-protocol`).
...
- [Generated configuration catalog](../../../docs/config-catalog.md#monolith-hooks-codex) — every accepted config field and its source declaration.
