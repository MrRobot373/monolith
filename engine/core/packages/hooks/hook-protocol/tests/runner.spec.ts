...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from '@monolith/shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@monolith/hook-protocol'
import type { RunHookOptions } from '@monolith/hook-protocol'
 ...
 * executor (monolith-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what monolith-bash-local's resolve does for the fields runHook sets.
      return {
