...
import type { SubprocessHandle, SubprocessOutputReader } from '@monolith/subprocess'
...
function shellArgv(command: string): string[] {
  ...
  switch (command) {
    ...
    case 'echo "[${MONOLITH_STALE:-absent}|$MONOLITH_SHELL|$MONOLITH_SESSION_ID]"':
      return node('console.log("[" + [process.env.MONOLITH_STALE ?? "absent", process.env.MONOLITH_SHELL, process.env.MONOLITH_SESSION_ID].join("|") + "]")')
    case 'echo "[${MONOLITH_TEST_API_KEY:-absent}|${MONOLITH_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${MONOLITH_TEST_PLAIN:-absent}]"':
      return node('console.log("[" + [process.env.MONOLITH_TEST_API_KEY ?? "absent", process.env.MONOLITH_TEST_TOKEN ?? "absent", process.env.SUBPROCESS_TEST_PASSWORD ?? "absent", process.env.MONOLITH_TEST_PLAIN ?? "absent"].join("|") + "]")')
...
const spillDir = mkdtempSync(join(tmpdir(), 'monolith-subprocess-spec-'))
...
describe('spawnSubprocess', () => {
  ...
  it('rejects with a spawn error for a nonexistent cwd', async () => {
    await expect(spawnSubprocess(spec('echo hi', { cwd: '/nonexistent-dir-monolith-test' })).done)
...
describe.skipIf(process.platform === 'win32')('tree-survivor escalation (terminate and bounded waits reach helpers the leader left behind)', () => {
  ...
  it('service teardown awaits tree survivors, not just handle settlement', async () => {
    const { Context } = await import('@monolith/cordis')
    const { default: LocalSubprocessRuntime } = await import('@monolith/subprocess-local')
...
describe('coverage seams', () => {
  ...
  it('childEnv keeps the POSIX spread on non-Windows hosts', () => {
    ...
    try {
      expect(childEnv({ MONOLITH_X: '1' }).MONOLITH_X).toBe('1')
    } finally {
  ...
  it('a spawn-failed handle rejects done while waitForExit reports gone', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-monolith-dispose-test' }))
  ...
  it('waitForExit on a failed spawn reports exited immediately', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-monolith-spawn-test' }))
    await expect(running.done).rejects.toThrow()
    await expect(running.waitForExit()).resolves.toBe(true)
  })
...
describe('environment and spill-file hardening', () => {
  it('scrubs credential-shaped and ambient MONOLITH env vars from child processes', async () => {
    process.env.MONOLITH_TEST_API_KEY = 'super-secret'
    process.env.MONOLITH_TEST_TOKEN = 'also-secret'
    process.env.SUBPROCESS_TEST_PASSWORD = 'password-secret'
    process.env.MONOLITH_TEST_PLAIN = 'visible'
    try {
      const result = await finish(spawnSubprocess(spec(
        'echo "[${MONOLITH_TEST_API_KEY:-absent}|${MONOLITH_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${MONOLITH_TEST_PLAIN:-absent}]"',
      )))
      expect(result.stdout.text.trim()).toBe('[absent|absent|absent|absent]')
    } finally {
      delete process.env.MONOLITH_TEST_API_KEY
      delete process.env.MONOLITH_TEST_TOKEN
      delete process.env.SUBPROCESS_TEST_PASSWORD
      delete process.env.MONOLITH_TEST_PLAIN
    }
  })

  it('forwards explicit MONOLITH_* env entries while scrubbing ambient ones', async () => {
    // Both facts through one explicit map: the ambient MONOLITH_STALE is dropped by
    // the scrub, and the deliberately supplied current values merge after it.
    process.env.MONOLITH_STALE = 'old-value'
    try {
      const result = await finish(spawnSubprocess(spec('echo "[${MONOLITH_STALE:-absent}|$MONOLITH_SHELL|$MONOLITH_SESSION_ID]"', {
        env: { MONOLITH_SHELL: '1', MONOLITH_SESSION_ID: 'current-session' },
      })))
      expect(result.stdout.text.trim()).toBe('[absent|1|current-session]')
    } finally {
      delete process.env.MONOLITH_STALE
    }
  })
  ...
  it.skipIf(process.platform === 'win32')('creates spill files with owner-only permissions and random names', async () => {
    ...
    expect(path).toMatch(/monolith-subprocess-\d+-\d+-[0-9a-f]{12}-stdout\.log$/)
  ...
  it.skipIf(process.platform === 'win32')('defaults spills into a private per-process directory', async () => {
    ...
    expect(dir).toMatch(/monolith-subprocess-/)
