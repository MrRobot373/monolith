 ...
 * @module monolith-subprocess-local/spawn
 */
...
function privateSpillDir(): string {
  defaultSpillDir ??= mkdtempSync(join(tmpdir(), 'monolith-subprocess-'))
...
export class OutputCollector {
  ...
  private spillAll(chunk: Buffer): void {
    ...
    if (this.spillFd === undefined) {
      ...
      this.spillFile = join(
        this.spillDir,
        `monolith-subprocess-${process.pid}-${++spillCounter}-${randomBytes(6).toString('hex')}-${this.label}.log`,
      )
