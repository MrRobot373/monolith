...
Children never inherit the harness's ambient secrets: credential-shaped names and ambient `MONOLITH_*` facts are scrubbed, and the caller's explicit `env` merges after that scrub. A deliberately forwarded credential or a current `MONOLITH_*` deployment fact still reaches the child; an explicit `undefined` tombstone removes an ordinary ambient entry.
...
| [`src/types.ts`](src/types.ts) | Vocabulary: spawn spec, stdio modes, handles, readers, outcomes, `MONOLITH_*` namespace |
...
- [Subprocess subsystem](../../../docs/subsystems/subprocess.md) — spawn specs, output readers, outcomes, and the `MONOLITH_*` environment in full.
- [monolith-subprocess-local](../subprocess-local/README.md) — the local host provider that implements this contract.
- [monolith-subprocess-e2b](../../e2b/subprocess-e2b/README.md) — the remote E2B provider for the same seam.
- [monolith-bash-local](../../shell/bash-local/README.md) — the largest consumer: bash commands over this service.
