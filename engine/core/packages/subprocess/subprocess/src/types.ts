 ...
 * @module monolith-subprocess/types
 */

import type { Readable, Writable } from 'node:stream'

/** Namespace prefix reserved for MONOLITH-managed child environment facts. */
export const MONOLITH_ENV_PREFIX = 'MONOLITH_' as const

/** One environment key inside the managed {@link MONOLITH_ENV_PREFIX} namespace. */
export type MonolithEnvironmentKey = `${typeof MONOLITH_ENV_PREFIX}${string}`

/** Trusted MONOLITH variables for one child-process execution. */
export type MonolithEnvironment = Readonly<Record<MonolithEnvironmentKey, string>>
 ...
 * to `TerminalSignal` in `@monolith/terminal` without a cross-seam dependency;
