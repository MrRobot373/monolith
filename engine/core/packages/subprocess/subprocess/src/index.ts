 ...
 * `@monolith/subprocess-local`.
 * @module @monolith/subprocess
 */

import { Context, Service } from '@monolith/cordis'
import { proxyEnvironmentForChild } from '@monolith/http-proxy'
import { MONOLITH_ENV_PREFIX } from './types.ts'
import type { SubprocessHandle, SubprocessSpawnSpec } from './types.ts'
import type { SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from './types.ts'

export { MONOLITH_ENV_PREFIX } from './types.ts'
export type {
  CollectedOutput,
  MonolithEnvironment,
  MonolithEnvironmentKey,
  SubprocessCollect,
 ...
 * `MONOLITH_*` names — the canonical base every harness child starts from. `PATH`,
 ...
 * credential or current `MONOLITH_*` fact goes through the spec's explicit `env`,
 * which merges after this scrub). Both scrubs match case-insensitively:
 * Windows environment names are case-insensitive, so a parent `monolith_*` entry
 * would otherwise survive and read back as `$env:MONOLITH_*` in the child;
 * deliberate lowercase `monolith_*` names on POSIX are implausible. Exported as a plain function so spawners
...
export function scrubbedParentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (value !== undefined && !SENSITIVE_ENV_PATTERN.test(key) && !key.toUpperCase().startsWith(MONOLITH_ENV_PREFIX)) env[key] = value
  }
