...
import { scrubbedParentEnv, SubprocessRuntime } from '@monolith/subprocess'
import type {
  SubprocessHandle,
...
describe('SubprocessRuntime seam', () => {
  ...
  it('scrubbedParentEnv drops credential-shaped and MONOLITH_ names (case-insensitively) but keeps PATH', () => {
    process.env.MONOLITH_SCRUB_PROBE = 'stale'
    process.env.monolith_scrub_probe_lower = 'stale'
    process.env.SCRUB_PROBE_TOKEN = 'secret'
    process.env.SCRUB_PROBE_PASSWORD = 'secret'
    process.env.SCRUB_PROBE_PLAIN = 'visible'
    try {
      const env = scrubbedParentEnv()
      expect(env.MONOLITH_SCRUB_PROBE).toBeUndefined()
      expect(env.monolith_scrub_probe_lower).toBeUndefined()
      ...
      delete process.env.MONOLITH_SCRUB_PROBE
      delete process.env.monolith_scrub_probe_lower
