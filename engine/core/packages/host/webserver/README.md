...
- [Generated configuration catalog](../../../docs/config-catalog.md#monolith-host-webserver) — every accepted config field and its source declaration.
...
- **No server-wide TLS, authentication, or origin policy** — route owners such as `monolith-client-connection` enforce their own request policy. Binding a non-loopback address still exposes unprotected routes and static assets to that network.
