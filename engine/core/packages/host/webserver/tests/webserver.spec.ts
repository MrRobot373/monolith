...
async function loadComposition(port = 0, gzip = false): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'monolith-webserver-loader-'))
  ...
  it('collects injection rows fresh per render and layers taps over the rendered rows', { timeout: 60_000 }, async () => {
    ...
    loaded.on('webserver/index-inject', (table) => {
      table.push(
        ...
        { kind: 'global', name: '__MONOLITH_BOOT__', value: { rev: '</script><b>' } },
    ...
    const order = [
      ...
      'globalThis["__MONOLITH_BOOT__"] = {"rev":"\\u003c/script>\\u003cb>"}',
    ...
    expect(renderIndexInjections('<main>x</main>', [
      ...
      + '<script>(globalThis.__MONOLITH_BOOT_READY__ ??= Promise.withResolvers()).resolve()</script>')
  })
