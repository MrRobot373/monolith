...
import * as Connection from '@monolith/client-connection'
import LocalCredentials from '@monolith/credentials-local'
...
async function loadComposition(): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'monolith-frontend-static-'))
  ...
  await writeFile(configPath, [
    ...
    "    host: '127.0.0.1'",
    '    port: 0',
    "- name: '@monolith/client-connection'",
    '- id: frontend',
  ...
  const modules = new Map<string, unknown>([
    ['@monolith/credentials-local', LocalCredentials],
    ['@monolith/host-webserver', HttpServer],
    ['@monolith/client-connection', Connection],
    ['@monolith/host-frontend-static', FrontendStatic],
  ])
...
describe('real Loader composition', () => {
  it('serves explicit index entries and files while preserving HTTP error semantics', { timeout: 60_000 }, async () => {
    ...
    expect(await request(port, '/')).toMatchObject({
      ...
      body: 'monolith web authentication required; reopen the URL printed by monolith web.\n',
    })
