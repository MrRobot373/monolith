...
# @monolith/host-plugin-inventory
...
- **Presets appear only with a roster** — a deployment without `monolith-agent-presets` serves Loader entries alone; the `agentPresets` field is absent rather than empty.
