...
# @monolith/tool-cordis

## Summary

`monolith-tool-cordis` gives the model seven tools over the live Cordis runtime of the current MONOLITH process: inspect what is loaded and what a dynamic package may use, define a package with a host half, a browser half, or both, run it, stop it, and remove it. Packages are versioned — a plugin holds immutable package versions, and the model can append a corrected package and update to it after a failure. Definitions live only in process memory and vanish on MONOLITH restart; nothing here writes repository files, installs packages, or changes `cordis.yml`. It also adds a system-prompt section that teaches the workflow; compose it with `@monolith/cordis-host-runner`, the package that runs the sandbox and the run round trip.
...
```yaml
- name: '@monolith/cordis-host-runner'
  config:
    vmTimeoutMs: 5000
- name: '@monolith/tool-cordis'
```
...
Definitions are session-scoped and process-local: a package is visible and controllable only in the session that defined it, stays active across later turns, and can affect other sessions in the same process while running. Stopping, removing, unloading the toolset, or restarting MONOLITH clears it. The sandbox isolates globals but is not a security boundary — treat a dynamic package like bash access, and load this plugin as deliberately as you would grant one.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#monolith-tool-cordis) — the exact schemas the model receives.
...
The conversation model sees the generated [`cordis_inspect_list`, `cordis_inspect_query`, `cordis_inspect_self`, `cordis_define`, `cordis_run`, `cordis_stop`, and `cordis_undefine` schemas](../../../docs/tool-catalog.md#monolith-tool-cordis) whenever this plugin is visible.
...
Dynamic Cordis plugins temporarily extend the current MONOLITH process. A Plugin uses apply(ctx) to consume Services, listen to Events, provide Services, register model Tools, or register browser UI in Slots.
```
