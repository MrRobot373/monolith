...
import type { ShellRunResult, CollectedOutput } from '@monolith/shell'
import SessionProjectionRegistry from '@monolith/session-projection'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@monolith/sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@monolith/sandbox'
import { SandboxPolicyService } from '@monolith/sandbox-policy'
import { SandboxBashExecutor } from '@monolith/bash-sandbox'
...
import type { Config } from '@monolith/bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'monolith-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'MONOLITH_WRAP=1', 'bash', '-c', 'printf "%s" "$MONOLITH_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$MONOLITH_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'monolith-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.MONOLITH_ORDER_FILE, "runner\\n");',
    ...
    try {
      const result = await bash.run(bash.resolve({
        command: 'true',
        env: { BASH_ENV: hook },
        monolithEnv: { MONOLITH_ORDER_FILE: order },
      }))
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'monolith-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const lockedDir = join(mkdtempSync(join(tmpdir(), 'monolith-sandbox-denied-')), 'locked')
    mkdirSync(lockedDir)
