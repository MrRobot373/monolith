...
import type { ConfinedArgv, RunnerFailureRule, SandboxExecutionPolicy, SandboxPolicy } from '@monolith/sandbox'
...
const spillDir = mkdtempSync(join(tmpdir(), 'monolith-pwsh-sandbox-spec-'))
...
describe('helpers (pure)', () => {
  const workdir = mkdtempSync(join(tmpdir(), 'monolith-pwsh-sandbox-helpers-'))
  afterAll(() => {
...
describe.skipIf(!pwshAvailable())('SandboxPwshExecutor', () => {
  ...
  const readOnlyDir = mkdtempSync(join(tmpdir(), 'monolith-pwsh-sandbox-ro-'))
