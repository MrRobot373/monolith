 ...
 *
 * @module @monolith/pwsh-local
 */

/* jscpd:ignore-start -- this executor mirrors monolith-bash-local call-for-call by
...
import { SHELL_SETTINGS_NAMESPACE, ShellExecutor } from '@monolith/shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult, CollectedOutput } from '@monolith/shell'
import type { SubprocessCollect, SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@monolith/subprocess'
import type {} from '@monolith/settings'
import { clampTimeout, deadline, MAX_TIMER_DELAY_MS, timeoutOf } from '@monolith/timeout'
/* jscpd:ignore-end */
import { resolvePwshPath } from './resolve.ts'

/* jscpd:ignore-start -- deliberate call-for-call mirror of monolith-bash-local (Agent Note: pwsh-tool-and-executor). */
/**
...
export class PwshLocalExecutor extends ShellExecutor {
   ...
   * `monolith-bash-local`'s `runArgv`/`startArgv` hooks; see
  ...
  private spawnSpec(
    ...
    return {
      ...
      signal,
      env: { ...ENV_OVERRIDES, ...spec.env, ...spec.monolithEnv },
    }
  }
   ...
   * `monolith-bash-local` (whose sandboxing subclass consumes the same hook); the
   * pwsh-confining consumer is `@monolith/pwsh-sandbox`.
