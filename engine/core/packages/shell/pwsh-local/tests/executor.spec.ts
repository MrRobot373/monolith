/**
 * Real-process tests for `@monolith/pwsh-local`: the LOCAL subprocess
...
import { PwshLocalExecutor, ENCODING_PREAMBLE, candidatePwshPaths, resolvePwshPath } from '@monolith/pwsh-local'
import LocalSubprocessRuntime from '@monolith/subprocess-local'
import SubprocessRuntime from '@monolith/subprocess'
import type { SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@monolith/subprocess'
import { MAX_TIMER_DELAY_MS } from '@monolith/timeout'
import type { ShellProcess } from '@monolith/shell'

const spillDir = mkdtempSync(join(tmpdir(), 'monolith-pwsh-exec-spec-'))
...
describe('resolvePwshPath and candidatePwshPaths (pure, every platform)', () => {
  ...
  it('returns the first EXISTING win32 candidate, else pwsh', () => {
    const dir = mkdtempSync(join(tmpdir(), 'monolith-pwsh-resolve-'))
  ...
  it('accepts a link-shaped PATH candidate whose target cannot be stat-ed', () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'monolith-pwsh-resolve-link-'))
    const store = join(dir, 'store')
    mkdirSync(store, { recursive: true })
  ...
  it('skips a directory candidate and falls through to the PATH-resolution default', () => {
    const dir = mkdtempSync(join(tmpdir(), 'monolith-pwsh-resolve-dir-'))
    const store = join(dir, 'store')
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.run', () => {
  ...
  it('uses config cwd, overridable per call', async () => {
    const first = mkdtempSync(join(tmpdir(), 'monolith-pwsh-cwd-a-'))
    const second = mkdtempSync(join(tmpdir(), 'monolith-pwsh-cwd-b-'))
  ...
  it('rejects on spawn failure (bad workdir)', async () => {
    const { bash } = await setup()
    await expect(bash.run(bash.resolve({ command: 'Write-Output ok', workdir: '/nonexistent-monolith' }))).rejects.toThrow(/ENOENT/)
  })

  it('resolve() carries stdin/env/monolithEnv onto the spec, and run() threads them to the command', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:SEAM_VAR][$env:MONOLITH_SEAM_VAR]"',
      stdin: 'piped\n',
      env: { SEAM_VAR: 'env-ok' },
      monolithEnv: { MONOLITH_SEAM_VAR: 'monolith-ok' },
    })
    ...
    expect(spec.monolithEnv).toEqual({ MONOLITH_SEAM_VAR: 'monolith-ok' })
    const result = await bash.run(spec)
    expect(lf(result.stdout.text)).toBe('piped\n[env-ok][monolith-ok]\n')
  })

  it('resolve() omits stdin/env/monolithEnv when the request supplies none', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({ command: 'Write-Output ok' })
    expect('stdin' in spec).toBe(false)
    expect('env' in spec).toBe(false)
    expect('monolithEnv' in spec).toBe(false)
  })
})
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.start (background process handles)', () => {
  ...
  it('threads stdin and extra env into a background process', async () => {
    ...
    const proc = bash.start(bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:BG_VAR][$env:MONOLITH_BG_VAR]"',
      stdin: 'bg-stdin\n',
      env: { BG_VAR: 'bg-env' },
      monolithEnv: { MONOLITH_BG_VAR: 'bg-monolith-env' },
    }))
    const partialOutput = await readUntil(proc, '[bg-env][bg-monolith-env]')
    await proc.done
    const output = partialOutput + lf(proc.readOutput().delta)
    expect(output).toBe('bg-stdin\n[bg-env][bg-monolith-env]\n')
    expect(proc.exitCode).toBe(0)
  })
  ...
  it('a background spawn failure settles as killed with the error readable on stderr', async () => {
    const { bash } = await setup()
    const proc = bash.start(bash.resolve({ command: 'Write-Output ok', workdir: '/nonexistent-monolith' }))
