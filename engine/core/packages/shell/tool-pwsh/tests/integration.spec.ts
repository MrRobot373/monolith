/**
 * Integration tests: the REAL `@monolith/pwsh-local` executor plus the
...
import ToolRuntime, { TOOL_ABORTED } from '@monolith/tools'
...
import { PwshLocalExecutor, resolvePwshPath } from '@monolith/pwsh-local'
...
describe.skipIf(!hasPwsh)('pwsh tool over the real pwsh executor', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'monolith-tool-pwsh-'))
