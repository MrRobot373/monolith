 ...
 * managed `MONOLITH_*` collection, abort translation, canonical result projection,
...
import SystemPrompt, { renderPrompt } from '@monolith/system-prompt'
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from '@monolith/tools'
...
import { SessionId, SessionLogOffset, SessionSeq } from '@monolith/session'
import ApprovalService from '@monolith/user-approval'
import type { ApprovalOutcome } from '@monolith/user-approval'
import { ShellExecutor } from '@monolith/shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellRunResult } from '@monolith/shell'
import SessionProjectionRegistry from '@monolith/session-projection'
import { turnBoundaryProjectionDefinition } from '@monolith/agent-loop'
...
import type { ShellProcessRead } from '@monolith/shell'
...
class FakeBash extends ShellExecutor {
  ...
  override resolve(request: ShellExecRequest): ShellExecSpec {
    ...
    return {
      ...
      ...request.monolithEnv !== undefined ? { monolithEnv: request.monolithEnv } : {},
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, monolithHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
  await ctx.plugin(BashEnvPlugin, monolithHome === undefined ? {} : { monolithHome })
...
async function setupWithTasks(toolConfig: Partial<ToolPwsh.Config> = {}, monolithHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed MONOLITH_* environment', async () => {
    const monolithHome = mkdtempSync(join(tmpdir(), 'monolith-tool-pwsh-home-'))
    const { ctx, bash } = await setup({}, monolithHome)
    ...
    expect(request?.monolithEnv).toEqual({
      MONOLITH_HOME: monolithHome,
      MONOLITH_SHELL: '1',
      MONOLITH_SESSION_ID: 'session-1',
    })
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    const monolithEnv = bash.requests[0]?.monolithEnv
    expect(monolithEnv).toBeDefined()
    expect(monolithEnv?.['MONOLITH_SHELL']).toBe('1')
    expect(monolithEnv?.['MONOLITH_HOME']).toEqual(expect.any(String))
    expect(monolithEnv).not.toHaveProperty('MONOLITH_SESSION_ID')
  })
...
describe('per-call sandbox policy resolution', () => {
  it('stamps the CALLING SESSION\'s resolved policy onto the request (session cwd, not the server launch dir)', async () => {
    ...
    const sessionCwd = mkdtempSync(join(tmpdir(), 'monolith-tool-pwsh-policy-'))
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @monolith/jobs and @monolith/tool-jobs')
  })
