 ...
 * `@monolith/pwsh-local`) backs `ctx.shell`; the tool contract is
 * PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables.
 *
 * Behavior mirrors `monolith-tool-bash` call-for-call: foreground and
 * `run_in_background` execution (background handles register with the
 * generic `ctx.jobs` runtime), the managed `MONOLITH_*` environment through the
 ...
 * exit-status parse from `@monolith/shell`.
 *
 * @module @monolith/tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@monolith/cordis'
import z from '@monolith/schemastery'
import { defineTool, TOOL_ABORTED } from '@monolith/tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@monolith/tools'
import { HarnessError } from '@monolith/llm'
import type { Agent } from '@monolith/agent'
import type {} from '@monolith/jobs'
import type {} from '@monolith/shell-env'
import type {} from '@monolith/user-approval'
import type { SandboxExecutionPolicy, SandboxMode } from '@monolith/sandbox'
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@monolith/sandbox'
import type { SandboxPolicyService } from '@monolith/sandbox-policy'
import type { ShellRunResult } from '@monolith/shell'
import { parseExitStatus } from '@monolith/shell'
import { processOutcome } from './background.ts'
import { renderPwshProcessRead, renderPwshResult } from './render.ts'
import type { RenderablePwshResult } from './render.ts'

declare module '@monolith/jobs' {
...
/* jscpd:ignore-start -- minimal mirror of monolith-tool-bash's validation and execute plumbing (Agent Note). */
...
function pwshDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a PowerShell command (`pwsh -Command`) and return its stdout/stderr. '
    ...
    + 'Current harness environment facts are exposed through managed `$env:MONOLITH_*` variables; inspect them when needed. '
...
function canonicalPwshResult(result: ShellRunResult): PwshForegroundResult {
  ...
  return {
    ...
    /* jscpd:ignore-start -- the canonical projection and background-handle shape mirror monolith-tool-bash's by design (Agent Note). */
...
/* jscpd:ignore-start -- deliberate mirror of monolith-tool-bash's apply() preamble (pwsh-tool-and-executor Agent Note). */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  /* jscpd:ignore-start -- deliberate mirror of monolith-tool-bash's escalation resolver (pwsh-tool-and-executor Agent Note). */
  /**
  ...
  ctx.tools.register(defineTool({
    ...
    /* jscpd:ignore-start -- deliberate mirror of monolith-tool-bash's parameter surface (pwsh-tool-and-executor Agent Note). */
    parameters: {
    ...
    /* jscpd:ignore-end */
    output: {
      // The foreground result wire shape mirrors monolith-tool-bash's by contract —
      // consumers of one must accept the other (see the pwsh-tool-and-executor
      // Agent Note).
      /* jscpd:ignore-start -- deliberate result-schema symmetry with monolith-tool-bash. */
      schema: {
        oneOf: [
          {
            type: 'object',
    ...
    /* jscpd:ignore-start -- the execute path mirrors monolith-tool-bash's by design (see the pwsh-tool-and-executor Agent Note). */
    async execute(args: PwshToolArgs, exec) {
      ...
      const request = {
        ...
        monolithEnv: ctx.shellEnv.collect(exec),
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @monolith/jobs and @monolith/tool-jobs')
        }
