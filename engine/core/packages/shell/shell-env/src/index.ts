 ...
 * trusted, per-execution `MONOLITH_*` variables consumed by the model-facing shell
 * tools (`monolith-tool-bash`, `monolith-tool-pwsh`). Built-in shell facts are owned by
 * the registry itself while plugins can register additional, enumerable facts
 * with effect-scoped disposal.
 *
 * @module @monolith/shell-env
 */

import { Service, type Context } from '@monolith/cordis'
import z from '@monolith/schemastery'
import { MONOLITH_ENV_PREFIX } from '@monolith/shell'
import type { MonolithEnvironment, MonolithEnvironmentKey } from '@monolith/shell'
import { MONOLITH_HOME_ENV, resolveMonolithHome } from '@monolith/home-paths'
...
export interface Config {
  /** MONOLITH home directory exposed as `MONOLITH_HOME`; defaults to `$MONOLITH_HOME` or `~/.monolith`. */
  monolithHome?: string
}

/** Runtime configuration schema for the shell-env plugin. */
export const Config: z<Config> = z.object({
  monolithHome: z.string(),
})

/** Model-visible metadata for one managed `MONOLITH_*` environment variable. */
...
export interface BashEnvContributor {
  ...
  name: string
  /** Complete set of `MONOLITH_*` keys this contributor may return. */
  variables: Readonly<Record<MonolithEnvironmentKey, BashEnvVariable>>
  /**
   ...
   */
  resolve(execution: ToolExecution): Readonly<Partial<Record<MonolithEnvironmentKey, string>>>
}
...
export interface BashEnvVariableInfo extends BashEnvVariable {
  ...
  contributor: string
  /** Declared `MONOLITH_*` environment variable name. */
  key: MonolithEnvironmentKey
}

const MONOLITH_SHELL_KEY = `${MONOLITH_ENV_PREFIX}SHELL` as const
const MONOLITH_SESSION_ID_KEY = `${MONOLITH_ENV_PREFIX}SESSION_ID` as const
const RESERVED_BASH_ENV_KEYS = new Set<MonolithEnvironmentKey>([
  MONOLITH_HOME_ENV,
  MONOLITH_SHELL_KEY,
  MONOLITH_SESSION_ID_KEY,
])
const BASH_ENV_KEY_SUFFIX = /^[A-Z][A-Z0-9_]*$/

/**
 * Registry (`ctx.shellEnv`) for trusted, per-execution `MONOLITH_*` variables.
 * The namespace is rebuilt for every model shell call: ambient `MONOLITH_*` values
...
export class ShellEnvRegistry extends Service {
  ...
  private readonly keyOwners = new Map<MonolithEnvironmentKey, string>()
  private readonly monolithHome: string
  ...
  constructor(ctx: Context, config: Config = {}) {
    super(ctx, 'shellEnv')
    this.monolithHome = resolveMonolithHome(config.monolithHome)
  }
  ...
  register(contributor: BashEnvContributor): () => void {
    const dispose = this.ctx.effect(function* (this: ShellEnvRegistry) {
      ...
      const variables = Object.entries(contributor.variables) as [MonolithEnvironmentKey, BashEnvVariable][]
      for (const [key, variable] of variables) {
        if (!key.startsWith(MONOLITH_ENV_PREFIX)
          || !BASH_ENV_KEY_SUFFIX.test(key.slice(MONOLITH_ENV_PREFIX.length))) {
  ...
  collect(execution: ToolExecution): MonolithEnvironment {
    const values: Record<MonolithEnvironmentKey, string> = {
      [MONOLITH_HOME_ENV]: this.monolithHome,
      [MONOLITH_SHELL_KEY]: '1',
    }
    if (execution.agent !== undefined) {
      values[MONOLITH_SESSION_ID_KEY] = execution.agent.session.header.id
    }
    ...
    for (const contributor of [...this.contributors.values()].sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      for (const [rawKey, value] of Object.entries(resolved)) {
        const key = rawKey as MonolithEnvironmentKey
  ...
  list(): BashEnvVariableInfo[] {
    return [...this.contributors.values()]
      .flatMap(contributor => Object.entries(contributor.variables).map(([key, variable]) => ({
        ...
        key: key as MonolithEnvironmentKey,
      })))
