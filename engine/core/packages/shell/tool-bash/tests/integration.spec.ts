...
import { SessionId, type SessionEvent } from '@monolith/session'
 ...
 */
async function harness(adapter: MockAdapter, sessionRoot?: string, monolithHome?: string) {
...
describe('bash tool through the agent loop', () => {
  it('first-turn bash receives session identity in a scrubbed MONOLITH_* namespace', async () => {
    const root = mkdtempSync(join(tmpdir(), 'monolith-bash-session-env-'))
    dirs.push(root)
    const monolithHome = join(root, 'monolith-home')
    vi.stubEnv('MONOLITH_STALE_PARENT', 'stale')
    const adapter = new MockAdapter([
      toolCallResponse('call-1', 'bash', {
        command: 'printf \'%s\\n%s\\n%s\\n%s\\n\' "$MONOLITH_HOME" "$MONOLITH_SHELL" "$MONOLITH_SESSION_ID" "${MONOLITH_STALE_PARENT-unset}"',
        description: 'inspect session environment',
      }),
      textResponse('Session environment inspected.'),
    ])
    const ctx = await harness(adapter, root, monolithHome)
    ...
    expect(resultText(result)).toBe(`${monolithHome}\n1\nsession-env-id\nunset\n`)
  ...
  it('background: start ack → completion wakes the idle agent → job_output collects it', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'monolith-bg-'))
    dirs.push(dir)
