...
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult } from '@monolith/shell'
...
const spillDir = mkdtempSync(join(tmpdir(), 'monolith-tool-bash-spec-'))
...
describe('bash tool', () => {
  ...
  it('surfaces spawn failures as isError', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'bash', { command: 'true', description: 'test command', workdir: '/nonexistent-monolith' })
...
describe('the model-facing bash tool builds its request from named args only (no {...args} forward)', () => {
  const recordingMonolithHome = join(spillDir, 'monolith-home')
   ...
   * (the credential scrub in monolith-bash-local is the security control; see the
  ...
  async function setupRecording() {
    ...
    await ctx.plugin(LocalJobRegistry)
    await ctx.plugin(ToolTasks)
    await ctx.plugin(BashEnvPlugin, { monolithHome: recordingMonolithHome })
  ...
  it('describes the managed harness environment namespace to the model', async () => {
    ...
    expect(description).toContain('$MONOLITH_*')
  })
  ...
  it('injects built-ins and the stable session id into a foreground request', async () => {
    ...
    const ambient = process.env.MONOLITH_SESSION_ID
    ...
    expect(bash.requests[0]?.monolithEnv).toEqual({
      MONOLITH_HOME: recordingMonolithHome,
      MONOLITH_SESSION_ID: 'request-fg',
      MONOLITH_SHELL: '1',
    })
    expect(process.env.MONOLITH_SESSION_ID).toBe(ambient)
  })
  ...
  it('injects the same trusted variables into a background request without forwarding model env', async () => {
    ...
    await ctx.tools.execute({
      ...
      arguments: {
        ...
        env: { MONOLITH_SESSION_ID: 'spoofed' },
      },
      agent,
    })
    ...
    expect(bash.requests[0]?.monolithEnv).toEqual({
      MONOLITH_HOME: recordingMonolithHome,
      MONOLITH_SESSION_ID: 'request-bg',
      MONOLITH_SHELL: '1',
    })
  })
  ...
  it('keeps parent and child agent session environments isolated', async () => {
    ...
    expect(bash.requests.map(request => request.monolithEnv)).toEqual([
      {
        MONOLITH_HOME: recordingMonolithHome,
        MONOLITH_SESSION_ID: 'request-parent',
        MONOLITH_SHELL: '1',
      },
      {
        MONOLITH_HOME: recordingMonolithHome,
        MONOLITH_SESSION_ID: 'request-child',
        MONOLITH_SHELL: '1',
      },
    ])
  })
