...
`monolith-tool-bash` gives the agent a `bash` tool that runs commands through the mounted shell executor and returns stdout, stderr, and exit markers. Each call runs in a fresh shell — no cwd, variables, or functions survive — and `run_in_background` turns long-running commands into background jobs the agent collects with `job_output` and stops with `job_kill`. Every call runs with the managed `MONOLITH_*` environment from `monolith-shell-env`, and under a sandboxing executor a denied command may be retried once with a wider `sandbox_permissions` mode plus a `justification` through user approval. Non-zero exits are reported, not failed, so the agent decides how to react. Mount it together with an executor provider such as `monolith-bash-local` or `monolith-bash-sandbox` and the `monolith-shell-env` plugin.
...
Load this plugin in any composition where the agent should run bash commands: it registers the `bash` tool once an executor provider and the `monolith-shell-env` registry are mounted, and stays pending until the `tools`, `shell`, `systemPrompt`, and `shellEnv` services exist.
...
```yaml
- name: '@monolith/bash-local'
- name: '@monolith/shell-env'
- name: '@monolith/tool-bash'

# Optional: background jobs
- name: '@monolith/jobs-local'
- name: '@monolith/tool-jobs'
```
...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-tool-bash) is the exhaustive source for every accepted field and its JSDoc; the generated [tool catalog](../../../docs/tool-catalog.md#monolith-tool-bash) carries the full argument schema.
...
- **Request from named args only.** The tool never exposes `stdin`, `env`, or `stdoutMaxBytes`; it builds each request from command/workdir/timeout/signal fields plus the registry-collected `monolithEnv`, so model-supplied keys cannot replace managed values ([bash stdin/env Agent Note](../../../.agents/notes/implemented/architecture/2026-06-30-bash-stdin-env-trusted-plugin-api.md)).
...
- [shell-env](../shell-env/README.md) — the managed `MONOLITH_*` environment every call receives.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#monolith-tool-bash) — the exact `bash` argument schema.
- [Generated configuration catalog](../../../docs/config-catalog.md#monolith-tool-bash) — every accepted config field and its source declaration.
...
The model sees the generated [`bash` schema](../../../docs/tool-catalog.md#monolith-tool-bash). `run_in_background` appears only when this producer enables it; `sandbox_permissions` and `justification` appear only when the mounted executor advertises sandboxing. Agent-scoped tool restrictions can remove the definition for that agent.
