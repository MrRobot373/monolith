 ...
 * @module @monolith/tool-bash-persistent
 */
...
import type { TerminalReadResult, TerminalSessionId } from '@monolith/terminal'
...
function markers(): CommandMarkers {
  ...
  return {
    start: `__MONOLITH_PERSISTENT_BASH_START_${nonce}__`,
    end: `__MONOLITH_PERSISTENT_BASH_END_${nonce}:`,
  }
}
...
function wrapCommand(command: string, marker: CommandMarkers): string {
  ...
  return `printf '%s\\n' ${quoteForBash(marker.start)}; eval -- ${quoteForBash(command)}; __monolith_persistent_bash_status=$?; printf '%s%s\\n' ${quoteForBash(marker.end)} "$__monolith_persistent_bash_status"`
}
