 ...
 * `__monolith_main__`/`__builtins__`/`__name__` (the Python backend's bootstrap
...
export const RESERVED_BINDING_GLOBALS: ReadonlySet<string> = new Set([
  'console',
  '__monolith_main__', '__builtins__', '__name__', '__debug__',
])
...
export abstract class CodeRuntime extends Service {
   ...
   * `monolith-tools` presents; the TypeScript backend is released, the Python
