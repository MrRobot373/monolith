...
import type { StorageBackend } from '@monolith/storage'
import { DomainFacility } from '@monolith/storage-domain'
import type { DomainChanged } from '@monolith/storage-domain'
import SessionStore, { SessionId } from '@monolith/session'
import type { SessionHeader } from '@monolith/session'
import { SessionPersistenceRevision } from '@monolith/session-persistence'
import type { SessionPersistenceSnapshot } from '@monolith/session-persistence'
...
async function makeDir(name: string): Promise<string> {
  base ??= await realpath(await mkdtemp(join(tmpdir(), 'monolith-workspace-')))
