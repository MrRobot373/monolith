...
- **DeepSeek requests** — every provider request carries the `x-monolith-user-id` header, so usage can be attributed per installation.

### Observing and resetting the id

The id lives in `$MONOLITH_HOME/.anonymous-user-id` (`~/.monolith` by default) as a plain UUID text file. Delete that file to get a fresh id at the next launch; the running process keeps its current id until it exits. Separate harness homes keep separate ids, and no machine or account detail ever goes into the value.
...
- [monolith-home-paths](../../util/home-paths/README.md) — owns `$MONOLITH_HOME` and `~/.monolith` resolution.
- [monolith-session-telemetry-otel](../../session/session-telemetry-otel/README.md) — reports the id as the OTel Resource `user.id`.
- [monolith-command-feedback](../../feedback/command-feedback/README.md) — embeds the id in the feedback acknowledgement.
- [monolith-llm-deepseek](../../llm/llm-deepseek/README.md) — sends `x-monolith-user-id` on provider requests.
...
- **No cross-home identity** — different `$MONOLITH_HOME` values cannot be correlated.
- **Configured DeepSeek gateways receive the id** — `monolith-llm-deepseek` sends the stable header to its resolved `baseURL`, including deployment overrides, independently of telemetry sharing mode.
