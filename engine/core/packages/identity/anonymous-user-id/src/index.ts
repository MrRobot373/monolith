 ...
 * harness home resolved by {@link resolveMonolithHome} (`$MONOLITH_HOME` > `~/.monolith`),
 ...
 * machine: every process sharing one `$MONOLITH_HOME` reports the same id, and
 ...
 *
 * @module @monolith/anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `MONOLITH_HOME`; defaults to `process.env`. */
...
export function getOrCreateAnonymousUserId(options: AnonymousUserIdOptions = {}): AnonymousUserId {
  const file = join(resolveMonolithHome(undefined, options.env ?? process.env), ANONYMOUS_USER_ID_FILE_NAME)
