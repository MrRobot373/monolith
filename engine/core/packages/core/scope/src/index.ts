 ...
 *
 * @module @monolith/scope
 */

import type { Context, Fiber } from '@monolith/cordis'
import { Context as CordisContext } from '@monolith/cordis'
...
const kScope = Symbol('monolith.scope')
...
function linkScopeParent(key: ScopeKey, parent: ScopeKey): void {
  for (let cursor: ScopeKey | undefined = parent; cursor !== undefined; cursor = scopeParents.get(cursor)) {
    if (cursor === key) throw new Error('monolith-scope: scope parent link would form a cycle')
  }
...
export function bindScopeParent(key: ScopeKey, parent: ScopeKey): ScopeParentBinding {
  if (scopeParents.has(key)) {
    throw new Error('monolith-scope: scope key is already bound to a parent; re-linking requires the binding returned by the original bind')
  }
