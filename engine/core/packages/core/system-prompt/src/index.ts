 ...
 *
 * @module @monolith/system-prompt
 */

import { Context, Service } from '@monolith/cordis'
import z from '@monolith/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from '@monolith/scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@monolith/scope'
import type { ContextSnapshotSection, ToolSchema } from '@monolith/llm'
...
declare module '@monolith/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@monolith/scope`): scoped listeners
...
export interface Config {
  /** Include the fixed MONOLITH identity before the deployment persona (default true). */
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        text: 'You are an AI agent powered by MONOLITH.',
      })
    }
    this.section({
