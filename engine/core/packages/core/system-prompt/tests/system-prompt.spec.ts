...
} from '@monolith/system-prompt'
import type { PromptContextOrderName, PromptSectionOrderName } from '@monolith/system-prompt'
...
const IDENTITY = 'You are an AI agent powered by MONOLITH.'
...
describe('SystemPrompt', () => {
  ...
  describe('built-in sections', () => {
    it('registers the harness identity and the configured deployment persona', async () => {
      const ctx = new Context()
      await ctx.plugin(SystemPrompt, { persona: 'You are MONOLITH.' })
  ...
  it('assembles sections in order with context-resolved text and collected tools', async () => {
    ...
    expect(assembly.sections.map(s => s.text)).toEqual([IDENTITY, 'You are MONOLITH.', 'Be precise.', 'cwd: /tmp'])
    expect(assembly.contexts).toEqual([
