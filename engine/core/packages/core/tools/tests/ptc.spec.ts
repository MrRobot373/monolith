...
import type { Scope } from '@monolith/scope'
...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@monolith/tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@monolith/tools'
import type { Agent } from '@monolith/agent'
import { Session, SessionId } from '@monolith/session'
import type { SessionEventMap } from '@monolith/session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@monolith/scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
    ...
    // `monolith-agent-tool-presentation` produces.
