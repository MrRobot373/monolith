 ...
 * @module @monolith/tools/src/ptc
 */

import { brandString } from '@monolith/brand'
import { createUserMessage, HarnessError } from '@monolith/llm'
import type { ContentBlock, ToolCallId } from '@monolith/llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@monolith/code-runtime'
import { snapshotJsonValue, type JsonValue } from '@monolith/util-values'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`monolith-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}
