 ...
 * execution pipeline.
 * @module @monolith/tools
 */

import { Context, Service } from '@monolith/cordis'
import z from '@monolith/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@monolith/scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@monolith/scope'
import type { ToolCallId, ContentBlock, ToolSchema } from '@monolith/llm'
import { HarnessError } from '@monolith/llm'
import type { Agent } from '@monolith/agent'
import type { UserMessage } from '@monolith/session'
import { assertNever, deepFreeze, snapshotJsonValue, type JsonValue } from '@monolith/util-values'
import type { ToolProviderResult } from '@monolith/system-prompt'
import type { CodeRuntime } from '@monolith/code-runtime'
 ...
 * instead of deriving them: the seam's `monolith-code-runtime` README pair, its
...
// lives in its own UI-facing module; re-export it so `@monolith/tools`
...
declare module '@monolith/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@monolith/scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@monolith/scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@monolith/scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@monolith/tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@monolith/tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `monolith-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`monolith-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`monolith-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @monolith/code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`monolith-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `monolith-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('monolith.tool.execution') as ToolExecutionToken
}
