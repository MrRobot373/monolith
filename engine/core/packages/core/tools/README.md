...
Mount `monolith-tools` wherever agents call tools: it provides `ctx.tools`, the registry every tool plugin registers into and the loop dispatches through. Registering a tool is enough to make it visible — the registry feeds its schemas into the system-prompt assembly automatically.
...
import { defineTool } from '@monolith/tools'
...
- [Generated tool catalog](../../../docs/tool-catalog.md#monolith-tools) — the shipped tools' schemas the model receives.
...
In normal mode the model sees each visible definition's exact name, description, and JSON schema; the shipped definitions are recorded in the generated [tool catalog](../../../docs/tool-catalog.md#monolith-tools). Agent-scoped restrictions, shadows, and extension registrations change that agent's end-tool set.
...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@monolith/tool-call-timeout-policy` wrapper.
