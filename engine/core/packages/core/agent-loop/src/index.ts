 ...
 *
 * @module @monolith/agent-loop
 */

import { Context, FiberState, Service } from '@monolith/cordis'
import { randomUUID } from 'node:crypto'
import z from '@monolith/schemastery'
import { z as zod } from 'zod'
import { brandString } from '@monolith/brand'
import { emitAgentEvent } from '@monolith/agent'
import type {
  Agent,
  AgentFactory,
  AgentHandle,
...
import { errorChain, ReasoningEffortId } from '@monolith/llm'
import type {} from '@monolith/settings'
import { interruptedTurnClosers, SessionLogOffset, SessionPreparation, SessionSeq } from '@monolith/session'
import type { Session, SessionHeader, SessionId } from '@monolith/session'
...
import { SessionPersistenceNotFoundError } from '@monolith/session-persistence'
import type { SessionHandle, SessionPersistence } from '@monolith/session-persistence'
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  async resume(ownerCtx: Context, options: ResumeAgentOptions): Promise<AgentHandle> {
    const persistence = this.runtime.ctx.get('sessionPersistence')
    if (persistence === undefined) {
      throw new Error('cannot resume: session persistence is not configured (load a monolith-session-persistence backend)')
    }
