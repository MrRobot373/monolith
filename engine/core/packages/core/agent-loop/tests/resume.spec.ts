import { ToolCallId, createMessage, createUserMessage } from '@monolith/llm'
...
import SessionStore, { SessionLogOffset, SessionSeq, Session, SessionId, TOOL_OUTCOME_UNKNOWN } from '@monolith/session'
...
async function persistentHarness(adapter: MockAdapter): Promise<{ ctx: Context; root: string }> {
  const root = await mkdtemp(join(tmpdir(), 'monolith-resume-'))
  dirs.push(root)
...
describe('the session-persistence Agent Note: AgentLoop factory create/resume', () => {
  ...
  it('resume over a torn physical tail continues from the committed prefix', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'monolith-resume-torn-'))
    dirs.push(root)
