 ...
 *
 * @module @monolith/agent
 */

import { Context, FiberState, getTraceable, Service, symbols } from '@monolith/cordis'
import type { Fiber } from '@monolith/cordis'
import { AsyncLocalStorage } from 'node:async_hooks'
import { isPromise } from 'node:util/types'
import { scopeTarget } from '@monolith/scope'
import type { Scoped } from '@monolith/scope'
import type { SessionEvent, SessionId, SessionLogOffset } from '@monolith/session'
...
declare module '@monolith/cordis' {
  interface Context {
     ...
     * `monolith-scope` tag while retaining it, so this field is DX context rather
...
export interface CreateAgentOptions {
   ...
   * {@link CreateSessionOptions.meta} in monolith-session (the internal-only
 ...
 * via {@link AgentRegistry.setFactory}. Kept on the `monolith-agent` interface so
 * consumers (e.g. the ACP bridge) program against `ctx.agents` without
 * depending on the concrete `monolith-agent-loop` package.
 */
 ...
 * (`@monolith/agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: '@monolith/agent#Agent',
        wireTypeSymbol: '@monolith/session/types#SessionId',
