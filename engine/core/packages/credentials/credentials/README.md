...
The local store README owns the full configuration surface; the generated [configuration catalog](../../../docs/config-catalog.md#monolith-credentials-local) is the exhaustive field list.
...
```ts
import type { Context } from '@monolith/cordis'
import { credentialKey } from '@monolith/credentials'
...
- **A key the launching environment supplies cannot be overwritten** — `DEEPSEEK_API_KEY=… monolith` (or a CI secret, a container `-e`) wins for this run and is reported read-only; clear the variable in the launching shell before storing a different value.
