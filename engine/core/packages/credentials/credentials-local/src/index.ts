/**
 * File-backed credentials provider over `$MONOLITH_HOME/.credentials.yaml`, layered
 * against the environment by how much each layer is trusted:
 *
 * ```text
 * inherited process environment      (read-only, wins)
 * > $MONOLITH_HOME/.credentials.yaml      (provider-managed, writable)
 * > <invocation cwd>/.env            (read-only fallback)
 * > $MONOLITH_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `DEEPSEEK_API_KEY=… monolith`, a CI
 ...
 * @module @monolith/credentials-local
 */
...
import { withFileLock, writeFileAtomic } from '@monolith/atomic-write'
import { canonicalizeWatchPath, resolveMonolithHome } from '@monolith/home-paths'
import { launchEnvironmentOf } from '@monolith/launch-environment'
import { CredentialProvider, credentialRef, parseCredentialKey } from '@monolith/credentials'
import type {
  ApiKeyRecord,
  CredentialInfo,
  CredentialKey,
  ...
  CredentialRef,
  ResolvedCredential,
} from '@monolith/credentials'
import type { LaunchEnvironmentEntry } from '@monolith/launch-environment'
...
export interface Config {
  ...
  path?: string
  /** MONOLITH home used when `path` is omitted; defaults to `$MONOLITH_HOME` or `~/.monolith`. */
  monolithHome?: string
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveMonolithHome(config.monolithHome), CREDENTIALS_FILENAME)),
 ...
 * holds it. Like the retry cadence in `monolith-atomic-write`, this is a
...
/** File-backed credentials provider (`$MONOLITH_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start monolith from instead',
      )
    }
  }
