...
import { credentialRef } from '@monolith/credentials'
import { createLaunchEnvironmentSnapshot, MONOLITH_LAUNCH_ENVIRONMENT_KEY } from '@monolith/launch-environment'
import type { CredentialRef } from '@monolith/credentials'
...
const KEY = credentialRef('MONOLITH_CRED_TEST')
const OTHER = credentialRef('MONOLITH_CRED_OTHER')
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'monolith-credentials-local-'))
...
describe('resolveSpec', () => {
  it('defaults to .credentials.yaml under the harness home with watching on', () => {
    const spec = resolveSpec({ monolithHome: '/custom/home' })
  ...
  it('lets an explicit path win over the home', () => {
    const spec = resolveSpec({ path: '/etc/monolith/creds.yaml', monolithHome: '/ignored', watch: false, debounceMs: 5 })
    expect(spec).toEqual({ filename: resolve('/etc/monolith/creds.yaml'), watch: false, debounceMs: 5 })
  })
})
...
describe('layering and reads', () => {
  ...
  it('serves file entries alongside comments and quoted values', async () => {
    ...
    await writeCredentials(path, 'version: 1\nrefs:\n  # notes\n  MONOLITH_CRED_TEST: plain\n  MONOLITH_CRED_OTHER: "with space"\n')
  ...
  it('lets a non-empty process environment win read-only over the file', async () => {
    const dir = await tempDir()
    const path = join(dir, '.credentials.yaml')
    await writeCredentials(path, 'version: 1\nrefs:\n  MONOLITH_CRED_TEST: from-file\n')
    const ctx = await boot({ path, watch: false })
    vi.stubEnv('MONOLITH_CRED_TEST', 'from-env')
  ...
  it('treats an empty environment value as absent, falling through to the file', async () => {
    const dir = await tempDir()
    const path = join(dir, '.credentials.yaml')
    await writeCredentials(path, 'version: 1\nrefs:\n  MONOLITH_CRED_TEST: stored\n')
    const ctx = await boot({ path, watch: false })
    vi.stubEnv('MONOLITH_CRED_TEST', '')
...
describe('layer ladder', () => {
  // inherited process env > .credentials.yaml > $MONOLITH_HOME/.env, and the
  ...
  async function bootLayered(
    ...
    const ctx = new Context()
    ctx.provide(MONOLITH_LAUNCH_ENVIRONMENT_KEY, createLaunchEnvironmentSnapshot(layers))
  ...
  it('lets the stored value beat the user .env, so a UI write takes effect immediately', async () => {
    ...
    const ctx = await bootLayered(path, [
      ...
      { source: 'user-env', path: '/home/.monolith/.env', values: { MONOLITH_CRED_TEST: 'older-user-env' } },
    ])
    expect(await ctx.credentials.resolve(KEY)).toEqual({ value: 'stored', source: 'file' })
  ...
  it('serves the user .env only when nothing is stored', async () => {
    ...
    const ctx = await bootLayered(join(dir, '.credentials.yaml'), [
      { source: 'process', values: {} },
      { source: 'user-env', path: '/home/.monolith/.env', values: { MONOLITH_CRED_TEST: 'from-user-env' } },
    ])
  ...
  it('serves the invoking project .env over the user one, but never over the store', async () => {
    ...
    const layers = [
      ...
      { source: 'project-env' as const, path: '/work/.env', values: { MONOLITH_CRED_TEST: 'from-project' } },
      { source: 'user-env' as const, path: '/home/.monolith/.env', values: { MONOLITH_CRED_TEST: 'from-user' } },
    ]
  ...
  it('lets only the inherited environment shadow the store, read-only', async () => {
    ...
    await writeCredentials(path, 'version: 1\nrefs:\n  MONOLITH_CRED_TEST: stored\n')
    const ctx = await bootLayered(path, [
      { source: 'process', values: { MONOLITH_CRED_TEST: 'from-shell' } },
      { source: 'user-env', path: '/home/.monolith/.env', values: { MONOLITH_CRED_TEST: 'from-user-env' } },
    ])
...
describe('document validation', () => {
  ...
  it.each([
    ...
    ['a sequence root', '- MONOLITH_CRED_TEST\n', /must be a mapping/],
    ...
    ['the flat layout with a non-string value', 'MONOLITH_CRED_TEST: [nope]\n', /nest the existing 1 entry under/],
    ['the flat layout with several entries and a non-string value', 'MONOLITH_CRED_TEST: a\nMONOLITH_CRED_OTHER: [b]\n',
      /nest the existing 2 entries under/],
    ['the flat layout with an empty value', 'MONOLITH_CRED_TEST: ""\n', /pre-release flat layout/],
    ['the flat layout with an unaddressable key', 'not-a-ref: value\n', /pre-release flat layout/],
    ['the flat layout with a non-string key', '1: a\n', /pre-release flat layout/],
    ['the flat layout under document directives', '%YAML 1.2\n---\nMONOLITH_CRED_TEST: a\n',
      /pre-release flat layout/],
    ...
    ['duplicate keys', 'version: 1\nrefs:\n  MONOLITH_CRED_TEST: one\n  MONOLITH_CRED_TEST: two\n', /invalid document/],
    ['malformed yaml', 'MONOLITH_CRED_TEST: "unterminated\n', /invalid document/],
  ...
  it('never puts a credential value in a diagnostic', async () => {
    ...
    await writeCredentials(path, `MONOLITH_CRED_TEST: "${secret}\n`)
    let failure: unknown
    try {
...
describe('document writes', () => {
  ...
  it('patches one entry, preserving comments and every untouched entry', async () => {
    const dir = await tempDir()
    const path = join(dir, '.credentials.yaml')
    await writeCredentials(path, 'version: 1\nrefs:\n  # deployment notes\n  MONOLITH_CRED_OTHER: keep\n\n  # the one under edit\n  MONOLITH_CRED_TEST: old\n')
    const ctx = await boot({ path, watch: false })
    await ctx.credentials.set(KEY, 'new value!')
    expect(await readFile(path, 'utf8')).toBe(
      'version: 1\nrefs:\n  # deployment notes\n  MONOLITH_CRED_OTHER: keep\n\n'
      + '  # the one under edit\n  MONOLITH_CRED_TEST: new value!\n',
    )
  })
  ...
  it('unsets only the owning entry, with its own annotation, and keeps an absent unset silent', async () => {
    ...
    await writeCredentials(path, 'version: 1\nrefs:\n  # about the doomed one\n  MONOLITH_CRED_TEST: gone\n  # about the survivor\n  MONOLITH_CRED_OTHER: stays\n')
    const ctx = await boot({ path, watch: false })
    const seen = updates(ctx)
  ...
  it('rejects empty values and writes the environment would shadow', async () => {
    ...
    vi.stubEnv('MONOLITH_CRED_TEST', 'shadowing')
  ...
  it('fails a write loud when the on-disk document became invalid', async () => {
    ...
    await writeCredentials(path, 'MONOLITH_CRED_TEST: "unterminated\n')
...
describe('real hot reload', () => {
  it('publishes external edits, replaces the snapshot wholesale, and suppresses self-writes', async () => {
    ...
    await writeCredentials(path, 'version: 1\nrefs:\n  MONOLITH_CRED_TEST: boot\n')
    const ctx = await boot({ path, debounceMs: 10 })
    const seen = updates(ctx)

    await writeCredentials(path, 'version: 1\nrefs:\n  MONOLITH_CRED_TEST: live\n  MONOLITH_CRED_OTHER: extra\n')
