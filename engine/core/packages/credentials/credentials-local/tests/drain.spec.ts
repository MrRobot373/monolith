...
import { credentialKey, credentialRef } from '@monolith/credentials'
...
vi.mock('@monolith/atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@monolith/atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@monolith/atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
  mocked.__setGate(next)
}

const KEY = credentialRef('MONOLITH_CRED_DRAIN_A')
const OTHER = credentialRef('MONOLITH_CRED_DRAIN_B')
...
describe('write-drain teardown', () => {
  it('lets the in-flight write land and fails the queued one after disposal', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'monolith-credentials-drain-'))
  ...
  it('fails a queued record write after disposal on the same terms', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'monolith-credentials-drain-record-'))
    cleanups.push(() => rm(dir, { recursive: true, force: true }))
    const ctx = new Context()
    const fiber = ctx.plugin(LocalCredentialProvider, { path: join(dir, '.credentials.yaml'), watch: false })
    await fiber
