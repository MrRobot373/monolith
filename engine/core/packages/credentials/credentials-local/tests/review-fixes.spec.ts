...
const ALPHA = credentialRef('MONOLITH_REVIEW_ALPHA')
const BETA = credentialRef('MONOLITH_REVIEW_BETA')
const INNER = credentialRef('MONOLITH_REVIEW_INNER')
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'monolith-cred-review-'))
...
describe('contained update fan-out', () => {
  ...
  it('rethrows an invariant-coded failure after the commit and the remaining listeners', async () => {
    ...
    // fatal by design — but the write itself committed first.
    expect(second).toHaveBeenCalledWith(ALPHA)
