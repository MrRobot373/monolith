...
| `monolithHome` | `$MONOLITH_HOME` or `~/.monolith` | MONOLITH home used when `path` is omitted |
...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-credentials-local) is the exhaustive source for every accepted field and its JSDoc.
...
- **A key the launching environment supplies is read-only** — `DEEPSEEK_API_KEY=… monolith` wins for this run; saving or removing it is refused. Clear the variable in the launching shell first.
...
| — | No runtime invariant companion is published; the Service Definition companion (`monolith-credentials/invariant`) owns the `credentials/reference-updated` lifecycle contract; this provider's file/environment layering is asynchronous I/O pinned by its unit suite. |
...
- [App boot and MONOLITH-home layers](../../boot/app-boot/README.md) — how the product CLI loads `.env` into the snapshot and `process.env`.
...
- **Atomic, not crash-durable** — inherited from `monolith-atomic-write`; the store re-reads on boot.
