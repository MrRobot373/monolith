...
- name: '@monolith/lsp'
- name: '@monolith/lsp-stdio'
- name: '@monolith/tool-lsp'
```

Server commands, extension mappings, and the filesystem/subprocess pairing are configured in the provider and tool packages; see [monolith-lsp-stdio](../lsp-stdio/README.md) and [monolith-tool-lsp](../tool-lsp/README.md).
...
- **Capability seam, Service Definition role.** The package owns `ctx.lsp` and the provider registry; providers register capabilities, not tools, and `monolith-tool-lsp` is the only owner of the model-facing surface.
...
- [monolith-lsp-stdio](../lsp-stdio/README.md) — the stdio provider that registers against this seam.
- [monolith-tool-lsp](../tool-lsp/README.md) — the model-facing tool over this seam.
...
Indirectly, through `monolith-tool-lsp`, which owns the model-facing `lsp` schema, prompt guidance, and rendered results while this registry contributes no prompt or schema itself.

#### KV Cache effect

No direct invalidation; `monolith-tool-lsp` owns request-prefix changes.
