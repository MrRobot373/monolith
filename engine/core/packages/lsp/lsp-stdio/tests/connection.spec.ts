...
import { LspConnection } from '@monolith/lsp-stdio'
import type { ConnectionWriter } from '@monolith/lsp-stdio/src/connection.ts'
...
describe('LspConnection', () => {
  ...
  it('forwards explicit MONOLITH_* env entries to the child', async () => {
    // A configured MONOLITH_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'MONOLITH_LSP_TEST_FACT', MONOLITH_LSP_TEST_FACT: 'managed' })
