---
description: "Automation-only Agent Client Protocol server for programmatic clients and maintainers driving MONOLITH agents over JSON-RPC stdio."
...
`monolith-acp` lets trusted programs drive persistent MONOLITH agents over the standard [Agent Client Protocol](https://agentclientprotocol.com): create or resume sessions, list resumable sessions, attach standard MCP servers, select a model and reasoning effort, prompt or cancel work, receive semantic execution updates, and close one session without affecting others. It is built for automation — out-of-process subagents, test runners, and scripted controllers — rather than the MONOLITH user interface: it emits standard ACP messages, thoughts, generic tool lifecycle, configuration, and context usage, never private MONOLITH presentation data or methods. Session persistence enables list, resume, and close across process restarts, while deletion, fork, transcript replay, additional directories, and interactive UI surfaces remain unsupported. The repository's own ACP client is `monolith-subagent-acp`, and `pnpm monolith --profile acp` starts a ready-to-use server. Setup and usage come first; the implementation details live in a collapsible developer section below.
...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-acp) is the exhaustive source for every accepted field and its JSDoc.

### Start a server

`pnpm monolith --profile acp` starts the shipped stdio server. The `acp` profile mounts session persistence, so clients can list, resume, and close persistent sessions. [`@monolith/subagent-acp`](../../subagent/subagent-acp/README.md) starts the same profile for out-of-process delegation.
...
- **Standard semantic updates only.** The wire carries committed messages and thoughts, generic tool lifecycle, configuration, and context usage; raw provider deltas, retry attempts, MONOLITH presentation data, and unsupported content stay off the wire.
...
- [monolith-subagent-acp](../../subagent/subagent-acp/README.md) — the out-of-process ACP client that spawns and drives this server.
...
- **MCP tools only** — MCP resources and prompts have no MONOLITH consumer.
