...
import { errorChain } from '@monolith/llm'
import {
...
import type { ModelSelection } from '@monolith/agent'
import type { SessionId } from '@monolith/session'
import type {} from '@monolith/session-persistence'
...
export function apply(ctx: Context, config: AcpConfig): void {
  ...
  // monolith-subagent-acp. The bridge offers one-shot choices only and never infers a
  ...
  const implementation = {
    async initialize(_params: InitializeRequest): Promise<InitializeResponse> {
      ...
      return {
        ...
        agentInfo: { name: 'monolith-acp', version: '0.0.1' },
        agentCapabilities: {
  ...
  const app = createAcpAgentApp({ name: 'monolith-acp' })
