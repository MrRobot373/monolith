...
import { type GenerateOptions, LlmAdapter, ReasoningEffortId, type LlmResolvedModelInfo, type StreamChunk } from '@monolith/llm'
...
export async function makeBridgeHarness(options: {
  ...
  const persistenceRoot = options.persistenceRoot ?? await mkdtemp(join(tmpdir(), 'monolith-acp-test-'))
