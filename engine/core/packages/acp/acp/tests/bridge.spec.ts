...
import { AttachmentError } from '@monolith/attachment'
import { ToolCallId, type StreamChunk } from '@monolith/llm'
import { SessionId } from '@monolith/session'
import type { SessionHeader } from '@monolith/session'
import { SessionPersistenceRevision, type SessionPersistenceSnapshot } from '@monolith/session-persistence'
...
describe('automation-only ACP bridge', () => {
  ...
  it('lists and resumes persisted sessions after an equivalent process restart', async () => {
    const persistenceRoot = await mkdtemp(join(tmpdir(), 'monolith-acp-restart-'))
    try {
