...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-sandbox-local) is the exhaustive source for every accepted field and its JSDoc.
...
The Landlock launcher ships as an npm-distributed native addon (`@monolith/node-addon-landlock-run`) that supplies the platform launcher, functional probe, and grant vocabulary; this provider maps mode to grants only, keeping path resolution and probe parsing with the versioned binary.
...
Indirectly, through [`monolith-bash-sandbox`](../../shell/bash-sandbox/README.md) and [`monolith-tool-bash`](../../shell/tool-bash/README.md), which render this provider's enforcement and denial facts, while the [`monolith-sandbox`](../sandbox/README.md) seam owns the `SANDBOX_UNAVAILABLE` text and this provider owns runner selection, and profiles stay outside context.
