...
import type { SandboxPolicy } from '@monolith/sandbox'
import {
  LocalSandboxProvider,
} from '@monolith/sandbox-local'
import type { Config } from '@monolith/sandbox-local'
...
function absentRunnerEntry(): string {
  return join(mkdtempSync(join(tmpdir(), 'monolith-absent-acl-entry-')), 'runner.js')
}
...
function fakeLauncher(report = 'landlock: fully enforced'): string {
  const dir = mkdtempSync(join(tmpdir(), 'monolith-fake-landlock-'))
...
function fakeSeatbeltExec(status: number): string {
  const dir = mkdtempSync(join(tmpdir(), 'monolith-fake-seatbelt-'))
...
describe('the platform chains', () => {
  ...
  // live in @monolith/sandbox-windows-acl/tests/provider-chain.spec.ts
...
describe('probeTimeoutMs config', () => {
  ...
  it('bounds the default probes: a launcher slower than the configured timeout reads as unusable', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'monolith-slow-landlock-'))
    const launcher = join(dir, 'landlock-run')
...
describe('the windows-acl probe (runner invocation contract)', () => {
  ...
  it('prefers the built lib/runner.js entry when the resolved file exists', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'monolith-fake-acl-entry-'))
