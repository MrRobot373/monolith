...
const platformPackageName = `@monolith/node-addon-landlock-run-linux-${process.arch}`

const NATIVE_PACKAGE_PREFIX = '@monolith/node-addon-landlock-run'
...
describe.skipIf(!packable)('sandbox-local: packed-tarball distribution (publish-path rehearsal)', () => {
  beforeAll(async () => {
    const packDest = mkdtempSync(join(tmpdir(), 'monolith-pack-'))
    consumerDir = mkdtempSync(join(tmpdir(), 'monolith-packed-consumer-'))
    workDir = mkdtempSync(join(tmpdir(), 'monolith-packed-work-'))
    ...
    writeFileSync(join(consumerDir, 'consumer.mjs'), `
      ...
      import { launcherPath } from '@monolith/node-addon-landlock-run'
