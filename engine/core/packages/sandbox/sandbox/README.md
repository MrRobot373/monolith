...
```yaml
- id: sandbox
  name: '@monolith/sandbox-local'     # the per-platform backend provider (ctx.sandbox)
- id: sandbox-policy
  name: '@monolith/sandbox-policy'    # the deployment default mode and workspace-write root
  config:
...
- id: bash
  name: '@monolith/bash-sandbox'      # the confined executor behind ctx.shell
```
...
Through [`monolith-bash-sandbox`](../../shell/bash-sandbox/README.md) and [`monolith-tool-bash`](../../shell/tool-bash/README.md), a requested confined mode with no usable backend produces code `SANDBOX_UNAVAILABLE` and the exact error below; an execution-time runner failure appends ` Runner failure: <detail>`.
