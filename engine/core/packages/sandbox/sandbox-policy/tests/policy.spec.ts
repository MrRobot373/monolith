...
import SandboxPolicyService, { SANDBOX_MODES, setSandboxMode } from '@monolith/sandbox-policy'
import SessionProjectionRegistry from '@monolith/session-projection'
import SystemPrompt, { renderContextSnapshot, renderPrompt } from '@monolith/system-prompt'
...
describe('SandboxPolicyService', () => {
  ...
  it.skipIf(process.platform === 'win32')('resolves a symlink-sensitive session cwd with POSIX component semantics', async () => {
    const root = mkdtempSync(join(tmpdir(), 'monolith-policy-cwd-'))
    try {
...
describe('sandbox:policy request context', () => {
  ...
  it.each(['read-only', 'workspace-write', 'danger-full-access'] as const)('renders the exact %s policy without a capability inventory', async (mode) => {
    ...
    const expected = {
      'read-only': 'Current MONOLITH file policy: read-only. Any available operation enforced by the MONOLITH file sandbox cannot modify files in the standing mode. Do not refuse a required modification from this policy alone: try an available tool normally and follow any denial and escalation guidance it returns.',
      'workspace-write': `Current MONOLITH file policy: workspace-write. Any available operation enforced by the MONOLITH file sandbox may modify files under the session workspace: ${JSON.stringify(workspaceRoot)}. Some platform temporary areas may also be writable.`,
      'danger-full-access': 'Current MONOLITH file policy: danger-full-access. The MONOLITH file sandbox does not restrict file modifications by available operations.',
    } as const
  ...
  it('reflects the latest durable switch on the next assembly and stays byte-stable otherwise', async () => {
    ...
    expect(danger).toBe('Current MONOLITH file policy: danger-full-access. The MONOLITH file sandbox does not restrict file modifications by available operations.')
    expect(await policyContext(ctx, active)).toBe(danger)

    setSandboxMode(active, 'workspace-write')
    expect(await policyContext(ctx, active)).toBe(`Current MONOLITH file policy: workspace-write. Any available operation enforced by the MONOLITH file sandbox may modify files under the session workspace: ${JSON.stringify(resolve('/projects/current'))}. Some platform temporary areas may also be writable.`)
  })
