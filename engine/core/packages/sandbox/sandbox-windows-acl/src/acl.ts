 ...
 * @module @monolith/sandbox-windows-acl/acl
 */
...
/**
 * One lock file per protected path: `<GetTempPathW()>\monolith-acl-locks\<first 16
 * hex of sha256(lowercased path)>.lock`. The lock root derives from
 * GetTempPathW (never from runner argv or MONOLITH_HOME), and the lowercasing
...
export function lockFilePath(api: Win32Bindings, path: string): string {
  ...
  return join(getTempPath(api), 'monolith-acl-locks', `${digest}.lock`)
}
