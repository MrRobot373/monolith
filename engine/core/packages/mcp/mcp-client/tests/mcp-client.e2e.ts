/**
 * End-to-end tests for monolith-mcp-client. Exercises the REAL MCP protocol against:
...
import { apply } from '@monolith/mcp-client/src/index.ts'
import { publicToolName } from '@monolith/mcp-client/src/tools.ts'
...
async function mountImageRegistry(monolithHome: string): Promise<Context> {
  const ctx = await mountRegistry()
  await ctx.plugin(LocalAttachmentStore, { monolithHome })
