...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-mcp-client) is the exhaustive source for every accepted field.
...
The child environment starts from the subprocess seam's `scrubbedParentEnv()` — ambient names matching `/KEY|PASSWORD|SECRET|TOKEN/i` and ambient `MONOLITH_*` names are dropped — and the configured `env` merges on top, so explicit overrides survive. The MCP SDK owns the actual spawn; this package shares the scrub definition, not the spawn path.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#monolith-mcp-client) — every accepted config field and its source declaration.
...
- An explicit MONOLITH-owned connection and discovery timeout is an open direction; the SDK's 60-second default bounds startup and teardown.
