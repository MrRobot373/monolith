...
- name: '@monolith/storage-sqlite'
  config:
    path: /var/lib/monolith/data.db
...
`wal` suits local disks; a rollback-journal mode (`delete`/`truncate`/`persist`) fits filesystems where WAL's shared-memory files do not work, such as network mounts. The generated [configuration catalog](../../../docs/config-catalog.md#monolith-storage-sqlite) is the exhaustive source for every accepted field and its JSDoc.
