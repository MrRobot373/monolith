 ...
 * @module @monolith/jobs-local
 */

import { Context } from '@monolith/cordis'
import z from '@monolith/schemastery'
import type { Agent } from '@monolith/agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@monolith/scope'
import type { ScopeLayer } from '@monolith/scope'
import { deadline, timeoutOf } from '@monolith/timeout'
import { JobRegistry, JobId } from '@monolith/jobs'
import type {
 ...
 * `@monolith/jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @monolith/tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @monolith/agent)')
    }
