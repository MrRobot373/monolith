...
import type { ScopeKey } from '@monolith/scope'
import { JobId } from '@monolith/jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@monolith/jobs'
import LocalJobRegistry, { type Config as JobsConfig } from '@monolith/jobs-local'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @monolith/tool-jobs in its composition)')
  })
