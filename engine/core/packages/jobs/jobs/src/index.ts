 ...
 * process-local registry lives in `@monolith/jobs-local`.
 * @module @monolith/jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@monolith/jobs is the abstract job registry seam; load an implementation such as @monolith/jobs-local instead')
    }
    super(ctx, 'jobs')
  }
