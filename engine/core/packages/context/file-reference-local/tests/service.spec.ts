...
import ToolRegistry, { defineContentToolFixture } from '@monolith/tools'
import { FILE_REFERENCE_PROMPT } from '@monolith/file-reference'
...
async function stubAgent(
  ...
  const root = await mkdtemp(join(tmpdir(), 'monolith-file-reference-service-'))
  roots.push(root)
