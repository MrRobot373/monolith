 ...
 *
 * @module @monolith/agent-instructions/files
 */

import { createReadStream } from 'node:fs'
import { stat } from 'node:fs/promises'
import { dirname, isAbsolute, join, relative, resolve } from 'node:path'
import type { FileSystem, FsInfo, FsTarget, FsVersion } from '@monolith/fs'
import { monolithHomeDisplay } from '@monolith/home-paths'
...
async function discoverInstructionFiles(
  ...
  const userGlobal = join(config.monolithHome, USER_GLOBAL_FILE)
  ...
  switch (userGlobalProbe.kind) {
    case 'present':
      addFile({
        ...
        displayPath: userGlobalDisplayPath(config.monolithHome),
...
export async function probeScopeInstruction(
  ...
  const file: ProbedInstructionFile = {
    absolutePath,
    displayPath: directory === USER_GLOBAL_DIRECTORY ? userGlobalDisplayPath(resolved.monolithHome) : relativeDisplay(projectRoot, absolutePath),
    target,
...
function userGlobalDisplayPath(monolithHome: string): string {
  return `${monolithHomeDisplay(monolithHome)}/AGENTS.md`
}
