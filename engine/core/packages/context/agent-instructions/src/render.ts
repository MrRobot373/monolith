 ...
 *
 * @module @monolith/agent-instructions/render
 */
...
/**
 * File name of the single user-global instruction file under `$MONOLITH_HOME`.
 * Discovery (`$MONOLITH_HOME/<name>`) and reconciliation (the user-global scope key's
...
export function scopeForDisplayPath(displayPath: string): string {
  if (displayPath === '~/.monolith/AGENTS.md' || displayPath === '$MONOLITH_HOME/AGENTS.md') return USER_GLOBAL_DIRECTORY
