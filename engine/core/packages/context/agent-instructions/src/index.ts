 ...
 * mount it as a no-op.
 *
 * @module @monolith/agent-instructions
 */
...
import type { ToolExecution, ToolExecutionResult, ToolExecutionToken } from '@monolith/tools'
  ...
  const compose = async (
    ...
    if (!baselinePresent || !keepVisibleBaseline || excludedBaselineScopes === undefined) {
      ...
      const instructions = await loadBaselineInstructionSet({
        cwd,
        monolithHome: resolved.monolithHome,
