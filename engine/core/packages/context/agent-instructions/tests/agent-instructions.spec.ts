...
import * as workspaceContext from '@monolith/agent-instructions'
import LlmRuntime, { createUserMessage, ToolCallId, type Message, type StreamChunk } from '@monolith/llm'
import SessionStore, { Session, SessionId, SessionSeq, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@monolith/session'
import SessionProjectionRegistry from '@monolith/session-projection'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@monolith/agent'
import AgentLoop, { turnBoundaryProjectionDefinition } from '@monolith/agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@monolith/fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
import ToolRuntime, { defineContentToolFixture } from '@monolith/tools'
import type {
  ToolExecution,
  ToolExecutionToken,
} from '@monolith/tools'
...
} from '@monolith/agent-instructions'
import {
...
async function tempRepo(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'monolith-workspace-context-'))
}
...
describe('workspace context instruction discovery', () => {
  it('treats ENOTDIR while probing a host candidate as confirmed absence', async () => {
    ...
    try {
      ...
      const files = await discoverBaselineInstructionFiles({ cwd: root, monolithHome: homeFile })
  ...
  it('loads user-global first, then every root-to-cwd candidate in precedence order', async () => {
    ...
    try {
      ...
      const files = await discoverBaselineInstructionFiles({ cwd, monolithHome: home })

      expect(files.map(file => file.displayPath)).toEqual([
        '$MONOLITH_HOME/AGENTS.md',
        'AGENTS.md',
        'CLAUDE.md',
  ...
  it('re-reads content after a same-version, same-size rewrite', async () => {
    ...
    try {
      ...
      expect(await loadBaselineInstructions({ cwd, monolithHome: home, maxBytes: 65536 })).toBeUndefined()

      const leaf = join(cwd, 'AGENTS.md')
      await write(leaf, 'first')
      const first = await loadBaselineInstructions({ cwd, monolithHome: home, maxBytes: 65536 })
      expect(first?.text).toContain('first')
      const again = await loadBaselineInstructions({ cwd, monolithHome: home, maxBytes: 65536 })
      ...
      const second = await loadBaselineInstructions({ cwd, monolithHome: home, maxBytes: 65536 })
  ...
  it.skipIf(process.platform === 'win32')('skips a file that becomes unreadable after discovery without failing the request', async () => {
    ...
    try {
      ...
      const loaded = await loadBaselineInstructions({ cwd, monolithHome: home, maxBytes: 65536 })
  ...
  it('follows a symlinked instruction file to its target content', async () => {
    ...
    try {
      ...
      const files = await discoverBaselineInstructionFiles({ cwd: root, monolithHome: home })
      const loaded = await loadBaselineInstructions({ cwd: root, monolithHome: home, maxBytes: 65536 })
  ...
  it('follows a symlinked instruction file through ctx.fs to its target content', async () => {
    ...
    try {
      ...
      await mountWorkspaceContext(ctx, { monolithHome: home, maxBytes: 65536 })
  ...
  it('disables baseline loading when the byte budget is zero', async () => {
    ...
    try {
      ...
      await expect(loadBaselineInstructions({ cwd: root, monolithHome: home, maxBytes: 0 })).resolves.toBeUndefined()
      await expect(loadBaselineInstructions({ cwd: root, monolithHome: home, maxBytes: 65536, maxSourceBytes: 0 })).resolves.toBeUndefined()
      await expect(loadBaselineInstructions({
        cwd: root, monolithHome: home, maxBytes: 65536, maxSourceBytes: Infinity,
  ...
  it('defaults monolithHome and uses cwd itself as root when no project marker exists', async () => {
    const root = await tempRepo()
    const emptyHome = await tempRepo()
    // Isolate the default-home fallback: blank MONOLITH_HOME is treated as unset, and
    // the home dirs point at an empty dir so the default ~/.monolith holds no global
    // scope. Windows homedir() reads USERPROFILE (not HOME), so both must be
    // stubbed or a real ~/.monolith/AGENTS.md would otherwise leak in.
    vi.stubEnv('MONOLITH_HOME', '')
  ...
  it('honors MONOLITH_HOME when monolithHome is not configured explicitly', async () => {
    const root = await tempRepo()
    const envHome = await tempRepo()
    try {
      await write(join(envHome, 'AGENTS.md'), 'env global rule')
      vi.stubEnv('MONOLITH_HOME', envHome)

      const files = await discoverBaselineInstructionFiles({ cwd: root })

      expect(files).toEqual([{ absolutePath: join(envHome, 'AGENTS.md'), displayPath: '$MONOLITH_HOME/AGENTS.md' }])
    } finally {
      vi.unstubAllEnvs()
      await rm(root, { recursive: true, force: true })
  ...
  it('labels the default MONOLITH home as ~/.monolith when HOME points at the configured default', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await write(join(home, '.monolith/AGENTS.md'), 'global default rule')

      // A set MONOLITH_HOME would override the homedir default and relabel the home.
      vi.stubEnv('MONOLITH_HOME', '')
      vi.resetModules()
      vi.doMock('node:os', () => ({ homedir: () => home }))
      const isolated = await import('@monolith/agent-instructions')
      const files = await isolated.discoverBaselineInstructionFiles({ cwd: root })

      expect(files.map(file => file.displayPath)).toEqual(['~/.monolith/AGENTS.md'])
    } finally {
      vi.unstubAllEnvs()
      vi.doUnmock('node:os')
      vi.resetModules()
  ...
  it('expands a configured ~/.monolith home to the operating-system home directory', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await write(join(home, '.monolith/AGENTS.md'), 'global tilde rule')

      vi.resetModules()
      vi.doMock('node:os', () => ({ homedir: () => home }))
      const isolated = await import('@monolith/agent-instructions')
      const files = await isolated.discoverBaselineInstructionFiles({ cwd: root, monolithHome: '~/.monolith' })

      expect(files).toEqual([{ absolutePath: join(home, '.monolith/AGENTS.md'), displayPath: '~/.monolith/AGENTS.md' }])
    } finally {
      vi.doUnmock('node:os')
      vi.resetModules()
      await rm(root, { recursive: true, force: true })
  ...
  it('deduplicates user-global instructions when monolithHome points at the project root', async () => {
    const root = await tempRepo()
    try {
      await mkdir(join(root, '.git'), { recursive: true })
      await write(join(root, 'AGENTS.md'), 'same file')

      const files = await discoverBaselineInstructionFiles({ cwd: root, monolithHome: root })

      expect(files).toEqual([{ absolutePath: join(root, 'AGENTS.md'), displayPath: '$MONOLITH_HOME/AGENTS.md' }])
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
...
describe('workspace context request injection', () => {
  ...
  it('preserves a visible baseline when its source is unavailable during resume', async () => {
    ...
    try {
      ...
      await mountWorkspaceContextPlugin(ctx, { monolithHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
      await composeBaselinePrefix(ctx, original)
  ...
  it('does not promote an unchanged budget-omitted baseline file during resume', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { monolithHome: home, maxBytes: 700 })
  ...
  it('recomposes the baseline when candidate precedence changes between resumes', async () => {
    ...
    try {
      ...
      await write(join(root, 'CLAUDE.md'), 'claude rule')
      await mountWorkspaceContext(originalCtx, { monolithHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
  ...
  it('reuses an inserted but unadmitted baseline after session recovery and plugin reload', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      const fiber = await mountWorkspaceContext(ctx, { monolithHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
  ...
  it.each([
    ...
    try {
      ...
      await mountWorkspaceContextPlugin(resumedCtx, { monolithHome: home, maxBytes })
      const resumed = stubAgent(root, original.session.snapshotEvents())
  ...
  it('reports a changed user-global instruction through the visible-scope reconcile path', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { monolithHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
  ...
  it('retains a visible baseline after a plugin remount', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      const fiber = await mountFileToolsAndWorkspaceContext(ctx, { monolithHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
  ...
  it('restores a compacted baseline on a hot plugin remount', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await ctx.plugin(LocalFileSystem, { cwd: '/' })
      const fiber = await mountWorkspaceContextPlugin(ctx, { monolithHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
      const baseline = baselineEvents(agent)[0]
  ...
  it('deduplicates one AGENTS.md that is both user-global and the project-root candidate', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { monolithHome: root, maxBytes: 65536 })
      const agent = stubAgent(root)
  ...
  it.each([10, 120])('does not expose state markers when baseline content is omitted at %i bytes', async (maxBytes) => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { monolithHome: home, maxBytes })
      const agent = stubAgent(root)
  ...
  it('keeps the direct provider API usable without an operation signal', async () => {
    ...
    try {
      ...
      const rendered = await loadBaselineInstructions({ cwd: root, monolithHome: home, maxBytes: 65536 }, fs)
  ...
  it('rejects a provider-sized instruction file before reading content', async () => {
    ...
    try {
      ...
      await mountWorkspaceContextPlugin(ctx, { monolithHome: home, maxBytes: 65536, maxSourceBytes: 4 })
  ...
  it('does not inject anything when maxBytes is zero', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { monolithHome: home, maxBytes: 0 })
      const agent = stubAgent(root)
  ...
  it('does not inject an empty agent-instructions message when maxBytes is negative', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { monolithHome: home, maxBytes: -1 })
      const agent = stubAgent(root)
  ...
  it('labels a custom monolithHome as MONOLITH_HOME instead of pretending it is ~/.monolith', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      ...
      expect(files.map(file => file.displayPath)).toEqual(['$MONOLITH_HOME/AGENTS.md'])
    } finally {
      await rm(root, { recursive: true, force: true })
      await rm(home, { recursive: true, force: true })
    }
  ...
  it('does not repeat a candidate metadata probe during one discovery and read pass', async () => {
    ...
    try {
      ...
      const isolated = await import('@monolith/agent-instructions')
      await isolated.loadBaselineInstructions({ cwd: root, monolithHome: home, maxBytes: 65536 })
      observedStats.clear()
      await isolated.loadBaselineInstructions({ cwd: root, monolithHome: home, maxBytes: 65536 })
  ...
  it('skips an unavailable host candidate but still loads its available sibling', async () => {
    ...
    try {
      ...
      const rendered = await isolated.loadBaselineInstructions({ cwd: root, monolithHome: home, maxBytes: 65536 })
...
describe('dynamic nested workspace context injection', () => {
  ...
  it('skips visible baseline scopes when baseline scopes are excluded from reconciliation', async () => {
    ...
    try {
      ...
      agent.session.append('user/message', authoritative, { surfaceOp: 'append' })
      const resolved = resolveConfig({ monolithHome: home, maxBytes: 65536, localInstructionFileCandidates: [] })
      const cache: InstructionVersionCache = new WeakMap()
  ...
  it('does not mark omitted nested files as pending-loaded', async () => {
    ...
    try {
      ...
      await write(join(root, 'pkg/sub/file.txt'), 'subtree file')
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { monolithHome: home, maxBytes: 700 })
      const agent = stubAgent(root)
      await ctx.tools.execute({
        signal: testToolSignal,
  ...
  it('does not attach nested instructions when the byte budget is disabled', async () => {
    ...
    try {
      ...
      await write(join(root, 'pkg/deep/file.txt'), 'hello')
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { monolithHome: home, maxBytes: 0 })
  ...
  it('retries a nested instruction touch when only a truncated budget notice was rendered', async () => {
    ...
    try {
      ...
      fs.entries.set(join(root, 'pkg/file.txt'), { type: 'file', content: 'hello' })
      await ctx.plugin(ToolFs)
      await mountWorkspaceContextPlugin(ctx, { monolithHome: home, maxBytes: 20 })
      const agent = stubAgent(root)
...
describe('workspace context inbox synchronization', () => {
  ...
  it('holds back a dynamic change a one-byte positive render budget cannot represent', async () => {
    ...
    try {
      ...
      await mountWorkspaceContextPlugin(ctx, { monolithHome: home, maxBytes: 1 })
      const agent = stubAgent(root)
      ctx.emit('tools/result', stubToolExecution({
        signal: testToolSignal,
