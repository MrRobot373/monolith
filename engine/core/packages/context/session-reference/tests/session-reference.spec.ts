...
import { agentEvents, type Agent } from '@monolith/agent'
import { CompactionId, compactCheckpointSource } from '@monolith/compaction'
import { createUserMessage, ToolCallId , createMessage, createToolResultMessage } from '@monolith/llm'
import SessionStore, { Session, SessionId, SessionSeq } from '@monolith/session'
import SessionProjectionRegistry from '@monolith/session-projection'
import SessionQueryEngine from '@monolith/session-query'
import SessionTitleService from '@monolith/session-title'
...
} from '@monolith/session-reference'
...
describe('session reference URI and inline mentions', () => {
  it('round-trips arbitrary session ids and replaces mentions with readable labels', () => {
    ...
    expect(parseSessionReferenceText('what is a monolith-session: URI?')).toEqual({
      text: 'what is a monolith-session: URI?',
      references: [],
    })
    expect(parseSessionReferenceText('see monolith-session:%%%')).toEqual({
      text: 'see monolith-session:%%%',
      references: [],
    })
  })
  ...
  it('rejects malformed explicit references and base64url-shaped bare candidates', () => {
    ...
    expect(() => parseSessionReferenceText('see monolith-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => parseSessionReferenceText('@[bad](monolith-session:%%%)')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    const nonString = `monolith-session:${Buffer.from(JSON.stringify({ id: 'x' })).toString('base64url')}`
    expect(() => decodeSessionReferenceUri(nonString)).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => decodeSessionReferenceUri('monolith-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
  })
})
...
describe('session reference discovery and preparation', () => {
  ...
  it('does not prepare a rejected pre-step and rejects malformed direct mentions', async () => {
    ...
    const malformed = createUserMessage({
      content: [{ type: 'text', text: '@[bad](monolith-session:not-canonical)' }],
      source: { kind: 'user' },
    })
