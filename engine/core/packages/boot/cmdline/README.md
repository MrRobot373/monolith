---
description: "App-owned command lines for monolith app bins: your app parses its own flags, --help, and exit behavior from the launcher's remaining arguments."
...
- `ctx.cmdlineArgs` — the inner arguments of your invocation. Reading them returns an immutable snapshot and never consumes or changes them: `monolith --profile tui --resume abc` gives your app `['--resume', 'abc']`.
...
The outcomes: `monolith --profile web --port 8080` starts the server on port 8080 even when the config says 3080, because the flag wins. `--help` prints your app's help and exits 0 without starting anything; a rejected value (for example a non-numeric port) prints your error and exits nonzero, and no row that depends on the parsed values ever starts.
...
- [monolith-app-boot](../app-boot/README.md) — the boot sequence that provides these launcher values.
- [monolith-web-app bundle](../../bundle/web-app/README.md) — an app that owns the Web flag family through this package.
- [monolith-headless bundle](../../bundle/headless/README.md) — the one-shot runner that reads its task from the command line.
