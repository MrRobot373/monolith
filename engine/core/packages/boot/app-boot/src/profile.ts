 ...
 * `monolith --profile` launcher family.
 *
 * A profile is a directory under `$MONOLITH_HOME/profiles/<name>` holding a
 * `package.json` (out-of-tree plugin dependencies plus the profile manifest
 * `monolith.profile` with its ordered `bundles` list) and a `cordis.patch.yml`
 * (the user's own patch layer, applied after every bundle layer). Bundles are
 * npm packages whose manifest declares
 * `"monolith": { "bundle": { "patch": "./cordis.patch.yml" } }`; the tree is
 * composed by applying each bundle's patch list in `monolith.profile.bundles` order over
 ...
 * first from the monolith installation (the launcher's own package), then from the
 * profile directory. Pnpm-managed entries in the profile's `node_modules`
 * resolve first. Monolith-owned links add packages carried only by selected
 * bundles, while `$MONOLITH_HOME/profiles/node_modules` supplies the installation
 ...
 * @module @monolith/app-boot/profile
 */
...
import { withFileLock } from '@monolith/atomic-write'
import type { EntryOptions } from '@monolith/cordis-plugin-loader'
import { applyEntryPatches, type PatchOptions } from '@monolith/cordis-plugin-include'
import { resolveMonolithHome } from '@monolith/home-paths'
import { resolve as resolvePackage, type Package as ResolvePackageManifest } from 'resolve.exports'
import { loadOverlayPatches } from './index.ts'

/** Directory under the MONOLITH home holding every profile. */
...
const PROFILE_MODULE_FALLBACK_DIR = '.monolith-module-fallback'

/** The bundle half of the `monolith` manifest section: what a bundle package exports. */
export interface MonolithBundleManifest {
  /** The patch layer this bundle exports, relative to its package root. */
  patch: string
}

/** The profile half of the `monolith` manifest section: what a profile directory composes. */
export interface MonolithProfileManifest {
...
/**
 * The profile-launcher slice of the `monolith`-owned package.json section. A
 * manifest may declare both roles; other consumers own additional keys.
 */
export interface MonolithManifestSection {
  /** Bundle metadata consumed by the profile launcher. */
  bundle?: MonolithBundleManifest
  /** Profile metadata consumed by the profile launcher. */
  profile?: MonolithProfileManifest
}
...
export interface ProfileManifest {
  ...
  monolith?: MonolithManifestSection
}

/** One resolved bundle layer of a profile. */
export interface ProfileLayer {
  /** The bundle's package name, as listed in `monolith.profile.bundles`. */
  packageName: string
...
export interface Profile {
  ...
  dir: string
  /** Bundle layers in `monolith.profile.bundles` order. */
  layers: ProfileLayer[]
...
/**
 * Resolve a profile's directory under the MONOLITH home.
 * @param name - the profile name (`monolith --profile <name>`).
 * @param home - the MONOLITH home; defaults to {@link resolveMonolithHome}.
 * @returns the absolute profile directory (which may not exist yet).
 */
export function resolveProfileDir(name: string, home: string = resolveMonolithHome()): string {
  if (name === '' || name.includes('/') || name.includes('\\') || name === '.' || name === '..'
    // The launcher-maintained flat module fallback lives at this sibling path.
    || name === 'node_modules') {
    throw new Error(`monolith: invalid profile name ${JSON.stringify(name)}`)
  }
...
export const PROFILE_TEMPLATES: Record<string, ProfileTemplate> = {
  acp: {
    bundles: ['@monolith/base', '@monolith/acp-app'],
    patchReload: 'startup',
  },
  web: {
    bundles: ['@monolith/base', '@monolith/web-app'],
    patchReload: 'live',
  },
  headless: {
    bundles: ['@monolith/base', '@monolith/headless'],
    patchReload: 'startup',
  },
  sdk: {
    bundles: ['@monolith/base', '@monolith/sdk-app'],
    patchReload: 'startup',
  },
  'sdk-minimal': {
    bundles: ['@monolith/sdk-minimal'],
    patchReload: 'startup',
  },
}
...
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  headless: ['@monolith/base', '@monolith/web-app', '@monolith/headless'],
}

/** The bundle list a `monolith plugin` init uses for a name with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['@monolith/base']
...
const PROFILE_PATCH_TEMPLATE = `# Your patch layer for this monolith profile, applied after every bundle layer:
 ...
 * @param bundles - the initial `monolith.profile.bundles` layer list.
...
export function initProfile(
  ...
  if (!existsSync(manifestPath)) {
    const manifest: ProfileManifest & { private: boolean } = {
      name: `monolith-profile-${basename(dir)}`,
      private: true,
      dependencies: {},
      monolith: { profile: { bundles: [...bundles], patchReload } },
    }
...
/** Ensure `link` is a symlink to `target`, replacing a wrong link or a monolith-managed packaged proxy. */
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      ...
      if (existing?.monolith?.moduleFallback?.targets === undefined) {
        throw new Error(`monolith: ${link} exists and is not a symlink or monolith-managed module proxy; remove it so monolith can manage the installation fallback`)
      }
...
interface ModuleProxyManifest {
  ...
  monolith: { moduleFallback: { targets: Record<string, string> } }
}

interface ModuleProxyRecord {
  version?: unknown
  monolith?: { moduleFallback?: { targets?: unknown } }
}
...
function packageEntryFromPackage(
  ...
  try {
    ...
    throw new Error(`monolith: cannot resolve ESM export ${specifier} from installed package ${packageName}`, { cause: error })
  }
  for (const candidate of candidates ?? []) {
    ...
    if (!target.startsWith('./') || /^\.\.(?:[\\/]|$)/u.test(relativeEntry)) {
      throw new Error(`monolith: installed package ${packageName} export ${subpath} resolves outside its package: ${target}`)
    }
...
function packageProxySource(
  ...
  if (typeof manifest.version !== 'string' || manifest.version.length === 0) {
    throw new Error(`monolith: installed package ${packageName} must declare a non-empty version`)
  }
  ...
  if (declared === undefined) {
    ...
    try {
      ...
      throw new Error(`monolith: installed package ${packageName} main entry is missing at ${entry}`, { cause: error })
    }
  }
...
function ensureModuleProxy(
  ...
  const manifest: ModuleProxyManifest = {
    ...
    monolith: { moduleFallback: { targets } },
  }
  let stat
  try {
    stat = lstatSync(link)
  ...
  if (stat !== undefined) {
    ...
    if (existing?.monolith?.moduleFallback?.targets === undefined) {
      throw new Error(`monolith: ${link} exists and is not a monolith-managed module proxy; remove it so monolith can manage the installation fallback`)
    }
    if (existing.version === version
      && JSON.stringify(existing.monolith.moduleFallback.targets) === JSON.stringify(targets)
...
function resolveModuleFallbackEntries(
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    // Peer dependencies participate: Service Definition packages (monolith-subprocess,
    // monolith-compaction, ...) are peers of their implementations, never plain
...
function moduleFallbackEntryCurrent(modulesDir: string, entry: ModuleFallbackEntry): boolean {
  ...
  try {
    ...
    return existing?.version === entry.version
      && JSON.stringify(existing.monolith?.moduleFallback?.targets) === JSON.stringify(entry.targets)
...
export interface ProfileModuleFallbackOptions {
  /** Absolute package.json path of the running monolith installation. */
  installAnchor: string
  /** Loaded profile whose selected bundles may carry profile-local plugins. */
  profile?: Profile
  /** MONOLITH home; defaults to {@link resolveMonolithHome}. */
  home?: string
}

/**
 * Maintain module fallbacks for one profile launch. The shared
 * `$MONOLITH_HOME/profiles/node_modules` mirrors the monolith installation dependency
 ...
 * @param options - installation anchor, optional loaded profile, and MONOLITH home.
...
export async function healProfilesModuleFallback(options: ProfileModuleFallbackOptions): Promise<void> {
  const { installAnchor, profile, home = resolveMonolithHome() } = options
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const bundles = manifest.monolith?.profile?.bundles
  ...
  const needsReloadDefault = manifest.monolith?.profile?.patchReload === undefined && isCurrentTuple
  ...
  const normalized: ProfileManifest = {
    ...
    monolith: {
      ...manifest.monolith,
      profile: {
        ...manifest.monolith?.profile,
        bundles: [...template.bundles],
        patchReload: manifest.monolith?.profile?.patchReload ?? template.patchReload,
      },
    },
  }
 ...
 * `@monolith/base` (and every other in-box bundle) always comes from
 * the same installation as the running monolith, never from a profile-local copy.
 * Resolution does not require the package to export `./package.json`.
 * @param binName - the diagnostic prefix on the thrown error.
 * @param packageName - the bundle's package name from `monolith.profile.bundles`.
 * @param installAnchor - absolute path of a file inside the monolith app package (its package.json).
...
export function resolveBundleDir(
  ...
  throw new Error(
    `${binName}: cannot resolve profile bundle ${JSON.stringify(packageName)} from the monolith installation or ${profileDir}; `
    + `run 'monolith plugin --profile ${basename(profileDir)} install' if its dependency is not installed`,
  )
}

/**
 * Load a profile: resolve every `monolith.profile.bundles` entry to its patch
 * layer and parse the profile's own patch file. A listed bundle without a
 * `monolith.bundle` manifest fails loud — naming a bundle-less package as a layer
 * is a misconfiguration, not "no patches".
 * @param binName - the diagnostic prefix on thrown errors.
 * @param name - the profile name.
 * @param installAnchor - absolute path of the monolith app's package.json (first resolution anchor).
 * @param home - the MONOLITH home; defaults to {@link resolveMonolithHome}.
...
export function loadProfile(
  binName: string, name: string, installAnchor: string, home: string = resolveMonolithHome(),
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create it with 'monolith plugin --profile ${name} add <package>'`,
      )
    }
  ...
  // A hand-written profile manifest may omit the monolith section entirely.
  const bundles = manifest.monolith?.profile?.bundles ?? []
  const rawPatchReload: unknown = manifest.monolith?.profile?.patchReload
  if (rawPatchReload !== undefined && rawPatchReload !== 'live' && rawPatchReload !== 'startup') {
    throw new Error(
      `${binName}: profile manifest ${join(dir, 'package.json')} monolith.profile.patchReload must be "live" or "startup"`,
    )
  }
  ...
  const layers = bundles.map((packageName): ProfileLayer => {
    ...
    const declared = bundleManifest.monolith?.bundle?.patch
    if (declared === undefined) {
      throw new Error(`${binName}: profile bundle ${JSON.stringify(packageName)} declares no monolith.bundle in its package.json`)
    }
