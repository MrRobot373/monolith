/**
 * Shared boot glue for `monolith` profiles, including the CLI packaged by the Python runtime wheel: load the gitignored
 * `.env`, install the fail-loud Loader guards, resolve the config path (snapshot-aware), load the
 * optional user patch layers from the MONOLITH home (`~/.monolith`), expose its path resolver to
 * config expressions, and drive the Cordis Loader against a leaf `cordis.yml` until the tree settles.
 * @module @monolith/app-boot
 */
...
import { Context, type FiberState } from '@monolith/cordis'
import Loader, { type Entry, type EntryOptions } from '@monolith/cordis-plugin-loader'
import Include, { applyEntryPatches, entryListSchema, type PatchOptions } from '@monolith/cordis-plugin-include'
import Group from '@monolith/cordis-plugin-group'
import { monolithHomePath, resolveMonolithHome } from '@monolith/home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@monolith/launch-environment'
import type {} from '@monolith/cordis-plugin-hmr'
import type {} from '@monolith/system-prompt'

declare module '@monolith/cordis' {
  interface Context {
    /** MONOLITH-home path resolver available to Loader `!!js` config expressions. */
    monolithHomePath?: typeof monolithHomePath
  }
}
...
export {
  ...
  type MonolithBundleManifest,
  type MonolithManifestSection,
  type MonolithProfileManifest,
  type Profile,
  type ProfileLayer,
 ...
 * @param snapshotMode - the bin's `$MONOLITH_SNAPSHOT` value; only `'replay'` swaps the
 *   basename.
...
const BOOTSTRAP_PREFIXES = ['MONOLITH_', 'XDG_', 'DYLD_', 'BASH_FUNC_']

/**
 * The bootstrap names the MONOLITH-home `.env` alone may set. A proxy chooses the route every
 * request takes, so the invoking directory's file — which arrives with a clone — keeps refusing
 * them; the home file is the user's own, and `MONOLITH_HOME` is itself bootstrap-only, so no `.env` can
 ...
 * because it changes process, runtime, VCS, or network bootstrap. The MONOLITH-home
 ...
 * @param home - the resolved MONOLITH home; when `dir` is it, {@link HOME_LAYER_PROXY_NAMES} are accepted.
...
/**
 * Load the product CLI's inherited > invoking-directory `.env` > MONOLITH-home
 * `.env` snapshot. The MONOLITH home resolves before either file; both files
 ...
 * @throws when either file declares a bootstrap-only variable, except {@link HOME_LAYER_PROXY_NAMES} in the MONOLITH-home file.
 */
export function loadLayeredEnv(
  binName: string, cwd: string = process.cwd(),
  warn: (line: string) => void = line => void process.stderr.write(line),
): LaunchEnvironmentSnapshot {
  const home = resolveMonolithHome()
 ...
 * entries (`@monolith/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@monolith/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@monolith/cordis-plugin-group`
 ...
 * directory. The self-referential `monolith-tool-cordis` toolset reads and edits this
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    text: `The MONOLITH implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend MONOLITH itself.`,
  })
}
