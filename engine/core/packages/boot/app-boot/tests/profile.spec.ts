/**
 * Profile machinery of `monolith-app-boot`: directory resolution and init,
...
const tmp = (): string => mkdtempSync(join(tmpdir(), 'monolith-profile-'))
...
function stageInstallation(
  ...
  for (const [name, spec] of Object.entries(bundles)) {
    ...
    writeFileSync(join(dir, 'package.json'), JSON.stringify({
      ...
      ...spec.patch === undefined ? {} : { monolith: { bundle: { patch: './cordis.patch.yml' } } },
    }))
...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    initProfile(dir, ['@monolith/base'])
    const manifest = readProfileManifest('t', dir)
    expect(manifest.monolith?.profile?.bundles).toEqual(['@monolith/base'])
    expect(manifest.monolith?.profile?.patchReload).toBe('live')
    expect(readFileSync(join(dir, PROFILE_PATCH_FILENAME), 'utf8')).toContain('[]')
    expect(readFileSync(join(dir, 'pnpm-workspace.yaml'), 'utf8')).toContain('nodeLinker: hoisted')
    // Re-init keeps user edits.
    writeFileSync(join(dir, PROFILE_PATCH_FILENAME), '- id: x\n  config: {}\n')
    initProfile(dir, ['other'], 'startup')
    expect(readProfileManifest('t', dir).monolith?.profile?.bundles).toEqual(['@monolith/base'])
    expect(readProfileManifest('t', dir).monolith?.profile?.patchReload).toBe('live')
...
describe('manifest round-trip', () => {
  it('writes and reads back, and fails loud on a broken manifest', () => {
    const dir = tmp()
    writeProfileManifest(dir, { name: 'p', monolith: { profile: { bundles: ['a'] } } })
    expect(readProfileManifest('t', dir).monolith?.profile?.bundles).toEqual(['a'])
...
describe('loadProfile', () => {
  it('resolves each monolith.profile.bundles entry to its patch layer in order, plus the user layer', () => {
    const anchor = stageInstallation({
    ...
    // A hand-made profile without the user layer file or monolith section: empty layers, no throw.
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    // @monolith/* through tsconfig paths regardless of the staged anchor.
    expect(PROFILE_TEMPLATES.web?.bundles).toContain('@monolith/base')
    ...
    expect(readProfileManifest('t', resolveProfileDir('web', home)).monolith?.profile?.bundles)
      .toEqual([...PROFILE_TEMPLATES.web?.bundles ?? []])
    expect(readProfileManifest('t', resolveProfileDir('web', home)).monolith?.profile?.patchReload)
      .toBe('live')
  })
  ...
  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      '@monolith/base': { patch: '[]\n' },
      '@monolith/web-app': { patch: '[]\n' },
      '@monolith/headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    const stock = resolveProfileDir('headless', home)
    initProfile(stock, [
      '@monolith/base', '@monolith/web-app', '@monolith/headless',
    ])
    const retiredManifest = readProfileManifest('t', stock)
    delete retiredManifest.monolith!.profile!.patchReload
    writeProfileManifest(stock, retiredManifest)
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).monolith?.profile).toEqual({
    ...
    initProfile(custom, [
      '@monolith/base', '@monolith/web-app', '@monolith/headless', 'custom-bundle',
    ])
    loadProfile('t', 'headless', anchor, customHome)
    expect(readProfileManifest('t', custom).monolith?.profile?.bundles).toEqual([
      '@monolith/base', '@monolith/web-app', '@monolith/headless', 'custom-bundle',
    ])
  })
  ...
  it('adds a shipped reload default only to an exact stock tuple and preserves explicit choices', () => {
    ...
    delete stockManifest.monolith!.profile!.patchReload
    writeProfileManifest(stock, stockManifest)
    expect(loadProfile('t', 'web', anchor, stockHome).patchReload).toBe('live')
    expect(readProfileManifest('t', stock).monolith?.profile?.patchReload).toBe('live')
  ...
  it('fails loud on an unknown patch reload value from disk', () => {
    ...
    initProfile(dir, [])
    const manifest = readProfileManifest('t', dir)
    const rawProfile = manifest.monolith!.profile as { patchReload?: string }
  ...
  it('fails loud when a listed bundle declares no monolith.bundle', () => {
    const anchor = stageInstallation({ 'not-a-bundle': {} })
    const home = tmp()
    const dir = resolveProfileDir('demo', home)
    initProfile(dir, ['not-a-bundle'])
    expect(() => loadProfile('t', 'demo', anchor, home)).toThrow('declares no monolith.bundle')
  })
})
...
describe('healProfilesModuleFallback', () => {
  it('links the app and bundle dependency surface flat under profiles/node_modules', async () => {
    ...
    for (const name of ['bundle-a', 'plain-lib', 'dep-of-a', 'monolith-app']) {
  ...
  it('throws when a fallback entry is a foreign file or directory', async () => {
    ...
    for (const kind of ['file', 'directory']) {
      const home = tmp()
      const entry = join(home, 'profiles', 'node_modules', 'monolith-app')
  ...
  it('keeps selected bundle closures profile-local without overriding installation packages', async () => {
    ...
    const ownedA = join(profileA.dir, '.monolith-module-fallback', 'node_modules', '@scope', 'bundle-only')
    const ownedB = join(profileB.dir, '.monolith-module-fallback', 'node_modules', '@scope', 'bundle-only')
  ...
  it('discovers dependencies beside a symlinked bundle real path', async () => {
    ...
    expect(readlinkSync(join(dir, '.monolith-module-fallback', 'node_modules', 'bundle-only')))
  ...
  it('traverses every explicit bundle root even when a nested package has the same name', async () => {
    ...
    const ownedModules = join(dir, '.monolith-module-fallback', 'node_modules')
  ...
  it('ignores owned projections while recomputing an ordered bundle closure', async () => {
    ...
    const owned = join(dir, '.monolith-module-fallback', 'node_modules', 'bundle-only')
  ...
  it('cleans owned projections without removing profile-managed entries', async () => {
    ...
    await healProfilesModuleFallback({ installAnchor: installationAnchor, profile, home })
    const ownedModules = join(profile.dir, '.monolith-module-fallback', 'node_modules')
  ...
  it('replaces a wrong symlink', async () => {
    ...
    symlinkSync(tmp(), join(fallback, 'monolith-app'), 'junction')
    await healProfilesModuleFallback({ installAnchor: anchor, home })
    expect(readlinkSync(join(fallback, 'monolith-app'))).toContain('app')
  })
  ...
  it('retains current links while repairing a missing sibling', async () => {
    ...
    const fallback = join(home, 'profiles', 'node_modules')
    await healProfilesModuleFallback({ installAnchor: anchor, home })
    const appTarget = readlinkSync(join(fallback, 'monolith-app'))
    unlinkSync(join(fallback, 'bundle-a'))

    await healProfilesModuleFallback({ installAnchor: anchor, home })

    expect(readlinkSync(join(fallback, 'monolith-app'))).toBe(appTarget)
  ...
  it('serializes concurrent healers and retains the identical link', async () => {
    ...
    const fallback = join(home, 'profiles', 'node_modules')
    expect(lstatSync(join(fallback, 'monolith-app')).isSymbolicLink()).toBe(true)
  })
  ...
  it('waits for the module-fallback writer lock before publishing entries', async () => {
    ...
    expect(existsSync(join(modules, 'monolith-app'))).toBe(false)
    releaseLock?.()
    await Promise.all([holder, healer])
    expect(lstatSync(join(modules, 'monolith-app')).isSymbolicLink()).toBe(true)
  })
  ...
  it('writes real ESM proxies for a packaged executable', async () => {
    ...
    try {
      ...
      const proxyManifest = JSON.parse(readFileSync(join(proxy, 'package.json'), 'utf8')) as {
        version: unknown
        exports: unknown
        monolith: { moduleFallback: { targets: Record<string, unknown> } }
      }
      expect(proxyManifest).toMatchObject({
        version: '0.0.0',
        exports: { '.': './entry-0.js', './feature': './entry-1.js' },
      })
      expect(proxyManifest.monolith.moduleFallback.targets['.']).toEqual(expect.stringContaining('/bundle-a/index.js'))
  ...
  it('preserves the installation path while resolving packaged exports', async () => {
    ...
    try {
      ...
      const proxyManifest = JSON.parse(readFileSync(
        ...
        'utf8',
      )) as { monolith: { moduleFallback: { targets: Record<string, string> } } }
      expect(proxyManifest.monolith.moduleFallback.targets['.']).toContain('/app/node_modules/linked-esm/index.js')
    } finally {
      delete (process as NodeJS.Process & { pkg?: unknown }).pkg
    }
  })
  ...
  it('skips executable-only and declaration-only packages without import entries', async () => {
    for (const marker of ['bin', 'types', 'typings']) {
      ...
      delete manifest.main
      manifest[marker] = marker === 'bin' ? { monolith: './lib/bin.js' } : './index.d.ts'
      ...
      try {
        ...
        await healProfilesModuleFallback({ installAnchor: anchor, home })
        const fallback = join(home, 'profiles', 'node_modules')
        expect(existsSync(join(fallback, 'monolith-app'))).toBe(false)
  ...
  it('rejects foreign packaged fallback directories with valid or invalid metadata', async () => {
    ...
    try {
      for (const metadata of ['{}', '{']) {
        ...
        await expect(healProfilesModuleFallback({ installAnchor: anchor, home })).rejects.toThrow(
          'exists and is not a monolith-managed module proxy',
        )
      }
    } finally {
      delete (process as NodeJS.Process & { pkg?: unknown }).pkg
