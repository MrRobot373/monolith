...
const NAME = 'monolith-test-bin'

const tmp = (): string => mkdtempSync(join(tmpdir(), 'monolith-app-boot-'))
...
describe('loadEnv', () => {
  it('loads variables from .env in the given dir', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'MONOLITH_APP_BOOT_SPEC_VAR=loaded\n')
    const warn = vi.fn()
    loadEnv(NAME, dir, warn)
    expect(process.env['MONOLITH_APP_BOOT_SPEC_VAR']).toBe('loaded')
    expect(warn).not.toHaveBeenCalled()
    delete process.env['MONOLITH_APP_BOOT_SPEC_VAR']
  })
  ...
  it('defaults dir to the process cwd and warn to a stderr write', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'MONOLITH_APP_BOOT_SPEC_DEFAULTS=yes\n')
    ...
    expect(process.env['MONOLITH_APP_BOOT_SPEC_DEFAULTS']).toBe('yes')
    delete process.env['MONOLITH_APP_BOOT_SPEC_DEFAULTS']
...
describe('loadLayeredEnv', () => {
  ...
  it.each([
    ['a harness switch', 'MONOLITH_PERMISSION_MODE=danger-full-access\n'],
    ['the executable search path', 'PATH=/tmp/evil\n'],
    ['a module preload', 'NODE_OPTIONS=--require /tmp/evil.js\n'],
    ['a skill root', 'MONOLITH_AGENTS_HOME=/tmp/injected\n'],
  ...
  it('accepts the proxy names from the MONOLITH-home .env, below an exported one', () => {
    const home = tmp()
    const project = tmp()
  ...
  it('still refuses every other bootstrap name in the MONOLITH-home .env', () => {
    const home = tmp()
    const project = tmp()
  ...
  it('names the MONOLITH-home file as the way out when a project .env sets a proxy', () => {
    const home = tmp()
    const project = tmp()
  ...
  it('treats the invoking directory as the MONOLITH home when they are the same directory', () => {
    const home = tmp()
    writeFileSync(join(home, '.env'), 'HTTP_PROXY=http://from-home:8080\n')
    clear(); clearProxy()
    vi.stubEnv('MONOLITH_HOME', home)
...
describe('boot', () => {
  ...
  it('can resolve bare plugins from the harness when the config project shadows their package name', async () => {
    ...
    const shadow = join(dir, 'node_modules', '@monolith', 'system-prompt')
    const harnessPlugin = join(harness, 'node_modules', '@monolith', 'system-prompt')
  ...
  it('exposes monolithHomePath to Loader config expressions', async () => {
    const dir = tmp()
    const monolithHome = join(dir, 'home')
    vi.stubEnv('MONOLITH_HOME', monolithHome)
    ...
    writeFileSync(join(dir, 'cordis.yml'), [
      ...
      '  name: ./capture.mjs',
      '  config:',
      "    path: !!js monolithHomePath('sessions')",
      '',
    ].join('\n'))
    let ctx: Context | undefined
    try {
      ctx = await boot(NAME, join(dir, 'cordis.yml'))
      expect(ctx.get('capturedPath')).toBe(join(monolithHome, 'sessions'))
    } finally {
      await ctx?.fiber.dispose()
      vi.unstubAllEnvs()
    }
...
describe('addHarnessSourceSection', () => {
  ...
  it('distinguishes the source path from the current workdir between identity and persona', async () => {
    ...
    try {
      ...
      // through `-1 < n`.
      const identityAt = rendered.indexOf('You are an AI agent powered by MONOLITH.')
