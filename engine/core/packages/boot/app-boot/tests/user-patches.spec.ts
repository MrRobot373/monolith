/**
 * User patch-layer behavior of `monolith-app-boot`: the optional patch-list loader
...
const tmp = (): string => mkdtempSync(join(tmpdir(), 'monolith-user-patches-'))
...
describe('loadOptionalPatches', () => {
  ...
  it('parses a patch list and preserves !!js expressions as loader expression nodes', () => {
    ...
    writeFileSync(join(dir, PROFILE_PATCH_FILENAME), [
      ...
      '  config:',
      '    model: !!js process.env.MONOLITH_SPEC_MODEL',
      '- insert:',
      '    - id: llm',
    ...
    expect(patches?.[0]).toMatchObject({
      id: 'agent-loop',
      config: { model: { __jsExpr: 'process.env.MONOLITH_SPEC_MODEL' } },
    })
...
describe('boot with user patches', () => {
  it('applies id-targeted overrides, inserts, and interpolates !!js from the environment', async () => {
    ...
    writeFileSync(join(userDir, PROFILE_PATCH_FILENAME), [
      ...
      '  name: ./noop.mjs',
      '  config:',
      '    value: !!js process.env.MONOLITH_APP_BOOT_USER_SPEC',
      '- insert:',
      '    - id: user-extra',
      '      name: ./noop.mjs',
      '',
    ].join('\n'))
    process.env['MONOLITH_APP_BOOT_USER_SPEC'] = 'user-value'
    ...
    try {
      ...
      await ctx.fiber.dispose()
      delete process.env['MONOLITH_APP_BOOT_USER_SPEC']
    }
  })
