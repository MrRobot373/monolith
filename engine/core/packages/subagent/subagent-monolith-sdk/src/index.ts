 ...
 * MONOLITH runtime in its own process — own named profile and patch composition,
 ...
 * @module @monolith/subagent-monolith-sdk
 */
...
import type { AgentOptions } from '@monolith/agent'
import type { SubagentCapabilities, SubagentProvider, SubagentStartRequest } from '@monolith/subagent'
import { assertPositiveFinite, NO_START_CAPABILITIES, resolveChildCwd, validateConfiguredCwd } from '@monolith/subagent'
import {
...
export const name = 'subagent-monolith-sdk'
export const inject = ['subagents']

/** Config: how to spawn and drive the child SDK runtime process. */
export interface Config {
  /** Provider name on `ctx.subagents` (default `monolith-sdk`). */
  providerName: string
  /** Explicit monolith CLI module, resolved and checked at plugin load; omission uses the SDK dependency. */
  monolithBin?: string
  /** Named child profile (default `sdk`). */
  profile: string
  /** Ordered per-launch profile patch files, resolved and checked at plugin load. */
  patches: string[]
  /** Absolute isolated MONOLITH home for every nested child process. */
  monolithHome: string
  /**
...
export const Config: z<Config> = z.object({
  providerName: z.string().default('monolith-sdk'),
  monolithBin: z.string(),
  profile: z.string().default('sdk'),
  patches: z.array(z.string()).default([]),
  monolithHome: z.string().required(),
  cwd: z.string(),
...
type ResolvedConfig = Required<Omit<Config, 'cwd' | 'maxTokens' | 'monolithBin'>> & Pick<Config, 'cwd' | 'maxTokens' | 'monolithBin'>
...
function resolveConfiguredFile(field: string, value: string): string {
  ...
  throw new TypeError(`subagent-monolith-sdk ${field} must name an existing file: ${path}`)
}

/** MONOLITH SDK can apply Agent route options while the other start features remain child-owned. */
...
class SdkSubagentProvider implements SubagentProvider {
  ...
  start(request: SubagentStartRequest) {
    ...
    let cwd: string
    try {
      cwd = resolveChildCwd('subagent-monolith-sdk', this.config.cwd, request.parent.session.header.cwd)
    } catch (error: unknown) {
      const failure = sdkConfigurationFailure(error)
      this.ctx.logger.warn(`subagent-monolith-sdk "${this.name}": child start failed: %o`, error)
      throw failure
    }
    const route = resolveSdkRoute(this.config, request.agentOptions)
    const spec: SdkRunSpec = {
      ...this.config.monolithBin === undefined ? {} : { monolithBin: this.config.monolithBin },
      profile: this.config.profile,
      patches: this.config.patches,
      monolithHome: this.config.monolithHome,
      cwd,
      ...route,
      env: this.config.env,
      ...
      onError: (error, stopReason) => {
        ...
        this.ctx.logger.warn(`subagent-monolith-sdk "${this.name}": child run failed (${stopReason}): ${error.message}`)
      },
    }
...
export function apply(ctx: Context, config: Config): void {
  ...
  assertPositiveFinite('subagent-monolith-sdk', 'shutdownTimeoutMs', resolved.shutdownTimeoutMs)
  assertPositiveFinite('subagent-monolith-sdk', 'disposeEofGraceMs', resolved.disposeEofGraceMs)
  assertPositiveFinite('subagent-monolith-sdk', 'disposeGraceMs', resolved.disposeGraceMs)
  if (resolved.maxTokens !== undefined && (!Number.isSafeInteger(resolved.maxTokens) || resolved.maxTokens <= 0)) {
    throw new TypeError('subagent-monolith-sdk maxTokens must be a positive safe integer')
  }
  if (!isAbsolute(resolved.monolithHome)) throw new TypeError('subagent-monolith-sdk monolithHome must be an absolute path')
  const launchPaths: ResolvedConfig = {
    ...resolved,
    patches: resolved.patches.map((path, index) => resolveConfiguredFile(`patches[${String(index)}]`, path)),
    ...resolved.monolithBin === undefined ? {} : { monolithBin: resolveConfiguredFile('monolithBin', resolved.monolithBin) },
  }
  ...
  const configuredCwd = validateConfiguredCwd('subagent-monolith-sdk', resolved.cwd)
