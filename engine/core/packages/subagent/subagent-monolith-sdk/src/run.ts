/**
 * Fresh-process SDK subagent client. Drives one child MONOLITH
 * runtime over stdio JSON-RPC through `@monolith/sdk-client` and owns
 ...
 *
 * @module @monolith/subagent-monolith-sdk/run
 */
...
import type { ContentBlock, ReasoningEffortId } from '@monolith/llm'
import type { SessionEvent, SessionId, TurnEndReason } from '@monolith/session'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@monolith/subagent'
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from '@monolith/subagent'
...
export interface SdkRunSpec {
  /** Explicit monolith CLI module; omission resolves the SDK client's same-version dependency. */
  monolithBin?: string
  /** Named child profile. */
  profile: string
  /** Ordered per-launch profile patch files. */
  patches: string[]
  /** Absolute isolated MONOLITH home for the nested runtime. */
  monolithHome: string
  /**
   ...
   * current `MONOLITH_*` fact survives while ambient namesakes never leak.
   */
...
class SdkRunFailure extends Error {
  constructor(readonly facts: SdkFailureFacts, cause: unknown) {
    super(`subagent-monolith-sdk: ${failureDiagnostic(facts)}`, { cause })
    this.name = 'SdkRunFailure'
  }
}

/** Runtime constructor seam replaced only by package-local fake-runtime tests. */
export const internals: { createHarness(options: MONOLITHOptions): MONOLITH } = {
  createHarness: options => new MONOLITH(options),
}
 ...
 * @returns an Error whose message contains only the fixed MONOLITH SDK failure line.
 */
...
export async function startSdkRun(request: SubagentStartRequest, spec: SdkRunSpec): Promise<SubagentRun> {
  ...
  const harness = internals.createHarness({
    ...spec.monolithBin === undefined ? {} : { monolithBin: spec.monolithBin },
    profile: spec.profile,
    patches: spec.patches,
    monolithHome: spec.monolithHome,
    processCwd: spec.cwd,
