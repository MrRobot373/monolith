...
import type { Agent, AgentOptions } from '@monolith/agent'
import {
  MONOLITH,
  HarnessClient,
  HarnessSession,
  SdkProtocolError,
} from '@monolith/sdk-client'
import { createProcessMONOLITH } from '../../../sdk/client/src/api.ts'
import type { RuntimeProcessOptions } from '../../../sdk/client/src/launch.ts'
import type { MONOLITHOptions } from '@monolith/sdk-client'
import { ReasoningEffortId } from '@monolith/llm'
...
let createdHarnessOptions: MONOLITHOptions[] = []
...
beforeEach(() => {
  ...
  runInternals.createHarness = (options) => {
    ...
    return createProcessMONOLITH(runtime, options)
  }
})
...
async function setup(fakeEnv: Record<string, string> = {}, config: Partial<sdk.Config> = {}) {
  ...
  await ctx.plugin(sdk, {
    providerName: 'monolith-sdk',
    profile: 'sdk',
    patches: [],
    monolithHome: process.cwd(),
...
function expectedFailure(fields: string): string {
  return `Subagent failure (provider: MONOLITH SDK; ${fields})`
}
...
describe('monolith-subagent-monolith-sdk provider', () => {
  it('constructs the production monolith-backed harness lazily', async () => {
    const harness = defaultCreateHarness({})
    expect(harness).toBeInstanceOf((await import('@monolith/sdk-client')).MONOLITH)
    await harness.close()
  })

  it('runs a child turn end to end with a parent-unique run id', async () => {
    const ctx = await setup({ FAKE_TEXT: 'hello from sdk child' })
    const run = await ctx.subagents.start('monolith-sdk', request('do X'))
    ...
    const nextRun = await ctx.subagents.start('monolith-sdk', request('again'))
  ...
  it('resolves relative launch files at load and forwards absolute paths', async () => {
    const ctx = await setup({ FAKE_TEXT: 'explicit monolith child' }, {
      monolithBin: relative(process.cwd(), fakeRuntime),
      patches: [relative(process.cwd(), existingPatch)],
    })
    const run = await ctx.subagents.start('monolith-sdk', request())
    expect(text((await run.result).output)).toBe('explicit monolith child')
    expect(createdHarnessOptions[0]).toMatchObject({
      monolithBin: fakeRuntime,
  ...
  it('initializes the child with the configured provider/model/maxTokens and the parent cwd', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-monolith-sdk-init-'))
  ...
  it('preserves instance defaults around a partial request override', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-monolith-sdk-partial-route-'))
    const recordFile = join(tmp, 'init.jsonl')
    try {
      const ctx = await setup({ FAKE_RECORD_INIT: recordFile }, { maxTokens: 4096 })
      const run = await ctx.subagents.start('monolith-sdk', request('partial', new AbortController().signal, {
  ...
  it('isolates complete per-run route overrides on concurrent children', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-monolith-sdk-routes-'))
    const recordFile = join(tmp, 'init.jsonl')
    try {
      const ctx = await setup({ FAKE_RECORD_INIT: recordFile }, { maxTokens: 4096 })
      const runs = await Promise.all([
        ctx.subagents.start('monolith-sdk', request('first', new AbortController().signal, {
          provider: 'provider-a',
          model: 'model-a',
          reasoningEffort: ReasoningEffortId('high'),
          maxTokens: 111,
        })),
        ctx.subagents.start('monolith-sdk', request('second', new AbortController().signal, {
          provider: 'provider-b',
          model: 'model-b',
  ...
  it('scrubs ambient credentials but forwards explicit config env', async () => {
    process.env.MONOLITH_TEST_AMBIENT_SECRET_KEY = 'leak-me-not'
    try {
      const ctx = await setup({
        FAKE_ECHO_ENV: 'MONOLITH_TEST_AMBIENT_SECRET_KEY,DEEPSEEK_API_KEY',
      ...
      expect(answer).toContain('MONOLITH_TEST_AMBIENT_SECRET_KEY=\n')
      expect(answer).toContain('DEEPSEEK_API_KEY=explicit-child-key')
      await run.dispose()
      await ctx.fiber.dispose()
    } finally {
      delete process.env.MONOLITH_TEST_AMBIENT_SECRET_KEY
    }
  })
  ...
  it('aggregates safe initialize and shutdown facts when startup rollback fails', async () => {
    ...
    try {
      ...
      const error = await ctx.subagents.start('monolith-sdk', request()).catch((cause: unknown) => cause)
      expect(error).toBeInstanceOf(AggregateError)
      expect((error as Error).message).toBe(
        `subagent-monolith-sdk: ${expectedFailure('stage: initialize; category: protocol')}; `
        + `subagent-monolith-sdk: ${expectedFailure('stage: shutdown; category: unknown')}`,
      )
  ...
  it('reports only safe shutdown facts when cancelled startup rollback fails', async () => {
    ...
    const spy = vi.spyOn(MONOLITH.prototype, 'close').mockImplementation(async function (this: MONOLITH) {
      spy.mockRestore()
      await this.close()
      throw new Error(rawCleanup)
    })
  ...
  it('keeps an initialize failure authoritative when a later abort flag is already set', async () => {
    ...
    const start = vi.spyOn(MONOLITH.prototype, 'start').mockRejectedValue(rawFailure)
    const close = vi.spyOn(MONOLITH.prototype, 'close').mockResolvedValue()
    try {
      const controller = new AbortController()
      const pending = startSdkRun(request('p', controller.signal), {
        profile: 'sdk',
  ...
  it('aborting the required signal settles a hung child as aborted', async () => {
    ...
    const controller = new AbortController()
    const run = await ctx.subagents.start('monolith-sdk', request('p', controller.signal))
  ...
  it('cancelling between handshake and publish rejects start after reap', async () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-monolith-sdk-midcancel-'))
  ...
  it('rejects WITHOUT spawning when the signal is already aborted', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-monolith-sdk-preabort-'))
  ...
  it('rejects a pre-aborted request through the registered provider before cwd resolution', async () => {
    ...
    await expect(ctx.subagents.start('monolith-sdk', {
      label: 'p',
  ...
  it('rejects after reaping when the child dies before the handshake', async () => {
    ...
    const failure = await ctx.subagents.start('monolith-sdk', request()).then(
      () => { throw new Error('start unexpectedly succeeded') },
      (error: unknown) => error,
    )
    expect(String(failure)).toBe(
      `SdkRunFailure: subagent-monolith-sdk: ${expectedFailure('stage: initialize; category: transport')}`,
    )
  ...
  it.each([
    ...
    await expect(ctx.subagents.start('monolith-sdk', request())).rejects.toThrow(
      `subagent-monolith-sdk: ${expectedFailure(`stage: initialize; category: ${category}`)}`,
    )
    await ctx.fiber.dispose()
  })
  ...
  it('routes provider-level onError through ctx.logger.warn', async () => {
    ...
    expect(warnings[0]).toContain('subagent-monolith-sdk "monolith-sdk": child run failed (error)')
    await run.dispose()
    await ctx.fiber.dispose()
  })
  ...
  it('rejects non-positive timing bounds at load', async () => {
    ...
    await ctx.plugin(SessionProjectionRegistry)
    await ctx.plugin(SubagentRuntime)
    const base = { providerName: 'sdk', profile: 'sdk', patches: [], monolithHome: process.cwd(), provider: 'p', model: 'm', env: {} }
  ...
  it('requires an explicit absolute MONOLITH home for nested monolith runtimes', async () => {
    const ctx = new Context()
    await ctx.plugin(SubagentRuntime)
    await expect(ctx.plugin(sdk, {
      ...
      profile: 'sdk',
      patches: [],
      monolithHome: './personal-home',
      provider: 'p',
      model: 'm',
      env: {},
    })).rejects.toThrow('monolithHome must be an absolute path')
    await ctx.fiber.dispose()
  })

  it.each([
    { field: 'monolithBin', override: { monolithBin: './missing-monolith-bin' } },
    { field: 'monolithBin', override: { monolithBin: '.' } },
  ...
  it('uses a validated config cwd override instead of the parent session cwd', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-monolith-sdk-cwd-'))
    try {
  ...
  it('fails loud when neither config cwd nor parent session cwd exists', async () => {
    ...
    await expect(ctx.subagents.start('monolith-sdk', {
      ...
      .rejects.toThrow(
        `subagent-monolith-sdk: ${expectedFailure('stage: initialize; category: configuration')}`,
      )
    await ctx.fiber.dispose()
  })

  it('keeps named plugin exports with no default export (loader shape)', () => {
    expect(sdk.name).toBe('subagent-monolith-sdk')
