...
import SandboxedFileSystem from '@monolith/fs-sandbox'
...
beforeEach(async () => {
  workspace = await realpath(await mkdtemp(join(tmpdir(), 'monolith-inherit-')))
})
  ...
  it('records the parent sandbox override and the approval pin before publishing a spawn child', async () => {
    ...
    try {
      ...
      const contextText = runtimeContext.data.content
        ...
        .map(block => block.text)
        .join('\n')
      expect(contextText).toContain('Current MONOLITH file policy: read-only')
