...
- [Generated configuration catalog](../../../docs/config-catalog.md#monolith-subagent-codex) — every accepted config field and its source declaration.
...
- **No human approval path** — known unattended approval requests are denied and unknown server requests fail closed; the three Profile modes never create a MONOLITH interaction channel or per-call allow policy.
