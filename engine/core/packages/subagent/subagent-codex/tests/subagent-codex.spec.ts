...
describe('task admission and package contracts', () => {
  it('ships one independently installable provider-only Bundle patch', () => {
    ...
    expect(manifest.dependencies).not.toHaveProperty('@monolith/subagent-claude-code')
  ...
  it('keeps named instances, runs, and HMR ownership isolated', async () => {
    ...
    vi.spyOn(ctx.subprocess, 'spawn').mockImplementation((spec) => {
      ...
      return spec.env?.MONOLITH_CODEX_INSTANCE === 'safe'
        ? safeChild.handle
        : bypassChild.handle
    })
    const added: string[] = []
    ...
    const safeFiber = await ctx.plugin(codex, {
      ...
      env: { MONOLITH_CODEX_INSTANCE: 'safe' },
    ...
    const bypassFiber = await ctx.plugin(codex, {
      ...
      env: { MONOLITH_CODEX_INSTANCE: 'bypass' },
    ...
    expect(spawnSpecs.map(spec => ({
      instance: spec.env?.MONOLITH_CODEX_INSTANCE,
