 ...
 * quiescent disposal.
 *
 * @module @monolith/subagent-acp/run
 */
...
import type { SubprocessHandle, SubprocessOutcome, SubprocessSpawnSpec } from '@monolith/subprocess'
...
export interface AcpRunSpec {
   ...
   * Explicit `MONOLITH_*` entries are deployment-owned facts for the child harness
   * (e.g. `MONOLITH_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and MONOLITH_* names
  ...
  const clientApp = createAcpClientApp({ name: 'monolith-subagent-acp' })
