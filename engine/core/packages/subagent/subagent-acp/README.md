...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-subagent-acp) is the exhaustive source for every accepted field and its JSDoc.

A MONOLITH child uses the product launcher and an explicit absolute `MONOLITH_HOME`. The isolated home prevents a nested runtime from discovering the launching person's profiles or credentials; the generic ACP provider does not impose this requirement on non-MONOLITH agents.

```yaml
- id: subagent-acp
  name: '@monolith/subagent-acp'
  config:
    providerName: acp
    command: monolith
    ...
    permission: reject
    env:
      MONOLITH_HOME: /absolute/path/to/isolated-child-home
...
- [monolith-subprocess seam](../../subprocess/subprocess/README.md) — the process-spawn and teardown machinery behind each run.
- [Generated configuration catalog](../../../docs/config-catalog.md#monolith-subagent-acp) — every accepted config field and its source declaration.
