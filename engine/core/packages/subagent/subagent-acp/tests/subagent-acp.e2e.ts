...
/**
 * With-key cross-process boundary proof: the backend spawns the real monolith ACP profile, speaks ACP over
...
// The real ACP profile: monolith plus the example's live DeepSeek patch.
...
// How to launch the child ACP profile (src via tsx / lib via plain node, per MONOLITH_EXAMPLE_MODE).
...
function resolveChildLaunch(monolithHome: string) {
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('ACP backend with-key e2e (drive our own acp-agent)', () => {
  it('drives the real acp-agent example process to answer a prompt', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'monolith-subagent-acp-e2e-'))
    const childLaunch = resolveChildLaunch(join(workdir, '.monolith-child'))
    ctx = new Context()
