...
import type { SubprocessHandle, SubprocessOutcome } from '@monolith/subprocess'
...
describe('child env layering (through the subprocess seam)', () => {
  ...
  it('forwards explicit MONOLITH_* config entries to the child', async () => {
    // A deployment sets child-harness facts like MONOLITH_PERMISSION_MODE in
    ...
    const ctx = await setup({ MOCK_ECHO_ENV: 'MONOLITH_ACP_TEST_FACT', MONOLITH_ACP_TEST_FACT: 'managed' })
...
describe('disposeAcpChild (the backend-owned teardown ladder over seam verbs)', () => {
  ...
  it('observes a spawn-level rejection and returns without a process to reap', async () => {
    const child = spawnSubprocess({
      ...
      cwd: '/nonexistent-dir-monolith-acp-ladder-test',
      stdio: { stdin: 'ignore', stdout: { maxBytes: 1000 }, stderr: { maxBytes: 1000 } },
      graceMs: 200,
    })
...
describe('cwd resolution', () => {
  ...
  it('resolves a relative config cwd against the launch directory at load', async () => {
    ...
    // ABSOLUTE form — MONOLITH's own ACP server rejects a relative session cwd, and
...
describe('monolith-subagent-acp', () => {
