...
import { ReasoningEffortId, boundContextSummary, contentHasImage, createUserMessage, errorChain } from '@monolith/llm'
import type { ContentBlock, MessageId, MessageSource } from '@monolith/llm'
import { SessionLogOffset } from '@monolith/session'
import type { SessionEvent, SessionId , SessionLogOffset as SessionLogOffsetType } from '@monolith/session'
import type { SessionPersistence } from '@monolith/session-persistence'
import type { SessionObservation, SessionQueryEngine } from '@monolith/session-query'
import type { ToolRestriction } from '@monolith/tools'
...
export class SubagentContinuationManager {
  ...
  private requirePersistence(): SessionPersistence {
    ...
    if (persistence === undefined) {
      throw new SubagentError(
        'continuable subagents require session persistence (load a monolith-session-persistence backend)',
  ...
  private requireSessionQuery(): SessionQueryEngine {
    ...
    if (query === undefined) {
      throw new SubagentError(
        'continuable subagents require session query (load @monolith/session-query)',
