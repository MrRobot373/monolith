...
import type { ContentBlock, GenerateOptions, MessageId, StreamChunk } from '@monolith/llm'
import { ToolCallId, createUserMessage, LlmAdapter, ReasoningEffortId } from '@monolith/llm'
...
async function setupWith(
  ...
  if (options.persistence !== false) {
    root = mkdtempSync(join(tmpdir(), 'monolith-subagent-continuation-'))
...
describe('continuable settlement delivery', () => {
  ...
  it('reports a turn that failed before reaching its first step', async () => {
    ...
    const { ctx, parent } = await setupWith(adapter)
    // The shipped durability checkpoint (`monolith-session-checkpoint-policy`) is
...
describe('continuable errors', () => {
  ...
  it('unloading the manager drains its live activations', async () => {
    ...
    await mountAgentLoopTestDependencies(ctx)
    await ctx.plugin(SessionProjectionRegistry)
    const root = mkdtempSync(join(tmpdir(), 'monolith-subagent-continuation-'))
    const persistenceFiber = await ctx.plugin(JsonlSessionPersistence, { root })
    cleanups.push(async () => {
      await persistenceFiber.dispose()
