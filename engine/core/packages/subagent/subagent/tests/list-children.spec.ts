...
import SessionStore, { SessionLogOffset, SessionSeq, SESSION_FORMAT_VERSION, SessionId } from '@monolith/session'
...
import SessionProjectionCache from '@monolith/session-projection-cache'
import Storage from '@monolith/storage'
import {
  apply as storageJsonApply, Config as storageJsonConfig, inject as storageJsonInject, name as storageJsonName,
} from '@monolith/storage-json'
import {
  apply as storageDomainApply, Config as storageDomainConfig, inject as storageDomainInject, name as storageDomainName,
} from '@monolith/storage-domain'
import SubagentRuntime, {
  SUBAGENT_DESCRIPTOR_VERSION,
  SubagentError,
} from '@monolith/subagent'
import * as SubagentSpawn from '@monolith/subagent-spawn-in-process'
import * as SubagentFork from '@monolith/subagent-fork-in-process'
...
async function setup(
  ...
  const root = mkdtempSync(join(tmpdir(), 'monolith-subagent-list-'))
  roots.push(root)
  ...
  if (options.projectionCache === true) {
    const root = mkdtempSync(join(tmpdir(), 'monolith-subagent-projcache-'))
