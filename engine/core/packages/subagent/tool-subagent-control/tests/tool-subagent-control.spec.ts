...
import { ToolCallId, createUserMessage } from '@monolith/llm'
...
async function setupWith(adapter: MockAdapter | GatedAdapter, park = true) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'monolith-tool-subagent-control-'))
  roots.push(root)
...
describe('monolith-tool-subagent-control', () => {
...
describe('monolith-tool-subagent-control interrupt_agent', () => {
