...
import type { SubagentListEntry } from '@monolith/subagent'
...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'monolith-tool-list-agents-'))
  roots.push(root)
...
describe('monolith-tool-subagent-control/list-agents', () => {
