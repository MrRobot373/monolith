...
import { ToolCallId, ReasoningEffortId } from '@monolith/llm'
import SystemPrompt from '@monolith/system-prompt'
import ToolRuntime, { TOOL_ABORTED_BEFORE_DISPATCH } from '@monolith/tools'
import { assembleContextFor, type Agent } from '@monolith/agent'
...
import type { SubagentStartRequest } from '@monolith/subagent'
import LocalJobRegistry from '@monolith/jobs-local'
import * as SubagentSpawn from '@monolith/subagent-spawn-in-process'
...
/**
 * Drives the REAL plugin body: mounts `monolith-tool-subagent` on a real
...
describe('monolith-tool-subagent', () => {
...
describe('monolith-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @monolith/jobs')
  })
...
describe('monolith-tool-subagent continuable background mode', () => {
  const roots: string[] = []
  afterEach(() => {
  ...
  async function continuableSetup() {
    ...
    const root = mkdtempSync(path.join(tmpdir(), 'monolith-tool-subagent-continuable-'))
    roots.push(root)
