...
import type { SubprocessHandle } from '@monolith/subprocess'
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)(
  ...
  () => {
    it('returns one unique nonce through the production provider and real SDK/CLI', async () => {
      ...
      const root = mkdtempSync(join(tmpdir(), 'monolith-claude-deepseek-e2e-'))
      roots.push(root)
      ...
      const nonce = `MONOLITH_CLAUDE_DEEPSEEK_${randomUUID()}`
      const parent = {
