...
import type { ContentBlock } from '@monolith/llm'
...
describe('task admission and package contracts', () => {
  it('ships one independently installable provider-only Bundle patch', () => {
    ...
    expect(manifest.dependencies).not.toHaveProperty('@monolith/subagent-codex')
    ...
    const parsed = yaml.load(readFileSync(resolve(root, manifest.monolith!.bundle!.patch!), 'utf8'))
    const rows = Array.isArray(parsed)
      ? (parsed as Array<{ insert?: Array<{ id?: string; name?: string }> }>).flatMap(entry => entry.insert ?? [])
      : []
    expect(rows).toEqual([{
      id: 'subagent-claude-code',
      name: '@monolith/subagent-claude-code',
    }])
  ...
  it('keeps named instances, runs, and HMR ownership isolated', async () => {
    ...
    vi.spyOn(ctx.subprocess, 'spawn').mockImplementation((spec) => {
      ...
      return spec.env?.MONOLITH_CLAUDE_INSTANCE === 'safe'
        ? safeChild.handle
        : bypassChild.handle
    })
    ...
    const safeFiber = await ctx.plugin(claudeCode, {
      ...
      env: { MONOLITH_CLAUDE_INSTANCE: 'safe' },
    ...
    const bypassFiber = await ctx.plugin(claudeCode, {
      ...
      env: { MONOLITH_CLAUDE_INSTANCE: 'bypass' },
    ...
    expect(queryOptions.map(options => ({
      instance: options.env?.MONOLITH_CLAUDE_INSTANCE,
      model: options.model,
    ...
    expect(spawnSpecs.map(spec => ({
      instance: spec.env?.MONOLITH_CLAUDE_INSTANCE,
  ...
  it('starts through the registered provider with its resolved config and diagnostics', async () => {
    ...
    await ctx.plugin(claudeCode, {
      ...
      env: {
        ...
        CLAUDE_CONFIG_DIR: '/private/tmp/monolith-claude-code-unit-config',
        HOME: '/private/tmp/monolith-claude-code-unit-home',
      },
      permissionMode: 'auto',
      disposeGraceMs: 29,
    })
...
describe('query options and result mapping', () => {
  it('builds the fixed unattended options over the scrubbed environment', async () => {
    ...
    vi.stubEnv('MONOLITH_INTERNAL', 'must-not-leak')
    const child = fakeChild()
    ...
    expect(options.env).not.toHaveProperty('MONOLITH_INTERNAL')
