---
description: "Shared resolution of the MONOLITH home and user-data paths for packages that need one consistent root, tilde expansion, and stable watch paths."
...
`monolith-home-paths` resolves the single MONOLITH home that all user data lives under, and joins child paths onto it, so every product package agrees on where its files go. Precedence is explicit: a configured path wins, then `$MONOLITH_HOME`, then `~/.monolith`, and an empty or whitespace-only `$MONOLITH_HOME` counts as unset. The package also expands `~`, `~/...`, and `~\...` prefixes against the operating-system home, and canonicalizes a watch target so a native filesystem watcher gets one stable path spelling even when the final components do not exist yet. It is a zero-dependency library that product packages import directly; a `cordis.yml` cannot load it.
...
```ts
import { resolveMonolithHome, monolithHomePath } from '@monolith/home-paths'

const home = resolveMonolithHome()                // configured path, else $MONOLITH_HOME, else ~/.monolith
const settings = monolithHomePath('settings')     // join one child onto the resolved home
```

An explicit configured path has the highest precedence, then `$MONOLITH_HOME`, then the default `~/.monolith`. An empty or whitespace-only `$MONOLITH_HOME` is treated as unset, so a blank override never resolves the home to the current working directory.

### Displaying a home

For user-facing paths, render the root symbolically rather than as a machine path: the default home displays as `~/.monolith` and any configured home displays as `$MONOLITH_HOME`. The display form never leaks an absolute machine path.
...
`resolveMonolithHome` reads the explicit override, then `$MONOLITH_HOME`, then falls back to the operating-system home joined with `.monolith`. The chosen value is tilde-expanded and normalized to an absolute path; `monolithHomePath` joins child segments with Node's platform path rules. `monolithHomeDisplay` compares the resolved path against the default root and returns the symbolic label, so a configured home never leaks its absolute path.
...
- [Shell environment](../../shell/shell-env/README.md) — how `MONOLITH_HOME` reaches model shell calls.
