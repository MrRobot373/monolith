...
```ts
import { ItemRetainer } from '@monolith/output-retention'
...
```text
import { TextRetainer } from '@monolith/output-retention'
...
```ts
import { formatRetentionNotice } from '@monolith/output-retention'

declare const grepMaxMatches: number
declare const items: { length: number }
import type { Omitted } from '@monolith/output-retention'
