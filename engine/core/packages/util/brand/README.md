...
```ts
import { brandString, type Branded } from '@monolith/brand'
...
```ts
import { brandNumber, type BrandedNumber } from '@monolith/brand'
...
Brand values that cross package boundaries and could plausibly be confused — `ToolCallId` in `monolith-llm`, the shared agent/session `SessionId` in `monolith-session`, `JobId` in `monolith-jobs`, and `SessionSeq` versus `SessionLogOffset` in `monolith-session`. Values that stay local or cannot be confused do not need this abstraction.
...
Keeping these helpers in their own package means `monolith-jobs` can brand `JobId` without importing an unrelated capability package, while each capability still owns the meaning and validation of its concrete ids.
