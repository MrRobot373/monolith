...
```ts
import { writeFileAtomic } from '@monolith/atomic-write'

declare const text: string
await writeFileAtomic('/home/u/.monolith/settings.yaml', text, { mode: 0o600 })
```
...
await withFileLock('/home/u/.monolith/settings.yaml', async () => {
  const previous = await readCurrent()
  await writeFileAtomic('/home/u/.monolith/settings.yaml', render(previous), { mode: 0o600 })
})
```
