...
import type { WebSearchProvider, WebSearchResult } from '@monolith/web'
...
} from '@monolith/tool-web'
import type { ContentBlock } from '@monolith/llm'
import type { ToolResult } from '@monolith/tools'
...
async function mountTools(opts: {
  ...
  fetchProvider?: import('@monolith/web').WebFetchProvider
...
describe('tool-web execution through the real registry', () => {
  ...
  it('executes web_fetch, forwarding the url (no timeout param) and the abort signal to the seam', async () => {
    ...
    // tool-call budget is owned by monolith-tool-call-timeout-policy over exec.signal.
