/**
 * Integration: the real fetch backend (`monolith-web-fetch-http`) + a real search provider
 * (`monolith-web-search-exa`) + the real seam (`monolith-web`) + the model tool (`monolith-tool-web`) + the
 * tool-call timeout policy (`monolith-tool-call-timeout-policy`), exercised through `ctx.tools.execute()` —
...
import ToolRuntime, { type ToolExecutionResult } from '@monolith/tools'
...
import * as TimeoutPolicy from '@monolith/tool-call-timeout-policy'
...
describe('tool-call timeout returns TOOL_TIMEOUT (deadline wins over a slow fetch)', () => {
  ...
  it('returns a structured TOOL_TIMEOUT (not the provider WEB_FETCH_TIMEOUT) when the tool-call budget wins', async () => {
    ...
    expect(out.isError).toBe(true)
    // The outer tool-call deadline won: TOOL_TIMEOUT, owned by monolith-tool-call-timeout-policy,
