 ...
 * (`monolith-spill-local` backend + `monolith-spill-policy`), exercised through
...
import LocalSpillStore from '@monolith/spill-local'
...
beforeEach(async () => {
  ...
  spillRoot = mkdtempSync(join(tmpdir(), 'monolith-spill-web-'))
