...
import WebRuntime, { WebError } from '@monolith/web'
import {
  DeepSeekSearchProvider,
  DEEPSEEK_PROVIDER_ID,
} from '@monolith/web-search-deepseek'
import * as deepseekPlugin from '@monolith/web-search-deepseek'
import { citationSnippets, mapAnthropicResponse } from '../src/provider.ts'
import type { AnthropicResponse } from '@monolith/web-search-deepseek/src/types.ts'

/** Construct the provider over a fixed options value; production passes a live thunk. */
import type { DeepSeekSearchProviderOptions } from '@monolith/web-search-deepseek'
...
describe('web-search-deepseek plugin registration', () => {
  ...
  it('resolves the credential for each search so a stored or rotated key needs no restart', async () => {
    ...
    const dir = await mkdtemp(join(tmpdir(), 'monolith-web-search-credentials-'))
