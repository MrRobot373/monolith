...
export class WebRuntime extends Service {
   ...
   * `$MONOLITH_WEB_SEARCH_PROVIDER` / `$MONOLITH_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.MONOLITH_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.MONOLITH_WEB_FETCH_PROVIDER
  }
