...
import * as typertLoader from '@monolith/typert-loader'
import { validateTypertManifest } from '@monolith/typert-loader'
import { z } from 'zod'
...
afterEach(async () => {
  ...
  context = undefined
  Reflect.deleteProperty(globalThis, '__monolithTypertLoaderGate')
...
describe('typert loader', () => {
  it('registers an explicit package without a Loader entry and withdraws it with the loader', LOADER_TEST_TIMEOUT, async () => {
    root = await mkdtemp(join(tmpdir(), 'monolith-typert-loader-'))
    await linkZod(root)
  ...
  it('drops an in-flight manifest when the loader is disposed before import settles', LOADER_TEST_TIMEOUT, async () => {
    ...
    Reflect.set(globalThis, '__monolithTypertLoaderGate', {
    ...
    await writePackage(root, '@fixture/pending', {
      typertSource: [
        'import { z } from \'zod\'',
        'globalThis.__monolithTypertLoaderGate.started()',
        'await globalThis.__monolithTypertLoaderGate.wait',
