...
import type { TypertContribution } from '@monolith/typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@monolith/tool-cordis/src/api-catalog.ts'
...
describe('model-driven monolith-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@monolith/tools'],
    }).analyze()
    const host = workspace.faces.find(candidate => candidate.face === 'host')
    if (host === undefined) throw new Error('monolith-tools has no analyzed host face')
    const artifact = new FaceModelEmitter(host).emit('@monolith/tools')
    ...
    const record = ctx.typert.getPackage('@monolith/tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@monolith/tools', 'host')).toBeUndefined()
  })
})
