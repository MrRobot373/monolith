...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@monolith/tools', 'ToolInput')).toBe('@monolith/tools#ToolInput')
    expect(typertPackageKey('@monolith/tools', 'host')).toBe('@monolith/tools#host')
    expect(ctx.typert.get('@monolith/tools#ToolInput')).toMatchObject({
      package: '@monolith/tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@monolith/tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@monolith/tools', 'host')).toMatchObject({
      key: '@monolith/tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@monolith/tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@monolith/tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@monolith/tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@monolith/tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@monolith/tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@monolith/tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@monolith/tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@monolith/tools#ToolInput')).not.toBe(projected)
  })
