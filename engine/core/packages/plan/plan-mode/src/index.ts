...
import type { Session, UserMessage } from '@monolith/session'
import { defineTool } from '@monolith/tools'
import { UserQuestionError } from '@monolith/user-questions'
import type { CommandId } from '@monolith/commands'
...
declare module '@monolith/session/types' {
  ...
  constructor(ctx: Context, config: PlanModeConfig = { section: '' }) {
    ...
    ctx.on('agent/pre-step', async (
      ...
      try {
        ...
        ctx.logger.warn('monolith-plan-mode: failed to append selected plan mode at step start: %o', error)
        return decision
      }
    ...
    ctx.effect(() => () => { disposed = true }, 'monolith-plan-mode: close service lifetime')
