...
import type { IndexInjection, WebServer } from '@monolith/host-webserver'
...
describe('experimental Inspector Host plugin', () => {
  ...
  it('starts the Worker, provides ctx.inspector, injects Client bootstrap, and disposes', async () => {
    ...
    const bootstrap = rows.find(row => row.kind === 'global' && row.name === '__MONOLITH_INSPECTOR__')
    expect(bootstrap).toMatchObject({ kind: 'global', name: '__MONOLITH_INSPECTOR__' })
    expect(log).toHaveBeenCalledWith(expect.stringMatching(/^monolith inspector: devtools:\/\//u))
    ...
    socket.send(JSON.stringify({ id: 1, method: 'MONOLITHInspector.getSources' }))
    await vi.waitFor(async () => {
