...
describe('experimental Inspector through a real Loader composition', () => {
  it('loads the named-export Host face from cordis.yml and releases its endpoint', async () => {
    root = await mkdtemp(join(tmpdir(), 'monolith-inspector-loader-'))
    ...
    await writeFile(configPath, [
      ...
      "    host: '127.0.0.1'",
      '    port: 0',
      "- name: '@monolith/experimental-inspector'",
      '  config:',
      '    port: 0',
      '    captureFetch: false',
      '',
    ...
    const modules = new Map<string, unknown>([
      ['@monolith/host-webserver', WebServer],
      ['@monolith/experimental-inspector', Inspector],
    ])
    ...
    const inspectorEntry = [...context.loader.entries()]
      .find(entry => entry.options.name === '@monolith/experimental-inspector')
