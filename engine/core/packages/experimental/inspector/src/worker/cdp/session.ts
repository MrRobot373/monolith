...
export class CdpSession implements NetworkSink {
  ...
  constructor(
    ...
    this.unsubscribeSources = sources.subscribeStatus(() => {
      if (this.diagnosticsEnabled) this.sendEvent('MONOLITHInspector.sourcesChanged', { sources: this.sources.describe() })
    })
  }
  ...
  receive(value: unknown): void {
    ...
    try {
      ...
      } else if (request.method === 'MONOLITHInspector.enable') {
        this.diagnosticsEnabled = true
        result = { sources: this.sources.describe() }
      } else if (request.method === 'MONOLITHInspector.disable') {
        this.diagnosticsEnabled = false
        result = {}
      } else if (request.method === 'MONOLITHInspector.getSources') {
        result = { sources: this.sources.describe() }
      } else if (request.method === 'MONOLITHInspector.getCordisTree') {
