...
import { MONOLITH_TMP } from '../../storage/paths.ts'
...
function vfsPath(path: string, cwd: string): string {
  ...
  if (absolute === '/tmp') return MONOLITH_TMP
  if (absolute.startsWith('/tmp/')) return `${MONOLITH_TMP}${absolute.slice('/tmp'.length)}`
  return absolute
}
...
/** Throw the denial dialect consumed by `monolith-bash-sandbox`. */
