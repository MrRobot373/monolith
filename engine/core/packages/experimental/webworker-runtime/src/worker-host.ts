 ...
 * @module @monolith/experimental-webworker-runtime/src/worker-host
 */
import { setActiveModuleLoader, WorkerModuleLoader, type StaticModuleFactory } from './module-system/module-loader.ts'
import type { TypertGateway } from '@monolith/api-gateway'
import type { HostConnectionHandle } from '@monolith/client-connection'
...
export interface WorkerHostOptions {
  ...
  /** Environment for the process shim; `MONOLITH_HOME` defaults to `<root>/home`. */
...
export function createWorkerHost(options: WorkerHostOptions): WorkerHost {
  ...
  const start = async (): Promise<void> => {
    try {
      ...
      installProcessGlobal({ cwd: root, env: { MONOLITH_HOME: home, HOME: home, ...options.env } })
      ...
      const appBoot = require('@monolith/app-boot') as {
        boot(
          binName: string,
          configPath: string,
          patches: unknown[],
      ...
      const cmdline = require('@monolith/cmdline') as {
      ...
      const ctx = await appBoot.boot('monolith-webworker', configPath, patches, (hostCtx) => {
...
export function installLogSink(ctx: HostContext, require: (specifier: string) => unknown): void {
  const { Logger } = require('@monolith/cordis') as { Logger: LogRenderer }
...
function bootPatches(
  ...
  if (configPath.endsWith('.json')) {
    ...
    const include = loader.load(loader.resolve('@monolith/cordis-plugin-include', root)) as { entryListSchema: unknown }
