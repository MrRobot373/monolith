 ...
 * @module @monolith/experimental-webworker-runtime/src/compile/transform
 */
import { parse } from 'acorn'

const HELPER_SOURCE: Record<string, string> = {
  def: 'const __monolith$def=(t,k,get)=>Object.defineProperty(t,k,{enumerable:true,configurable:true,get});',
  default: 'const __monolith$default=(m)=>(m&&m.__esModule?m.default:m);',
  ns: 'const __monolith$ns=(m)=>(m&&m.__esModule?m:Object.assign({},m,{default:m}));',
  exportAll: 'const __monolith$exportAll=(t,m)=>{for(const k of Object.keys(m))if(k!=="default"&&!(k in t))__monolith$def(t,k,()=>m[k]);};',
  dynImport: 'const __monolith$dynImport=(s)=>Promise.resolve().then(()=>__monolith$ns(require(s)));',
}
...
class Transformer {
  ...
  private helper(name: string): string {
    ...
    return `__monolith$${name}`
  }

  private moduleTemp(): string {
    this.modules += 1
    return `__monolith$m${this.modules}`
  }
  ...
  private visit(node: unknown, context: {
    ...
    switch (record.type) {
      ...
      case 'MetaProperty': {
        ...
        if (meta.name === 'import') {
          this.moduleSyntax = true
          this.replace(record.start, record.end, '__monolith$meta')
        }
        break
      }
  ...
  run(): string {
    ...
    for (const { exported, local } of this.bindings) {
      prologue.push(`__monolith$def(exports,${JSON.stringify(exported)},()=>${local});`)
    }
