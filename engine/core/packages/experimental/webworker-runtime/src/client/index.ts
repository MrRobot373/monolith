 ...
 * @module @monolith/experimental-webworker-runtime/client
 */
...
interface BootReadyGlobal {
  __MONOLITH_BOOT_READY__?: PromiseWithResolvers<void>
}

function bootReadyGate(): PromiseWithResolvers<void> {
  return (globalThis as BootReadyGlobal).__MONOLITH_BOOT_READY__ ??= Promise.withResolvers<void>()
}
 ...
 * bundles, `__MONOLITH_BOOT__`, the theme bootstrap — in table order. The
 * boot-readiness deferred (`__MONOLITH_BOOT_READY__`) is installed before the
...
export async function connectWorkerHost(worker: Worker, options?: WorkerHostConnectOptions): Promise<WorkerHostConnection> {
  ...
  try {
    ...
    ;(globalThis as ClientTransportGlobal).__MONOLITH_TRANSPORT__ = {
