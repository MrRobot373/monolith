 ...
 * @module @monolith/experimental-webworker-runtime/src/node/globals/process
 */
...
export interface ProcessShimOptions {
  ...
  readonly cwd: string
  /** Environment the tree reads; `MONOLITH_HOME` belongs here. */
...
export function installProcessGlobal(options: ProcessShimOptions): ProcessShim {
  ...
  const shim: ProcessShim = {
    env: { ...options.env },
    argv: [...(options.argv ?? ['node', 'monolith-webworker'])],
    execArgv: [],
    title: 'monolith-webworker',
    platform: 'linux',
    arch: 'x64',
    pid: 1,
    version: 'v0.0.0',
