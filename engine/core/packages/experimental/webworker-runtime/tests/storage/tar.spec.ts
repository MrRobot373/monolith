...
describe('tar codec', () => {
  ...
  it('mounts as a VFS with directories synthesized along file paths', () => {
    ...
    expect(vfs.existsSync('/monolith/config/cordis.yml')).toBe(true)
    expect(vfs.readFileSync('/monolith/config/cordis.yml', 'utf8')).toBe('- id: subject\n')
    expect(vfs.existsSync('/monolith/config')).toBe(true)
    expect(vfs.existsSync('/monolith/workspace')).toBe(true)
    expect(vfs.existsSync('/monolith/absent')).toBe(false)
  })
  ...
  it('applies ordered data overlays without exposing runtime paths', () => {
    ...
    expect(vfs.readFileSync('/monolith/workspace/status.txt', 'utf8')).toBe('fixture')
    expect(vfs.readFileSync('/monolith/home/sessions/example/session.jsonl', 'utf8')).toBe('{}\n')
    expect(() => loadVfsOverlay(packTar({
      'config/cordis.yml': encoder.encode('replaced'),
    }), '/monolith', vfs)).toThrow(/overlay entry must stay under home\/ or workspace/)
    expect(vfs.readFileSync('/monolith/config/cordis.yml', 'utf8')).toBe('- id: subject\n')
  })
})
