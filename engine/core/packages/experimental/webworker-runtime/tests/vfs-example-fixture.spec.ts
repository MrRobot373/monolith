...
import { scanLog } from '@monolith/session-persistence-jsonl/src/format.ts'
import { foldSubagentDescriptor } from '@monolith/subagent'
import {
...
function readSession(id: string): ReturnType<typeof scanLog> {
  return scanLog(readFileSync(
    join(VFS_EXAMPLE_ROOT, 'home/sessions/--monolith-workspace--', id, 'session.jsonl'),
  ))
}
