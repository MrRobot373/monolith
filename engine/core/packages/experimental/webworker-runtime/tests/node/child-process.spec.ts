...
import { spawn, spawnSync } from '@monolith/experimental-webworker-runtime/src/node/builtin_modules/implemented/child_process.ts'
import {
  LAUNCHER_FAILURE_EXIT, grantArgs, launcherPath, probe,
} from '@monolith/node-addon-landlock-run'
import { processAlive, signalProcess } from '@monolith/experimental-webworker-runtime/src/node/process-table.ts'
import { hostFileSystem } from '@monolith/experimental-webworker-runtime/src/shell/fs-access.ts'
import {
  LANDLOCK_EXECUTABLE, landlockFileSystem, parseLandlockArguments,
} from '@monolith/experimental-webworker-runtime/src/shell/process/landlock.ts'
import { spawnSubprocess } from '@monolith/subprocess-local/src/spawn.ts'

vi.mock('node:child_process', async () =>
  await import('@monolith/experimental-webworker-runtime/src/node/builtin_modules/implemented/child_process.ts'))

const WORKSPACE = '/monolith/workspace'
const HOME = '/monolith/home'
const TMP = '/monolith/tmp'
...
it('keeps the native Landlock package API and CLI failure contract', async () => {
  ...
  const missingGrant = spawn(launcherPath(), ['--rw', '/monolith/missing', '--', 'touch', `${WORKSPACE}/never`], { cwd: WORKSPACE })
  expect(await collect(missingGrant)).toEqual({
    stdout: '',
    stderr: 'landlock-run: cannot open rule path: /monolith/missing: No such file or directory\n',
    code: 125,
  })
