...
import { MONOLITH_HOME, MONOLITH_TMP } from '../../src/storage/paths.ts'
...
describe('replaced external packages', () => {
  it('lists the packages the loader serves from the bundle', () => {
    ...
    expect(REPLACED_EXTERNAL_PACKAGES).not.toContain('@monolith/node-addon-landlock-run')
...
describe('node:os', () => {
  it('reports the virtual platform identity and the VFS directories', () => {
    expect([os.EOL, os.tmpdir(), os.homedir()]).toEqual(['\n', MONOLITH_TMP, MONOLITH_HOME])
    expect([os.platform(), os.type(), os.arch()]).toEqual(['linux', 'Linux', 'x64'])
    expect([os.release(), os.hostname()]).toEqual(['0.0.0-monolith-worker', 'monolith-worker'])
  })
