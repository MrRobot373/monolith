...
function loaderRequire(): WorkerRequire {
  ...
  const loader = new WorkerModuleLoader({ vfs, root: '/monolith', staticModules: createNodeBuiltins() })
  return loader.createRequire('/monolith/')
}
...
describe('module identity through the loader', () => {
  it('exposes async and synchronous resolution through the Cordis internal seam', async () => {
    const vfs = new MemoryVfs()
    vfs.seedDirectory('/monolith/node_modules/example')
    vfs.writeFileSync('/monolith/node_modules/example/package.json', JSON.stringify({ main: 'index.js' }))
    vfs.writeFileSync('/monolith/node_modules/example/index.js', 'module.exports = {}\n')
    const loader = new WorkerModuleLoader({ vfs, root: '/monolith', staticModules: createNodeBuiltins() })

    expect(loader.internal.resolveSync('example', 'file:///monolith/app.js')).toEqual({
      format: 'commonjs',
      url: 'file:///monolith/node_modules/example/index.js',
    })
    await expect(loader.internal.resolve('node:fs', 'file:///monolith/app.js')).resolves.toEqual({
      format: 'builtin',
      url: 'node:fs',
    })
  })
  ...
  it('exposes the package search paths used by the VFS resolver', () => {
    ...
    expect(require.resolve.paths('workspace-package')).toEqual(['/monolith/node_modules'])
    expect(require.resolve.paths('./local.js')).toEqual(['/monolith'])
  })
})
