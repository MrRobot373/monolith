...
describe('process shim', () => {
  it('publishes cwd, env, and version zero for the loader probe', () => {
    const shim = installProcessGlobal({ cwd: '/monolith', env: { MONOLITH_HOME: '/monolith/home' } })
    expect(shim.cwd()).toBe('/monolith')
    expect(shim.env.MONOLITH_HOME).toBe('/monolith/home')
    expect(shim.title).toBe('monolith-webworker')
  ...
  it('answers getBuiltinModule from the module proxies and undefined otherwise', () => {
    ...
    const shim = installProcessGlobal({ cwd: '/monolith', env: {} })
