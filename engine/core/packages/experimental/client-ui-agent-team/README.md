...
# @monolith/experimental-client-ui-agent-team
...
Install the package through [`@monolith/experimental-agent-team-web-profile`](../agent-team-web-profile/README.md) after the stable Web bundle and the Host-side Agent Teams profile. The Web Client loader mounts the `/client` export; the root Host export is inert, and the package has no user configuration fields.
...
The Client export mounts the generated `ctx.remote.agentTeams` contribution from [`@monolith/experimental-agent-team/remote`](../agent-team/README.md), then registers its locale dictionaries and one conversation-header slot through Cordis effects. Disposing the plugin fiber removes both registrations.
