...
describe('logTruncationMarker', () => {
  it('names the configured byte budget', () => {
    expect(logTruncationMarker(65536)).toBe('[monolith-code-runtime-python] log capture truncated at 65536 bytes')
    expect(logTruncationMarker(1)).toBe('[monolith-code-runtime-python] log capture truncated at 1 bytes')
  })
})
