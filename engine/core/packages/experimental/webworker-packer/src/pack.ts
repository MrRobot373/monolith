 ...
 * @module @monolith/experimental-webworker-packer/src/pack
 */
...
} from '@monolith/experimental-webworker-runtime'
import picomatch from 'picomatch'
import yaml from 'js-yaml'
import { entryListSchema } from '@monolith/cordis-plugin-include'
import { REPLACED_EXTERNAL_PACKAGES } from '@monolith/experimental-webworker-runtime/src/node/external_packages/replaced-externals.ts'
import { MODULE_PROXIES, MODULE_PROXY_PREFIXES } from '@monolith/experimental-webworker-runtime/src/module-proxies.ts'
...
export { DEFAULT_ROOT } from '@monolith/experimental-webworker-runtime'
...
function sweepImage(
  ...
  for (let entry = queue.shift(); entry !== undefined; entry = queue.shift()) {
    ...
    try {
      ...
      const external = importer.startsWith('node_modules/') && !importer.startsWith('node_modules/@monolith/')
...
function materialize(
  ...
  for (let entry = queue.shift(); entry !== undefined; entry = queue.shift()) {
    ...
    for (const field of ['dependencies', 'peerDependencies'] as const) {
      ...
      // (@monolith/cordis is a peerDependency of every harness package),
