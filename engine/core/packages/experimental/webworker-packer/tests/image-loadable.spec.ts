...
import { FiberState } from '@monolith/cordis'
import { createNodeBuiltins, REPLACED_PREFIXES } from '@monolith/experimental-webworker-runtime/src/node/builtins.ts'
import {
  setActiveModuleLoader, WorkerModuleLoader,
} from '@monolith/experimental-webworker-runtime/src/module-system/module-loader.ts'
import { inflateImage } from '@monolith/experimental-webworker-runtime/src/storage/image-gzip.ts'
import { loadVfsImage } from '@monolith/experimental-webworker-runtime/src/storage/memory.ts'
...
const SUBJECT = '@monolith/timeout'
const LANDLOCK = '@monolith/node-addon-landlock-run'
const PLUGIN_INVENTORY = '@monolith/plugin-package-inventory-deepseek'
const WEB_SERVER = '@monolith/host-webserver'
    ...
    inventory.apply({
      ...
      deepseekLlmApiExtensions: {
        register: (field: string, contribution: { readonly prepare: Prepare }): void => {
          expect(field).toBe('monolith_plugin_packages')
