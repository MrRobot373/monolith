...
import { SessionLogOffset, SessionId, type Session, type SessionEvent } from '@monolith/session'
...
import SubagentService from '@monolith/subagent'
import { deliverSubagentPrompt, type HostPromptDeliverer } from '@monolith/subagent/internal'
...
async function setup(
  ...
  const storageRoot = mkdtempSync(join(tmpdir(), 'monolith-team-'))
...
describe('Team identity and provisioning', () => {
  ...
  it('supports direct-constructor defaults and recovers roots that already exist', async () => {
    ...
    await mountAgentLoopTestDependencies(ctx)
    await ctx.plugin(SessionProjectionRegistry)
    const storageRoot = mkdtempSync(join(tmpdir(), 'monolith-team-direct-'))
    roots.push(storageRoot)
    await ctx.plugin(JsonlSessionPersistence, { root: storageRoot })
    await ctx.plugin(AgentLoop, { agents: [] })
    await ctx.plugin(SubagentService)
...
describe('Team mailbox and waiting', () => {
  ...
  it('waits for one change, supports cancellation, times out, and releases waiters on HMR disposal', async () => {
    ...
    await mountAgentLoopTestDependencies(ctx)
    await ctx.plugin(SessionProjectionRegistry)
    const storageRoot = mkdtempSync(join(tmpdir(), 'monolith-team-wait-'))
    roots.push(storageRoot)
    await ctx.plugin(JsonlSessionPersistence, { root: storageRoot })
    await ctx.plugin(AgentLoop, { agents: [] })
    await ctx.plugin(SubagentService)
