...
import { scopeOf } from '@monolith/scope'
...
import { renderPrompt } from '@monolith/system-prompt'
...
async function setup(script: ConstructorParameters<typeof MockAdapter>[0], legacyControl = false) {
  ...
  const storageRoot = mkdtempSync(join(tmpdir(), 'monolith-tool-team-'))
