...
Loading the engine registers `ctx.workflowEngine`; adding `monolith-tool-workflow` on top gives the model the `workflow` tool. Every config field is optional:
