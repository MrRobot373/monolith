...
import type { SubagentCapabilities, SubagentProvider, SubagentResult, SubagentRun, SubagentStartRequest } from '@monolith/subagent'
import type { WorkflowMeta, WorkflowResult, WorkflowResultInfo, WorkflowRun, WorkflowRunInfo } from '@monolith/workflow'
...
describe('monolith-workflow-worker-thread', { timeout: 120_000 }, () => {
  describe('script execution over a real worker thread', () => {
    ...
    it('the unbuilt worker forwards exactly TSX_TSCONFIG_PATH through the scrub: the paths-map pin survives, secrets do not', async () => {
      ...
      // must inherit the pin (or its monolith-* imports silently resolve to
