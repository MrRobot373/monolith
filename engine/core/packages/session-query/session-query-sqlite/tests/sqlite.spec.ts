import { createAssistantMessage, createUserMessage } from '@monolith/llm'
...
import type { SessionEvent, SessionHeader, SessionId as SessionIdType } from '@monolith/session'
...
} from '@monolith/session-query-sqlite'
import {
...
async function temporaryPath(name = 'search.db'): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'monolith-session-search-'))
