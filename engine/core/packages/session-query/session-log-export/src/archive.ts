...
import type { AttachmentStore, ImageAttachmentRef } from '@monolith/attachment'
import type { SessionLineageNode, SessionQueryEngine } from '@monolith/session-query'
import type { SessionEvent, SessionHeader, SessionId, SessionStore } from '@monolith/session'
...
export function sessionLogZipFilename(sessionId: string): string {
  return `monolith-session-${safeSessionIdSegment(sessionId)}.zip`
}
