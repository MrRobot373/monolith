...
import type { SessionEvent, SessionHeader, SessionId } from '@monolith/session'
import type { SessionLineageNode } from '@monolith/session-query'
import { SessionPersistenceNotFoundError } from '@monolith/session-persistence'
import type { SessionAccess, SessionHandle } from '@monolith/session-persistence'
import { HostConnectionService } from '@monolith/client-connection'
import type { BrowserAuth } from '@monolith/client-connection/src/browser-auth.ts'
...
describe('session.export download endpoint', () => {
  it('streams a ZIP with the root log serialized as canonical JSONL', async () => {
    ...
    expect(response.headers.get('content-disposition')).toContain('monolith-session-session-root.zip')
