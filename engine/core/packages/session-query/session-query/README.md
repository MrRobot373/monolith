...
- **One canonical surface fold.** `listEvents`, `readSurface`, and `traceEvent` validate the whole log with the same `monolith-session` fold, so search and traces agree with model-history derivation.
...
- [monolith-session-query-sqlite](../session-query-sqlite/README.md) — the shipped full-text backend and its index lifecycle.
- [monolith-tool-session-query](../tool-session-query/README.md) — the model-facing consumer built on this service.
