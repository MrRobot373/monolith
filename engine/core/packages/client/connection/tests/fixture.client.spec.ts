...
import type { DirectoryListing } from '@monolith/host-directory-picker/types'
import type { ModelCatalog } from '@monolith/api-session-controller/types'
import type { ModelSelection } from '@monolith/api-session-controller/types'
...
describe('createFixtureApi', () => {
  ...
  it('serves raw history entries with replayable tool-result metadata', async () => {
    ...
    expect(webSearch).toHaveProperty('data.meta.sources', expect.arrayContaining([
      expect.objectContaining({ url: 'https://github.com/MrRobot373/monolith' }),
    ]))
