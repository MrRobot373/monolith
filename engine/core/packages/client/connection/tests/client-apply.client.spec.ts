...
type Win = {
  ...
  __MONOLITH_TRANSPORT__?: ClientTransportHooks
}

afterEach(() => {
  delete (globalThis as Win).location
  delete (globalThis as Win).__MONOLITH_TRANSPORT__
  vi.unstubAllGlobals()
  vi.useRealTimers()
})
...
describe('connection client apply', () => {
  ...
  it('carries RPC calls without requiring secure-context randomUUID', async () => {
    ...
    expect(seen[0]?.url).toBe('http://monolith.internal/api/goals/create')
  ...
  it('exposes a worker-local Gateway stream through connection.rpc.open', async () => {
    ...
    ;(globalThis as Win).__MONOLITH_TRANSPORT__ = {
  ...
  it('validates generic RPC transport failures, correlation, and targets', async () => {
    ...
    try {
      ...
      expect(fetch.mock.calls[0]?.[0]).toEqual(new URL('http://monolith.internal/api/goals/create'))
