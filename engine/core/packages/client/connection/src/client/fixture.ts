...
import {
  ...
  createUserMessage,
} from '@monolith/llm/message'
import { brandString } from '@monolith/brand'
import type { MessageId, ToolCallId } from '@monolith/llm/brand'
import type {
  AssistantMessage,
  ContentBlock,
  MessageSource,
...
import type { AttachmentIdType, ImageAttachmentRef } from '@monolith/attachment'
import type {
  SessionEvent,
  SessionId,
} from '@monolith/session/types'
import { SessionSeq } from '@monolith/session/types'
import type { JsonValue } from '@monolith/util-values'
import { isChunkRow, packChunkRuns } from '@monolith/session/chunk-rows'
import type { ChunkRow } from '@monolith/session/chunk-rows'
import type { TodoItem } from '@monolith/tool-todo/client'
// Type-only: the brand constructor is host-side; the fixture casts at its
// wire-fabrication boundary (the schema layer's one-cast-point posture).
import type { CommandId } from '@monolith/commands/brand'
import type { CommandDescriptor, CommandExecution, CommandResult } from '@monolith/commands/types'
import type { CredentialInfo } from '@monolith/credentials/types'
import type { DirectoryListing as FixtureDirectoryListing } from '@monolith/host-directory-picker/types'
import type { SettingsDescribeValue, SettingsNamespaceView } from '@monolith/settings/types'
import { deriveEventMessage, foldSurface } from '@monolith/session/surface'
...
const WEB_SEARCH_META = {
  answer: 'MONOLITH is a plugin-based agent harness on vendored Cordis where **every capability is a plugin**.',
  sources: [
    {
      url: 'https://github.com/MrRobot373/monolith',
      title: 'MONOLITH — plugin-based agent harness',
...
/** Fixture-local mirror of the goal projection value (monolith-goal's GoalProjection shape). */
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const directoryTree = new Map<string, string[]>([
    ...
    [`${FIXTURE_HOME}/Documents`, [
      ...
      'deepseek-web', 'monolith', 'deepseek-app', 'deepseek-landing-blog',
    ]],
  ])
      ...
      const value = sessions
        ...
        .map((item) => {
          ...
          return {
            ...
            mention: `@[${label}](monolith-session:${encoded})`,
          }
        })
  ...
  const sessionApi: FixtureSessionApi = {
    ...
    prompt: (request) => {
      ...
      // user-rpc source member is declared by monolith-api-session-controller,
