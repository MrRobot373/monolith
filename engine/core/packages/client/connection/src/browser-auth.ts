...
import type { CredentialProvider, CredentialRecord } from '@monolith/credentials'
import type {
...
const COOKIE_PREFIX = 'monolith-auth-'
...
export class BrowserAuth {
   ...
   * when this MONOLITH home has none.
  ...
  authorizeIndex(req: ConnectionIndexRequest, res: ConnectionIndexResponse): boolean {
    ...
    const url = new URL(req.url ?? '/', 'http://monolith.invalid')
  ...
  private writeUnauthorized(req: ConnectionIndexRequest, res: ConnectionIndexResponse): void {
    ...
    res.end(req.method === 'HEAD'
      ? undefined
      : 'monolith web authentication required; reopen the URL printed by monolith web.\n')
  }
}
