...
import {
  MarkLogo, IconNewChatOutline16, IconPanelLeftOutline16, Tooltip,
...
function localBuildVersion(): string | undefined {
  const version = process.env.MONOLITH_CLIENT_VERSION
  if (version === undefined) return undefined
  const commit = process.env.MONOLITH_CLIENT_COMMIT_HASH
  return version
    + (commit === undefined ? '' : `-${commit}`)
    + (process.env.MONOLITH_CLIENT_GIT_DIRTY === 'true' ? '-dirty' : '')
}
          ...
          >
            <span className={css.brandIdentity} aria-hidden="true">
              <span className={css.brandMark}>
                {renderSlot('sidebar.brand.mark', { size: 24 }, { fallback: <MarkLogo size={24} /> })}
              </span>
