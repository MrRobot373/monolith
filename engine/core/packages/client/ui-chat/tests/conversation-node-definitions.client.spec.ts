...
import { isChunkRow, packChunkRuns, type ChunkRow } from '@monolith/session/chunk-rows'
...
import { inspectRequestPrompt } from '@monolith/client-ui-conversation/client'
  ...
  it('orders the system field before the request messages while preserving message order', () => {
    const value = assembler([
      ...
      at(4, 'user/message', {
        ...
        source: { kind: 'plugin', plugin: '@monolith/system-prompt', form: 'snapshot' },
      }, { surfaceOp: 'append' }),
      at(5, 'request/header', {
        reason: 'initial',
