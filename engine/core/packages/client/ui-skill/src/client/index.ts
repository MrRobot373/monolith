 ...
 * determinism
 * lives host-side — the pre-step boundary (`monolith-tool-skill`) recognizes a
...
import type { SkillEntry } from '@monolith/api-remotes/client'
import type {} from '@monolith/api-session-controller/client'
import type { SessionId } from '@monolith/session/types'
import type { InputTriggerServiceContract, InputTriggerSource } from '@monolith/client-ui-input-trigger/client'
...
export function apply(ctx: ClientContext): void {
  ...
  const source: InputTriggerSource = {
    ...
    onPick({ candidate }) {
      ...
      // pre-step boundary (monolith-tool-skill) recognizes the leading /name and
