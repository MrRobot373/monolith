...
The shipped Web graph already resolves `@monolith/client-ui-schedule` through a disabled `ui-schedule` row; the overlay enables that row together with `@monolith/schedule`. The trigger appears only while the Session is successfully open and the projection contains at least one active record. Opening it shows overdue rows first, then future rows by target time, with exact ties preserving the projection's creation order.
