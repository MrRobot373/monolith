...
describe('MessageFeedbackActions theme styles', () => {
  it('names only theme variables the token sheet defines', () => {
    ...
    // not just `--dsw-`: a `--monolith-` name reads as a plausible sibling and would
    // otherwise slip past into an invalid declaration.
    const named = [...css.matchAll(/var\((--(?:dsw|monolith|ds)-[a-z0-9-]+)/g)].map(match => match[1])
  ...
  it('slot-injected actions ride the content font-size axis like their host row', () => {
    ...
    expect(block('.action')).toMatch(/width:\s*calc\(28px \+ var\(--monolith-content-font-delta, 0px\)\)/)
    expect(block('.action')).toMatch(/height:\s*calc\(28px \+ var\(--monolith-content-font-delta, 0px\)\)/)
    expect(block('.action svg')).toMatch(/width:\s*calc\(15px \+ var\(--monolith-content-font-delta, 0px\)\)/)
    expect(block('.noteOpen')).toMatch(/line-height:\s*calc\(28px \+ var\(--monolith-content-font-delta, 0px\)\)/)
  })
})
