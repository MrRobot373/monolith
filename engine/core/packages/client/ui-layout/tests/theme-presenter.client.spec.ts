...
import type { ThemeSnapshot } from '@monolith/client-ui-theme/client'
import { DARK_ATTRIBUTE, ThemePresenter } from '@monolith/client-ui-layout/src/client/theme-presenter.ts'
...
describe('ThemePresenter', () => {
  ...
  it('publishes the content font size and follows changes', () => {
    const presenter = new ThemePresenter()
    presenter.apply(snapshot('light'))
    expect(document.body.style.getPropertyValue('--monolith-content-font-size')).toBe('14px')
    presenter.apply(snapshot('light', {}, 17))
    expect(document.body.style.getPropertyValue('--monolith-content-font-size')).toBe('17px')
  })
  ...
  it('dispose removes color-scheme, the attribute, the font-size axis, and every applied variable, sparing foreign inline styles', () => {
    ...
    expect(document.body.style.getPropertyValue('--monolith-content-font-size')).toBe('')
