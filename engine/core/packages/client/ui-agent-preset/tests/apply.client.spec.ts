...
import { apply as settingsApply, inject as settingsInject } from '@monolith/client-ui-settings/client'
import { apply, inject } from '@monolith/client-ui-agent-preset/client'
...
describe('AgentPresetSeatController reconciliation', () => {
  ...
  it('keeps the bare cause of a mount failure, not the frame that names the preset again', async () => {
    const reason = 'failed to import loader entry ctx (@monolith/gone): Cannot find package'
    const controller = new AgentPresetSeatController({
      remote: {
        agentPresets: {
