 ...
 */
const CSS_VIRTUAL_PREFIX = '\0monolith-css:'
const GLOBAL_CSS_VIRTUAL_PREFIX = '\0monolith-global-css:'
const INLINE_CSS_VIRTUAL_PREFIX = '\0monolith-inline-css:'
 ...
 * Everything else under @monolith/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^(?:@monolith\/(?:file-reference|session|llm|tools|brand|deque|typert-protocol|util-crypto|util-values|util-workspace-path)(?:\/|$)|@monolith\/token-meter\/client$|@monolith\/agent-presets\/display$)/
...
const GENERATED_REMOTE = /^@monolith\/[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
...
export function clientBundle(
  ...
  return ({ env }) => {
    const face = buildFace(env?.MONOLITH_BUILD_FACE)
...
export function clientOnly(configs: readonly UserConfig[]): BuildFaceConfig {
  return ({ env }) => buildFace(env?.MONOLITH_BUILD_FACE) === 'host'
...
interface WorkspaceManifest {
  ...
  readonly monolith?: { readonly client?: { readonly external?: unknown } }
}
...
/**
 * Module-table specifiers one `monolith.client` declaration requests. Matching is
 ...
 * @param declaration - the package's `monolith.client` object.
...
export function requestedExternals(
  ...
  return new Set(optionalStringArray(subject, 'monolith.client.external', declaration.external) ?? [])
}

/**
 * Module-table specifiers one package requests. The shell baseline is implicit
 * for every dynamic bundle; `monolith.client.external` only adds package-specific
...
function clientExternals(id: string): ReadonlySet<string> {
  ...
  const externals = new Set([
    ...
    ...requestedExternals(id, workspaceManifest(id).monolith?.client ?? {}),
  ])
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @monolith value import is a build error — a
      ...
      name: 'monolith-client-bundle-purity',
      resolveId(source: string) {
        if (!source.startsWith('@monolith/')) return null
        if (isRequested(source)) return null // requested module-table row: external wins
        if (VENDORED_LIBRARY.test(source)) return null // vendored library: inline, no shared identity
        if (INLINE_SAFE.test(source) || GENERATED_REMOTE.test(source)) return null // wire contribution: inline is the point
        throw new Error(
          `client bundle purity: "${source}" is not in the default client externals or ${id}'s monolith.client.external, an inline-safe wire layer, or a generated /remote contribution — `
      ...
      name: 'monolith-css-modules-inline',
      ...
      name: 'monolith-css-global-inline',
      resolveId(source: string, importer: string | undefined) {
...
const STATIC_LINKED_PLUGIN = 'monolith-static-linked-external'
