...
import { Context, type Fiber } from '@monolith/cordis'
import { afterEach, describe, expect, it, vi } from 'vitest'
import { renderIndexInjections, type WebServer, type WebRoute } from '@monolith/host-webserver'
...
const MODULES_ID = '@monolith/client-modules'
const UI_RENDERER_ID = '@monolith/client-ui-renderer'
...
function writePackage(
  packageName: string,
  metadata: Record<string, unknown> = { monolith: { client: { platform: 'web' } } },
): string {
  root ??= realpathSync(mkdtempSync(join(tmpdir(), 'monolith-client-modules-')))
...
function writeBuiltPackage(packageName: string, client: Record<string, unknown>): void {
  const clientPath = writePackage(packageName, { monolith: { client: { platform: 'web', ...client } } })
...
describe('HTML bootstrap facade', () => {
  it('precedes blocking preloads and the boot graph, then becomes the live registration target', async () => {
    ...
    const graphAt = html.indexOf('globalThis["__MONOLITH_BOOT__"] = ')
...
describe('client bundle activation', () => {
  ...
  it('allows sibling monolith roles', () => {
...
describe('shared module declarations', () => {
  ...
  it('rejects a non-array external', () => {
    ...
    expect(() => construct([packageName]))
      .toThrow(`client-modules: ${packageName} monolith.client.external must be a string array`)
  })
})
...
describe('module graph order', () => {
  ...
  it('leaves a request no row answers to the static assembly channel', () => {
    expect(ids(orderByModuleGraph([
      entry('consumer', { external: ['@monolith/cordis'] }),
      entry('other'),
