/**
 * Node half of the client module system (`monolith.client` dual-face package): scans
 * the host Loader's entries for packages declaring `monolith.client`, composes the
 * `window.__MONOLITH_BOOT__` entry graph (wire single source: {@link WebBootEntry}
 ...
 * @module @monolith/client-modules
 */
...
import type { Entry } from '@monolith/cordis-plugin-loader'
import type { IndexInjection } from '@monolith/host-webserver'
...
/** package.json `monolith.client` declaration fields, validated one by one after reading the file. */
interface MonolithClientDeclaration {
  inject?: string[]
  platform: string
...
/** Narrow an unknown parsed JSON value to the `monolith.client` declaration, throwing on malformed fields. */
function parseMonolithClient(pkgName: string, value: unknown): MonolithClientDeclaration | undefined {
  if (value === undefined) return undefined
  if (typeof value !== 'object' || value === null) {
    throw new Error(`client-modules: ${pkgName} has a non-object monolith.client declaration`)
  }
  const decl = value as Record<string, unknown>
  if (typeof decl.platform !== 'string') {
    throw new Error(`client-modules: ${pkgName} monolith.client.platform must be a string`)
  }
  const inject = optionalStringArray(pkgName, 'monolith.client.inject', decl.inject)
  const external = optionalStringArray(pkgName, 'monolith.client.external', decl.external)
  if (decl.immediately !== undefined && typeof decl.immediately !== 'boolean') {
    throw new Error(`client-modules: ${pkgName} monolith.client.immediately must be a boolean`)
  }
  return {
...
function comboSectionMap(record: WebPluginRecord): Record<string, unknown> {
  ...
  const base = new URL(`/plugins/${record.entry.id}/client.js.map`, 'http://monolith.invalid')
    ...
    for (const name of entry.external ?? []) {
      ...
      if (dependency === entry) {
        throw new Error(
          ...
          + '— a row must not declare its own package in monolith.client.external',
        )
      }
...
const CLIENT_MODULES_ID = '@monolith/client-modules'
...
window.__ModuleLoader__={
  ...
  rows.push({ kind: 'global', name: '__MONOLITH_BOOT__', value: graph })
  return rows
}

/**
 * The web plugin table service: incremental `monolith.client` scan + wire composition
...
export class ClientModuleRegistry extends Service {
   ...
   * @returns the graph served as `window.__MONOLITH_BOOT__`.
   */
  graph(): WebBootGraph {
  ...
  private resolveMeta(loaderName: string, baseUrl: string): ResolvedPkgMeta | null {
    ...
    const monolith = pkg.monolith
    const decl = parseMonolithClient(
      packageName,
      monolith !== null && typeof monolith === 'object' ? (monolith as Record<string, unknown>).client : undefined,
    )
    ...
    if (clientRel === undefined) {
      throw new Error(`client-modules: ${packageName} declares monolith.client but exports no "./client" bundle`)
    }
    const meta: PkgMeta = {
