/**
 * Package-owned invariant companion for `@monolith/client-modules`.
 * @module @monolith/client-modules/invariant
 */
...
const PACKAGE_NAME = '@monolith/client-modules'
  ...
  ctx.on('internal/plugin', () => {
    ...
    for (const row of host.graph().entries) {
      if (host.clientPath(row.id) === undefined) {
        fail(`web plugin graph row "${row.id}" advertises ${row.url} but resolves no client bundle path — the served __MONOLITH_BOOT__ would 404 on fetch`)
      }
    }
  }, { global: true })
}
