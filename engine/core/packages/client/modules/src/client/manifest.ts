 ...
 * `__MONOLITH_BOOT__` wire types, the boot-manifest parser, and the boundaries around
 * {@link ClientModuleSystem}. The package root is the host-side service that
 * composes the wire.
 */

import type {} from '@monolith/cordis'
...
/**
 * Validate an optional string-array field read from a `monolith.client` declaration
...
/**
 * Parse `window.__MONOLITH_BOOT__` into the two consumer views. Wire boundary:
 * a missing or malformed graph throws (the shell shows the loud failure —
 * a page without a valid manifest cannot boot anything).
 * @param wire - the raw `window.__MONOLITH_BOOT__` value.
...
export function parseBootManifest(wire: unknown): BootManifest {
  if (typeof wire !== 'object' || wire === null) {
    throw new Error('client-modules: window.__MONOLITH_BOOT__ is missing or not an object')
  }
