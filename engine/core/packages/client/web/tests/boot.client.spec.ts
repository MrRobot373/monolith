...
import * as modulesClient from '@monolith/client-modules/client'
import type {
  ClientBundleRegistration, ClientModuleCreateOptions, ClientModuleLoaderTarget, MonolithWindow,
  WebBootEntry,
} from '@monolith/client-modules/client'
...
const win = globalThis as MonolithWindow
const transportGlobal = globalThis as {
  __MONOLITH_TRANSPORT__?: { loadBundle(url: string): Promise<void> }
}
...
afterEach(() => {
  ...
  delete transportGlobal.__MONOLITH_TRANSPORT__
...
describe('bootstrap failure rendering', () => {
  ...
  it('renders a malformed boot manifest', async () => {
    await expectBootFailure(() => {
      installFacade()
      delete win.__MONOLITH_BOOT__
    }, 'window.__MONOLITH_BOOT__ is missing or not an object')
  })
...
describe('plugin activation', () => {
  it('prefetches a parser-loaded immediate row through the injected bundle transport', async () => {
    ...
    transportGlobal.__MONOLITH_TRANSPORT__ = {
