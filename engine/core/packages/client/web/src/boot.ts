 ...
 * @module @monolith/client-web/src/boot
 */
import { Context } from '@monolith/cordis'
import Loader from '@monolith/cordis-plugin-loader'
import type {
  BootManifest, ClientModuleCreateOptions, ClientModuleSystem, MonolithWindow,
...
export class AppWebEntry {
  ...
  async run(): Promise<void> {
    try {
      ...
      await (globalThis as { __MONOLITH_BOOT_READY__?: { promise: Promise<void> } }).__MONOLITH_BOOT_READY__?.promise
      ...
      // is `ClientTransportHooks`, owned by @monolith/client-connection;
      // this structural slice reads one optional member without adding a
      // package edge.
      const transport = (globalThis as {
        __MONOLITH_TRANSPORT__?: { loadBundle?: ClientModuleCreateOptions['loadBundle'] }
      }).__MONOLITH_TRANSPORT__
      this.modules = moduleLoader.create({
        boot: win.__MONOLITH_BOOT__,
