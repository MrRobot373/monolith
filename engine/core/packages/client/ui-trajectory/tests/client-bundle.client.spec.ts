 ...
 * not built (`pnpm --filter @monolith/client-ui-trajectory bundle`).
 */
import { readFileSync } from 'node:fs'
import { resolve } from 'node:path'
import { Context } from '@monolith/cordis'
import { stubSettingsScope } from '@monolith/client-test-runtime'
import { afterEach, describe, expect, it } from 'vitest'
import { UiConversation } from '@monolith/client-ui-conversation/client'
import { SlotRegistry } from '@monolith/client-ui-renderer/client'

const PLUGIN_ID = '@monolith/client-ui-trajectory'
...
describe('tsdown client artifact', () => {
  ...
  async function loadArtifact() {
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@monolith/client-store', await import('@monolith/client-store')],
      ['@monolith/client-ui-conversation/client', await import('@monolith/client-ui-conversation/client')],
      ['@monolith/client-ui-primitives', await import('@monolith/client-ui-primitives')],
    ])
  ...
  it.skipIf(code === undefined)('mounted as an object plugin, apply registers the view tab on the real ring', async () => {
    ...
    const locale = await import('@monolith/client-locale/client')
