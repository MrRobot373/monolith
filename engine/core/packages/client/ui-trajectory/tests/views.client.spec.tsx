...
import { bindSnapshotSelector, SlotTestRuntime, stubSettingsScope } from '@monolith/client-test-runtime'
import { resolveSlotLabel } from '@monolith/client-ui-slots'
import {
  EMPTY_CONVERSATION_SNAPSHOT, UiConversation,
} from '@monolith/client-ui-conversation/client'
import type {
...
import { EMPTY_CHAT_SNAPSHOT } from '@monolith/client-ui-chat/client'
import type {
  ChatSnapshot, LegacyConversationSlice,
} from '@monolith/client-ui-chat/client'
import { SlotRegistry } from '@monolith/client-ui-renderer/client'
import { createSnapshotStore } from '@monolith/client-store'
import type { ObservableSnapshot } from '@monolith/client-store'
import type {
  SessionBinding, SessionListState, SessionProjectionMap, SessionSnapshot, UseProjection,
} from '@monolith/api-session-controller/client'
import type { WorkspaceSnapshot } from '@monolith/api-workspace-controller/client'
import type { SessionId } from '@monolith/session/types'
import type { SessionPendingInteractionSnapshot } from '@monolith/client-ui-session/client'
import {
  ConversationSession, ConversationSessionHeader,
  type ConversationSessionHeaderProps, type ConversationSessionProps,
} from '@monolith/client-ui-conversation/src/client/skeleton/ConversationSession.tsx'
import { createConversationStore } from '@monolith/client-ui-conversation/src/client/stores.ts'
import { zh as conversationZh } from '@monolith/client-ui-conversation/src/client/locales.ts'
import { apply as localeApply, inject as localeInject } from '@monolith/client-locale/client'
import { apply, inject } from '@monolith/client-ui-trajectory/client'
import { apply as nodeApply } from '@monolith/client-ui-trajectory'
...
describe('plugin registration', () => {
  ...
  it('shares one browser-wide duration preference across session injections', async () => {
    ...
    expect(localStorage.getItem('monolith.trajectory.duration')).toBe('true')
    expect(localStorage.getItem(`monolith.trajectory.duration.${SID}`)).toBeNull()
  })
