...
import { LocaleRuntime } from '@monolith/client-locale/client'
import type { SessionId } from '@monolith/session/types'
import { RemoteError } from '@monolith/client-test-runtime'
import type {
  CandidateRequest, ClientSessionContext, InputTriggerCandidate, InputTriggerSource,
} from '@monolith/client-ui-input-trigger/client'
import type { FileReferenceCandidate } from '@monolith/file-reference/types'
import type { SessionReferenceMentionCandidate } from '@monolith/session-reference/types'
...
async function bench(
  ...
  sessions: RemoteLookup<SessionReferenceMentionCandidate> = vi.fn(() => Promise.resolve({
    ...
    value: [{
      ...
      mention: '@[Research](monolith-session:InNvdXJjZSI)',
    }],
  })),
  ...
  it('labels a session without a cwd and still dates it', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: false,
        createdAt: CREATED_AT,
        mention: '@[same](monolith-session:InNhbWUi)',
      }],
    }))
    const { source } = await bench(files, sessions)
  ...
  it('falls back to the candidate createdAt for a session the Host list does not carry', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: true,
        createdAt: CREATED_AT,
        mention: '@[Unlisted run](monolith-session:InVubGlzdGVkIg)',
      }],
    }))
  ...
  it('reads a session opened moments ago as the present, not a zero distance', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: true,
        createdAt: NOW - 1_000,
        mention: '@[Just now](monolith-session:Imp1c3Qtbm93Ig)',
      }],
    }))
  ...
  it('dates a session in the current workspace without repeating that workspace', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: true,
        createdAt: CREATED_AT,
        mention: '@[Sibling run](monolith-session:InNpYmxpbmdyIg)',
      }],
    }))
    const { source } = await bench(files, sessions)
...
describe('pick and codec', () => {
  ...
  it('inserts sessions as atomic chips whose clipboard and model forms are canonical mentions', async () => {
    ...
    const mention = '@[Research](monolith-session:InNvdXJjZSI)'
