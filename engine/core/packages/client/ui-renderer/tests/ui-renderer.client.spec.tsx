...
import { apply as nodeApply } from '@monolith/client-ui-renderer'
...
describe('UI renderer plugin', () => {
  ...
  it('hydrates the boot page before switching to the assembled application', async () => {
    ...
    slots.register({ name: 'root' }, () => <div data-testid="root-probe" />)
    const el = container()
    el.innerHTML = '<div class="boot" data-monolith-boot=""><div><div class="spinner" data-monolith-boot-spinner="" style="--monolith-boot-arc: 180deg"></div><div>Loading plugins…</div></div></div>'
