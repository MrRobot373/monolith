...
import type { SessionSnapshot } from '@monolith/api-session-controller/client'
...
describe('matrix row: claimed', () => {
  it('publishes the claim currency, colors the token, hints while args are blank, and edits stay free', () => {
    ...
    expect(textarea.style.getPropertyValue('--monolith-composer-hint')).toBe(JSON.stringify('输入目标，智能体将持续执行'))
    ...
    expect(textarea.style.getPropertyValue('--monolith-composer-hint')).toBe('')
  })
