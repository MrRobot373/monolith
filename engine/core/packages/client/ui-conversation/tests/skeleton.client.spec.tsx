...
import type { SessionListState, SessionSnapshot } from '@monolith/api-session-controller/client'
import type { WorkspaceSnapshot, WorkspaceView } from '@monolith/api-workspace-controller/client'
...
} from '@monolith/client-test-runtime'
...
import { en as commonEn } from '@monolith/client-locale/src/locales/en.ts'
import { zh as commonZh } from '@monolith/client-locale/src/locales/zh.ts'
...
describe('ConversationRoot resident composer', () => {
  ...
  it('publishes the column width as a px variable for the shared width axis', () => {
    ...
    act(() => { fireResize(root) })
    expect(root.style.getPropertyValue('--monolith-conversation-column-width')).toBe('1200px')
    // No dragged preference: the user-width override stays absent so the
    // adaptive clamp term applies.
    expect(root.style.getPropertyValue('--monolith-chat-user-width')).toBe('')
  })
  ...
  it('drag → persist → window clamp round-trip on a width handle', () => {
    ...
    try {
      ...
      expect(root.style.getPropertyValue('--monolith-chat-user-width')).toBe('970px')
      expect(localStorage.getItem('monolith.conversation.contentWidth')).toBe('970')
      ...
      act(() => { fireResize(root) })
      expect(root.style.getPropertyValue('--monolith-chat-user-width')).toBe('724px')
      expect(localStorage.getItem('monolith.conversation.contentWidth')).toBe('970')
