...
const WIDTH_PREF_KEY = 'monolith.conversation.contentWidth'
...
function WidthHandle(props: {
  ...
  const onPointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    e.currentTarget.style.setProperty('--monolith-width-handle-pointer-y', `${e.clientY - box.top}px`)
...
export function ConversationRoot({
  ...
  // scroll body: the seat's height as --monolith-composer-height, so controls clear
  // the composer as it grows, and the scrollport's own height as
  // --monolith-conversation-viewport-height, so a control can sit in the band the
  ...
  const seatResizeRef = useCallback((seat: HTMLDivElement | null): void => {
    ...
    seatObserver.current = new ResizeObserver(() => {
      scroller.style.setProperty('--monolith-composer-height', `${seat.offsetHeight}px`)
      scroller.style.setProperty(
        '--monolith-conversation-viewport-height',
  ...
  // Publishes the column's live width as --monolith-conversation-column-width so
  ...
  const publishWidths = useCallback((root: HTMLDivElement): void => {
    ...
    root.style.setProperty('--monolith-conversation-column-width', `${column}px`)
    const preference = readWidthPreference()
    if (preference === null) {
      root.style.removeProperty('--monolith-chat-user-width')
    } else {
      root.style.setProperty('--monolith-chat-user-width', `${resolveContentWidth(column, preference)}px`)
    }
  }, [])
  ...
  const onHandleDrag = useCallback((width: number): void => {
    ...
    root.style.setProperty('--monolith-chat-user-width', `${clamped}px`)
  }, [])
