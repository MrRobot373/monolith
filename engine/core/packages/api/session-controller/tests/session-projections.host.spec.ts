...
import SessionProjectionCache, { projectionCacheDomainSpec } from '@monolith/session-projection-cache'
...
import type { SessionControlFrame, SessionFollowFrame } from '@monolith/api-session-controller/types'
...
describe('session.list projections column', () => {
  ...
  it('keeps persisted host-only state out of a cold session.list response', async () => {
    const root = await mkdtemp(join(tmpdir(), 'monolith-api-projcache-'))
    const ctx = new Context()
    try {
