...
import { agentPresetProjectionDefinition } from '@monolith/agent-presets'
import SessionStore, { SessionLogOffset, SessionId } from '@monolith/session'
import type { SessionEvent, SessionHeader } from '@monolith/session'
import type { SessionObservation } from '@monolith/session-query'
import TypertRegistry from '@monolith/typert-registry'
...
describe('ApiSession create or adoption', () => {
  it('shares one in-flight creation between concurrent callers', async () => {
    const { ctx, agents } = await harness()
    const cwd = mkdtempSync(join(tmpdir(), 'monolith-session-controller-concurrent-'))
  ...
  it('accepts a raced ordinary creation and rejects a raced attached child', async () => {
    const ordinary = await harness()
    const cwd = mkdtempSync(join(tmpdir(), 'monolith-session-controller-create-'))
    ...
    const child = await harness()
    const childCwd = mkdtempSync(join(tmpdir(), 'monolith-session-controller-child-'))
  ...
  it('validates ownership and cwd on the Agent returned by creation', async () => {
    const child = await harness()
    const childCwd = mkdtempSync(join(tmpdir(), 'monolith-session-controller-returned-child-'))
    const childMeta = {
    ...
    const requestedCwd = mkdtempSync(join(tmpdir(), 'monolith-session-controller-wrong-cwd-'))
  ...
  it('surfaces directory creation failure and rejects setup without a scoped Agent', async () => {
    const { agents } = await harness()
    const parent = mkdtempSync(join(tmpdir(), 'monolith-session-controller-file-'))
