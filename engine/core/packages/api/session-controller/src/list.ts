...
import type { ImageAttachmentLimits } from '@monolith/attachment'
...
import { SessionQueryError, type SessionSearchCursor } from '@monolith/session-query'
  ...
  async search(query: string, signal: AbortSignal): Promise<SessionSearchValue> {
    ...
    if (provider === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'session search is unavailable: this deployment does not mount @monolith/session-query',
        {},
      )
    }
    try {
