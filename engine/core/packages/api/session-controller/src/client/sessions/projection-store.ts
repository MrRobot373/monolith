...
import type { SessionSeqCursor } from '@monolith/session/types'
...
// (`/types`, zero imports), never the package root: the root's monolith-agent →
// monolith-session chain would drag the host `Context.sessions` merge into the
...
export type { SessionProjectionMap } from '@monolith/session-projection/types'
