...
import { remoteErrorOf, remoteMethods } from '@monolith/typert-protocol'
  ...
  it('reports the actionable configuration error while no credential provider is mounted', async () => {
    ...
    for (const call of [
      ...
      expect(remoteErrorOf(failure)).toMatchObject({
        ...
        message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @monolith/credentials-local) in its composition',
        details: {},
      })
    }
  })
