...
import type { SettingsDescriptor } from '@monolith/settings'
import { RemoteError, remoteErrorOf, remoteMethods } from '@monolith/typert-protocol'
  ...
  it('reports the actionable configuration error while no settings provider is mounted', async () => {
    ...
    for (const call of calls) {
      ...
      expect(remoteErrorOf(failure)).toMatchObject({
        ...
        message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @monolith/settings-file) in its composition',
        details: {},
      })
    }
  })
