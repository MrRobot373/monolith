 ...
 *
 * @module @monolith/api-settings-controller/src/credentials.ts
 */

import { Context } from '@monolith/cordis'
import { credentialRef } from '@monolith/credentials'
import type { CredentialProvider } from '@monolith/credentials'
  ...
  private provider(): CredentialProvider {
    ...
    if (credentials === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'credentials service is absent: this deployment does not mount a credential provider (e.g. @monolith/credentials-local) in its composition',
        {},
      )
    }
    return credentials
