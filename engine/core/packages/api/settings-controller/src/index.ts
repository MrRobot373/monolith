 ...
 *
 * @module @monolith/api-settings-controller
 */

import { dirname } from 'node:path'
import { Context } from '@monolith/cordis'
import Schema from '@monolith/schemastery'
...
import {
  ...
  openNativePath,
  openNativeTextFile,
} from '@monolith/native-command'
import type { SettingsDescriptor, SettingsPathOp, SettingsProvider } from '@monolith/settings'
import type {
...
export class SettingsController extends TypertRemoteService {
  ...
  private provider(): SettingsProvider {
    ...
    if (settings === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'settings service is absent: this deployment does not mount a settings provider (e.g. @monolith/settings-file) in its composition',
        {},
      )
    }
    return settings
