...
```yaml
- name: '@monolith/permission-presets'
  config:
    presets:
      workspace-write:
...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-permission-presets) is the exhaustive source for every accepted field and its JSDoc. The name `custom` is reserved for the derived not-a-preset state and cannot name a table entry. Mounting requires a confining bash executor (one that reports a `sandboxMode`) and the approval service.
...
Indirectly, through `monolith-user-approval` and `monolith-tool-bash`, which render the approval-policy prompt, switch notice, and sandboxed tool outcomes selected by this service's knob events; `permission/preset` itself is log-only.
