...
import type { WebServer } from '@monolith/host-webserver'
...
function stageDist(): string {
  dist = mkdtempSync(join(tmpdir(), 'monolith-web-app-'))
...
describe('web-app runtime glue', () => {
  it('mounts dist serving, prompt section, bash variables, and publishes the URL with the LAN snapshot', async () => {
    ...
    ctx.provide(MONOLITH_LAUNCH_ENVIRONMENT_KEY, createLaunchEnvironmentSnapshot([
    ...
    expect(log).toHaveBeenCalledWith('monolith web: http://127.0.0.1:4567/?token=test-token (LAN: http://192.168.1.5:4567/?token=test-token)')
    expect(log).toHaveBeenCalledWith('monolith web: opening the default browser; pass --no-open to disable')
    expect(openBrowser).toHaveBeenCalledWith('http://127.0.0.1:4567/?token=test-token')
    expect(lifecycle).toEqual([
      'monolith web: http://127.0.0.1:4567/?token=test-token (LAN: http://192.168.1.5:4567/?token=test-token)',
      'monolith web: opening the default browser; pass --no-open to disable',
      'open:http://127.0.0.1:4567/?token=test-token',
    ])
    const assembly = await ctx.systemPrompt.assemble()
    expect(assembly.sections.find(entry => entry.name === 'harness:source')?.text).toContain('MONOLITH implementation checkout')
    ...
    expect(webRuntime?.resolve()).toEqual({ MONOLITH_WEB_URL: 'http://127.0.0.1:4567' })
  ...
  it('prints the loopback-only URL line when no LAN snapshot exists', async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).toHaveBeenCalledWith('monolith web: http://127.0.0.1:4567/?token=test-token')
    await ctx.fiber.dispose()
  })
  ...
  it.each([
    ...
    expect(diagnostic).toHaveBeenCalledWith(
      `web-app: could not open the default browser because ${reason}; use the monolith web URL printed at startup`,
    )
  ...
  it('scrubs the helper environment and reports helper spawn or exit failures', async () => {
    ...
    vi.stubEnv('MONOLITH_HOME', '/must-not-reach-browser')
    ...
    expect(options?.env).not.toHaveProperty('MONOLITH_HOME')
