...
import WebServer from '@monolith/host-webserver'
...
afterEach(async () => {
  ...
  vi.unstubAllEnvs()
  Reflect.deleteProperty(globalThis, '__monolithWebAppApply')
  Reflect.deleteProperty(globalThis, '__monolithWebServer')
  Reflect.deleteProperty(globalThis, '__monolithConnection')
})

describe('web app browser startup', () => {
  it('opens the canonical URL only after the complete page is reachable', async () => {
    const root = mkdtempSync(join(tmpdir(), 'monolith-web-browser-open-'))
    tempRoots.push(root)
    ...
    writeFileSync(webserverModule, 'export default globalThis.__monolithWebServer\n')
    writeFileSync(connectionModule, [
      "export const inject = ['webServer']",
      "export const apply = ctx => ctx.provide('connection', globalThis.__monolithConnection)",
      '',
    ].join('\n'))
    writeFileSync(webAppModule, [
      "export const name = 'fixture-web-app'",
      "export const inject = ['webServer']",
      'export const apply = (ctx, config) => globalThis.__monolithWebAppApply(ctx, config)',
      '',
    ].join('\n'))
    ...
    const globals = globalThis as unknown as {
      __monolithWebAppApply: typeof apply
      __monolithWebServer: typeof WebServer
      __monolithConnection: {
    ...
    globals.__monolithWebAppApply = apply
    globals.__monolithWebServer = WebServer
    globals.__monolithConnection = {
