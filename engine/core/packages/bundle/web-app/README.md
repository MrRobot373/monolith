---
description: "The browser GUI for monolith: interactive chat, model and settings management, and session history, for users running the monolith web surface."
kind: "package-bundle"
---
...
Run `monolith --profile web` and the interface opens in your default browser, ready for interactive chat with the agent. You get the conversation view, model and settings management, and session history, backed by the same model access, tools, and safety defaults as every other surface. The command prints a tokenized startup URL; the browser exchanges that token for a signed session cookie and redirects to the clean root URL. You can change the port, suppress the browser handoff, and allow extra hosts from the command line; binding all network interfaces is intentionally not supported. Choose it for interactive work in the browser; `monolith-headless` is the one-shot command-line sibling.
...
| `printUrl` | `true` | Print the `monolith web:` URL line at startup |
| `surfaceContext` | `true` | Give the agent GUI-orientation context and expose `MONOLITH_WEB_URL` to its shell commands |
| `trustedHosts` | `[]` | Extra hosts allowed to reach the GUI from the network |

The generated [configuration catalog](../../../docs/config-catalog.md#monolith-web-app) is the exhaustive source for every accepted field and its JSDoc.
...
When you launch `monolith --profile web` over SSH, the URL line still prints but the browser is not opened for you: the SSH client or editor owns the local forwarding address. Open the forwarded URL on your machine yourself; the printed URL names the remote host's loopback endpoint.

### Per-session agent setup

Each browser session composes its own agent from the shipped presets (the `standard` preset by default), instead of sharing one process-wide tool set. You can change the default preset or add your own presets under `$MONOLITH_HOME/.agent-presets`.
...
- [monolith-base](../base/README.md) — the shared core the GUI runs on.
- [monolith-client-hmr](../../client/hmr/README.md) — how client-plugin changes reload during development.
- [frontend-static](../../host/frontend-static/README.md) — how the built frontend is served.
- [Generated configuration catalog](../../../docs/config-catalog.md#monolith-web-app) — every accepted config field and its source declaration.
