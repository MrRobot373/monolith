/**
 * @monolith/web-app — the browser-surface bundle's runtime glue plugin
 * plus the bundle patch (`cordis.patch.yml`, declared by the `monolith.bundle.patch`
...
import { addHarnessSourceSection } from '@monolith/app-boot'
import type {} from '@monolith/client-connection'
import * as FrontendStatic from '@monolith/host-frontend-static'
import { launchEnvironmentOf } from '@monolith/launch-environment'
import { scrubbedParentEnv } from '@monolith/subprocess'
import type {} from '@monolith/cordis-plugin-loader'
...
/** This monolith installation's root, from either this package's source or built entry. */
...
export interface Config {
   ...
   * section and the `MONOLITH_WEB_URL` bash variable). A one-shot non-interactive
...
const MONOLITH_WEB_URL = 'MONOLITH_WEB_URL' as const
...
/** Model-visible orientation and acceptance boundary for sessions created through `monolith web`. */
function webSurfacePrompt(webUrl: string): string {
  const updateContract = 'The client-plugin HMR receiver is active, but client-plugin changes reload without a refresh only while '
    + '`pnpm run dev:web` is also running from this same checkout to rebuild their bundles; verify that watcher before promising automatic updates. '
    + 'Every other change — the apps/web shell and plain packages — requires rebuilding the affected Web artifacts and verifying this existing URL after a page refresh. '
  return `You are interacting with the user through the MONOLITH Web GUI at ${webUrl}. `
    + 'When the user refers to "this page", "this GUI", or "this app" without naming another target, they mean this GUI. '
    + 'The browser provides no implicit DOM, route, or screenshot context. '
    + updateContract
    + 'Starting another server does not update this GUI. '
    + 'The apps/web Vite entry builds the shell but is not a standalone application because only monolith web injects window.__MONOLITH_BOOT__. '
...
function resolveDistIndex(): string {
  ...
  try {
    return join(dirname(require.resolve('@monolith/web-frontend/package.json')), 'dist', 'index.html')
  } catch {
    /* v8 ignore next 2 -- reachable only when the frontend package is absent from the checkout */
    throw new Error('web-app: @monolith/web-frontend is not resolvable from this composition')
  }
}
  ...
  if (config.surfaceContext) {
    ...
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'web-runtime',
        variables: {
          [MONOLITH_WEB_URL]: { description: 'Canonical local URL of the MONOLITH Web GUI serving this session.' },
        },
        resolve: () => ({ [MONOLITH_WEB_URL]: localWebUrl(runtimeCtx) }),
      })
    })
  }
  if (config.printUrl || handoffBrowser) {
    ctx.inject(['connection'], (connectionCtx) => {
      ...
      const announceReady = (): void => {
        ...
        if (config.printUrl) {
          console.log(`monolith web: ${authenticatedUrl}${lanUrl === undefined ? '' : ` (LAN: ${lanUrl})`}`)
        }
        if (handoffBrowser) {
          console.log('monolith web: opening the default browser; pass --no-open to disable')
          void internals.openBrowser(authenticatedUrl).catch((error: unknown) => {
            const reason = error instanceof Error ? error.message : String(error)
            console.error(`web-app: could not open the default browser because ${reason}; use the monolith web URL printed at startup`)
          })
        }
      }
