/**
 * @monolith/headless — one-shot direct Agent driver. The bundle patch
 * rides over monolith-base without Host, HTTP, or browser plugins; this runner
...
import { installModelSelection } from '@monolith/agent'
import type { Agent, ModelSelectionRef } from '@monolith/agent'
import type {} from '@monolith/agent-default-model'
...
import type { Session, SessionEvent, SessionId, SessionLogOffset } from '@monolith/session'
// Empty type imports carry the loader Context merge for the settlement await
// and the cmdline Context merge for the appExit host value.
import type {} from '@monolith/cordis-plugin-loader'
import type {} from '@monolith/cmdline'
  ...
  const dispose = ctx.on('session/event', (session, event) => {
    ...
    switch (chunk.type) {
      case 'reasoning-delta':
        ...
        if (!open) {
          stderr.write('monolith: reasoning:\n')
          open = true
        }
...
function fail(io: HeadlessIo, error: unknown): void {
  io.stderr.write(`monolith: ${error instanceof Error ? error.message : String(error)}\n`)
  io.exit(1)
}
...
async function run(ctx: Context, task: string, io: HeadlessIo): Promise<void> {
  ...
  // (@monolith/agent-presets README, "Composing a child agent").
  ...
  if (outcome.reason?.kind === 'error') {
    io.stderr.write(`monolith: ${outcome.reason.error.code}: ${outcome.reason.error.message}\n`)
  }
