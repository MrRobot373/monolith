...
import type { Agent, AgentHandle, CreateAgentOptions } from '@monolith/agent'
import AgentDefaultModelConfig from '@monolith/agent-default-model'
import { createAssistantMessage } from '@monolith/llm'
...
describe('headless runner', () => {
  ...
  it('prints the durable model failure when the final turn ends in error', async () => {
    ...
    expect(await test.run()).toMatchObject({
      code: 1,
      out: '\n',
      err: 'monolith: SERVER: provider unavailable\n',
    })
    await test.ctx.fiber.dispose()
  })
  ...
  it('separates an unterminated reasoning prefix from the terminal model failure', async () => {
    ...
    expect(await test.run()).toMatchObject({
      code: 1,
      out: '\n',
      err: 'monolith: reasoning:\ntrying recovery\nmonolith: SERVER: provider unavailable\n',
    })
    await test.ctx.fiber.dispose()
  })
  ...
  it('fails when an event below the captured Session length cannot be read', async () => {
    ...
    expect(await test.run()).toMatchObject({
      code: 1,
      out: '',
      err: 'monolith: headless summary cannot read seq 0 below captured length 7\n',
    })
    await test.ctx.fiber.dispose()
  })
  ...
  it('reports a direct Agent creation failure', async () => {
    ...
    expect(err).toBe('monolith: factory exploded\n')
