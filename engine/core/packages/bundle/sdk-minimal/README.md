...
# `@monolith/sdk-minimal`

## Summary

Use `monolith --profile sdk-minimal` when an SDK client needs a small, explicit coding-agent runtime. The profile advertises a platform-selected persistent shell and `str_replace_editor`, persists sessions as uncompressed JSONL, and selects the model from the SDK initialization request. It supplies a complete Cordis tree and deliberately excludes `monolith-base`, Web, settings, managed credentials, telemetry, compaction, workspace instructions, skills, jobs, and subagents. Its danger-full-access policy lets the shell and editor modify any path available to the process, so use it only with an isolated workspace.
...
Launch the profile directly or select it from the Python SDK. Supply an explicit `MONOLITH_HOME`, use a disposable workspace, and provide the model credential through `DEEPSEEK_API_KEY`.

```sh
export MONOLITH_HOME=/absolute/path/to/example-monolith-home
monolith --profile sdk-minimal
```

`MONOLITH_CONTEXT_WINDOW` sets the fallback capacity for a model absent from the adapter's advisory catalog. `MONOLITH_SYSTEM_PROMPT` replaces the default persona. The SDK initialization request is the sole model selection and overrides environment defaults.

Use `monolith plugin --profile sdk-minimal` to manage persistent external dependencies. Profile, home, and ordered `--patch` files can replace rows or insert bundles above the complete default tree. The shipped template applies patches only at startup.
...
The bundle's single insert is the complete application tree: SDK stdio startup and JSON-RPC serving, one environment-configured DeepSeek adapter, the explicit agent core, local subprocess and unrestricted filesystem providers, a platform-selected persistent shell PTY, the string-replace editor, and uncompressed JSONL persistence under `$MONOLITH_HOME/sessions`. It does not inherit another bundle, so every extra row is an explicit profile change.
...
- [Python SDK example](../../../python/sdk/examples/README.md) — launches this profile from Python against an explicit MONOLITH home.
...
- **The composition intentionally omits shared product services** — select `monolith --profile sdk` when settings, managed credentials, policy presets, telemetry, Web tools, or the full default tool roster are required.
