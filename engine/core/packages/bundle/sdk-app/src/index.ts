 ...
 * @module @monolith/sdk-app
 */

import { Command } from 'commander'
import type { Context } from '@monolith/cordis'
import z from '@monolith/schemastery'
import { exitOnStdinEnd, parseCmdline } from '@monolith/cmdline'
...
function sdkCommand(profile: string): Command {
  return new Command()
    .name(`monolith --profile ${profile}`)
    .description('Serve MONOLITH SDK clients over stdio JSON-RPC.')
    .helpOption('-h, --help', 'show this help')
    .addHelpText('after', `
Example:
  monolith --profile ${profile}     serve one SDK runtime until its client disconnects
`)
}
