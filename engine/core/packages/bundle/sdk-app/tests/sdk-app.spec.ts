...
describe('monolith-sdk-app bundle', () => {
  it('declares startup-gated JSON-RPC serving without overriding base HMR policy', () => {
    ...
    expect(manifest.dependencies).toHaveProperty('@monolith/sdk-jsonrpc-server')
    ...
    expect(rows.find(row => row.id === 'sdk-app-startup')?.name).toBe('@monolith/sdk-app')
