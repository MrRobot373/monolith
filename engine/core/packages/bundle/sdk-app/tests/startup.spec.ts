...
import { internals, provideCmdline } from '@monolith/cmdline'
...
describe('SDK app startup', () => {
  ...
  it('prints app help without publishing readiness or binding stdin', () => {
    ...
    expect(out()).toContain('monolith --profile sdk')
  ...
  it('renders the selected SDK profile name in help', () => {
    ...
    expect(out()).toContain('Usage: monolith --profile sdk-minimal')
    expect(out()).toContain('monolith --profile sdk-minimal')
  })
})
