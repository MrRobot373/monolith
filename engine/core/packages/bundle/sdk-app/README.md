...
The SDK stdio application as a `monolith` profile bundle over [`monolith-base`](../base/README.md). It inherits the base's disabled module-HMR policy; its patch sets the coding-agent persona, mounts an app-owned zero-option command provider, and starts [`monolith-sdk-jsonrpc-server`](../../sdk/server/README.md) only after that provider accepts the invocation. `monolith --profile sdk --help` therefore writes help and exits without claiming stdin or stdout. The standalone [`sdk-minimal`](../sdk-minimal/README.md) bundle reuses the same startup provider with its own profile name.
...
- **A profile can omit the SDK server** — a custom profile selected by the TypeScript client must retain this bundle or another `monolith-sdk-jsonrpc-server` row; client initialization fails when no peer answers.
