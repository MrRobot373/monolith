...
describe('monolith-acp-app bundle', () => {
  it('declares startup-gated ACP serving without overriding base HMR policy', () => {
    ...
    expect(manifest.dependencies).toHaveProperty('@monolith/acp')
    ...
    expect(rows.find(row => row.id === 'acp-app-startup')?.name).toBe('@monolith/acp-app')
