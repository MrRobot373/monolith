...
The automation-only ACP stdio application as a `monolith` profile bundle over [`monolith-base`](../base/README.md). It inherits the base's disabled module-HMR policy; its patch sets the coding-agent persona and default model route, mounts an app-owned zero-option command provider, and starts [`monolith-acp`](../../acp/acp/README.md) only after that provider accepts the invocation. `monolith --profile acp --help` therefore writes help and exits without claiming stdin or stdout.
...
- **A profile can omit the ACP bridge** — a custom ACP launch profile must retain this bundle or another `monolith-acp` row; otherwise no peer answers the client.
