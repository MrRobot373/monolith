/**
 * The bundle's substance is its patch file: the `monolith.bundle.patch` manifest
...
describe('monolith-base bundle', () => {
  it('declares a parseable patch list through the monolith.bundle.patch manifest field', () => {
    ...
    expect(rows.find(row => row.id === 'session-telemetry-otel')?.config?.['mode']).toEqual({
      __jsExpr: "process.env.MONOLITH_TELEMETRY_MODE || 'FEEDBACK_ONLY'",
    })
    ...
    expect(manifest.dependencies).toHaveProperty('@monolith/web-fetch-http')
  })
