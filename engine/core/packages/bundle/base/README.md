---
description: "The shared monolith core: model access, tools, durable sessions, and safety defaults for every monolith --profile surface, for users composing or customizing a profile."
kind: "package-bundle"
---
...
Every base-backed `monolith --profile` surface runs on `monolith-base`, so those surfaces share a model connection, the full tool set, durable session history, and workspace safety defaults. The shipped `sdk-minimal` profile deliberately uses a complete standalone tree instead. You rarely touch this bundle directly — shipped base-backed profiles already include it, and a custom base-backed profile names it first. When you need different defaults, change your profile patch or add a later bundle; this package is not a library you import.
...
You get the monolith core automatically: the shipped `web`, `headless`, `sdk`, and `acp` profiles already include it, and a custom profile names it as its first bundle. After that, everything works with no further configuration.

### A minimal custom profile

To build a profile on the shared core, create a profile with a `package.json` that names `@monolith/base` first:
...
```json
{
  ...
  "monolith": {
    "profile": {
      "bundles": ["@monolith/base"]
    }
  }
}
```

Run `monolith --profile my-profile "your task"` and you get a working agent with model access, tools, persistence, and the default permission policy. The shipped `web`, `headless`, `sdk`, and `acp` profiles are created for you on first use. To add more bundles, run `monolith plugin --profile <name> add <package>`; in-box bundles resolve from the monolith installation. The profile contract is documented in the [app-boot profile section](../../boot/app-boot/README.md).
...
The patch gates the two shell stacks by platform on its own rows: `bash-sandbox` and `tool-bash` carry `disabled: !!js process.platform === 'win32'`, and their twins `pwsh-sandbox` and `tool-pwsh` mount on win32 only with the inverted expression. The permission surface stays identical to POSIX: the sandbox policy executes the same file-effect policy through the Windows ACL restricted-token runner (`monolith-sandbox-local` → `@monolith/sandbox-windows-acl`), and `fs-sandbox` keeps fencing `ctx.fs` writes — mounting `monolith-fs-local` alongside it would double-register `ctx.fs` and fail the load.
...
- **Windows temp grants are private per-session subdirectories** — `workspace-write` confines writes to the workspace plus the session's own temp subdirectory (`<temp>\monolith-<hash>`, TMP/TEMP rewritten for confined children); `read-only` grants nothing. See `@monolith/sandbox-windows-acl`.
