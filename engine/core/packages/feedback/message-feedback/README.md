...
```yaml
- id: message-feedback
  name: '@monolith/message-feedback'
  config:
    maxNoteBytes: 8192
```

A note must contain at least one non-whitespace character and fit within the configured byte length; a blank note is rejected with `note-blank` and an oversized one with `note-too-large`. Accepted text is stored exactly as submitted — nothing is trimmed. The generated [configuration catalog](../../../docs/config-catalog.md#monolith-message-feedback) is the exhaustive source for every accepted field and its JSDoc.
...
- [monolith-client-ui-message-feedback](../../client/ui-message-feedback/README.md) — the browser consumer that drives the Host Remote contract.
...
- The browser controls and the client Remote mount live in `monolith-client-ui-message-feedback` and `monolith-api-remotes`; their open items belong to those packages' notes.
