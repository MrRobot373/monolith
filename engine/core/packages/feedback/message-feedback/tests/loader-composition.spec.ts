...
import * as StorageDomain from '@monolith/storage-domain'
import * as StorageJson from '@monolith/storage-json'
import { remoteMethods } from '@monolith/typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@monolith/session-persistence-jsonl', JsonlSessionPersistence],
    ['@monolith/storage', Storage],
    ['@monolith/storage-json', StorageJson],
    ['@monolith/storage-domain', StorageDomain],
    ['@monolith/message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    root = await mkdtemp(join(tmpdir(), 'monolith-message-feedback-loader-'))
    ...
    await writeFile(configPath, [
      ...
      "- name: '@monolith/storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@monolith/storage-domain'",
      '  config:',
      '    backend: json',
