...
import type { Agent, AgentStatus } from '@monolith/agent'
import CommandRuntime from '@monolith/commands'
import SessionStore, { SessionId } from '@monolith/session'
import * as CommandFeedback from '@monolith/command-feedback'
...
describe('/feedback real Loader composition through cordis.yml', () => {
  it('boots cordis.yml and records feedback without model-visible output', async () => {
    root = await mkdtemp(join(tmpdir(), 'monolith-command-feedback-loader-'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@monolith/commands', CommandRuntime],
      ['@monolith/command-feedback', CommandFeedback],
    ])
    ...
    const userId = getOrCreateAnonymousUserId({ env: { MONOLITH_HOME: root } })
