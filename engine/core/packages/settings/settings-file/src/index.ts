...
import { SettingsProvider, type SettingsNamespace } from '@monolith/settings'
...
export function resolveSpec(config: Config): ResolvedSpec {
  const filename = resolve(config.path ?? join(resolveMonolithHome(config.monolithHome), 'settings.yaml'))
