...
The tools need `ctx.terminals` — a backend must be mounted — and the system-prompt service for the guidance section. Background sends additionally require the jobs service and its model-facing controller (`@monolith/tool-jobs`).
...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-tool-terminal) and [tool catalog](../../../docs/tool-catalog.md#monolith-tool-terminal) are the exhaustive sources for config fields and schemas.
...
- [Tool catalog](../../../docs/tool-catalog.md#monolith-tool-terminal) — the six generated schemas and result shapes.
...
The six generated schemas are listed in the [`monolith-tool-terminal` catalog section](../../../docs/tool-catalog.md#monolith-tool-terminal). Their fixed schema tokens are present whenever this plugin is active; agent-scoped tool filtering may hide them.
...
- **Background mode requires the jobs surface** — `run_in_background` needs both `@monolith/jobs` and its model-facing controller (`@monolith/tool-jobs`); without them the argument is rejected.
