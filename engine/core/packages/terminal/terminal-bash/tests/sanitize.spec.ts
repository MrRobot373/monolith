...
import { normalizeTerminalText, TerminalSanitizer } from '@monolith/terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07monolith> ')).toEqual({ text: 'monolith> ', prompt: true, promptTail: 'monolith> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('monolith> ')).toEqual({ text: 'monolith> ', prompt: false, promptTail: 'monolith> ' })
  })
