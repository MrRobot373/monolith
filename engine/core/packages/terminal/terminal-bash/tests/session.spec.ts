...
import { LocalPtySession } from '@monolith/terminal-bash/src/session.ts'
import type { ResolvedConfig } from '@monolith/terminal-bash/src/config.ts'
import type { TerminalSendOperation, TerminalSessionStatus, TerminalSignal } from '@monolith/terminal'
import type {
  SubprocessOutcome,
  SubprocessTerminalHandle,
  SubprocessTerminalSignal,
} from '@monolith/subprocess'
import { TerminalError } from '@monolith/terminal'
import type {
  ProcessIdentity,
  ProcessInspector,
} from '@monolith/subprocess-local/src/process-inspector.ts'
...
async function initialize(session: LocalPtySession, terminal: FakeTerminal): Promise<void> {
  ...
  terminal.emitData('\x1b]133;D;0\x07monolith> ')
...
describe('LocalPtySession readiness and output', () => {
  it('answers split cursor-position queries before publishing prompt readiness', async () => {
    ...
    terminal.emitData('\x1b]133;D;0\x07monolith> \x1b[')
  ...
  it('cancels with foreground-group SIGINT, observes AbortSignal, and contains write failures', async () => {
    ...
    terminal.emitData('\x1b]133;D;130\x07monolith> ')
    await vi.advanceTimersByTimeAsync(10)
  ...
  it('does not attribute a delayed prior prompt to the current send', async () => {
    ...
    terminal.emitData('\x1b]133;D;0\x07monolith> printf \'PID=%s\\n\' "$!"\r\n')
    await vi.advanceTimersByTimeAsync(20)
    expect(settled).toBe(false)

    terminal.emitData('PID=123\r\n\x1b]133;D;0\x07monolith> ')
    await vi.advanceTimersByTimeAsync(10)
