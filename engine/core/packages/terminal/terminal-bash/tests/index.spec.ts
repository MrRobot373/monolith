...
import AgentRegistry, { Inbox, type Agent } from '@monolith/agent'
import SandboxProvider from '@monolith/sandbox'
import type { ConfinedArgv, SandboxPolicy } from '@monolith/sandbox'
import SandboxPolicyService, { setSandboxMode } from '@monolith/sandbox-policy'
import SessionProjectionRegistry from '@monolith/session-projection'
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@monolith/terminal'
import type { TerminalSendRequest, TerminalWaitReason } from '@monolith/terminal'
import { BashTerminalBackend, PWSH_PROMPT_SETUP } from '@monolith/terminal-bash'
import { ENCODING_PREAMBLE } from '@monolith/pwsh-local'
import * as ptyLocal from '@monolith/terminal-bash'
import type { ResolvedConfig } from '@monolith/terminal-bash/src/config.ts'
import type { LocalPtySession } from '@monolith/terminal-bash/src/session.ts'
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      graceMs: 10,
      env: {
        TERM: 'dumb', PAGER: 'cat', GIT_PAGER: 'cat', PS1: 'monolith> ', BASH_SILENCE_DEPRECATION_WARNING: '1',
        PROMPT_COMMAND: 'printf "\\033]133;D;%s\\007" "$?"; PS1=\'monolith> \'',
        MONOLITH_SHELL: '1', MONOLITH_SESSION_ID: 'agent', MONOLITH_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('composes the default local session around a spawned terminal', async () => {
    ...
    queueMicrotask(() => { output.write(Buffer.from('\x1b]133;D;0\x07monolith> ')) })
    const backend = new BashTerminalBackend(
      ctx,
      config(),
      async () => terminal,
    ...
    expect(session.motd).toBe('monolith> ')
  ...
  it('bootstraps a pwsh dialect through the prompt function and scrubs bash-only env', async () => {
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'setup-echo monolith> ', waitReason: 'stdin_read' as const,
    ...
    expect(session.motd).toBe('setup-echo monolith> ')
    expect(spawned?.env).toMatchObject({
      TERM: 'dumb', NO_COLOR: '1', MONOLITH_SHELL: '1', MONOLITH_SESSION_ID: 'agent', MONOLITH_PTY_SESSION_ID: 'pty-1',
    })
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: second ? 'monolith> ' : "function prompt { 'monolith> ' }\n",
  ...
  it('forwards the spawn signal into the pwsh bootstrap sends', async () => {
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'monolith> ', waitReason: 'stdin_read' as const,
            sessionStatus: { kind: 'running' as const }, truncated: false,
          }),
          readOutput: () => ({ delta: '', truncated: false }),
          cancel: () => false,
    ...
    expect(spawned.motd).toBe('monolith> ')
