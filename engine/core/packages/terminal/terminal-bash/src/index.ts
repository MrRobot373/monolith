 ...
 * @module @monolith/terminal-bash
 */

import { Context } from '@monolith/cordis'
import type { Agent } from '@monolith/agent'
import type { Session, SessionEvent } from '@monolith/session'
import { TerminalBackendCleanupError } from '@monolith/terminal'
import type { TerminalBackend, TerminalBackendSpawnSpec, TerminalSendOperation } from '@monolith/terminal'
import type { SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from '@monolith/subprocess'
import type { SandboxExecutionPolicy } from '@monolith/sandbox'
...
function childEnvironment(spec: TerminalBackendSpawnSpec, dialect: ShellDialect): Record<string, string> {
  ...
  const common = {
    ...
    GIT_PAGER: 'cat',
    MONOLITH_SHELL: '1',
    MONOLITH_SESSION_ID: spec.owner.id,
    MONOLITH_PTY_SESSION_ID: spec.sessionId,
  }
  if (dialect === 'pwsh') {
