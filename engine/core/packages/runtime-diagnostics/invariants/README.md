...
`monolith-invariants` runs package-owned runtime checks — invariants — inside a MONOLITH composition: any package can ship a `./invariant` companion that verifies its own durable relationships (authoritative event streams and mutable snapshots) while the composition runs. Checks run automatically, and a failed check reports an `InvariantError` attributed to the package that owns the violated relationship. Choose it for compositions that want self-checking diagnostics with a global switch and package-name filters; the standard agent composition already mounts it with the four core companions, and loading the service alone installs no checks.
...
Use the registry for compositions that want live diagnostics. [`monolith-sdk-minimal`](../../bundle/sdk-minimal/README.md) mounts it with the four core stateful companions — `monolith-session`, `monolith-agent`, `monolith-scope`, and `monolith-agent-loop`; `monolith-base` deliberately omits runtime diagnostics. Custom compositions mount the registry and add companions for any other loaded package whose contracts they want checked. Loading the registry alone installs no checks: it ships no product checks of its own, so a composition that never mounts a companion observes no diagnostic behavior.
...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-invariants) is the exhaustive source for every accepted field and its JSDoc.
...
| Companion | Checks |
|---|---|
| `monolith-session`, `monolith-agent`, `monolith-scope`, `monolith-agent-loop` | Session log enclosure and call/result trace, agent-status transitions, scope-filtered dispatch subjects, loop-built request reconstruction |
| `monolith-llm`, `monolith-llm-retry`, `monolith-tools`, `monolith-system-prompt` | LLM stream grammar, retry-failure shape, tool-pipeline stage pairing and frozen results, prompt-assembly section names |
| `monolith-compaction`, `monolith-hook-protocol`, `monolith-sandbox-policy` | Compaction stream pairing, hook invocation/result pairing, sandbox mode values |
| `monolith-fs`, `monolith-subagent`, `monolith-workflow`, `monolith-tool-workflow` | Filesystem event identity, subagent provider and start/end pairing, workflow lifecycle identity, workflow record shape |
| `monolith-goal`, `monolith-goal-round-driver` | Durable goal-stream folds and reconstructed continuation prompts |
| `monolith-permission-presets`, `monolith-user-approval`, `monolith-commands` | Preset references to live presets, approval asked/decided pairing, command run/done pairing |
| `monolith-jobs`, `monolith-tool-todo`, `monolith-time-context` | Job snapshot field relationships, whole-list todo shape, durable clock readings |
| `monolith-credentials`, `monolith-settings`, `monolith-storage-domain`, `monolith-workspace` | Commit events against the live service or memory state, entity-cache mirroring |
| `monolith-agent-presets`, `monolith-session-title`, `monolith-plan-mode`, `monolith-schedule` | Preset mount placement, title source citation, plan-mode payload, schedule stream |
| `monolith-client-hmr`, `monolith-client-modules`, `monolith-client-runtime` | Browser/node-half stat-watcher lifecycle, boot entry graph, slot mutation versioning |
...
import InvariantRegistry from '@monolith/invariants'
import * as SessionInvariant from '@monolith/session/invariant'
...
- [Generated configuration catalog](../../../docs/config-catalog.md#monolith-invariants) — every accepted config field and its source declaration.
...
- **Request reconstruction covers loop-built requests only** — the `monolith-agent-loop` companion reconstructs requests explicitly built by the loop; direct one-shot LLM calls remain outside that contract even when callers freeze them or attach a session id.
