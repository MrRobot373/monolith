 ...
 * @module @monolith/session-log-deepseek
 */
...
import { isSurfaceEvent, SessionLogOffset, SessionSeq } from '@monolith/session'
import type {
  Session,
  SessionEvent,
  SessionId,
...
export interface Config {
  /** Contribute `monolith_session_log` to official DeepSeek requests. Defaults to `false`. */
  enabled?: boolean
}
...
/**
 * Register the incremental `monolith_session_log` request contribution when enabled.
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.deepseekLlmApiExtensions.register('monolith_session_log', {
    prepare: (request) => {
