...
import SessionStore, { Session, SessionId, SessionLogOffset, SessionSeq, type CreateSessionOptions, type SessionEvent } from '@monolith/session'
...
describe('incremental DeepSeek session-log upload', () => {
  ...
  it('does not contribute the session log under its default configuration', async () => {
    ...
    expect(prepared.fields).not.toHaveProperty('monolith_session_log')
  })
  ...
  it('uploads the full first prefix, records acceptance, then sends only the appended suffix', async () => {
    ...
    const firstPayload = first.fields.monolith_session_log
    ...
    expect(second.fields.monolith_session_log).toMatchObject({ afterSeq: 1, throughSeq: 3 })
    expect(second.fields.monolith_session_log?.events).toHaveLength(2)
    expect(second.fields.monolith_session_log?.events[0]).toMatchObject({
  ...
  it('reconstructs a persisted cursor and ignores an inherited parent watermark in a fork', async () => {
    ...
    expect(resumedPayload.fields.monolith_session_log?.afterSeq).toBe(0)
    ...
    expect(forkPayload.fields.monolith_session_log).toMatchObject({ afterSeq: -1, throughSeq: fork.session.seq - 1 })
  })
  ...
  it('omits the field for direct or stale requests and uploads the prior acceptance marker next', async () => {
    ...
    expect(current.fields.monolith_session_log).toMatchObject({
      afterSeq: 0,
      throughSeq: 1,
  ...
  it('contributes complete events without reading request messages', async () => {
    ...
    expect(prepared.fields.monolith_session_log?.events).toEqual(session.snapshotEvents())
  })
  ...
  it('translates logical brands and isSeeded into the raw v0 upload DTO', async () => {
    ...
    const wire = JSON.parse(JSON.stringify(prepared.fields.monolith_session_log)) as Record<string, unknown>
  ...
  it('withdraws its request field when the contributing plugin reloads', async () => {
    ...
    expect((await ctx.deepseekLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })).fields)
      .toHaveProperty('monolith_session_log')
    await disposeUpload()
    expect((await ctx.deepseekLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })).fields)
      .not.toHaveProperty('monolith_session_log')
  })
})
