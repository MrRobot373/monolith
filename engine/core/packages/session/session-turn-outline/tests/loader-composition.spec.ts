...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'monolith-session-turn-outline-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@monolith/session-turn-outline', SessionTurnOutlinePlugin],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped session-turn-outline YAML shape and serves the outline', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@monolith/session-turn-outline'",
    ])
