...
Mount `monolith-session-projection` wherever client carriers need current values of log-derived session state. Domain plugins register units; carriers read snapshots and subscribe to the change feed; neither knows the other.
...
- **Registry cells live in memory only** — a restart rebuilds by folding the log on first touch; compositions that mount `monolith-session-projection-cache` seed that fold from persisted rows instead.
