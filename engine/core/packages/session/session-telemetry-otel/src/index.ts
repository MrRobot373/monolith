/**
 * OpenTelemetry Service Provider for the MONOLITH telemetry capability.
 *
...
} from '@monolith/session-telemetry'
import { APP_IDENTITY } from '@monolith/llm'
...
// version (same pattern as monolith-llm's attribution identity).
 ...
 * and one MONOLITH-owned shutdown bound. Uploading modes validate their endpoint
...
export class OpenTelemetrySessionBackend extends SessionTelemetryBackend {
  ...
  constructor(ctx: Context, config: Config) {
    ...
    const ledger = this.provider.getLogger('@monolith/session-telemetry-otel', version)
    const ops = this.provider.getLogger('@monolith/session-telemetry-otel/ops', version)
