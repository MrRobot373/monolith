...
import { recordFeedback } from '@monolith/command-feedback'
import SessionStore, { SessionId, SessionSeq } from '@monolith/session'
...
// pin MONOLITH_HOME to a temp dir so the suite never touches the ambient ~/.monolith.
let tempHome: string
let previousMonolithHome: string | undefined
beforeAll(() => {
  tempHome = mkdtempSync(join(tmpdir(), 'monolith-otel-home-'))
  previousMonolithHome = process.env.MONOLITH_HOME
  process.env.MONOLITH_HOME = tempHome
})
afterAll(() => {
  if (previousMonolithHome === undefined) delete process.env.MONOLITH_HOME
  else process.env.MONOLITH_HOME = previousMonolithHome
...
describe('OpenTelemetrySessionBackend wire', () => {
  it('ships session records and the ops shutdown marker through the real SDK pipeline', async () => {
    ...
    expect(resource).toContainEqual({ key: 'service.name', value: { stringValue: 'monolith' } })
    expect(resource).toContainEqual({ key: 'user.id', value: { stringValue: getOrCreateAnonymousUserId() } })

    const records = allRecords(captures)
    const ledger = records.filter(r => r.scope === '@monolith/session-telemetry-otel')
    const ops = records.filter(r => r.scope === '@monolith/session-telemetry-otel/ops')
  ...
  it('drains records enqueued after a timer export began: dispose during an in-flight batch', async () => {
    ...
    const ops = allRecords(captures).filter(r => r.scope === '@monolith/session-telemetry-otel/ops')
    expect(ops).toHaveLength(1)
    expect(ops[0]!.record.attributes).toContainEqual({ key: 'telemetry.op', value: { stringValue: 'shutdown' } })
  })
...
describe('monolith-session-telemetry-otel real-load-path guard', () => {
