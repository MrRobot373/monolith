...
import { resolveConfigPath } from '@monolith/app-boot'
import { recordFeedback } from '@monolith/command-feedback'
import { runFixtureTurn } from '@monolith/loader-smoke'
...
process.env.MONOLITH_TELEMETRY_E2E_URL = `http://127.0.0.1:${address.port}/v1/logs`
process.env.MONOLITH_TELEMETRY_OTLP_URL = process.env.MONOLITH_TELEMETRY_E2E_URL
process.env.MONOLITH_TELEMETRY_MODE = process.env.MONOLITH_TELEMETRY_E2E_MODE ?? 'FULL'
...
try {
  ...
  const mode = process.env.MONOLITH_TELEMETRY_E2E_MODE ?? 'FULL'
  if (mode !== 'FULL') {
