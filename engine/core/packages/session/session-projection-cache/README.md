...
# @monolith/session-projection-cache
...
The cache opens its domain through the storage stack, so base mounts `storage`, `storage-json` (root `monolithHomePath('storages')`), and `storage-domain` (`backend: json`) before it:

```yaml
- id: session-projection-cache
  name: '@monolith/session-projection-cache'
  config:
...
The plugin injects `storageDomain`, `sessionProjections`, and `sessions`. The generated [configuration catalog](../../../docs/config-catalog.md#monolith-session-projection-cache) is the exhaustive source for every accepted field and its JSDoc.
