 ...
 * @module @monolith/session-title-llm
 */

import type { Context } from '@monolith/cordis'
import z from '@monolith/schemastery'
import { createUserMessage, BlockAssembler } from '@monolith/llm'
import type { FinishReason, GenerateOptions, Message } from '@monolith/llm'
import { deadline, MAX_TIMER_DELAY_MS } from '@monolith/timeout'
import { deepFreeze } from '@monolith/util-values'
import type { SessionSeq } from '@monolith/session'
import {
  normalizeSessionTitle,
  SessionTitleProviderId,
} from '@monolith/session-title'
import type {
...
export async function generateSessionTitleWithLlm(
  ...
  const messages: Message[] = [createUserMessage({
    ...
    source: { kind: 'plugin', plugin: 'monolith-session-title-llm' },
  })]
