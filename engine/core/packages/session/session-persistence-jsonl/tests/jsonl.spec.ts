import { MessageId, createMessage } from '@monolith/llm'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import { Context } from '@monolith/cordis'
import { appendFile, mkdtemp, mkdir, rm, readFile, writeFile, readdir, stat, symlink } from 'node:fs/promises'
import { tmpdir } from 'node:os'
import { dirname, join, relative, resolve } from 'node:path'
import { SessionLogOffset, SessionSeq, SessionId } from '@monolith/session'
...
async function freshRoot(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'monolith-jsonl-'))
  dirs.push(dir)
  return dir
}
...
runLiveWritePathContract('jsonl', LIVE_WRITE_BATCH_MAX_DELAY_MS, async () => {
  const dir = await mkdtemp(join(tmpdir(), 'monolith-jsonl-live-'))
  dirs.push(dir)
...
describe('JsonlSessionPersistence: format helpers', () => {
  ...
  it('projectKey normalizes project paths into bounded readable names', () => {
    expect(projectKey('/Users/qyj/work/monolith')).toBe('--Users-qyj-work-monolith--')
