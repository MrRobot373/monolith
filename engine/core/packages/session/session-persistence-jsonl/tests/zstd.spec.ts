...
import { SessionSeq, SessionId } from '@monolith/session'
...
async function freshRoot(prefix = 'monolith-jsonl-zstd-'): Promise<string> {
...
runPersistenceContract('jsonl-zstd', async () => {
  const root = await mkdtemp(join(tmpdir(), 'monolith-jsonl-zstd-contract-'))
...
describe('JsonlSessionPersistence: encoding selection', () => {
  it('rejects roots owned by the opposite encoding in both directions', async () => {
    const rawRoot = await freshRoot('monolith-jsonl-raw-mismatch-')
    ...
    const zstdRoot = await freshRoot('monolith-jsonl-zstd-mismatch-')
