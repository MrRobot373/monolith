...
import LlmRuntime, { createUserMessage, LlmAdapter  } from '@monolith/llm'
...
import * as providerPlugin from '@monolith/session-title-first-prompt-llm'
...
async function loadComposition(): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'monolith-title-loader-'))
  const configPath = join(root, 'cordis.yml')
  await writeFile(configPath, [
    "- name: '@monolith/llm'",
    "- name: '@monolith/session'",
    "- name: '@monolith/session-projection'",
    "- name: '@monolith/session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: '@monolith/session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@monolith/session-title', SessionTitleService],
    ['@monolith/session-title-first-prompt-llm', providerPlugin],
  ])
