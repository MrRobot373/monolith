...
import { Inbox } from '@monolith/agent'
import type { Agent } from '@monolith/agent'
import { runLoaderSmoke } from '@monolith/loader-smoke'
import {
  FileNotFoundError,
  Sandbox,
  SandboxNotFoundError,
} from '@monolith/e2b'
import TerminalSessionService, { TerminalSessionId } from '@monolith/terminal'
import { BashTerminalBackend } from '@monolith/terminal-bash'
...
describe.skipIf(!process.env.E2B_API_KEY)('E2B live Loader composition', () => {
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    const sandbox = await Sandbox.create({
      apiKey,
      envs: { NPM_TOKEN: 'sentinel-secret', MONOLITH_STALE: 'sentinel-stale', KEEP: 'visible' },
      timeoutMs: 60_000,
      secure: true,
      lifecycle: { onTimeout: 'kill' },
    })
    try {
      const profileLeakPath = '/home/user/monolith-e2b-bootstrap-profile-leak'
      ...
      ctx.provide('e2b', {
        cwd: '/home/user',
        runtimeRoot: '/home/user/.monolith-e2b',
      ...
      const environmentProbe = ctx.subprocess.spawn({
        argv: ['/bin/bash', '-c', [
          'monolith_leak=0',
          'for monolith_pid in "$PPID" $(ps -o pid= --ppid "$PPID"); do',
          '  [[ "$monolith_pid" == "$$" ]] && continue',
          '  if tr "\\0" "\\n" < "/proc/$monolith_pid/environ" 2>/dev/null | grep -Fqx "NPM_TOKEN=sentinel-secret"; then monolith_leak=1; fi',
          'done',
          'printf "DIRECT=<%s> LEAK=<%s>\\n" "${NPM_TOKEN-}" "$monolith_leak"',
        ].join('\n')],
        cwd: '/home/user',
      ...
      const result = await session.startSend({
        text: "printf 'NPM=<%s> MONOLITH=<%s> KEEP=<%s>\\n' \"$NPM_TOKEN\" \"$MONOLITH_STALE\" \"$KEEP\"",
        submit: true,
      }).done
      expect(result.viewport).toContain('NPM=<> MONOLITH=<> KEEP=<visible>')
  ...
  it('runs FS, Bash, PTY, and LSP in one sandbox and deletes it', async () => {
    const { stdout, stderr } = await runLoaderSmoke({
      ...
      tempDirPrefix: 'monolith-e2b-composition-',
      binScript,
    ...
    expect(terminalMotd).not.toContain('.monolith-e2b/terminals/')
