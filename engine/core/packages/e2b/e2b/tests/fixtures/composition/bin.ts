...
import { boot } from '@monolith/app-boot'
import { Inbox } from '@monolith/agent'
import type { Agent } from '@monolith/agent'
import { Session, SessionId } from '@monolith/session'
import type {} from '@monolith/fs-e2b'
import type {} from '@monolith/bash-local'
import type {} from '@monolith/lsp-stdio'
import type {} from '@monolith/terminal-bash'
...
try {
  ...
  const environmentHandle = ctx.subprocess.spawn({
    ...
    env: {
      ...
      MONOLITH_EXPLICIT: 'managed-value',
  ...
  const outputDrainHandle = ctx.subprocess.spawn({
    argv: ['bash', '-c', "bash -c 'exec -a monolith-output-drain-descendant sleep 30' & printf 'leader-done\\n'"],
    cwd: process.cwd(),
  ...
  const outputDrainClean = !outputDrainProcesses.some(processInfo =>
    JSON.stringify([processInfo.cmd, processInfo.args]).includes('monolith-output-drain-descendant'),
  )
  ...
  const sleeping = ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "printf 'MONOLITH_SLEEP_%s\\n' READY; sleep 30",
    submit: true,
  })
  let sleepReadyOutput = ''
  const sleepReadyDeadline = Date.now() + 5_000
  while (!sleepReadyOutput.includes('MONOLITH_SLEEP_READY\n')) {
    sleepReadyOutput += sleeping.readOutput().delta
    if (sleepReadyOutput.includes('MONOLITH_SLEEP_READY\n')) break
  ...
  const stubborn = await ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "bash -c 'trap \"\" TERM; exec sleep 30' & printf 'MONOLITH_STUBBORN_PID=%s\\n' \"$!\"",
    submit: true,
  }).done
  const stubbornMatch = /MONOLITH_STUBBORN_PID=([1-9][0-9]*)/.exec(stubborn.viewport)
