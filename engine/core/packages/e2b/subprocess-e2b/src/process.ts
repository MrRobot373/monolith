...
import type { CommandHandle, CommandResult, Sandbox } from '@monolith/e2b'
import type {
  SubprocessCollect,
  SubprocessHandle,
  SubprocessOutcome,
  ...
  SubprocessSpawnSpec,
} from '@monolith/subprocess'
import type E2BRuntime from '@monolith/e2b'
...
function commandText(spec: SubprocessSpawnSpec, paths: RemotePaths): string {
  const encoder = `"$monolith_e2b_env_bin" -i "$monolith_e2b_node" -e ${quoteE2BShellArg(OUTPUT_ENCODER_SOURCE)}`
  const stdoutRedirect = hasSpill(spec.stdio.stdout)
    ? `> >("$monolith_e2b_tee" --output-error=warn-nopipe >("$monolith_e2b_head" -c ${spec.stdio.stdout.spill.maxBytes} > ${quoteE2BShellArg(paths.stdout)}) | ${encoder} 2>/dev/null)`
    : `> >(${encoder} 2>/dev/null)`
  const stderrRedirect = hasSpill(spec.stdio.stderr)
    ? `2> >("$monolith_e2b_tee" --output-error=warn-nopipe >("$monolith_e2b_head" -c ${spec.stdio.stderr.spill.maxBytes} > ${quoteE2BShellArg(paths.stderr)}) | ${encoder} >&2 2>/dev/null)`
    : `2> >(${encoder} >&2 2>/dev/null)`
  const inner = [
    'set +e',
    'monolith_e2b_env_bin=$1',
    'monolith_e2b_node=$2',
    'monolith_e2b_ps=$3',
    'monolith_e2b_tr=$4',
    'monolith_e2b_tee=$5',
    ...
    'monolith_e2b_rm=$7',
    'shift 7',
    'monolith_e2b_pgid="$("$monolith_e2b_ps" -o pgid= -p "$$" | "$monolith_e2b_tr" -d " ")"',
    `printf '%s\\n' "$monolith_e2b_pgid" > ${quoteE2BShellArg(paths.pid)}`,
    `mapfile -d '' -t monolith_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    `"$monolith_e2b_rm" -f -- ${quoteE2BShellArg(paths.environment)}`,
    `"$monolith_e2b_env_bin" -i -- "\${monolith_e2b_env[@]}" "$@" ${stdoutRedirect} ${stderrRedirect}`.trimEnd(),
    'monolith_e2b_status=$?',
    `printf '%s\\n' "$monolith_e2b_status" > ${quoteE2BShellArg(paths.status)}`,
    'wait',
    'exit "$monolith_e2b_status"',
  ].join('\n')
  const argv = spec.argv.map(quoteE2BShellArg).join(' ')
  const bootstrap = [
    `mapfile -d '' -t monolith_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    'monolith_e2b_env_bin="$(command -v env)"',
    'monolith_e2b_setsid="$(command -v setsid)"',
    'monolith_e2b_bash="$(command -v bash)"',
    'monolith_e2b_node="$(command -v node)"',
    'monolith_e2b_ps="$(command -v ps)"',
    'monolith_e2b_tr="$(command -v tr)"',
    'monolith_e2b_tee="$(command -v tee)"',
    'monolith_e2b_head="$(command -v head)"',
    'monolith_e2b_rm="$(command -v rm)"',
    'for monolith_e2b_tool in "$monolith_e2b_env_bin" "$monolith_e2b_setsid" "$monolith_e2b_bash" "$monolith_e2b_node" "$monolith_e2b_ps" "$monolith_e2b_tr" "$monolith_e2b_tee" "$monolith_e2b_head" "$monolith_e2b_rm"; do',
    '  [[ "$monolith_e2b_tool" == /* && -x "$monolith_e2b_tool" ]] || exit 125',
    'done',
    `exec "$monolith_e2b_env_bin" -i -- "\${monolith_e2b_env[@]}" "$monolith_e2b_setsid" --wait -- "$monolith_e2b_bash" -c ${quoteE2BShellArg(inner)} monolith-e2b "$monolith_e2b_env_bin" "$monolith_e2b_node" "$monolith_e2b_ps" "$monolith_e2b_tr" "$monolith_e2b_tee" "$monolith_e2b_head" "$monolith_e2b_rm" ${argv}`,
  ].join('\n')
  return bootstrap
}
