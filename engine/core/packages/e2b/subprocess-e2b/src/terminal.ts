...
const TERMINAL_RUNNER_SOURCE = [
  ...
  'set -euo pipefail',
  'monolith_state=$1',
  'mapfile -d \'\' -t monolith_env < "$monolith_state/environment"',
  'mapfile -d \'\' -t monolith_argv < "$monolith_state/argv"',
  'monolith_output_marker=$(<"$monolith_state/output-marker")',
  'rm -f -- "$monolith_state/environment" "$monolith_state/argv" "$monolith_state/output-marker" "$monolith_state/runner.bash"',
  'if (( ${#monolith_argv[@]} == 0 )); then',
  "  printf 'terminal runner received empty argv\\n' >&2",
  '  exit 125',
  'fi',
  'printf \'%s\' "$monolith_output_marker"',
  'exec env -i -- "${monolith_env[@]}" "${monolith_argv[@]}"',
  '',
].join('\n')
...
export async function spawnE2BTerminal(
  ...
  const outputMarker = Buffer.from(`monolith-e2b-bootstrap:${randomUUID()}`)
