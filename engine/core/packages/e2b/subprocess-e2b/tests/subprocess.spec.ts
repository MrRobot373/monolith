...
import type { SubprocessSpawnSpec } from '@monolith/subprocess'
import E2BSubprocessRuntime from '@monolith/subprocess-e2b'
...
class FakeSandbox {
  ...
  alive = true
  zombieOnly = false
  ambient = 'PATH=/ambient/bin\0KEEP=safe\0UNICODE=你好\0NPM_TOKEN=secret\0MONOLITH_STALE=old\0BROKEN\0=bad\0'
...
function runtime(fake: FakeSandbox, getSandbox: () => Promise<Sandbox> = async () => fake.sandbox): E2BRuntime {
  return {
    cwd: '/workspace',
    runtimeRoot: '/workspace/.monolith-e2b',
    getSandbox,
...
describe('E2BSubprocessHandle', () => {
  it('starts asynchronously, keeps secrets out of the command, and supports deferred piped stdin/output', async () => {
    ...
    }), '/workspace/.monolith-e2b/processes/one')
    ...
    expect(controlEnvs?.HOME).toMatch(/^\/\.monolith-e2b-control-/)
    ...
    const command = fake.commandsSeen.find(value => value.includes('exec "$monolith_e2b_env_bin" -i'))!
    expect(command).toContain('"$monolith_e2b_setsid" --wait -- "$monolith_e2b_bash" -c')
    expect(command).not.toContain('DEEPSEEK_API_KEY')
    expect(command).not.toContain('MONOLITH_MODE')
    ...
    expect(command).not.toContain('${!monolith_e2b_name}')
    ...
    expect(environmentProbe).toContain('test -n "$monolith_e2b_home" -a -d "$monolith_e2b_home"')
    expect(environmentProbe).not.toContain('"$PWD"')
    expect(command).toContain('mapfile -d')
    expect(command).toContain('monolith_e2b_node="$(command -v node)"')
    expect(command).toContain('"$monolith_e2b_env_bin" -i "$monolith_e2b_node" -e')
    expect(command).toContain('"$monolith_e2b_env_bin" -i -- "${monolith_e2b_env[@]}" "$@"')
    expect(command).toContain('exec "$monolith_e2b_env_bin" -i -- "${monolith_e2b_env[@]}"')
    ...
    expect(fake.writtenFiles[0]).toEqual([
      '/workspace/.monolith-e2b/processes/one/pid',
      '/workspace/.monolith-e2b/processes/one/exit-code',
      '/workspace/.monolith-e2b/processes/one/environment',
      '/workspace/.monolith-e2b/processes/one/stderr.log',
    ])
    expect(fake.writtenFileData.get('/workspace/.monolith-e2b/processes/one/environment')).toBe(
      'PATH=/bin\0UNICODE=你好\0HOME=/home/user\0FOO-BAR=hyphen-value\0--split-string=literal-value\0DEEPSEEK_API_KEY=explicit-secret\0MONOLITH_MODE=test\0',
    )
    ...
    expect(fake.removed).toContain('/workspace/.monolith-e2b/processes/one/stderr.log')
  ...
  it('removes a spill once the complete stream exceeds its cap', async () => {
    ...
    const command = fake.commandsSeen.find(value => value.includes('monolith_e2b_tee='))!
    expect(command).toContain('"$monolith_e2b_head" -c 3')
    expect(command).toContain('/runtime/oversize/stdout.log')
    expect(command).toContain('"$monolith_e2b_tee" --output-error=warn-nopipe')
