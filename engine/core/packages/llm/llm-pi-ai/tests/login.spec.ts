...
import type { AuthorizationInteraction, AuthorizationNotice, AuthorizationPrompt } from '@monolith/authorization'
import LocalCredentialProvider from '@monolith/credentials-local'
import type { CredentialKey } from '@monolith/credentials'
...
async function harness(): Promise<Context> {
  const dir = await mkdtemp(join(tmpdir(), 'monolith-pi-login-'))
  dirs.push(dir)
