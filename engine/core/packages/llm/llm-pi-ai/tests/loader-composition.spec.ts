...
import LlmRuntime, { createMessage, createUserMessage, userAgent } from '@monolith/llm'
...
async function loadComposition(): Promise<{ ctx: Context; settingsPath: string }> {
  root = await mkdtemp(join(tmpdir(), 'monolith-pi-composition-'))
