...
import type { AttachmentStore, ImageAttachmentRef, ImageRequestPolicy, RequestImageAttachment } from '@monolith/attachment'
import { createUserMessage, ToolCallId, CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, createMessage } from '@monolith/llm'
import type { ContentBlock, StreamChunk } from '@monolith/llm'
...
describe('toPiContext', () => {
  ...
  it('degrades the flat pre-envelope replay state a legacy session log carries', () => {
    ...
    expect(context.messages[0]).toMatchObject({ role: 'assistant', api: 'monolith-foreign' })
