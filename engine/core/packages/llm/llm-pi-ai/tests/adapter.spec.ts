...
import { AttachmentId, AttachmentStore, ImageVariantId } from '@monolith/attachment'
import type {
...
import LlmRuntime, { createUserMessage, CONTEXT_WINDOW_EXCEEDED_CODE, LlmError, ReasoningEffortId, userAgent } from '@monolith/llm'
import * as LlmPiAi from '@monolith/llm-pi-ai'
import { PiAiAdapter } from '@monolith/llm-pi-ai'
...
const HOST_IMAGE_PATH = '/host/.monolith/attachments/objects/aa/object'
const MODEL_IMAGE_PATH = '/model/.monolith/attachments/objects/aa/object'
...
describe('PiAiAdapter provider routing', () => {
  ...
  it('merges profile headers with MONOLITH attribution winning', async () => {
    const server = await mockServer([{ events: textEvents }])
    const ctx = await harness(server.url, {
  ...
  it('forwards common stream options and profile reasoning', async () => {
    ...
    expect(server.requests[0]).not.toHaveProperty('monolith_session_log')
    expect(server.requests[0]).not.toHaveProperty('monolith_plugin_packages')
  })
