...
import LlmRuntime, { createUserMessage, ReasoningEffortId } from '@monolith/llm'
...
/** A throwaway $MONOLITH_HOME with an empty settings document. */
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'monolith-pi-catalog-'))
  homes.push(dir)
