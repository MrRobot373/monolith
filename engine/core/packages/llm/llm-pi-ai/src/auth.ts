 ...
 *
 * @module monolith-llm-pi-ai/auth
 */
...
import type { CredentialKey, CredentialProvider, CredentialRecord } from '@monolith/credentials'
import { launchEnvironmentOf } from '@monolith/launch-environment'
import { LlmError } from '@monolith/llm'
...
function writableStore(ctx: Context): CredentialProvider {
  ...
  if (credentials === undefined) {
    throw new LlmError(
      ...
      + ' credential a sign-in produces; mount one (monolith-credentials-local) to sign in',
      'NO_CREDENTIAL_STORE',
    )
  }
  return credentials
