 ...
 *
 * @module monolith-llm-pi-ai/replay
 */

import { LlmError } from '@monolith/llm'
import type { Message, ModelMessageSource, ReplayEnvelope } from '@monolith/llm'
...
function foreignAssistant(message: Message): AssistantMessage {
  ...
  return {
    ...
    provider: source?.provider ?? 'monolith-foreign',
    model: source?.model ?? 'monolith-foreign',
    usage: emptyPiUsage(),
