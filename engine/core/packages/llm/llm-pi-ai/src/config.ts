 ...
 *
 * @module monolith-llm-pi-ai/config
 */
...
import { resolveRetryPolicy, RetryPolicySchema } from '@monolith/llm'
import type { ResolvedRetryPolicy, RetryPolicyConfig } from '@monolith/llm'
import {
...
export interface PiAiProviderProfile {
  ...
  /** Provider request headers, validated against Fetch when the profile resolves; MONOLITH attribution wins reserved names. */
...
function rejectRemovedFields(provider: string, source: PiAiProviderProfile): void {
  ...
  if ('maxRetries' in legacy || 'maxRetryDelayMs' in legacy) {
    throw new Error(
      ...
      + ' compose agent recovery with monolith-llm-retry',
    )
  }
}
