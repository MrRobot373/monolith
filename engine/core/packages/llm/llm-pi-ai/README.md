...
Choose this adapter when the same composition serves several providers, when a route needs pi-ai's catalog defaults with a few fields corrected, or when a hand-declared gateway must be reached through its own endpoint and protocol. Choose `monolith-llm-deepseek` for the direct DeepSeek route when the deployment needs no other provider. Both adapters can be mounted together because their route names do not collide; registering a route another adapter already owns fails plugin loading.
...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-llm-pi-ai) is the exhaustive source for every accepted field and its JSDoc.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#monolith-llm-pi-ai) — every accepted config field and its source declaration.
