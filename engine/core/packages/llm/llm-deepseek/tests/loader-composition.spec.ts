...
import LocalCredentialProvider from '@monolith/credentials-local'
import FileSettingsProvider from '@monolith/settings-file'
import { getOrCreateAnonymousUserId } from '@monolith/anonymous-user-id'
import DeepSeekLlmApiExtensionRegistry from '@monolith/deepseek-llm-api-extensions'
import * as SessionLogDeepSeek from '@monolith/session-log-deepseek'
import * as DeepSeekPluginPackageInventory from '@monolith/plugin-package-inventory-deepseek'
...
async function loadComposition(
  ...
  root = options.reuseRoot ?? await mkdtemp(join(tmpdir(), 'monolith-llm-composition-'))
  vi.stubEnv('MONOLITH_HOME', root)
  ...
  await writeFile(configPath, [
    ...
    "  name: '@monolith/deepseek-llm-api-extensions'",
    '- id: session-log-deepseek',
    "  name: '@monolith/session-log-deepseek'",
    ...options.enableSessionLog === true
      ? ['  config:', '    enabled: true']
      : [],
    '- id: plugin-package-inventory-deepseek',
    "  name: '@monolith/plugin-package-inventory-deepseek'",
    ...options.withDynamic
      ? [
        '- id: settings',
        "  name: '@monolith/settings-file'",
        '  config:',
        `    path: ${JSON.stringify(settingsPath)}`,
        '    debounceMs: 10',
        '- id: credentials',
        "  name: '@monolith/credentials-local'",
        '  config:',
  ...
  const modules = new Map<string, unknown>([
    ['@monolith/llm', LlmRuntime],
    ['@monolith/session', SessionStore],
    ['@monolith/agent', AgentRegistry],
    ['@monolith/deepseek-llm-api-extensions', DeepSeekLlmApiExtensionRegistry],
    ['@monolith/session-log-deepseek', SessionLogDeepSeek],
    ['@monolith/plugin-package-inventory-deepseek', DeepSeekPluginPackageInventory],
    ['@monolith/settings-file', FileSettingsProvider],
    ['@monolith/credentials-local', LocalCredentialProvider],
    ['@monolith/llm-deepseek', LlmDeepSeek],
  ])
...
describe('llm-deepseek real dynamic composition', () => {
  it('keeps session upload off and package inventory on by default in the real Loader composition', async () => {
    ...
    const request = server.requests[0] as { monolith_plugin_packages: { version: number; packages: unknown[] } }
    expect(request).not.toHaveProperty('monolith_session_log')
    expect(request.monolith_plugin_packages.packages).toEqual(expect.arrayContaining([
      { name: '@monolith/deepseek-llm-api-extensions', version: '0.1.0-rc.8' },
      { name: '@monolith/llm-deepseek', version: '0.1.0-rc.8' },
      { name: '@monolith/session-log-deepseek', version: '0.1.0-rc.8' },
    ]))
    expect(request.monolith_plugin_packages.version).toBe(1)
  ...
  it('sends the canonical session suffix when the Loader composition explicitly enables upload', async () => {
    ...
    expect(request.monolith_session_log).toMatchObject({
      version: 1,
  ...
  it('boots from cordis.yml and routes the next request after external settings and credential edits', async () => {
    ...
    expect(serverA.headers[0]?.['x-monolith-user-id']).toBe(getOrCreateAnonymousUserId())
