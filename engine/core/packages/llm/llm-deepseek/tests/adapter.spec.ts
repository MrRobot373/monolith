...
import { AttachmentId, ImageVariantId } from '@monolith/attachment'
import type { AttachmentStore, ImageAttachmentRef, RequestImageAttachment } from '@monolith/attachment'
import { createLaunchEnvironmentSnapshot } from '@monolith/launch-environment'
...
import { getOrCreateAnonymousUserId, type AnonymousUserId } from '@monolith/anonymous-user-id'
import { SessionId } from '@monolith/session'
import DeepSeekLlmApiExtensionRegistry from '@monolith/deepseek-llm-api-extensions'
import type { PreparedDeepSeekLlmApiExtensions } from '@monolith/deepseek-llm-api-extensions'
import * as LlmDeepSeek from '@monolith/llm-deepseek'
import { DeepSeekAdapter, resolveAdapterOptions } from '@monolith/llm-deepseek'
...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), 'monolith-llm-deepseek-'))
  vi.stubEnv('MONOLITH_HOME', testHome)
})
...
describe('DeepSeekAdapter against a mock server', () => {
  it('merges prepared extension fields and accepts them once after HTTP 2xx', async () => {
    ...
    const prepareExtensions = vi.fn(async () => ({
      fields: { monolith_test: { version: 1 } },
      accept: async () => { accept() },
    }))
    const adapter = new DeepSeekAdapter({
    ...
    expect(server.requests[0]).toMatchObject({ monolith_test: { version: 1 } })
  ...
  it('does not accept extensions on non-2xx and does accept before a later stream failure', async () => {
    ...
    const adapter = new DeepSeekAdapter({
      ...
      resolveApiKey: () => Promise.resolve('k'),
      resolveUserId: () => TEST_USER_ID,
      prepareExtensions: () => Promise.resolve({ fields: { monolith_test: 1 }, accept: async () => { accept() } }) as never,
    })
  ...
  it('streams a text generation end to end through the assembler', async () => {
    ...
    expect(server.headers[0]?.['x-monolith-user-id']).toBe(getOrCreateAnonymousUserId())
    expect(server.headers[0]).not.toHaveProperty('x-monolith-session-id')
    ...
    expect(server.headers[0]).not.toHaveProperty('x-monolith-compact')
  })
  ...
  it('uploads a durable image once and sends only its Files API id to the vision model', async () => {
    ...
    expect(server.fileRequests).toEqual([{
      method: 'POST',
      path: '/files',
      filename: `monolith-${'a'.repeat(16)}-${'b'.repeat(8)}.png`,
      bytes: 3,
    }])
  ...
  it('reuses the exact request version between agent and compaction calls', async () => {
    ...
    expect(server.headers[1]?.['x-monolith-compact']).toBe('1')
  })
  ...
  it('forwards the harness user and session ids for host-side trajectory routing', async () => {
    ...
    expect(server.headers[0]?.['x-monolith-session-id']).toBe('child-session')
    expect(server.headers[0]?.['x-monolith-user-id']).toBe(getOrCreateAnonymousUserId())
  })
  ...
  it('marks the auxiliary compaction call on the wire', async () => {
    ...
    expect(server.headers[0]?.['x-monolith-compact']).toBe('1')
  })
...
describe('plugin registration and config', () => {
  ...
  it('takes DEEPSEEK_BASE_URL from any environment layer, with explicit config still on top', () => {
    const trusted = createLaunchEnvironmentSnapshot([
      { source: 'user-env', path: '/home/.monolith/.env', values: { DEEPSEEK_BASE_URL: 'https://user.example' } },
    ])
