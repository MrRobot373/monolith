...
import LlmRuntime, { createUserMessage, ToolCallId, ReasoningEffortId, createMessage } from '@monolith/llm'
import type { Message, ToolSchema } from '@monolith/llm'
import AttachmentStore, { AttachmentId, ImageVariantId } from '@monolith/attachment'
import type {
...
import * as PluginPackageInventoryDeepSeek from '@monolith/plugin-package-inventory-deepseek'
import * as SessionLogDeepSeek from '@monolith/session-log-deepseek'
import * as LlmDeepSeek from '@monolith/llm-deepseek'
import type { Config } from '@monolith/llm-deepseek'
...
beforeEach(async () => {
  identityHome = await mkdtemp(join(tmpdir(), 'monolith-e2e-user-id-'))
  vi.stubEnv('MONOLITH_HOME', identityHome)
})
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('llm-deepseek e2e (real API)', () => {
  ...
  it('serves a real request with the key held only by a credentials-local document', async () => {
    const key = process.env.DEEPSEEK_API_KEY
    if (key === undefined) throw new Error('e2e ran without DEEPSEEK_API_KEY')
    const dir = await mkdtemp(join(tmpdir(), 'monolith-e2e-credentials-'))
    try {
