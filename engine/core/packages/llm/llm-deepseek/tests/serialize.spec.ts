...
import type { ImageAttachmentRef, ImageMediaType, RequestImageAttachment } from '@monolith/attachment'
import { createUserMessage, ToolCallId, ReasoningEffortId, createMessage } from '@monolith/llm'
import type { ContentBlock, GenerateOptions, Message } from '@monolith/llm'
import {
  serializeMessages,
...
describe('image serialization', () => {
  ...
  it('includes provider-resolved normalized access in a retained image handle', async () => {
    ...
    version.width = 1130
    version.height = 565
    images.resolveImageAccess = () => ({ readonlyPath: '/tmp/monolith/objects/aa/object' })
    const wire = await serializeRequestWithImages(request({
      model: 'deepseek-v4-flash-vision-exp',
      messages: [createUserMessage({
        content: [{ type: 'image', attachment: ref }],
    ...
    expect(JSON.stringify(wire.messages[0])).toContain('/tmp/monolith/objects/aa/object')
  ...
  it('offloads oldest images before reads and keeps the newest image', async () => {
    ...
    images.resolveImageAccess = ref => ref.mediaType === 'image/png'
      ? { readonlyPath: '/tmp/monolith/objects/png' }
      : undefined
    const wire = await serializeRequestWithImages(request({
      model: 'deepseek-v4-flash-vision-exp',
      messages: [createUserMessage({
    ...
    expect(wire.messages[0]).toMatchObject({
      ...
      content: [
        {
          type: 'text',
          text: expect.stringContaining(`image omitted to fit request image limits; ${png.attachmentId}. Normalized copy (read-only; may be resized or re-encoded): "/tmp/monolith/objects/png"`) as string,
        },
