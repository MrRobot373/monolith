 ...
 *
 * @module monolith-llm-deepseek/adapter
 */

import { attributionHeaders, contentHasImage, CONTEXT_WINDOW_EXCEEDED_CODE, isContextWindowExceededError, isQuotaExceededError, LlmAdapter, LlmError, offloadedImageText, offloadRequestImagesWithPolicy, ProviderRequestId, QUOTA_EXCEEDED_CODE, ReasoningEffortId } from '@monolith/llm'
import type {
  ContentBlock,
  GenerateOptions,
...
import { deadline, idleWatchdog, timeoutOf } from '@monolith/timeout'
import type { AnonymousUserId } from '@monolith/anonymous-user-id'
import type {
  DeepSeekLlmApiExtensionRequest,
  DeepSeekLlmApiJson,
  PreparedDeepSeekLlmApiExtensions,
} from '@monolith/deepseek-llm-api-extensions'
...
export class DeepSeekAdapter extends LlmAdapter {
  ...
  private async * request(
    ...
    const headers = {
      ...
      ...attributionHeaders(),
      'x-monolith-user-id': String(userId),
      ...options.sessionId !== undefined
        ? { 'x-monolith-session-id': String(options.sessionId) }
        : {},
      ...options.purpose === 'compaction'
        ? { 'x-monolith-compact': '1' }
        : {},
    }
