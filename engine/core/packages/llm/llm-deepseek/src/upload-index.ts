/** Durable DeepSeek attachment-to-file-id index. @module monolith-llm-deepseek/upload-index */
...
import { ImageVariantId } from '@monolith/attachment'
import type { AttachmentId, ImageVariantId as ImageVariantIdType } from '@monolith/attachment'
...
/** Atomic local index shared by every DeepSeek session in this MONOLITH home. */
export class DeepSeekUploadIndex {
  /** Absolute owner-private JSON index path. */
  readonly path: string

  /**
   * @param path - explicit test path; omission uses `MONOLITH_HOME/llm-deepseek/files-v3.json`.
   */
  constructor(path = join(resolveMonolithHome(), 'llm-deepseek', 'files-v3.json')) {
    this.path = path
  }
