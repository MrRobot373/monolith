...
Choose this adapter when the deployment targets DeepSeek's official API, optionally behind an OpenAI-compatible gateway named by `baseURL`. Choose `monolith-llm-pi-ai` when the same composition also routes other providers or hand-declared gateways through pi-ai's catalogs; the two adapters can be mounted together because their route names do not collide. Registering any other adapter for `deepseek-official` fails with `DUPLICATE_ADAPTER`.
...
| `retryPolicy` | normal, 5 retries | Provider-owned retry policy executed by `monolith-llm-retry` |

The generated [configuration catalog](../../../docs/config-catalog.md#monolith-llm-deepseek) is the exhaustive source for every accepted field and its JSDoc.
...
When `ctx.deepseekLlmApiExtensions` is present, the adapter prepares its registered top-level fields from the exact serialized base request before `fetch`. Preparation or field collisions fail before HTTP; after a 2xx response, the adapter accepts every captured contribution before consuming SSE. Transport and non-2xx failures do not accept them. Shipped compositions use this for the optional incremental `monolith_session_log` field and the default-on active `monolith_plugin_packages` inventory; both stay outside model input.
...
- [monolith-llm service](../llm/README.md) — the provider-neutral service this adapter registers on.
...
- [Session-log upload](../../session/session-log-deepseek/README.md) — the opt-in incremental `monolith_session_log` contribution.
- [Plugin package inventory](../plugin-package-inventory-deepseek/README.md) — the default-on `monolith_plugin_packages` contribution.
