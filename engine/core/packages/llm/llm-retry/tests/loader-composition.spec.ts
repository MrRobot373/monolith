...
import LlmRuntime, { createUserMessage, LlmAdapter, LlmError, resolveRetryPolicy  } from '@monolith/llm'
import type { GenerateOptions, ResolvedRetryPolicy, StreamChunk } from '@monolith/llm'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'monolith-llm-retry-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@monolith/llm-retry', retry],
    ['@monolith/agent-loop', AgentLoop],
  ])
