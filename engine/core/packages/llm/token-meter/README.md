...
Both plugins have usable defaults. The meter consumes only the optional `llm` service, and only to resolve route-declared request-image pricing; compaction remains optional. A deployment configures capacity and image pricing on its LLM adapter and compaction policy on `monolith-compaction-basic`.
...
- [monolith-llm service](../llm/README.md) — the model-call service whose capacity metadata `resolveModelInfo()` serves.
...
Indirectly, through consumers such as `monolith-compaction-basic`; the service itself adds no prompt, message, schema, tool, or model call.
