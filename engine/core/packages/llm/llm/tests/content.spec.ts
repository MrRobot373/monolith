...
import type { AttachmentStore, ImageMediaType } from '@monolith/attachment'
import {
  ToolCallId,
  createUserMessage,
  offloadedImageText,
...
describe('model-facing image access', () => {
  it('describes the request preview, immutable normalized path, and source uncertainty', () => {
    ...
    const access = { readonlyPath: '/tmp/.monolith/attachments/v1/objects/bb/object' }
    const version = {
    ...
    expect(requestImageHandleText(attachment, version, access)).toBe(
      ...
      + ' Normalized copy (read-only; may be resized or re-encoded): "/tmp/.monolith/attachments/v1/objects/bb/object" (2048x1536px, image/png).'
  ...
  it('bridges a provider host object only through the mounted filesystem mapping', () => {
    ...
    const attachments = {
      imageHostPath: () => '/host/.monolith/attachments/object',
    } as unknown as AttachmentStore
    const mapped = (hostPath: string): string | undefined => hostPath === '/host/.monolith/attachments/object'
