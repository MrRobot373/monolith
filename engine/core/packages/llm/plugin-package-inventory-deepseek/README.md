...
| Key | Default | Meaning |
|---|---:|---|
| `enabled` | `true` | Register the `monolith_plugin_packages` contribution. Set it to `false` to omit package metadata. |
...
The version-1 `monolith_plugin_packages` field contains only `{ name, version }` pairs. Disabled, pending, failed, disposed, unloading, structural `cordis:` rows, ordinary dependencies, loose files without an owning package identity, programmatically mounted child fibers, and in-memory dynamic plugins are excluded.
...
Nothing. `monolith_plugin_packages` is provider metadata outside the model's messages, system prompt, and tool schemas.
