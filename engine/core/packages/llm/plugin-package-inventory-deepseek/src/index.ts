 ...
 * @module @monolith/plugin-package-inventory-deepseek
 */
...
import type { Entry, EntryTree } from '@monolith/cordis-plugin-loader'
...
export interface Config {
  /** Contribute `monolith_plugin_packages` to official DeepSeek requests. Defaults to `true`. */
  enabled?: boolean
}
...
async function collectActivePluginPackages(
  ...
  if (sessionId !== undefined && ctx.get('agentPresets') !== undefined) {
    ...
    if (agent !== undefined) {
      ...
      const { standingMountFor } = await import('@monolith/agent-presets')
...
/**
 * Register the complete `monolith_plugin_packages` request contribution when enabled.
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.deepseekLlmApiExtensions.register('monolith_plugin_packages', {
