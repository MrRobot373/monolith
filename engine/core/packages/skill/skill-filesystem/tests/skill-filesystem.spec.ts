...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@monolith/fs'
import * as SkillFileSystem from '../src/index.ts'

async function tempDir(name: string): Promise<string> {
  return await import('node:fs/promises').then(fs => fs.mkdtemp(join(tmpdir(), `monolith-${name}-`)))
}
...
async function setupLocal(home: string, config: Partial<SkillFileSystem.Config> = {}): Promise<Context> {
  ...
  await ctx.plugin(SkillFileSystem, {
    monolithHome: join(home, '.monolith'),
...
describe('monolith-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.monolith/skills'), 'same', 'user monolith skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.monolith/skills'), 'same', 'project monolith skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.monolith/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project monolith skill')
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-monolith')
    ...
    await writeSkill(join(noGit, '.monolith/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.monolith/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.monolith/skills'), 'runtime-name', 'User loses')
  ...
  it('parses flat skills and filters invalid skills from the invocation-neutral listing', async () => {
    ...
    const root = join(home, '.monolith/skills')
  ...
  it('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.monolith/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.monolith/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.monolith/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.monolith/skills/broken-link'))
    await symlink('/dev/null', join(home, '.monolith/skills/device-link'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { monolithHome: join(home, '.monolith'), agentsHome: join(home, '.agents'), watch: false })

    expect((await ctx.skills.list({ cwd: nestedCwd })).map(skill => [skill.name, skill.source])).toEqual([
      ['backend-root', 'project-agents'],
      ['text-skill', 'user-monolith'],
    ])
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.monolith/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.monolith/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    const previousMonolithHome = process.env.MONOLITH_HOME
    const previousAgentsHome = process.env.MONOLITH_AGENTS_HOME
    const previousBundledSkillDir = process.env.MONOLITH_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.MONOLITH_HOME = join(envHome, '.monolith')
      process.env.MONOLITH_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.MONOLITH_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.monolith/skills'), 'env-skill', 'Env skill')
      ...
      process.env.MONOLITH_HOME = join(envHome, 'empty-monolith')
      delete process.env.MONOLITH_BUNDLED_SKILL_DIR
      process.env.MONOLITH_AGENTS_HOME = join(envHome, 'empty-agents')
      const empty = new Context()
      await empty.plugin(SkillRegistry)
      SkillFileSystem.apply(empty, { watch: false })
      expect(await empty.skills.list()).toEqual([])

      delete process.env.MONOLITH_AGENTS_HOME
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        signal: new AbortController().signal,
        invalidate() {},
      }, { monolithHome: join(envHome, 'empty-monolith') }).name).toBe('filesystem')
    } finally {
      if (previousMonolithHome === undefined) {
        delete process.env.MONOLITH_HOME
      } else {
        process.env.MONOLITH_HOME = previousMonolithHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.MONOLITH_AGENTS_HOME
      } else {
        process.env.MONOLITH_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.MONOLITH_BUNDLED_SKILL_DIR
      } else {
        process.env.MONOLITH_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
