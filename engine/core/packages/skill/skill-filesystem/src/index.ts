 ...
 *
 * @module @monolith/skill-filesystem
 */
...
import type Schema from '@monolith/schemastery'
import { parse as parseYaml } from 'yaml'
import type { FileSystem, FsDirEntry, FsTarget } from '@monolith/fs'
...
const PROJECT_MONOLITH_RANK = 100
const PROJECT_AGENTS_RANK = 200
const CUSTOM_RANK = 300
const USER_MONOLITH_RANK = 400
...
export interface Config {
  ...
  /** MONOLITH config root. Defaults to `$MONOLITH_HOME` or `~/.monolith`. */
  monolithHome?: string
  /** Shared agent config root. Defaults to `$MONOLITH_AGENTS_HOME` or `~/.agents`. */
  agentsHome?: string
  ...
  /** Bundled skill root; defaults to `$MONOLITH_BUNDLED_SKILL_DIR` when default roots are included, otherwise mounts none. */
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  constructor(
    ...
    this.agentsHome = resolve(config.agentsHome ?? process.env.MONOLITH_AGENTS_HOME ?? join(homedir(), '.agents'))
    ...
    const bundledSkillDir = config.bundledSkillDir
      ?? (this.includeDefaultRoots ? process.env.MONOLITH_BUNDLED_SKILL_DIR : undefined)
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.monolith/skills'), source: 'project-monolith', rank: PROJECT_MONOLITH_RANK, projectRoot },
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        { path: join(this.monolithHome, 'skills'), source: 'user-monolith', rank: USER_MONOLITH_RANK, skipSystem: true },
