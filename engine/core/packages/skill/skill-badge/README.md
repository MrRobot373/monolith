---
description: "The bundled 'powered by monolith' badge skill for users and maintainers enabling, using, or debugging the optional badge provider."
kind: "package-reference"
---

# @monolith/skill-badge

## Summary

Agents can load the official "powered by monolith" badge skill from this bundled provider and follow its instructions for adding attribution badges to documents, pull requests, and other content produced with MONOLITH. The provider has no configuration, and the shipped CLI composition includes the plugin disabled, so deployments enable it explicitly. The skill ships both Markdown snippets and a packaged PNG for systems that cannot reliably import remote images.
...
Enable the plugin to make the `monolith-badge` skill available in the session skill catalog; the model can then load it like any other skill and follow its instructions for adding a "powered by monolith" badge.

### When to choose it

Choose this provider when content produced with MONOLITH should carry official attribution badges, and the deployment wants the badge skill available to agents without storing it in a local skill directory. Skip it when the badge is irrelevant to the deployment — the plugin is disabled by default and adds nothing until enabled.
...
```yaml
- name: '@monolith/skill-badge'
```

After enabling, `monolith-badge` appears in the available skills of the session catalog. The skill covers remote Markdown badges (Shields.io-based) and a packaged PNG badge asset for targets that cannot fetch remote images reliably.
...
- **Packaged PNG asset.** A `monolith-badge.png` resource (726×120 source, rendered at 121×20) that works where remote images cannot be imported.

### Observable success and failures

Enabling the plugin makes `monolith-badge` appear in the catalog and loadable by name; disabling or omitting the row keeps it out of every catalog. Because the provider is immutable, discovery always succeeds with exactly one skill and never reports partial results.
...
| [`assets/`](assets/) | Packaged skill body (`monolith-badge.md`) and PNG asset (`monolith-badge.png`) |
...
Indirectly, through `monolith-tool-skill`, which renders the provider's catalog entry and the selected skill body to the model.
...
- **One fixed skill, no runtime customization** — the provider contributes exactly the `monolith-badge` skill; deployments that need another badge variant author their own skill instead.
