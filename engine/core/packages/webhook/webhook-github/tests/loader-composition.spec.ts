...
describe('real Loader composition', () => {
  it('registers on a real WebServer and dispatches a signed request', { timeout: 60_000 }, async () => {
    root = await mkdtemp(join(tmpdir(), 'monolith-webhook-github-loader-'))
    ...
    await writeFile(configPath, [
      ...
      "    host: '127.0.0.1'",
      '    port: 0',
      "- name: '@monolith/webhook-github'",
      '  config:',
      '    source: loader',
      '    path: /github',
      '    secretEnv: MONOLITH_GITHUB_WEBHOOK_SECRET',
      '    maxBodyBytes: 1024',
      '',
    ].join('\n'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@monolith/webhook-github', GitHubAdapter],
    ])
