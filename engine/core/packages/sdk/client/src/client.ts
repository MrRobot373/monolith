/**
 * Low-level JSON-RPC client for a MONOLITH SDK runtime subprocess.
 * {@link HarnessClient} owns the child process: it spawns the runtime, speaks
 * the `@monolith/sdk-protocol` wire over the child's stdio, fans
 ...
 * spawns directly rather than through the `monolith-subprocess` service — the
 * seam's documented exception for SDK-managed transports.
 *
 * @module @monolith/sdk-client/client
 */
...
} from '@monolith/sdk-protocol'
import { disposeRuntimeProcess } from './dispose.ts'
import { resolveMonolithLaunch, type RuntimeProcessOptions } from './launch.ts'
...
/**
 * JSON-RPC client for the MONOLITH SDK runtime over subprocess stdio.
 *
...
export class HarnessClient {
  /** Original public monolith launch and timeout options for this client. */
  ...
  /** @param options - monolith profile, patch, home, process, environment, and timeout options. */
  constructor(options?: HarnessClientOptions)
  constructor(options: HarnessClientOptions = {}, runtime?: RuntimeProcessOptions) {
    this.options = options
    this.runtime = runtime ?? resolveMonolithLaunch(options)
  }
   ...
   */
  start(): void {
    if (this.closeTask !== undefined) throw new TransportClosedError('MONOLITH runtime client is closed')
    ...
    child.once('error', (error) => {
      ...
      this.failSubscriptions(this.closedError('MONOLITH runtime failed to start'))
    })
    ...
    child.once('exit', (code) => {
      ...
      settled.exited = true
      maybeSettle()
      this.failSubscriptions(this.closedError('MONOLITH runtime exited'))
    })
    child.once('close', () => {
  ...
  async request(method: string, params?: object, timeoutMs?: number): Promise<unknown> {
    ...
    if (this.exitCode !== undefined || this.spawnError !== undefined) {
      ...
      throw this.closedError('MONOLITH runtime is not running')
    }
    const transport = this.transport
    /* v8 ignore next -- start() either sets the transport or throws */
    if (transport === undefined) throw new TransportClosedError('MONOLITH runtime is not running')
  ...
  subscribe(filter?: NotificationFilter): NotificationSubscription {
    ...
    if (this.closeTask !== undefined || this.exitCode !== undefined || this.spawnError !== undefined) {
      subscription.fail(this.closedError('MONOLITH runtime closed'))
      return subscription
    }
  ...
  private async performClose(): Promise<void> {
    ...
    this.transport?.close()
    this.failSubscriptions(this.closedError('MONOLITH runtime closed'))
  }
