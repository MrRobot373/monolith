 ...
 *
 * @module @monolith/sdk-client/types
 */

import type { ContentBlock, ReasoningEffortId } from '@monolith/llm'
import type { SdkPromptContentBlock } from '@monolith/sdk-protocol'
...
export interface HarnessClientOptions {
  /** Absolute or caller-relative monolith CLI module; omitted resolves this package's same-version dependency. */
  monolithBin?: string
  /** Named profile serving the SDK protocol (default `sdk`). */
  profile?: string
  /** Ordered per-launch profile patches; relative paths resolve before spawn. */
  patches?: string[]
  /** Explicit MONOLITH home for this child; relative paths resolve before spawn. */
  monolithHome?: string
  /** Working directory for the monolith process itself. */
  processCwd?: string
  /**
   ...
   * credential policy (see `scrubbedParentEnv` in `@monolith/subprocess`
...
/** Options for the high-level {@link MONOLITH} wrapper. */
export interface MONOLITHOptions extends HarnessClientOptions {
