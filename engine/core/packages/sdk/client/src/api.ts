/**
 * High-level run API over {@link HarnessClient}: `MONOLITH` owns one
 ...
 *
 * @module @monolith/sdk-client/api
 */

import { randomUUID } from 'node:crypto'
import { resolve } from 'node:path'
import type { SessionEvent, TurnEndReason } from '@monolith/session'
...
import type { ContentBlock, MONOLITHOptions, HarnessNotification, RunResult, SdkPromptContentBlock } from './types.ts'

/**
 * Reusable SDK for running MONOLITH agent turns in a runtime
 ...
 * child is reaped.
 */
export class MONOLITH implements AsyncDisposable {
  private clientInstance: HarnessClient
  private readonly createClient: () => HarnessClient
  private readonly cwd: string
  private readonly provider: string
  private readonly model: string
  private readonly reasoningEffort: MONOLITHOptions['reasoningEffort']
  private readonly maxTokens: number | undefined
  private initialized: Promise<void> | undefined
  private closed = false

  /** @param options - monolith launch configuration plus the session route, effort, and output cap. */
  constructor(options?: MONOLITHOptions)
  constructor(options: MONOLITHOptions = {}, clientFactory?: () => HarnessClient) {
      ...
      try {
        ...
        try {
          ...
          throw new AggregateError(
            [error, cleanupError],
            'MONOLITH initialization and cleanup failed',
          )
        }
...
export function createProcessMONOLITH(
  runtime: RuntimeProcessOptions,
  options: MONOLITHOptions = {},
): MONOLITH {
  const Constructor = MONOLITH as unknown as new (
    publicOptions: MONOLITHOptions,
...
export class HarnessSession {
   ...
   */
  constructor(readonly harness: MONOLITH, readonly id: string) {}
