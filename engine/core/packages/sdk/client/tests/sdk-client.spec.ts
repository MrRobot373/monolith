...
import { createProcessMONOLITH, finalResponse, normalizeInput } from '../src/api.ts'
...
function harnessWith(env: Record<string, string> = {}, extra: LaunchOverrides = {}): MONOLITH {
  const harness = createProcessMONOLITH(fakeLaunch(env, extra))
...
describe('MONOLITH', () => {
  ...
  it('sends the configured cwd/provider/model/reasoningEffort/maxTokens in the handshake exactly once', async () => {
    ...
    const harness = createProcessMONOLITH(fakeLaunch({ FAKE_RECORD_INIT: recordFile }), {
      cwd: dir,
  ...
  it('resolves a relative launch cwd to an absolute workspace before the handshake', async () => {
    ...
    const dir = await mkdtemp(join(process.cwd(), '.monolith-sdk-client-relcwd-'))
    cleanups.push(() => rm(dir, { recursive: true, force: true }))
    const recordFile = join(dir, 'init.jsonl')
    ...
    const harness = createProcessMONOLITH(
  ...
  it('preserves both initialize and SDK-owned cleanup failures', async () => {
    ...
    try {
      const harness = createProcessMONOLITH(fakeLaunch())
      ...
      expect((failure as Error).message).toBe('MONOLITH initialization and cleanup failed')
  ...
  it('supports await using disposal', async () => {
    let captured: MONOLITH
    {
      await using harness = createProcessMONOLITH(fakeLaunch())
      captured = harness
  ...
  it('constructs the public monolith-backed client lazily', async () => {
...
describe('HarnessClient', () => {
  it('bounds profile initialization and names the selected profile in its diagnostic', async () => {
    const client = processClient(fakeLaunch(
      { FAKE_HANG_INIT: '1' },
      {
        description: 'monolith profile "profile-without-sdk-server"',
    ...
    await expect(client.initialize({ cwd: process.cwd(), provider: 'p', model: 'm' }))
      .rejects.toThrow(/initialize timed out after 50ms waiting for monolith profile "profile-without-sdk-server"/)
    await client.close()
  })
  ...
  it('fails when the configured monolith CLI module does not exist', async () => {
    const client = new HarnessClient({ monolithBin: join(tmpdir(), 'monolith-no-such-runtime-bin') })
    cleanups.push(() => client.close())
  ...
  it('reports a generic process spawn failure to internal transports', async () => {
    const client = processClient(fakeLaunch({}, {
      command: join(tmpdir(), 'monolith-no-such-process-command'),
      args: [],
    }))
    cleanups.push(() => client.close())
  ...
  it('delivers notifications to unfiltered and filtered subscriptions in wire order', async () => {
    ...
    expect(identity.serverInfo.name).toBe('monolith-sdk-runtime')
