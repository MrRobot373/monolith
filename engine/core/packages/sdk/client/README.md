---
description: "The TypeScript SDK client for callers that spawn a MONOLITH runtime subprocess and drive agent turns over stdio JSON-RPC: the MONOLITH run API and the lower-level HarnessClient."
...
`monolith-sdk-client` lets TypeScript programs drive a MONOLITH runtime as a subprocess over stdio JSON-RPC. With `MONOLITH` you can spawn the runtime, open sessions, send prompts, and collect the final response plus the event and notification streams; `HarnessClient` gives explicit control over the protocol layer. It is the design twin of the [Python SDK](../../../python/README.md), which shares the same runtime peer and protocol. The launch spec is explicit — callers may name the runtime executable via `monolithBin`, omitted resolves the same-version `@monolith/cli` package's bin, and the client constructs the arguments — so this client suits repository-adjacent TypeScript consumers such as the SDK subagent backend and automation that know which runtime they are launching. It is a pure library: it registers nothing on a Cordis context, and the runtime it spawns is a complete harness whose composition its own `cordis.yml` decides.
...
Use this client when TypeScript code must drive a complete MONOLITH runtime from another process and you can name the runtime executable explicitly. The common path is minimal: construct a `MONOLITH` with a launch spec, run prompts, and close it so the child process is always reaped.

### Running agent turns with MONOLITH

```ts
import { MONOLITH } from '@monolith/sdk-client'
import { ReasoningEffortId } from '@monolith/llm'

await using harness = new MONOLITH({
  profile: 'sdk',
...
The client is two layers over one wire: `MONOLITH` (owned runs) over `HarnessClient` (the protocol client), mirroring the Python SDK's layering. It runs outside any harness context, so it spawns the runtime directly rather than through the `monolith-subprocess` service — the seam's documented exception for SDK-managed transports — and its teardown ladder lives in this package. The runtime notifies for every session in its context; session-tree scoping is a client-side filter over `subagent.started` lineage edges.

### Source map

| File | Role |
|---|---|
| [`src/api.ts`](src/api.ts) | `MONOLITH` + `HarnessSession`: owned runs, receipt-to-idle collection, `finalResponse` |
...
- [SDK subagent backend](../../subagent/subagent-monolith-sdk/README.md) — a harness-internal consumer of this client.
- [SDK application bundle](../../bundle/sdk-app/README.md) — the `monolith --profile sdk` runtime application this client launches.
...
- **No bundled-runtime resolution** — the client resolves the same-version `@monolith/cli` package (or a caller-provided `monolithBin`); packaged-executable discovery stays Python-side until a TypeScript distribution consumer exists.
