...
describe('monolith-sdk-jsonrpc-server plugin apply', () => {
  it('serves initialize over the injected stdio pair', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'monolith-jsonrpc-apply-init-'))
    ...
    try {
      ...
      expect(response).toEqual({
        jsonrpc: '2.0',
        id: 'init-1',
        result: { serverInfo: { name: 'monolith-sdk-runtime', version: '0.0.1' } },
      })
  ...
  it('waits for Loader-owned adapter registration before initialize', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'monolith-jsonrpc-apply-readiness-'))
    vi.stubEnv('DEEPSEEK_API_KEY', 'test-key')
  ...
  it('drives a session/prompt turn end-to-end and forwards session notifications as output frames', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'monolith-jsonrpc-apply-prompt-'))
  ...
  it('answers shutdown before exiting 0 exactly once, even against a racing second shutdown', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'monolith-jsonrpc-apply-shutdown-'))
  ...
  it('still disposes and exits once when the flush callback fails', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'monolith-jsonrpc-apply-flush-failure-'))
  ...
  it('stops serving on a bare fiber dispose (HMR-style unload) without calling exit', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'monolith-jsonrpc-apply-dispose-'))
    const harness = await mountPlugin(storageDir)
    try {
      ...
      expect(error.error).toMatchObject({
        code: -32603,
        message: 'unknown MONOLITH SDK runtime method: nope/unknown',
      })
