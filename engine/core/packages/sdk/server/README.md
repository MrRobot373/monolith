...
The profile composition owns each root agent's tools. `input`, `output`, and `exit` are runtime-only transport hooks for tests; production uses process stdio and `process.exit`. The generated [configuration catalog](../../../docs/config-catalog.md#monolith-sdk-jsonrpc-server) is the exhaustive source for every accepted field.
...
- [SDK application bundle](../../bundle/sdk-app/README.md) — the `monolith --profile sdk` application that boots this plugin.
