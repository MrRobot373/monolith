 ...
 *
 * @module @monolith/sdk-jsonrpc-server/server
 */

import type { Context } from '@monolith/cordis'
import { resolve } from 'node:path'
import { brandString } from '@monolith/brand'
import type { Agent, AgentHandle } from '@monolith/agent'
import { admitEncodedImages, type EncodedImageAttachment, type ImageAttachmentRef } from '@monolith/attachment'
import { createUserMessage, ReasoningEffortId, type ContentBlock, type LlmRuntime } from '@monolith/llm'
import { carrierKeyOf, type Scoped } from '@monolith/scope'
import type { SessionId } from '@monolith/session'
import type SubagentRuntime from '@monolith/subagent'
import type { SubagentRunEndInfo } from '@monolith/subagent'
...
export class HarnessSdkJsonRpcServer {
  ...
  async initialize(params: InitializeParams): Promise<InitializeResult> {
    ...
    return { serverInfo: { name: 'monolith-sdk-runtime', version: '0.0.1' } }
  }
  ...
  async handleRequest(method: string, params: Record<string, unknown> | undefined): Promise<unknown> {
    switch (method) {
      ...
      default:
        throw new Error(`unknown MONOLITH SDK runtime method: ${method}`)
    }
  }
