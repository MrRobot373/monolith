/**
 * Named wire types for the MONOLITH SDK runtime protocol: the three
 * request/result pairs and the four server-to-client notification payloads
 * exchanged over the newline-delimited JSON-RPC stdio transport. The server
 * plugin (`@monolith/sdk-jsonrpc-server`) and SDK clients share these shapes;
 * `serverInfo.name` stays the wire-stable `monolith-sdk-runtime`.
 *
 * @module @monolith/sdk-protocol/types
 */

import type { ContentBlock, ReasoningEffortId } from '@monolith/llm'
import type { SessionEvent } from '@monolith/session'
import type { SubagentStopReason } from '@monolith/subagent'
...
export interface InitializeResult {
  /** Wire-stable server identity (`monolith-sdk-runtime`) and version. */
