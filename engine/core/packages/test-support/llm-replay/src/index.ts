 ...
 * @module @monolith/llm-replay
 */

import { existsSync, readFileSync, writeFileSync } from 'node:fs'
import { delimiter as pathDelimiter } from 'node:path'
import type { Context } from '@monolith/cordis'
import type {} from '@monolith/compaction'
import type {} from '@monolith/deepseek-llm-api-extensions'
import { decodeSeqRanges, decodeStorageRecord, SessionLogOffset, type SessionEvent } from '@monolith/session'
import type { SessionLogOffset as SessionLogOffsetType } from '@monolith/session'
import type {
  ContentBlock,
  GenerateOptions,
...
import { LlmAdapter, LlmError, ReasoningEffortId, requestImageHandleText, resolveRetryPolicy } from '@monolith/llm'
import { assertNever } from '@monolith/util-values'
...
/** Plugin config: the {@link ReplayConfig} inputs, each defaulting to its `MONOLITH_SNAPSHOT_*` env var in `apply`. */
export interface Config {
  /** Override the fixture path; defaults to `$MONOLITH_SNAPSHOT_FILE`. */
  file?: string
  /** Override the sidecar path; defaults to `$MONOLITH_SNAPSHOT_OVERRIDE`. */
  overrideFile?: string
  /**
   * Override the child-log paths; defaults to `$MONOLITH_SNAPSHOT_CHILD_FILES` (a
...
export function apply(ctx: Context, config: Config = {}): void {
  const file = config.file ?? process.env.MONOLITH_SNAPSHOT_FILE
  if (file === undefined || file.length === 0) {
    throw new Error('llm-replay: a fixture path is required (Config.file or $MONOLITH_SNAPSHOT_FILE)')
  }
  validateConfiguredModels(config.providers)
  const overrideFile = config.overrideFile ?? process.env.MONOLITH_SNAPSHOT_OVERRIDE
  const childEnv = process.env.MONOLITH_SNAPSHOT_CHILD_FILES
