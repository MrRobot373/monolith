    ...
    providers:
      - id: deepseek-official
        ...
        models:
          ...
          - id: deepseek-v4-pro
  # file/overrideFile/childFiles default to $MONOLITH_SNAPSHOT_FILE /
  # $MONOLITH_SNAPSHOT_OVERRIDE / $MONOLITH_SNAPSHOT_CHILD_FILES, set by the snapshot
  # harness per scenario.
```

| Field | Default | Meaning |
|---|---|---|
| `file` | `$MONOLITH_SNAPSHOT_FILE` | Path to the primary (parent) `session.jsonl` fixture; required (config or env) |
| `overrideFile` | `$MONOLITH_SNAPSHOT_OVERRIDE` | Optional `ReplayOverrideDoc` sidecar for the primary session |
| `childFiles` | `$MONOLITH_SNAPSHOT_CHILD_FILES` | Recorded subagent child-session logs for a nested scenario |
...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-llm-replay) is the exhaustive source for every accepted field and its JSDoc.
