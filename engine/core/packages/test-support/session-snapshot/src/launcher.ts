 ...
 *
 * @module @monolith/session-snapshot/launcher
 */
...
import { entryListSchema, type PatchOptions } from '@monolith/cordis-plugin-include'
import { resolveExampleLaunch } from '@monolith/loader-smoke'
...
export interface AgentUnderTest {
  ...
  configPath: string
  /** Named monolith profile; omitted only for test-only fake bins with their own config grammar. */
  profile?: string
...
export function launchAcpTestAgent(options: AcpTestLaunchOptions): LaunchedAcpTestAgent {
  ...
  const launch = resolveExampleLaunch({
    ...
    configArgs: agent.profile === undefined
      ...
      : profileArgs(agent.profile, agent.configPath, selectedConfig, options.env?.MONOLITH_SNAPSHOT, cwd),
    tsconfigPath: agent.tsconfigPath,
    ...agent.profile === undefined ? {} : { sourceImport: 'tsx/esm' },
    env: {
      ...options.env,
      MONOLITH_HOME: join(cwd, '.monolith'),
  ...
  const clientApp = createAcpClientApp({ name: 'monolith-acp-test-client' })
...
/** Build one monolith profile invocation from the base and optional scenario patches. */
function profileArgs(
  ...
  const materializedRoot = join(cwd, '.monolith-profile-patches')
 ...
 * profile fallback. This mirrors `monolith plugin` while retaining the bare entry
...
function linkProfilePackage(source: string, cwd: string, packageName: string): void {
  ...
  // The package may instead belong to the monolith installation; profile boot heals those links.
  if (packageDir === undefined) return
  const link = join(cwd, '.monolith', 'profiles', 'node_modules', packageName)
