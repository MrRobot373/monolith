 ...
 *
 * @module @monolith/session-snapshot/harness
 */
...
import { clearedProxyEnv } from '@monolith/http-proxy'
...
export interface RunOptions {
   ...
   * them to `monolith-llm-replay` via `$MONOLITH_SNAPSHOT_CHILD_FILES` so each child
...
export function snapshotSpillRoot(
  ...
  return `${root}/monolith-acp-snap-${key}`
}
...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const initialWorkspace = await captureWorkspaceSnapshot(cwd, {
      ignoredRootEntries: ['.agents', '.monolith', '.monolith-profile-patches', '.monolith-snapshot-stream-ready'],
    })
    const env: NodeJS.ProcessEnv = {
      ...
      ...clearedProxyEnv(),
      MONOLITH_SNAPSHOT: opts.mode,
      MONOLITH_SNAPSHOT_FILE: opts.fixtureFile,
      ...
      ...opts.overrideFile !== undefined ? { MONOLITH_SNAPSHOT_OVERRIDE: opts.overrideFile } : {},
      ...opts.childFiles !== undefined && opts.childFiles.length > 0
        ? { MONOLITH_SNAPSHOT_CHILD_FILES: opts.childFiles.join(delimiter) }
        : {},
    }
...
async function harvestSessionLogs(root: string): Promise<HarvestedLog[]> {
  ...
  // order must match the replay load order in monolith-llm-replay's loadSessionScripts
