...
it('keeps scenario-owned snapshot spill root length stable across platforms', () => {
  ...
  expect(posix).toMatch(/^\/tmp\/monolith-acp-snap-[0-9a-f]{9}$/)
  expect(windows).toMatch(/^\/t\/monolith-acp-snap-[0-9a-f]{9}$/)
...
describe('runScenario', () => {
  ...
  it('centralizes ACP boot, captures, updates, fail-closed permissions, and shutdown', { timeout: 20_000 }, async () => {
    ...
    const launched = launchAcpTestAgent({
      ...
      env: {
        ...
        MONOLITH_SNAPSHOT_FILE: fixtureFile,
        MONOLITH_SNAPSHOT_SESSIONS_ROOT: sessionsRoot,
      },
    })
  ...
  it('builds monolith profile argv and rebases relative modules in live and replay patches', async () => {
    ...
    const live = launchAcpTestAgent({
      ...
      env: { MONOLITH_SNAPSHOT: 'record', MONOLITH_SNAPSHOT_FILE: fixtureFile },
    })
    await live.spawned
    await live.close()
    const materializedRoot = join(dir, '.monolith-profile-patches')
    ...
    expect(await realpath(join(dir, '.monolith', 'profiles', 'node_modules', 'example-package')))
      .toBe(await realpath(packageDir))
    expect(await realpath(join(dir, '.monolith', 'profiles', 'node_modules', '@fixture', 'example-package')))
    ...
    const replay = launchAcpTestAgent({
      ...
      cwd: dir,
      configPath: selectedPatch,
      env: { MONOLITH_SNAPSHOT: 'replay', MONOLITH_SNAPSHOT_FILE: fixtureFile },
    })
    ...
    const conflictLink = join(dir, '.monolith', 'profiles', 'node_modules', 'conflict-package')
    await Promise.all([
  ...
  it('waits for inherited stdio and buffered ACP parsing after the parent exits', { timeout: 20_000 }, async () => {
    ...
    const launched = launchAcpTestAgent({
      agent: AGENT,
      cwd: dir,
      env: { MONOLITH_SNAPSHOT_FILE: fixtureFile },
    })
    await launched.client.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: {} })
  ...
  it('preserves launch-resolution errors when no child process exists', async () => {
    const { dir, fixtureFile } = await scenario({})
    vi.stubEnv('MONOLITH_EXAMPLE_MODE', 'lib')
    try {
      await expect(runScenario(
        { steps: [] },
        {
