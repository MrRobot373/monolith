/**
 * Scripted fake ACP agent bin for `monolith-session-snapshot`'s ACP adapter specs. Speaks
 * newline-delimited JSON-RPC on stdio like the real `monolith-acp-agent` bin, but
 ...
 * `behavior.json` sitting NEXT to the `$MONOLITH_SNAPSHOT_FILE` fixture, so a spec
...
interface ScriptedLog {
  /** Path relative to `$MONOLITH_SNAPSHOT_SESSIONS_ROOT`, e.g. `project/session/session.jsonl`. */
  file: string
  /**
...
interface Behavior {
  ...
  /** Echo the `MONOLITH_SNAPSHOT_*` env the harness set as a chunk (spec-side env-plumbing assertions). */
  echoEnv?: boolean
...
const sessionsRoot = process.env.MONOLITH_SNAPSHOT_SESSIONS_ROOT ?? ''
const fixtureFile = process.env.MONOLITH_SNAPSHOT_FILE ?? ''
...
async function handlePrompt(id: number | string): Promise<void> {
  ...
  if (behavior.echoEnv === true) {
    chunk(`env:${JSON.stringify({
      mode: process.env.MONOLITH_SNAPSHOT,
      override: process.env.MONOLITH_SNAPSHOT_OVERRIDE ?? null,
      childFiles: process.env.MONOLITH_SNAPSHOT_CHILD_FILES ?? null,
      spillRoot: process.env.MONOLITH_SNAPSHOT_SPILL_ROOT ?? null,
      // Scenario-supplied deployment env (the `Scenario.env` layering hook).
      permissionMode: process.env.MONOLITH_PERMISSION_MODE ?? null,
    })}`)
  }
