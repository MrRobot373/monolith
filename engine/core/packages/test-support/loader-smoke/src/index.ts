 ...
 * zero-build dev path, resolving `@monolith/*` / `@cordisjs/*` through the tsconfig `paths`
...
export const EXAMPLE_MODE_ENV = 'MONOLITH_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.MONOLITH_EXAMPLE_MODE`.
...
export function resolveExampleLaunch(options: ExampleLaunchOptions): ExampleLaunch {
  ...
  // A smoke launches a real `monolith` against local fixtures, so it must not inherit the machine's
...
export interface LoaderSmokeOptions {
  ...
  readonly mode?: ExampleMode
  /** Environment overrides layered over the parent and isolated MONOLITH homes. */
