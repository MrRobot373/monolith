...
import * as GoalSessionInvariant from '@monolith/goal-round-driver/invariant'
import { renderGoalRoundPrompt } from '@monolith/goal-round-driver'
import InvariantRegistry, { InvariantError } from '@monolith/invariants'
import SessionStore, { SessionId, type Session } from '@monolith/session'
...
describe('goal-round-driver prompt invariants', () => {
  ...
  it('rejects a continuation whose content differs from the package renderer', async () => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@monolith/goal-round-driver',
    }))
  })
