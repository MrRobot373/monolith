...
`defaultMaxGoalRounds` must be a positive safe integer; a create request that names its own cap overrides it. The generated [configuration catalog](../../../docs/config-catalog.md#monolith-goal) is the exhaustive source for every accepted field.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#monolith-goal) — every accepted config field and its source declaration.
...
- **State, not scheduling** — this package does not decide when an armed goal continues, retry abnormal failures, or cancel an active turn; those policies belong to consumer packages such as `monolith-goal-round-driver`.
