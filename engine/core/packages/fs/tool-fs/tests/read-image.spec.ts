...
import { CodeRuntime } from '@monolith/code-runtime'
import type { CodeRunRequest, CodeRunResult } from '@monolith/code-runtime'
import { ToolCallId, LlmAdapter, LlmRuntime } from '@monolith/llm'
import type { GenerateOptions, LlmModelInfo, LlmResolvedModelInfo, Message, StreamChunk } from '@monolith/llm'
import SystemPrompt from '@monolith/system-prompt'
import ToolRuntime, { RUN_CODE_NAME } from '@monolith/tools'
import type { Config as ToolConfig } from '@monolith/tools'
import LocalFileSystem from '@monolith/fs-local'
import * as FsPolicy from '@monolith/fs-observation-policy'
import LocalAttachmentStore from '@monolith/attachment-local'
import { AttachmentError, AttachmentId, AttachmentStore } from '@monolith/attachment'
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'monolith-read-image-'))
  home = await mkdtemp(join(tmpdir(), 'monolith-read-image-home-'))
})
afterEach(async () => {
...
async function setup(options: SetupOptions = {}) {
  ...
  if (options.attachments !== false) {
    await ctx.plugin(LocalAttachmentStore, { monolithHome: home, ...options.storeConfig })
  }
...
describe('registration surface', () => {
  it('withdraws read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    ...
    await ctx.plugin(LocalFileSystem, { cwd: dir })
    await ctx.plugin(FsPolicy)
    const attachmentsFiber = await ctx.plugin(LocalAttachmentStore, { monolithHome: home })
    ...
    const remounted = await ctx.plugin(LocalAttachmentStore, { monolithHome: home })
    expect(names()).toEqual(['edit', 'read', 'read_image', 'write'])
