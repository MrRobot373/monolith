...
// --------------------------------------------------------------------------
describe('default deployment (with monolith-fs-observation-policy)', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'monolith-tool-fs-'))
    ctx = new Context()
...
// --------------------------------------------------------------------------
describe('bare provider (no monolith-fs-observation-policy)', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'monolith-tool-fs-bare-'))
    ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
    await ctx.plugin(LocalFileSystem, { cwd: dir })
...
// caller-selected session workspace wins, matching monolith-tool-bash.
describe('per-session cwd', () => {
  let sessionDir: string
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'monolith-tool-fs-cfg-'))
    sessionDir = await mkdtemp(join(tmpdir(), 'monolith-tool-fs-session-'))
    ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
