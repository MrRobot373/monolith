...
import ToolRuntime, { type ToolResult } from '@monolith/tools'
import { FileSystem, FsError, FsTargetKey, FsVersion } from '@monolith/fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
describe('session cwd resolution', () => {
  ...
  it('retains ordinary spelling but resolves the cwd before parent traversal', () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'monolith-tool-fs-session-cwd-'))
