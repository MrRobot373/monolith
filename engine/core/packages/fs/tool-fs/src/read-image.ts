 ...
 * @module @monolith/tool-fs/src/read-image
 */

import { basename, extname } from 'node:path'
import type { Context } from '@monolith/cordis'
import { AttachmentError, AttachmentId } from '@monolith/attachment'
import type { AttachmentStore, ImageAttachmentRef, ImageMediaType } from '@monolith/attachment'
import type { ContentBlock } from '@monolith/llm'
import { defineTool } from '@monolith/tools'
import type { GenericCallView, ToolExecution } from '@monolith/tools'
...
export function applyReadImageTool(ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    description: 'Read a PNG/JPEG/WebP/GIF file and return the image itself. '
      ...
      + 'MONOLITH validates and downscales large supported images before the next model request, so use this tool directly instead of installing image libraries or creating thumbnails merely to inspect an image. '
