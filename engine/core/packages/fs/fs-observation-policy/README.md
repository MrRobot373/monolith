...
Load this plugin alongside a `ctx.fs` backend and the `monolith-tool-fs` tools when a deployment wants the model to read a file before it can overwrite or edit it. The plugin needs no configuration and injects no service; it only listens for the `fs/*` events the tools dispatch.
...
```yaml
- name: '@monolith/fs-local'
- name: '@monolith/fs-observation-policy'
- name: '@monolith/tool-fs'
```
...
- **Event gate, not method service.** The plugin influences the world only through the `fs/*` events, so it registers no `ctx.fsPolicy` service and has no public methods. Removing it cannot break `monolith-tool-fs` at a service-injection boundary — the tool falls through to the bare provider.
...
- [monolith-fs](../fs/README.md) — the `ctx.fs` contract and the `fs/*` event vocabulary.
