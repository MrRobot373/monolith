 ...
 * `monolith-fs-observation-policy`, so it is not exercised here.
 */
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'monolith-fs-'))
  ctx = new Context()
...
describe('resolve', () => {
  it('resolves a relative path against opts.cwd, not config.cwd', async () => {
    ...
    const other = await mkdtemp(join(tmpdir(), 'monolith-fs-other-'))
    try {
