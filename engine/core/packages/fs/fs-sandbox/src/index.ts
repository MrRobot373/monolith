 ...
 * `@monolith/fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * (`@monolith/bash-sandbox`). The residual
 ...
 * `FS_SANDBOX_DENIED`.
 *
 * @module @monolith/fs-sandbox
 */

import { Context } from '@monolith/cordis'
import { LocalFileSystem } from '@monolith/fs-local'
import type { Config as LocalConfig } from '@monolith/fs-local'
import { FsError } from '@monolith/fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@monolith/fs'
import { writableRoots } from '@monolith/sandbox'
import type { SandboxExecutionPolicy, SandboxMode } from '@monolith/sandbox'
import type {} from '@monolith/sandbox-policy'
 ...
 * INSTEAD OF `monolith-fs-local`, together with a `ctx.sandboxPolicy`, is the whole
 * swap — the model-facing tools are untouched). Its configured default mode is
 * the capability fact exposed by {@link sandboxMode}; `monolith-tool-fs` resolves
