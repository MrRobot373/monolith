...
import { FsError, FsTargetKey } from '@monolith/fs'
import type { FsTarget } from '@monolith/fs'
...
beforeEach(async () => {
  ...
  base = await mkdtemp(join(homedir(), '.monolith-fssbx-'))
...
describe('workspace-write containment', () => {
  ...
  it('a write to the platform temp area lands (parity with the bash runner grant)', async () => {
    const path = join(await mkdtemp(join(tmpdir(), 'monolith-fssbx-tmp-')), 'temp.txt')
