...
- name: '@monolith/fs-sandbox'
  config:
    cwd: /absolute/path/to/workspace
- name: '@monolith/tool-fs'
```

The backend's config is the local backend's unchanged (`cwd` resolution default and `diffBasisMaxBytes` overwrite bound); the [configuration catalog](../../../docs/config-catalog.md#monolith-fs-sandbox) is the exhaustive source.
...
The policy owner contributes capability-neutral `sandbox:policy` context. Indirectly, `monolith-tool-fs` renders this backend's `FS_SANDBOX_DENIED` refusals as the `[sandbox: file access denied under <mode> mode]` marker plus the same-turn escalation hint.
