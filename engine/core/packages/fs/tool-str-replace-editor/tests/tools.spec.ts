...
import { FsVersion } from '@monolith/fs'
...
import SandboxPolicy from '@monolith/sandbox-policy'
...
async function setup(
  ...
  const root = await mkdtemp(join(tmpdir(), 'monolith-tool-str-replace-editor-'))
  roots.push(root)
...
describe('tool-str-replace-editor', () => {
  ...
  it('reports missing sandbox-policy composition during plugin startup', async () => {
    const root = await mkdtemp(join(tmpdir(), 'monolith-tool-str-replace-editor-missing-policy-'))
    roots.push(root)
    const ctx = new Context()
    contexts.push(ctx)
    await ctx.plugin(SystemPrompt)
