...
# @monolith/tool-fs-search

## Summary

`monolith-tool-fs-search` provides the model-facing filesystem discovery tools — `glob` and `grep` — backed by a packaged ripgrep binary, so no host `rg` install and no filesystem backend are needed. Each call runs ripgrep itself with a fixed argument set and returns workdir-relative results, and the tools are always available because every carrier packages ripgrep. Results are bounded by configurable caps, and a capped result is saved in full through the optional spill store when one is mounted. Choose this package when the model should discover files by pattern or search file contents; text file reading, writing, and editing are the sibling `monolith-tool-fs` package's job.
...
The generated [configuration catalog](../../../docs/config-catalog.md#monolith-tool-fs-search) is the exhaustive source for every accepted field and its JSDoc.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#monolith-tool-fs-search) — the exhaustive schemas this package registers.
...
The glob description states the configured over-cap ordering. The generated [`glob` and `grep` schemas](../../../docs/tool-catalog.md#monolith-tool-fs-search) use `sampleOverCapGlobResults: true`; the tools are registered unconditionally.
