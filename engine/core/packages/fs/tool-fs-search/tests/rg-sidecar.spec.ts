...
describe('ripgrep resolution', () => {
  it('uses the native sidecar beside the current executable', async () => {
    ...
    process.execPath = '/runtime/monolith'
    existsSync.mockReturnValue(true)
    const sidecar = '/runtime/monolith-rg'
    const { resolveRgPath } = await import('@monolith/tool-fs-search')
  ...
  it('uses a conventional executable name for the Windows ripgrep sidecar', async () => {
    ...
    process.execPath = 'C:\\runtime\\monolith-sdk-runtime-win-x64.exe'
    existsSync.mockReturnValue(true)
    const sidecar = 'C:\\runtime\\monolith-sdk-runtime-win-x64-rg.exe'
    const { resolveRgPath } = await import('@monolith/tool-fs-search')
