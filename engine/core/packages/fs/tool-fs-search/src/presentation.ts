 ...
 * model-facing render consumes ({@link module:@monolith/tool-fs-search/search-core}
 ...
 * final output budget (`monolith-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @monolith/tool-fs-search/presentation
 */
 ...
 * ({@link module:@monolith/tool-fs-search/grep} `formatGrepMatches`), so
