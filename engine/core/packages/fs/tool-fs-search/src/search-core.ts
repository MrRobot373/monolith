 ...
 *
 * @module @monolith/tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@monolith/output-retention'
import type { RetainedItems } from '@monolith/output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@monolith/subprocess'
import type { SaveTextSpill, SpillRef } from '@monolith/spill'
 ...
 * `@monolith/tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`monolith-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * (`@monolith/tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@monolith/tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@monolith/tool-fs-search/presentation} `grepSearchMeta`)
