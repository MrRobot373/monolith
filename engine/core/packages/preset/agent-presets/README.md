...
The presets you can choose from come from two places: the presets shipped inside this package under `presets/`, and your own presets under `<monolithHome>/.agent-presets`. The picker shows each preset's display name and description; a preset whose composition cannot load is listed with the reason rather than hidden, so you can see what to fix or delete.
...
| `includeUserRoot` | `true` | Append `<monolithHome>/.agent-presets` as a `user` root, after every configured root |

The generated [configuration catalog](../../../docs/config-catalog.md#monolith-agent-presets) is the exhaustive source for every accepted field and its JSDoc.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#monolith-agent-presets) — every accepted config field and its source declaration.
...
- **A superseded generation is never reclaimed** — sessions already joined keep the generation they run on, and the roster holds no join count that could tell when the last one left, so the whole subtree stays mounted until the process ends. The cost is per generation rather than per session, but it is not free: `monolith-skill-filesystem` watches its roots by default, so each edit-then-create cycle adds a live watcher set.
...
Reclaiming a superseded standing mount needs a joined-agent count on `StandingMount`, incremented in `mount`/`composeFrom`/`recompose` and decremented when the agent's scope key dies — the `TODO` at `ensureStanding`. The subtree is not inert: `monolith-skill-filesystem` watches its roots, so an unreclaimed generation keeps a live watcher set alive until the process ends.
