...
import { Context, FiberState } from '@monolith/cordis'
...
import AgentPresets, { COMPOSITION_FILE, METADATA_FILE } from '@monolith/agent-presets'
...
describe('fileComposition', () => {
  it('flattens groups and keeps refused expressions conditional', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'monolith-composition-'))
...
describe('AgentPresets.compositionInventory', () => {
  it('reads unmounted presets from their files, marking the default and metadata', async () => {
    const userRoot = await mkdtemp(join(tmpdir(), 'monolith-composition-roster-'))
  ...
  it('prefers the standing mount over a file that broke after mounting', async () => {
    const root = await mkdtemp(join(tmpdir(), 'monolith-composition-volatile-'))
