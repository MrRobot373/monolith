 ...
 *
 * `$MONOLITH_HOME` is repointed per test for the same reason as the user-root
...
import Include, { entryListSchema } from '@monolith/cordis-plugin-include'
import SessionProjectionRegistry from '@monolith/session-projection'
import * as yaml from 'js-yaml'
import { afterEach, beforeEach, describe, expect, it } from 'vitest'
import AgentPresets, { SHIPPED_PRESET_ROOT, type Config } from '@monolith/agent-presets'
...
beforeEach(async () => {
  ...
  process.env.MONOLITH_HOME = await mkdtemp(join(tmpdir(), 'monolith-shipped-root-'))
})
