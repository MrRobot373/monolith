 ...
 * authors into `<monolithHome>/.agent-presets`, the way `monolith-skill-filesystem` owns
 * `<monolithHome>/skills`. `includeUserRoot: false` is how a deployment — or a test
 * pinning an exact roster — opts out.
 *
 * `$MONOLITH_HOME` is repointed per test because the derived root is resolved in the
...
import AgentPresets, { COMPOSITION_FILE, type Config } from '@monolith/agent-presets'
...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), 'monolith-preset-home-'))
...
describe('the harness-home preset root', () => {
  ...
  it('yields to a configured user root for authoring, which writableRoot takes first', async () => {
    const explicit = await mkdtemp(join(tmpdir(), 'monolith-preset-explicit-'))
    const ctx = await roster({
      roots: [
