/**
 * Package-owned invariant companion for `@monolith/agent-presets`.
 * @module @monolith/agent-presets/invariant
 */

import type { Context } from '@monolith/cordis'
import type { InvariantInstaller } from '@monolith/invariants'
// Type-only: resolves the `system-prompt/assemble` waterfall this companion
// joins, and the `agent` field `monolith-agent` merges into its context.
...
import { leakedServices, livePresetMounts } from '@monolith/agent-presets'

const PACKAGE_NAME = '@monolith/agent-presets'
