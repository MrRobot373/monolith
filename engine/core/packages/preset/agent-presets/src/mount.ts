 ...
 * @module @monolith/agent-presets/mount
 */

import { pathToFileURL } from 'node:url'
import { Context, type Fiber } from '@monolith/cordis'
import { Include } from '@monolith/cordis-plugin-include'
import type { EntryTree } from '@monolith/cordis-plugin-loader'
import { scopeOf, scopeParentOf, type ScopeKey } from '@monolith/scope'
...
class PresetTree extends Include {
   ...
   * dependencies, so every `@monolith/*` row would fail to import. The
 ...
 * invariant companion's service listener, and `monolith-invariants` is a
