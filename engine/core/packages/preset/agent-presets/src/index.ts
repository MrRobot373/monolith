 ...
 * @module @monolith/agent-presets
 */

import { stat } from 'node:fs/promises'
import { Context } from '@monolith/cordis'
import { evaluate } from '@monolith/cordis-plugin-loader'
import z from '@monolith/schemastery'
import { Remote, RemoteError, TypertRemoteService } from '@monolith/typert-protocol'
import { bindScopeParent, createScope, scopeOf, type Scope, type ScopeKey, type ScopeParentBinding } from '@monolith/scope'
...
import type SettingsService from '@monolith/settings'
import type { SettingsScope } from '@monolith/settings'
...
export class AgentPresets extends TypertRemoteService {
  ...
  constructor(ctx: Context, public config: Config) {
    ...
    this.resolvedRoots = [
      ...
      ...config.roots,
      ...config.includeUserRoot ? [{ path: monolithHomePath(USER_PRESET_DIR), trust: 'user' } satisfies PresetRoot] : [],
    ]
   ...
   * scope key. The binding is monolith-scope's only re-link capability; holding it
  ...
  private async ensureStanding(preset: AgentPreset): Promise<StandingMount> {
    ...
    if (pending !== undefined) {
      ...
      // it is gone. The subtree is not inert — `monolith-skill-filesystem` watches its
