...
describe('request image verification', () => {
  it('rejects an encoded request whose decoded facts disagree with the encoder result', async () => {
    const monolithHome = await mkdtemp(join(tmpdir(), 'monolith-request-verification-'))
    homes.push(monolithHome)
    const attachments = new LocalAttachmentStore(new Context(), { monolithHome })
