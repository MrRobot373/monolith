...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const monolithHome = await mkdtemp(join(tmpdir(), 'monolith-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { monolithHome })
      ...
      await rm(monolithHome, { recursive: true, force: true })
    }
  })

  it('commits a fully prepared image batch in input order', async () => {
    const monolithHome = await mkdtemp(join(tmpdir(), 'monolith-attachment-batch-success-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { monolithHome })
  ...
  it.each([3, 4] as const)('admits a 16-bit %s-channel PNG as an 8-bit normalized object', async (channels) => {
    const monolithHome = await mkdtemp(join(tmpdir(), 'monolith-attachment-16-bit-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { monolithHome })
  ...
  it('prepares every batch member before any write', async () => {
    const monolithHome = await mkdtemp(join(tmpdir(), 'monolith-attachment-batch-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { monolithHome })
  ...
  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const monolithHome = await mkdtemp(join(tmpdir(), 'monolith-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { monolithHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { monolithHome, maxImageBytes: 1 })
