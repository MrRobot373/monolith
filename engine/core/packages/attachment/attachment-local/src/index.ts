/** Local durable attachment backend rooted below `MONOLITH_HOME`. @module @monolith/attachment-local */
...
export interface Config {
  /** Explicit harness home; omitted follows `MONOLITH_HOME`, then `~/.monolith`. */
  monolithHome?: string
...
export class LocalAttachmentStore extends AttachmentStore {
  ...
  constructor(ctx: Context, config: Config) {
    super(ctx)
    this.root = resolve(join(resolveMonolithHome(config.monolithHome), 'attachments', 'v1'))
