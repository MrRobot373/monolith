...
`monolith-fs` owns three events the tool dispatches and the policy plugin listens for, so the emitter (`monolith-tool-fs`) and the listener (`monolith-fs-observation-policy`) share a vocabulary without the emitter depending on the policy plugin. They carry only `monolith-fs` vocabulary plus an opaque `object` actor — no model-facing concepts and no agent/session owner structure.
 ...
 * `FsObservationActor` without importing `monolith-tools`, `monolith-agent`, or `monolith-session`.
 *
