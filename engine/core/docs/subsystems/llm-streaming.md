...
- **An empty completion is a retryable error, not a silent success.** Both adapters map a terminal `stop` finish that carried no content blocks to `finish {kind:'error'}` with the canonical `EMPTY_RESPONSE` code, and `monolith-llm-retry` retries it by default; see [empty model responses are retryable](../../.agents/notes/implemented/bug-fix/2026-07-24-empty-model-response-is-retryable.md).
...
declare class BlockAssembler {
   ...
   */
  message(source: MessageSource = { kind: 'plugin', plugin: 'monolith-llm/assembler' }): Message;
}
```
...
interface GenerateOptions {
   ...
   * the derived history (monolith-agent-loop); a hand-built one-shot passes any list.
   */
  messages: Message[]
...
`GenerateOptions.tools` carries `ToolSchema` — the JSON-schema description of a tool, as sent to the model. It is declared in monolith-llm (not monolith-tools) precisely because it is part of the request the loop assembles every step:

```ts type-equiv
/**
 * JSON-schema description of a tool, as sent to the model.
 *
 * Declared here (not in monolith-tools) because it is part of {@link GenerateOptions};
 * monolith-tools' ToolDefinition and monolith-system-prompt's PromptAssembly both import
