...
```ts type-equiv
/** The composed client entry graph the host injects as `window.__MONOLITH_BOOT__`. */
...
The web plugin table service: incremental `monolith.client` scan + wire composition + bundle route + index injection rows. Construction runs the activation scan synchronously — a malformed declaration or missing bundle among the already-loaded entries aggregates into one loud throw (FAILED fiber; the boot activation audit reports it).
