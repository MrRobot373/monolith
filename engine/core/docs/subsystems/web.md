 ...
 * `monolith-tool-web`-layer bound passed through unchanged and enforced on the way
...
interface WebSearchRequest {
   ...
   * bound. `monolith-tool-web` always sets it. A provider whose API supports a
 ...
 * URL-only). `monolith-tool-web` renders `title ?? hostname(url)` for display.
 */
 ...
 * `monolith-web`: the provider decodes the kind and `monolith-tool-web` renders it, so a
