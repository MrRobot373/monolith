...
| [subprocess.md](subprocess.md) | the subprocess seam: fully-explicit `SubprocessSpawnSpec`, offset-based output readers, unclassified `SubprocessOutcome`, and the managed `MONOLITH_*` environment vocabulary |
...
| [client-modules.md](client-modules.md) | the web plugin table: `monolith.client` declarations, `WebBootGraph` wire composition, the bundle route and index tap |
