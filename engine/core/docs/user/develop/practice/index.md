...
- **Service Definition** (`monolith-shell`) — defines the Cordis service and Bash request and result types
- **Service Provider** (`monolith-bash-local`) — executes commands on the local machine
- **Consumer** (`monolith-tool-bash`) — exposes the capability as a model-callable tool

```
┌─────────────┐     ┌──────────────────┐     ┌──────────────┐
│  monolith-shell   │────▶│  monolith-bash-local  │     │ monolith-tool-bash│
...
import { MyCapService, type MyCapRequest, type MyCapResult } from '@monolith/my-cap'
...
```yaml
- name: '@monolith/my-cap-local'
- name: '@monolith/tool-my-cap'
```
