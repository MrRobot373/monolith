...
This opt-in overlay adds a signed GitHub endpoint to `monolith web`. When a pull request in the configured repository changes from draft to ready for review, the rule creates a titled root Session under the repository's Web Workspace and starts a read-only review prompt.

## Prerequisites

- A local checkout that MONOLITH may register as a Web Workspace.
- A high-entropy GitHub webhook secret available through the `MONOLITH_GITHUB_WEBHOOK_SECRET` credential reference.
...
The overlay defaults the Workspace to the launch directory and the listener to `127.0.0.1:3081`. Override them with `MONOLITH_GITHUB_REVIEW_WORKSPACE` and `MONOLITH_GITHUB_WEBHOOK_PORT`.
...
```sh
export MONOLITH_GITHUB_WEBHOOK_SECRET="$(openssl rand -hex 32)"
printf '%s\n' "$MONOLITH_GITHUB_WEBHOOK_SECRET"
```

From a development checkout:

```sh
export MONOLITH_GITHUB_REVIEW_WORKSPACE=/path/to/monolith
pnpm monolith web --patch apps/cli/config/examples/github-review/cordis.yml
```

An installed MONOLITH uses the same overlay through an absolute path:

```sh
monolith web --patch /absolute/path/to/github-review/cordis.yml
```

For a permanent profile, place `github-ready-review-rule.mjs` beside `$MONOLITH_HOME/profiles/web/cordis.patch.yml`, append the rows from `cordis.yml` to that patch, and start with `monolith web`. The shipped CLI already contains both webhook packages; the overlay alone activates them.
...
Secret:       MONOLITH_GITHUB_WEBHOOK_SECRET value
...
The rule accepts only source `primary-github`, repository `monolith/monolith`, event `pull_request`, and action `ready_for_review`. It passes the exact head SHA plus selected PR fields to the review prompt, labeling the JSON as untrusted metadata and forbidding file, branch, PR, or GitHub mutation.
...
```js
const workspacePath = {
  'monolith/monolith': '/path/to/monolith',
  'monolith/monolith-sdk': '/path/to/monolith-sdk',
