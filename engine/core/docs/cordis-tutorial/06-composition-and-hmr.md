...
Because unloading releases effects ([chapter 2](02-lifecycle-and-effects.md)) and loading follows dependencies ([chapter 3](03-services.md)), HMR can replace a running plugin by unloading and loading it. The `@monolith/cordis-plugin-hmr` plugin watches your files and does exactly that on save.

In `tmp/cordis-tutorial`, write `cordis.yml`:

```yaml
- id: logger
  name: '@monolith/cordis-plugin-logger-console'
- id: timer
  name: '@monolith/cordis-plugin-timer'
- id: hmr
  name: '@monolith/cordis-plugin-hmr'
  config:
    root: ['.']
- id: hello
  name: './hello.ts'
