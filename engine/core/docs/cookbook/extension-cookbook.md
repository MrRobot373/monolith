...
import type { PreToolDecision, ToolExecution } from '@monolith/tools'
...
import { createUserMessage } from '@monolith/llm'
...
|---|---|
| Hook system (user + project level) | listeners on `agent/session-start`, `agent/pre-step`, `agent/request`, `tools/pre-execute`, `tools/post-execute`, and `agent/turn-stopping`; the waterfalls return typed decisions, while `agent/turn-stopping` may steer another step; the `monolith-hooks-claude-code` / `monolith-hooks-codex` bridges map hook config files onto these extension points |
| `/goal` | `ctx.goals` owns durable state, `monolith-goal-round-driver` schedules same-session rounds through the public `Agent`, and separate command/tool producers expose human/model control |
...
| Context compaction (auto + manual) | the `ctx.compaction` seam + `monolith-compaction-basic`; automatic pressure runs on serial `agent/pre-step`, canonical overflow recovery runs on `agent/request-error`, and manual callers use the same compact service ([compaction Agent Note](../../.agents/notes/implemented/feature/2026-06-18-compaction-capability-seam.md)) |
...
| Built-in tools | `ctx.tools.register()`; schemas flow into the assembly automatically — the `monolith-tool-*` families (bash, fs, web, subagent, todo) are the shipped examples |
...
| Subprocess sandbox (landlock / sandbox-exec) | use a `ctx.sandbox` backend through `monolith-bash-sandbox`; use `tools/pre-execute` for capability-level denial |
...
| Plan mode | [`@monolith/plan-mode`](../../packages/plan/plan-mode/README.md) — logged `plan/mode` state, the `plan:policy` guidance section, `/plan [message]` entry, `/plan off` direct exit, and the user-reviewed `exit_plan_mode` exit; enforcement stays on the independent sandbox/approval axes |
| Sub-agent delegation | the `ctx.subagents` provider registry (`monolith-subagent-spawn-in-process`/`monolith-subagent-fork-in-process`/`monolith-subagent-acp`/`monolith-subagent-codex`/`monolith-subagent-claude-code`/`monolith-subagent-monolith-sdk`) + `monolith-tool-subagent` exposing one configured provider to the model |
...
| Model adapters | `LlmAdapter` subclass via `registerAdapter` (`monolith-llm-deepseek`, `monolith-llm-pi-ai`) |
