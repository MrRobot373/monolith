...
The two halves live in one package — the Host half under `src/`, the browser half under `src/client/`, exported as `./client` and declared with `monolith.client`. [`packages/client/ui-theme`](../../packages/client/ui-theme) is a worked example of that packaging; the cards this section ships live in [`packages/client/ui-settings-plugins`](../../packages/client/ui-settings-plugins).
...
import type {} from '@monolith/client-ui-settings-plugins/client'
...
```jsonc
{
  ...
  "monolith": { "client": { "platform": "web", "inject": ["@monolith/client-ui-settings-plugins"] } }
}
```
...
export default clientBundle('@monolith/client-my-plugin', ['lib/types/index.js', 'lib/types/invariant.js'])
```
