...
The neutral vocabulary lives in `monolith-tools`; tools never import a UI or transport type. Consumers of this API map each `card` into their own view. The design and the why are in [the render-intent-union Agent Note](../../.agents/notes/implemented/architecture/2026-07-02-tool-render-intent-union.md); `monolith-tool-fs` (generic/diff) and `monolith-tool-bash` (terminal) are the reference implementations.
