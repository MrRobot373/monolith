...
|---|---|---|
| HTTP field names | Lowercase kebab-case; HTTP matching remains case-insensitive | `user-agent`, `x-monolith-session-id` |
| DeepSeek request-body extension fields | Snake case with the reserved `monolith_` prefix | `monolith_plugin_packages`, `monolith_session_log` |
| MONOLITH-owned nested JSON members | Camel case | `afterSeq`, `throughSeq`, `sessionId` |
...
|---|---|---|
| `user-agent` | Every provider HTTP request, including Files API operations | Application identity in `product/version (+url)` form; the default product is `monolith` |
| `x-monolith-user-id` | Every authorized chat-completion request | The stable anonymous UUID for the resolved MONOLITH home |
| `x-monolith-session-id` | Chat-completion requests carrying a Session id | The exact request `sessionId` string |
| `x-monolith-compact` | Chat-completion requests whose purpose is `compaction` | The literal string `1` |

Credential failure happens before anonymous-user-id resolution, so an unauthorized request neither sends these headers nor creates the identity file. A direct request without a Session omits `x-monolith-session-id`. Session-title requests have no additional purpose header; the ordinary Session-id rule still applies when one carries a `sessionId`.
...
## `monolith_plugin_packages`

[`@monolith/plugin-package-inventory-deepseek`](../packages/llm/plugin-package-inventory-deepseek/README.md) contributes the complete active Loader-backed plugin package inventory. The field is enabled by default.
...
| Member | Type | Meaning |
|---|---|---|
| `version` | `1` | Schema version for `monolith_plugin_packages` |
...
An enabled inventory with no qualifying entries sends `packages: []`; disabling the contributor omits the entire `monolith_plugin_packages` field. Package identities are provider metadata and never enter model input.

## `monolith_session_log`
...
| Member | Type | Meaning |
|---|---|---|
| `version` | `1` | Schema version for `monolith_session_log` |
...
The `session` member is the exact `Session.header`, not a complete runtime Session. The outer `monolith_session_log.version` selects this extension schema, while `session.version` selects the canonical on-disk Session format; the two version values evolve independently.
...
The request headers expose the Harness application version, one anonymous MONOLITH-home identity, and an optional Session identity. `monolith_plugin_packages` exposes active npm package names and versions. When enabled, `monolith_session_log` may expose the Session working directory, system-prompt snapshots, user and assistant content, raw assistant chunks, tool arguments and results, compaction summaries, feedback, and plugin-owned events. Adapter API keys are not Session events and therefore do not enter the field. A gateway selected through `baseURL` receives the same values as the official endpoint.
