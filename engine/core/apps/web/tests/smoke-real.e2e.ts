// Real-host smoke: spawn `monolith web` with a real key, walk the full flow
...
function authenticatedWeb(launchUrl: string): Promise<{ origin: string; cookie: string }> {
  ...
  const exchange = (async () => {
    ...
    if (response.status !== 303 || setCookie === null) {
      throw new Error(`monolith web authentication returned HTTP ${String(response.status)}`)
    }
    return {
...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    let out = ''
    const timer = setTimeout(() => { reject(new Error(`monolith web not ready in 90s; output:\n${out}`)) }, 90_000)
    const onData = (chunk: Buffer): void => {
      out += chunk.toString()
      const match = /monolith web: (http:\/\/[^\s]+)/.exec(out)
    ...
    child.once('exit', (code) => {
      clearTimeout(timer)
      reject(new Error(`monolith web exited early (code ${code}); output:\n${out}`))
    })
  })
}
...
// Readiness gate: `monolith web` serves every production manifest plugin; until every UI
...
describe('monolith web keyless CLI smoke', () => {
  it('serves a usable app from two immutable plugin batches', async () => {
    requireDist()
    const sessionsDir = mkdtempSync(join(tmpdir(), 'monolith-web-keyless-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...
          MONOLITH_HOME: join(sessionsDir, '.monolith'),
          MONOLITH_AGENTS_HOME: join(sessionsDir, '.agents'),
    ...
    try {
      ...
      expect(batchPaths).toContainEqual(expect.stringMatching(
        /^\/plugins\/\?\?@monolith\/client-modules\/client\.js&rev=[a-f\d]{12}$/,
      ))
  ...
  it('routes web runtime context and workspace instructions through the real CLI request', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'monolith-web-workspace-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...
          MONOLITH_HOME: join(workspace, '.monolith'),
          MONOLITH_AGENTS_HOME: join(workspace, '.agents'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
        stdio: ['ignore', 'pipe', 'pipe'],
      },
  ...
  it('retries a partial transport failure through the shipped Web composition', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'monolith-web-retry-'))
  ...
  it('MONOLITH_TOOLS_MODE=ptc collapses the provider wire tools to run_code with the SDK prompt section', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'monolith-web-ptc-'))
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  beforeAll(async () => {
    requireDist()
    sessionsDir = mkdtempSync(join(tmpdir(), 'monolith-web-w5-'))
    const port = await probeFreePort()
    // tsx boot mirrors the runtime half of the root monolith script. Isolate
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    // This scenario spawns its own server against a fresh $MONOLITH_HOME with the
