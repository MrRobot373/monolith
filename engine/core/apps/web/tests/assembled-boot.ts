...
import { bootInjections, orderByModuleGraph } from '@monolith/client-modules'
import type { ClientModuleLoaderTarget, WebBootEntry, WebBootGraph } from '@monolith/client-modules/client'
import { AppWebEntry } from '@monolith/client-web'
...
const appBoot = await import(pathToFileURL(webBundleResolver.resolve('@monolith/app-boot')).href) as unknown as BootComposition
...
function resolveClientExport(packagePath: string, pkg: ClientPackageManifest): string {
  ...
  if (relative === undefined) {
    throw new Error(`assembled boot: ${pkg.name ?? packagePath} declares monolith.client without a ./client export`)
  }
...
/** Derive the assembled browser graph from the same bundle patches and package declarations as `monolith web`. */
function loadAssembledPlugins(): readonly AssembledPlugin[] {
  ...
  for (const entry of entries) {
    ...
    const declaration = pkg.monolith?.client
...
const BOOTSTRAP_IDS = ['@monolith/client-modules'] as const
...
interface FixtureWindow extends Window {
  __MONOLITH_BOOT__?: WebBootGraph
...
export function mountAssembledApp(search = '?fixture', options: AssembledBootOptions = {}): void {
  ...
  win.__MONOLITH_BOOT__ = graph
  const [facadeRow] = bootInjections(win.__MONOLITH_BOOT__)
 ...
 * the snapshot gate's `MONOLITH_SNAPSHOT` mode (`record` re-runs the scenarios from
 * scratch, `refresh` re-derives the expected text from the existing ones).
 */
export const REFRESHING_GOLDEN = process.env.MONOLITH_SNAPSHOT === 'record' || process.env.MONOLITH_SNAPSHOT === 'refresh'
