...
the deliberate composition divergences from `monolith web` — are documented in
...
## Do not import `@monolith/client-*` here
...
`@monolith/goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@monolith/client-web` and the boot-manifest
type from `@monolith/client-modules/client`: booting the real shell is what
