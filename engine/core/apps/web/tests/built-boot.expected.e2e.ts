...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  if (clientBuildValue('MONOLITH_CLIENT_BUILD_PROFILE') === 'official') {
    expect(document.querySelector('svg[viewBox="26 0 156 24"]')).not.toBeNull()
    expect(screen.queryByText('MONOLITH Local Build')).toBeNull()
  } else {
    expect(document.querySelector('svg[viewBox="0 0 24 24"]')).not.toBeNull()
    const version = clientBuildValue('MONOLITH_CLIENT_VERSION')
    if (version === undefined) throw new Error('default client build record must carry MONOLITH_CLIENT_VERSION')
    const commit = clientBuildValue('MONOLITH_CLIENT_COMMIT_HASH')
    const buildVersion = version
      + (commit === undefined ? '' : `-${commit}`)
      + (clientBuildValue('MONOLITH_CLIENT_GIT_DIRTY') === 'true' ? '-dirty' : '')
    screen.getByText('MONOLITH Local Build')
  ...
  for (const plugin of ['@monolith/client-ui-layout', '@monolith/client-ui-sidebar', '@monolith/client-ui-conversation', '@monolith/client-ui-tool']) {
...
it('boots without ui-chat and does not select another conversation view implicitly', async () => {
  mountAssembledApp('?fixture', { exclude: ['@monolith/client-ui-chat'] })

  const tree = await screen.findByRole('tree', { name: 'Sessions' }, { timeout: 10_000 })
  const boot = Reflect.get(window, '__MONOLITH_BOOT__') as { entries: Array<{ id: string }> } | undefined
  expect(boot?.entries.some(entry => entry.id === '@monolith/client-ui-chat')).toBe(false)
