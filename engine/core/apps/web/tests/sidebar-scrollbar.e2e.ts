...
function measureList(page: Page): Promise<ListMetrics> {
  return page.evaluate(() => {
    ...
    return {
      ...
      hoverRules,
      token: resolve('--monolith-scrollbar-thumb'),
      hoverToken: resolve('--monolith-scrollbar-thumb-hover'),
...
function renderGeometry(light: PaletteMetrics, dark: PaletteMetrics): string {
  const palette = (name: string, { hovered: metrics, quietThumb }: PaletteMetrics): string[] => [
    `## ${name}`,
    '',
    `- --monolith-scrollbar-thumb, pointer outside the sidebar: ${quietThumb}`,
    ...
    `- --monolith-scrollbar-thumb, pointer over the list: ${metrics.token}`,
    `- --monolith-scrollbar-thumb-hover, pointer over the list: ${metrics.hoverToken}`,
...
/**
 * Resolve `--monolith-scrollbar-thumb` as the list sees it, without the rest of the
...
function resolveThumb(page: Page): Promise<string> {
  return page.evaluate(() => {
    ...
    if (list === null) throw new Error('sidebar session list not in the DOM')
    const probe = document.createElement('span')
    probe.style.color = 'var(--monolith-scrollbar-thumb)'
    list.append(probe)
    const value = getComputedStyle(probe).color
    probe.remove()
    return value
...
describe('web e2e: sidebar session list scrollbar (reserved gutter / themed thumb)', () => {
  ...
  it('renders the themed thumb through the WebKit path in both palettes', async () => {
    ...
    expect(light.hoverRules).toEqual(['var(--monolith-scrollbar-thumb-hover)'])
