...
import type { StreamChunk } from '@monolith/llm'
import {
  ToolCallId,
...
import type { ReplayEntry, ReplayOverrideDoc } from '@monolith/llm-replay'
import type { SessionEvent, SessionSeq } from '@monolith/session'
import {
...
import type {} from '@monolith/session-title'
import {
  launchWebScaffold,
  seedSession,
  watchConsole,
...
async function startMutationProbe(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    Reflect.set(globalThis, '__monolithPerfMutationProbe', probe)
  })
}

async function stopMutationProbe(page: Page): Promise<MutationProbeResult> {
  return page.evaluate(() => {
    const probe = Reflect.get(globalThis, '__monolithPerfMutationProbe') as
    ...
    Reflect.deleteProperty(globalThis, '__monolithPerfMutationProbe')
...
async function startUserRenderProbe(
  ...
  await page.evaluate((expectedMarker) => {
    ...
    Reflect.set(globalThis, '__monolithPerfUserRenderProbe', probe)
  }, marker)
  await send.evaluate((button) => {
    button.addEventListener('click', (event) => {
      const probe = Reflect.get(globalThis, '__monolithPerfUserRenderProbe') as
...
async function stopUserRenderProbe(
  ...
  return page.evaluate(() => {
    const probe = Reflect.get(globalThis, '__monolithPerfUserRenderProbe') as
      ...
      | undefined
    Reflect.deleteProperty(globalThis, '__monolithPerfUserRenderProbe')
    if (
...
async function launchPerformanceWorld(
  ...
  try {
    if (options.replay === undefined) {
      ...
      replayDir = await mkdtemp(join(tmpdir(), 'monolith-web-perf-replay-'))
