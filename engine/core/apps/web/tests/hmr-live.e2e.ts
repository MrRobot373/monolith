/** Published monolith web + pnpm dev:web → browser HMR, with no page reload. */
...
import type { SubprocessHandle, SubprocessSpawnSpec } from '@monolith/subprocess'
...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  const world = await mkdtemp(join(tmpdir(), 'monolith-web-hmr-world-'))
  const sourcePath = join(REPO_ROOT, 'packages/client/ui-conversation/src/client/locales.ts')
  const binPath = join(REPO_ROOT, 'apps/cli/lib/bin.js')
  if (!existsSync(binPath)) throw new Error('HMR browser test needs the built monolith bin; run pnpm run build first')
  ...
  try {
    ...
    host = subprocessCtx.subprocess.spawn(spawnSpec(
      ...
      world,
      {
        ...
        MONOLITH_HOME: join(world, '.monolith'),
      },
    ))
    const baseUrl = await waitForOutput(host, /monolith web: (http:\/\/[^\s]+)/, 'built monolith web')
    ...
    const pageIdentity = await page.evaluate(() => {
      ...
      Object.defineProperty(window, '__monolithHmrPageIdentity', { value: identity })
      return identity
    })

    await writeFile(sourcePath, updatedSource)
    await page.getByText(newText, { exact: true }).waitFor({ timeout: 30_000 })
    expect(await page.evaluate(() => (window as Window & { __monolithHmrPageIdentity?: string }).__monolithHmrPageIdentity))
      .toBe(pageIdentity)
