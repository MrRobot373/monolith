...
// Boots the REAL web composition — the monolith-base and monolith-web-app bundle
...
// downlink, api-gateway, agent loop, tools, and persistence. Modes ride $MONOLITH_SNAPSHOT:
// replay (default, keyless: normally disables the llm-deepseek row and
// inserts monolith-llm-replay in providers mode), record (real adapter + key,
...
//
// Composition divergences from `monolith web`, all deliberate, all via include
...
import Include, { type PatchOptions } from '@monolith/cordis-plugin-include'
import Group from '@monolith/cordis-plugin-group'
import {
...
} from '@monolith/session-snapshot'
import {
  assertEntriesLoaded,
  composeEntries,
  ...
  loadOverlayPatches,
  type Profile,
} from '@monolith/app-boot'
import { monolithHomePath } from '@monolith/home-paths'
import { LlmAdapter } from '@monolith/llm'
import type {
  LlmModelInfo, LlmProviderInfo, LlmResolvedModelInfo, RetryPolicyConfig, StreamChunk,
} from '@monolith/llm'
import type { ReplayHandle } from '@monolith/llm-replay'
import { installLlmReplay, parseSessionLog } from '@monolith/llm-replay'
import {
  packChunkRuns,
...
import JsonlSessionPersistence from '@monolith/session-persistence-jsonl'
// Empty type imports carry the webServer/agents/sessionPersistence Context merges.
import type {} from '@monolith/host-webserver'
import type {} from '@monolith/agent'
import { provideCmdline } from '@monolith/cmdline'
...
// } from '@monolith/client-ui-settings-models'
...
export const WELCOME_NOTICE_COPY = {
  zh: {
    title: '内测声明',
    body: 'MONOLITH 目前的 0.1 版本仍处在面向 MONOLITH 开发者进行测试的阶段，还有许多地方需要持续改进和打磨，希望听取广大开发者的反馈建议。预计 MONOLITH 的核心插件以及基础 API 都会在接下来的一段时间内快速迭代、持续演化。\n\n我们期待与全球开发者一起，在开源、开放、可复用、可组合的基础设施之上，共同探索智能上限。欢迎全球 MONOLITH 开发者加入 MONOLITH 插件生态。',
    continueLabel: '继续',
  },
} as const

/** Snapshot mode for the lane, from $MONOLITH_SNAPSHOT (same vocabulary as the other snapshot suites). */
...
export function webSnapshotMode(): WebSnapshotMode {
  const value = process.env.MONOLITH_SNAPSHOT
  if (value === undefined || value === '' || value === 'replay') return 'replay'
  if (value === 'record' || value === 'refresh') return value
  throw new Error(`MONOLITH_SNAPSHOT must be replay, record, or refresh; got ${JSON.stringify(value)}`)
}
...
/** The shipped composition under test: the monolith-base and monolith-web-app bundle patches over the empty profile root. */
...
export interface WebScaffold {
  ...
  /** Isolated harness home the settings/credentials rows write ($MONOLITH_HOME double). */
  harnessHome: string
...
export interface LaunchOptions {
  ...
  /**
   * Replay fixture (session.jsonl) served by the inserted monolith-llm-replay row
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const workspaceCwd = await realpath(await mkdtemp(join(tmpdir(), 'monolith-web-e2e-ws-')))
  // Isolated harness home: the settings/credentials rows resolve $MONOLITH_HOME
  // paths at load, and an in-process boot must NEVER touch the developer's
  // real ~/.monolith document or credential file.
  const harnessHome = options.harnessHome ?? join(workspaceCwd, '.monolith-home')
  ...
  // session is created. Without this a developer's real ~/.monolith/skills silently
  // enters replay requests and goldens while CI sees none. `MONOLITH_HOME` follows
  ...
  const skillRootEnvironment = {
    MONOLITH_HOME: harnessHome,
    MONOLITH_AGENTS_HOME: join(workspaceCwd, '.agents-home'),
    MONOLITH_BUNDLED_SKILL_DIR: join(workspaceCwd, '.bundled-skills'),
  }
  ...
  try {
    persistenceRoot = await mkdtemp(join(tmpdir(), 'monolith-web-e2e-sessions-'))
  } catch (error) {
  ...
  // (bundle patches in monolith.profile.bundles order), applied over the SAME empty root (a
  ...
  const patches: PatchOptions[] = [
    ...
    // `monolith-agent-presets` and prepended by it. Pin only the machine-local
    // root away: a developer's own `~/.monolith/.agent-presets` must not be able
    // to change a golden.
    {
      id: 'agent-presets',
      config: {
    ...
    // storage-json's yml root is anchored to the real $MONOLITH_HOME; pin the row
    // to an absolute temp root (removed with the workspace at close) so tests
    // never write the user's harness home.
    { id: 'storage-json', config: { root: join(workspaceCwd, '.monolith-storages') } },
    // Skill discovery is model-visible input. Pin every host-level root inside
    // the owned temp world so ~/.monolith, ~/.agents, and a bundled-root env setting
    ...
    // enabled against the same empty temp workspace, preserving the real seam.
    {
      id: 'skill-filesystem',
      config: {
        monolithHome: join(workspaceCwd, '.monolith-home'),
    ...
    // to the production OTLP endpoint (or whatever MONOLITH_TELEMETRY_OTLP_URL
    ...
    // (apps/web IS @monolith/web-frontend); native browser opening and the
    ...
    { id: 'settings', config: { monolithHome: harnessHome } },
    { id: 'credentials', config: { monolithHome: harnessHome } },
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@monolith/host-directory-picker-browse' },
      { id: 'ui-directory-picker-browse', name: '@monolith/client-ui-directory-picker-browse' },
    ] },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@monolith/tool-cordis' },
      ] }]
      : [],
  ...
  try {
    ...
    ctx.provide('monolithHomePath', monolithHomePath)
    ...
    // `@monolith/cordis-plugin-group` by name.
...
export async function compareOrRefreshGolden(goldenPath: string, actual: string, mode: WebSnapshotMode): Promise<void> {
  ...
  if (!existsSync(goldenPath)) {
    throw new Error(`missing golden ${goldenPath} — run MONOLITH_SNAPSHOT=refresh pnpm run test:web to generate it`)
  }
