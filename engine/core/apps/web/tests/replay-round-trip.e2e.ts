...
// in the temp workspace) all run; the model adapter is monolith-llm-replay (keyless)
...
// Record: MONOLITH_SNAPSHOT=record rewrites session.jsonl, then a keyless
// MONOLITH_SNAPSHOT=refresh regenerates ui.expected.md.
...
describe('web e2e: fresh round trip through the real assembly', () => {
  ...
  it.skipIf(MODE === 'record')('renders the system prompt disclosure inside the expanded Turn process', async () => {
    ...
    expect(await opaque.textContent()).toContain('You are an AI agent powered by MONOLITH.')
