...
// monolith-llm-replay with override
...
import type { RetryPolicyConfig } from '@monolith/llm'
import { deriveReplayScript, parseSessionLog } from '@monolith/llm-replay'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  async function launch(
    ...
    if (buildOverride !== undefined) {
      ...
      sidecarDir = await mkdtemp(join(tmpdir(), 'monolith-web-e2e-sidecar-'))
