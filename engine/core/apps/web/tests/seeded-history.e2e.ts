...
// fixture under the same record discipline as every other: MONOLITH_SNAPSHOT=record drives the turn
...
import type { ContentBlock, Message } from '@monolith/llm'
import { deriveEventMessage, SessionId } from '@monolith/session'
...
describe('web e2e: seeded history renders through cold resume', () => {
  ...
  it.skipIf(MODE === 'record')('reports full feedback correlation ids in an expandable two-line row', async () => {
    ...
    process.env.MONOLITH_HOME = scaffold.harnessHome
    try {
      const input = page.locator('[data-composer-input]').first()
