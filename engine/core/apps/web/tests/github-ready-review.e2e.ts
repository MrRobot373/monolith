...
import type { GenerateOptions, StreamChunk } from '@monolith/llm'
import { LlmAdapter } from '@monolith/llm'
import type {} from '@monolith/webhook'
import {
...
const TITLE = 'Review monolith/monolith#314'
...
describe.skipIf(MODE === 'record')('web e2e: GitHub ready-for-review', () => {
  ...
  beforeAll(async () => {
    previousPort = process.env.MONOLITH_GITHUB_WEBHOOK_PORT
    previousSecret = process.env.MONOLITH_GITHUB_WEBHOOK_SECRET
    const port = await freePort()
    process.env.MONOLITH_GITHUB_WEBHOOK_PORT = String(port)
    process.env.MONOLITH_GITHUB_WEBHOOK_SECRET = SECRET
    ...
    await page.addInitScript(() => { localStorage.setItem('monolith.locale', 'en') })
  ...
  afterAll(async () => {
    ...
    if (previousPort === undefined) Reflect.deleteProperty(process.env, 'MONOLITH_GITHUB_WEBHOOK_PORT')
    else process.env.MONOLITH_GITHUB_WEBHOOK_PORT = previousPort
    if (previousSecret === undefined) Reflect.deleteProperty(process.env, 'MONOLITH_GITHUB_WEBHOOK_SECRET')
    else process.env.MONOLITH_GITHUB_WEBHOOK_SECRET = previousSecret
  })
  ...
  it('isolates ingress and creates a browsable Workspace Session', async () => {
    ...
    const payload = {
      ...
      pull_request: {
        ...
        html_url: 'https://github.com/monolith/monolith/pull/314',
        draft: false,
        user: { login: 'octocat' },
