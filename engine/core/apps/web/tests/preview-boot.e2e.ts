...
} from '@monolith/experimental-webworker-packer'
import {
...
function requireVfsAssets(): PreviewAssets {
  ...
  const directory = mkdtempSync(join(tmpdir(), 'monolith-preview-boot-'))
...
async function bootPreview(origin: string, browser: Browser): Promise<void> {
  ...
  try {
    ...
    const exercised = await page.evaluate(async () => {
      ...
      const transport = (globalThis as typeof globalThis & { __MONOLITH_TRANSPORT__?: PreviewTransport }).__MONOLITH_TRANSPORT__
      ...
      await createDirectory('/monolith/workspace/.agents/skills', 'runtime-created')
  ...
  try {
    ...
    const sessionCount = await page.evaluate(async () => {
      const transport = (globalThis as typeof globalThis & {
        __MONOLITH_TRANSPORT__?: { fetch(input: string, init: RequestInit): Promise<Response> }
