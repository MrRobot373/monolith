...
const STANDALONE_ERROR = 'apps/web is not a standalone application: bare Vite cannot inject window.__MONOLITH_BOOT__. '
  + 'From a repository checkout, run `pnpm monolith web`; an installed package uses `monolith web`. '
  + 'For client-plugin HMR, run `pnpm monolith web` together with `pnpm run dev:web`.'
const DEFAULT_CLIENT_TITLE = 'MONOLITH Local Build'
...
function clientDocumentTitle(): Plugin {
  const title = escapeHtmlText(process.env.MONOLITH_CLIENT_TITLE ?? DEFAULT_CLIENT_TITLE)
  return {
    name: 'monolith-client-document-title',
    transformIndexHtml(html) {
      return html.replace('<title>MONOLITH Local Build</title>', `<title>${title}</title>`)
    },
  }
}
...
function rejectStandaloneServe(): Plugin {
  return {
    name: 'monolith-reject-standalone-web-serve',
    config(_config, env) {
...
function emitPreviewPage(): Plugin {
  ...
  return {
    name: 'monolith-emit-preview-page',
