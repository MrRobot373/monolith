...
import { boot, healProfilesModuleFallback, loadOverlayPatches, loadProfile } from '@monolith/app-boot'
import { provideCmdline } from '@monolith/cmdline'
import { SessionId, SessionLogOffset } from '@monolith/session'
import type { Agent } from '@monolith/agent'
import type { PatchOptions } from '@monolith/cordis-plugin-include'
import { afterAll, beforeAll, describe, expect, it, vi } from 'vitest'
import { SUBAGENT_MODEL_SELECTION_SETTINGS_NAMESPACE } from '@monolith/tool-subagent/model-selection-settings'
import { SETTINGS_NAMESPACE, SHIPPED_PRESET_ROOT } from '@monolith/agent-presets'
import { applyChildComposition, childSessionMeta } from '@monolith/subagent'
import { ToolCallId } from '@monolith/llm'
import type {} from '@monolith/compaction-basic'
import type {} from '@monolith/skill'
import type {} from '@monolith/tools'
// Type-only: resolves `ctx.get('sessionProjections')` and `ctx.get('tokenMeter')`.
import type {} from '@monolith/session-projection'
import type {} from '@monolith/token-meter'

const REPO_ROOT = fileURLToPath(new URL('../../..', import.meta.url))
/** The shipped Web surface: the monolith-base and monolith-web-app bundle patches over an empty preset root. */
...
async function bootWeb(
  ...
  const overrides: PatchOptions[] = [
    // The settings row defaults to `$MONOLITH_HOME/settings.yaml`. Left alone it
    ...
    // storage-json's root is anchored to the real $MONOLITH_HOME. Unpinned, this
    // file writes the developer's own `~/.monolith/storages/` — and then reads it
    ...
    // false keeps `~/.monolith/.agent-presets` from changing a test's outcome.
  ...
  // Product Bundles are installed into the Profile, not the monolith app. Model
  ...
  let bundlePatches: PatchOptions[] = [
    ...loadOverlayPatches('monolith-test', BASE_PATCH),
    ...loadOverlayPatches('monolith-test', WEB_PATCH),
  ]
  if (profileBundles !== undefined) {
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      private: true,
      dependencies: Object.fromEntries(profileBundles.map(name => [name, 'workspace:*'])),
      monolith: { profile: { bundles: profileBundles } },
    }, null, 2) + '\n')
    const profile = loadProfile('monolith-test', 'spec', INSTALL_ANCHOR, home, { userLayer: false })
    bundlePatches = profile.layers.flatMap(layer => layer.patches)
  }
  const rootConfig = join(profileDir, 'cordis.yml')
  await writeFile(rootConfig, '[]\n')
  return await boot('monolith-test', rootConfig, [...bundlePatches, ...overrides], (bootCtx) => {
...
let ctx: Context
beforeAll(async () => {
  const settingsFile = join(await mkdtemp(join(tmpdir(), 'monolith-web-presets-')), 'settings.yaml')
...
describe('the shipped Web composition', () => {
  ...
  it('merges the global skill layer into a preset agent\'s catalog, keeping local discovery preset-side', async () => {
    const proj = await mkdtemp(join(tmpdir(), 'monolith-preset-skill-proj-'))
    await mkdir(join(proj, '.monolith', 'skills', 'project-proof'), { recursive: true })
    await writeFile(join(proj, '.monolith', 'skills', 'project-proof', 'SKILL.md'), [
      '---',
      'name: project-proof',
    ...
    const handle = await ctx.agents.create({
      // Unique per run: the composition persists into the ambient MONOLITH home,
    ...
    try {
      ...
      expect((await ctx.skills.list({ cwd: proj })).map(skill => skill.name)).toEqual(['monolith-badge'])
      ...
      expect(scoped).toContain('monolith-badge')
      ...
      expect(JSON.stringify(loaded.content)).toContain('powered by monolith')
    } finally {
      await handle.dispose()
    }
  })
  ...
  it('shows a minimal agent the global layer but no loader tool', async () => {
    ...
    try {
      ...
      expect((await ctx.skills.list({ scope: handle.agent })).map(skill => skill.name)).toContain('monolith-badge')
...
describe('product Bundle and user-preset intersection', () => {
  ...
  async function bootProducts(installed: readonly Product[]): Promise<Context> {
    const root = await mkdtemp(join(tmpdir(), 'monolith-product-presets-'))
    ...
    const packageName = (product: Product): string => (
      product === 'codex'
        ? '@monolith/subagent-codex'
        : '@monolith/subagent-claude-code'
    )
...
describe('a launcher that configures no writable root', () => {
  ...
  // roster derives `<monolithHome>/.agent-presets` itself. `$MONOLITH_HOME` is pointed
  ...
  beforeAll(async () => {
    const home = await mkdtemp(join(tmpdir(), 'monolith-preset-derived-'))
    previousHome = process.env.MONOLITH_HOME
    process.env.MONOLITH_HOME = home
    await mkdir(join(home, '.agent-presets', 'derived-mine'), { recursive: true })
    await writeFile(
      join(home, '.agent-presets', 'derived-mine', 'agent.cordis.yml'),
      '- id: tool-todo\n  name: \'@monolith/tool-todo\'\n  config:\n    allowParallelInProgress: true\n',
    )
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'monolith-preset-derived-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
  ...
  afterAll(async () => {
    if (previousHome === undefined) delete process.env.MONOLITH_HOME
    else process.env.MONOLITH_HOME = previousHome
...
describe('authoring a preset on the shipped composition', () => {
  ...
  beforeAll(async () => {
    userRoot = join(await mkdtemp(join(tmpdir(), 'monolith-preset-authoring-')), 'profiles')
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'monolith-preset-authoring-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
...
describe('a composition that configures its own preset roots', () => {
  ...
  beforeAll(async () => {
    const home = await mkdtemp(join(tmpdir(), 'monolith-preset-roots-'))
