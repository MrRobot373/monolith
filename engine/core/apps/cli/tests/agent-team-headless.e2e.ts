...
describe('monolith run with Agent Teams enabled', () => {
  it('runs two teammates, durable peer mail, dependent tasks, waiting, and final aggregation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'monolith-agent-team-headless-'))
    try {
      ...
      await writeFile(join(profileDir, 'package.json'), JSON.stringify({
        ...
        private: true,
        dependencies: {
          '@monolith/experimental-agent-team-profile': 'workspace:^',
        },
        monolith: {
          profile: {
            bundles: [
      ...
      expect(
        result.exitCode,
        `monolith headless profile exited unexpectedly.\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`,
      ).toBe(0)
