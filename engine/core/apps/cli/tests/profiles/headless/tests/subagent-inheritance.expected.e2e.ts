...
import { normalizeSessionSnapshot, type NormalizeContext } from '@monolith/session-snapshot'
import { LOADER_SMOKE_TEST_TIMEOUT_MS, runLoaderSmoke } from '@monolith/loader-smoke'
import { createUserMessage, ReasoningEffortId } from '@monolith/llm'
import { SessionSeq, SESSION_FORMAT_VERSION, SessionId, type SessionEvent, type SessionHeader } from '@monolith/session'
...
describe('parent-only override inheritance snapshot', () => {
  it('confines a delegated child through the assembled headless app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'monolith-subagent-inherit-',
      binScript,
      ...
      env: {
        ...
        MONOLITH_SNAPSHOT_FILE: replayOverride,
        MONOLITH_SNAPSHOT_OVERRIDE: replayOverride,
      ...
      inspect: async (runCwd) => {
        ...
        const runtimeContexts = (content: string): string[] => content.trimEnd().split('\n').flatMap((line) => {
          ...
          if (record.type !== 'user/message'
            ...
            || record.data.source.plugin !== '@monolith/system-prompt') return []
        ...
        for (const context of policyContexts) {
          expect(context).toContain('Any available operation enforced by the MONOLITH file sandbox cannot modify files in the standing mode.')
