...
import LlmRuntime, { createUserMessage, ToolCallId, HarnessError  } from '@monolith/llm'
...
import ToolRuntime, { RUN_CODE_NAME, defineTool } from '@monolith/tools'
import type { ToolExecutionResult } from '@monolith/tools'
...
import { WorkerThreadCodeRuntime } from '@monolith/code-runtime-worker-thread'
...
describe('PTC mode typed values: keyless real-worker contracts', () => {
  ...
  it('returns a background job id, settles the outer run, and polls that id to completion', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'monolith-ptc-background-'))
  ...
  it('pre-abort spawns nothing; post-publication abort leaves job_kill as the cancellation owner', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'monolith-ptc-task-cancel-'))
    ctx = await backgroundPtcModeHarness(workdir)
  ...
  it('keeps foreground bash coupled to the outer signal', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'monolith-ptc-foreground-cancel-'))
    ctx = await backgroundPtcModeHarness(workdir)
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('PTC mode: real model writes a program over real tools', () => {
  it('collapses the wire tool list to [run_code], bridges sub-calls, and returns curated output', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'monolith-ptc-e2e-'))
  ...
  it('projects nested workspace instructions discovered by an fs sub-call', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'monolith-ptc-workspace-e2e-'))
