...
import { renderWorkspaceContext } from '@monolith/agent-instructions'
import { resolveConfig, workspaceBaselineIdentity } from '@monolith/agent-instructions/src/config.ts'
...
async function seedVisibleBaseline(
  ...
  const config = resolveConfig({
    monolithHome: join(cwd, '.monolith'),
    maxBytes: 65536,
...
describe('agent-instructions resume snapshot', () => {
  it('appends an offline replacement without duplicating the visible baseline', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'monolith-workspace-context-resume-',
      binScript,
      ...
      tsconfigPath,
      env: {
        MONOLITH_SNAPSHOT_FILE: replayFixture,
  ...
  it('supersedes an incompatible baseline when precedence changed offline', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'monolith-workspace-context-precedence-',
      binScript,
      libBinScript: binScript,
      configPath,
      binArgs: [configPath, 'Acknowledge the current workspace instruction.'],
