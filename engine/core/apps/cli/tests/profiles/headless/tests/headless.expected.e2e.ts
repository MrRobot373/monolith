...
import {
  decompressZstdFrame,
  scanZstdFrames,
} from '@monolith/session-persistence-jsonl/src/zstd.ts'
...
const refreshing = process.env.MONOLITH_SNAPSHOT === 'refresh'
...
describe('headless stream-json snapshots', () => {
  ...
  it('runs one task through the product headless profile command', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      inspect: async (cwd) => {
        const logs = await persistedLogs(cwd, join(cwd, '.monolith', 'sessions'))
  ...
  it('logs actionable missing-credential guidance through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tsconfigPath,
      env: {
        // First-run posture: no key in the environment, none under ./.monolith.
        DEEPSEEK_API_KEY: '',
        DEEPSEEK_BASE_URL: '',
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
    ...
    try {
      const result = await runLoaderSmoke({
        ...
        env: {
          ...
          MONOLITH_SNAPSHOT_BASE_URL: server.url,
          NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
        },
      })
  ...
  it('replays persisted goal tools through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        MONOLITH_SNAPSHOT: 'replay',
        MONOLITH_SNAPSHOT_FILE: join(goalScenarioDir, 'session.jsonl'),
        MONOLITH_SNAPSHOT_OVERRIDE: join(goalScenarioDir, 'replay.override.json'),
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
  ...
  it('delivers a continuable child result without parent polling', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ...
        MONOLITH_SNAPSHOT_FILE: parentReplay,
        MONOLITH_SNAPSHOT_OVERRIDE: parentOverride,
        MONOLITH_SNAPSHOT_CHILD_FILES: childReplay,
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
