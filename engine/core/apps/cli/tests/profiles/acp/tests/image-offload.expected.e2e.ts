...
it('pins native DeepSeek Files offload and inline fallback in assembled requests', async () => {
      ...
      void (async () => {
        ...
        if (url.pathname === '/files' && request.method === 'POST') {
          ...
          response.writeHead(200, { 'content-type': 'application/json' }).end(JSON.stringify({
            ...
            filename: 'monolith-snapshot.png',
            purpose: 'user_data',
  ...
  try {
    const result = await runScenario(input, {
      ...
      env: {
        MONOLITH_SNAPSHOT_API_KEY: 'snapshot-key',
        MONOLITH_SNAPSHOT_BASE_URL: `http://127.0.0.1:${address.port}`,
      },
    })
    ...
    const fallback = await runScenario(input, {
      ...
      workspaceDir: READ_IMAGE_WORKSPACE,
      env: {
        MONOLITH_SNAPSHOT_API_KEY: 'snapshot-fallback-key',
        MONOLITH_SNAPSHOT_BASE_URL: `http://127.0.0.1:${address.port}`,
      },
    })
