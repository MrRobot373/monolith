/** Generic keyless ACP v1 automation-control conformance over the real monolith profile. */
...
describe('standard ACP v1 control surface', () => {
  it('selects, mounts MCP, closes, restarts, resumes, and cancels through the SDK only', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'monolith-acp-control-'))
    const persistenceRoot = join(cwd, '.sessions')
    const env = { MONOLITH_CONFORMANCE_PERSISTENCE_ROOT: persistenceRoot, MONOLITH_TELEMETRY_DISABLED: '1' }
