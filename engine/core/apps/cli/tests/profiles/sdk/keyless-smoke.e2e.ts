...
describe('Python SDK monolith profile keyless smoke', () => {
  it.each([
    ...
    const root = await mkdtemp(join(tmpdir(), 'monolith-python-sdk-runtime-smoke-'))
    ...
    const child = execa(process.execPath, [
      ...
      env: {
        ...
        ...(envValue === undefined ? {} : { MONOLITH_MAX_TOKENS_AS_SUCCESS: envValue }),
      },
      timeout: 35_000,
      killSignal: 'SIGKILL',
      reject: false,
    ...
    try {
      ...
      const sessionsRoot = join(root, '.monolith', 'sessions')
  ...
  it('boots the standalone minimal profile through its generated manifest', async () => {
    const root = await mkdtemp(join(tmpdir(), 'monolith-python-sdk-minimal-'))
    const modelServer = createServer((request, response) => {
      request.resume()
      request.on('end', () => {
        response.writeHead(200, { 'content-type': 'text/event-stream' })
    ...
    const child = execa(process.execPath, [
      ...
      env: {
        MONOLITH_HOME: join(root, '.monolith'),
        MONOLITH_SYSTEM_PROMPT: 'Minimal allowlist prompt.',
        DEEPSEEK_API_KEY: 'keyless-smoke-no-call',
        DEEPSEEK_BASE_URL: `http://127.0.0.1:${address.port}`,
      },
      timeout: 35_000,
    ...
    try {
      ...
      const profile = JSON.parse(
        await readFile(join(root, '.monolith', 'profiles', 'sdk-minimal', 'package.json'), 'utf8'),
      ) as { monolith?: { profile?: { bundles?: string[]; patchReload?: string } } }
      expect(profile.monolith?.profile).toEqual({
  ...
  it('rejects an invalid max-token success env value', async () => {
    const root = await mkdtemp(join(tmpdir(), 'monolith-python-sdk-runtime-invalid-'))
    try {
      const { exitCode, stdout, stderr } = await execa(process.execPath, [
        ...
        env: {
          MONOLITH_HOME: join(root, '.monolith'),
          DEEPSEEK_API_KEY: 'keyless-smoke-no-call',
          MONOLITH_MAX_TOKENS_AS_SUCCESS: 'sometimes',
        },
        stdin: 'ignore',
        timeout: 25_000,
        killSignal: 'SIGKILL',
      ...
      expect(stderr).toContain('failed to apply loader entry sdk-jsonrpc-server (@monolith/sdk-jsonrpc-server)')
