...
import { decodeStorageRecord } from '@monolith/session/chunk-rows'
...
const MARKER = 'MONOLITH_GITHUB_WEBHOOK_REAL_E2E_OK'
...
function observeProcess(child: ChildProcess): ProcessObservation {
  ...
  const timer = setTimeout(() => {
    if (!settled) rejectReady(new Error(`monolith web did not become ready within 90s:\n${output}`))
  }, 90_000)
  timer.unref()
  const append = (chunk: Buffer | string): void => {
    output = `${output}${String(chunk)}`.slice(-100_000)
    const match = /monolith web: (http:\/\/[^\s]+)/u.exec(output)
  ...
  child.once('exit', (code) => {
    if (!settled) rejectReady(new Error(`monolith web exited before readiness (code ${String(code)}):\n${output}`))
  })
...
async function eventually<T>(
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`monolith web exited while waiting for ${label} (code ${String(child.exitCode)}):\n${processOutput()}`)
    }
    try {
...
async function sendGitHubDelivery(origin: string): Promise<Response> {
  const body = JSON.stringify({
    ...
    number: 4242,
    repository: { full_name: 'monolith/monolith' },
    pull_request: {
      title: 'Real CLI webhook e2e',
      html_url: 'https://github.com/monolith/monolith/pull/4242',
      draft: false,
      user: { login: 'octocat' },
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('GitHub webhook through the real monolith CLI and model', () => {
  it('creates, attaches, prompts, and completes a Workspace Session', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'monolith-github-webhook-real-'))
    ...
    const child = spawn(process.execPath, [
      ...
      env: {
        ...
        MONOLITH_GITHUB_E2E_MARKER: MARKER,
        MONOLITH_GITHUB_E2E_WORKSPACE: workspacePath,
        MONOLITH_GITHUB_WEBHOOK_PORT: String(webhookPort),
        MONOLITH_GITHUB_WEBHOOK_SECRET: SECRET,
