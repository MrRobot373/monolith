 ...
 * web profile (monolith-base + monolith-web-app bundle patches, auto-initialized).
...
const requireBuiltArtifacts = process.env.MONOLITH_REQUIRE_BUILT_CLI_SMOKE === '1'
...
function runBuiltWeb(cwd: string): Promise<{ stdout: string; stderr: string; code: number }> {
  return new Promise((resolveRun, rejectRun) => {
    const env: NodeJS.ProcessEnv = {
      ...process.env,
      DEEPSEEK_API_KEY: 'monolith-cli-smoke-dummy-key',
    ...
    child.stdout.on('data', (chunk: string) => {
      stdout += chunk
      if (!settled && /monolith web: http:\/\/127\.0\.0\.1:\d+/u.test(stdout)) {
        settled = true
        child.kill('SIGTERM')
      }
    })
...
describe.skipIf(!requireBuiltArtifacts)('built CLI lazy-search startup', () => {
  it('boots and disposes the shipped composition with full-text search off by default', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'monolith-cli-lazy-search-'))
    try {
      const result = await runBuiltWeb(cwd)
      expect(result.stdout).toMatch(/monolith web: http:\/\/127\.0\.0\.1:\d+/u)
