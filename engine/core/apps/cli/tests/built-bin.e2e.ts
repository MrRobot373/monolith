...
import { startMockLlmServer } from '@monolith/llm-mock-server'
...
// The monolith built bin cold-starts slowly on the contended self-hosted Windows pool; the
...
const monolithBin = join(repoRoot, 'apps/cli/lib/bin.js')
...
async function runBuiltBin(
  ...
  const result = await execa(process.execPath, [monolithBin, ...args], {
    input: '',
  ...
  if (result.timedOut) {
    throw new Error(`monolith built bin did not exit within ${SPAWN_TIMEOUT_MS / 1_000}s. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
  }
...
async function waitForFile(file: string): Promise<void> {
  ...
  while (!existsSync(file)) {
    if (Date.now() >= deadline) throw new Error(`monolith profile lifecycle marker did not appear: ${file}`)
 ...
 * monolith.profile.bundles, no monolith-base — proving out-of-box composition machinery without
 * booting the entire product tree.
 */
function createProfileLifecycleFixture(): ProfileLifecycleFixture {
  const home = mkdtempSync(join(tmpdir(), 'monolith-profile-lifecycle-'))
  ...
  writeFileSync(join(bundleDir, 'plugin.mjs'), [
    ...
    "  writeFileSync(join(process.env.MONOLITH_HOME, 'config-echo'), String(config.generation ?? 'bundle-default'))",
  ...
  writeFileSync(join(bundleDir, 'package.json'), JSON.stringify({
    name: 'monolith-lifecycle-bundle',
    version: '0.0.0',
    type: 'module',
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'monolith-profile-lifecycle',
    private: true,
    dependencies: {},
    monolith: { profile: { bundles: ['monolith-lifecycle-bundle'] } },
  }, undefined, 2))
  // Hand-place the "installed" bundle where profile resolution finds it.
  writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
  const linkTarget = join(profileDir, 'node_modules', 'monolith-lifecycle-bundle')
  mkdirSync(join(profileDir, 'node_modules'), { recursive: true })
  try {
...
function startProfileLifecycle(fixture: ProfileLifecycleFixture, args: readonly string[] = []) {
  return execa(process.execPath, [monolithBin, '--profile', 'lifecycle', ...args], {
    cwd: fixture.home,
    input: '',
    timeout: SPAWN_TIMEOUT_MS,
    killSignal: 'SIGKILL',
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'monolith-profile-environment-probe',
    private: true,
    dependencies: {},
    monolith: { profile: { bundles: ['@monolith/base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@monolith/cmdline` and `commander` through the profile module
 * fallback, exactly as an installed out-of-tree bundle does.
 */
function createStartupFixture(): StartupFixture {
  const home = mkdtempSync(join(tmpdir(), 'monolith-profile-startup-'))
  ...
  const bundleDir = join(profileDir, 'node_modules', 'monolith-startup-bundle')
  mkdirSync(bundleDir, { recursive: true })
  writeFileSync(join(bundleDir, 'startup.mjs'), [
    "import { Command } from 'commander'",
    "import { parseCmdline } from '@monolith/cmdline'",
  ...
  writeFileSync(join(bundleDir, 'witness.mjs'), [
    ...
    'export function apply(ctx, config = {}) {',
    "  writeFileSync(join(process.env.MONOLITH_HOME, 'witness'), String(config.generation ?? 'bundle-default'))",
    '}',
    '',
  ].join('\n'))
  writeFileSync(join(bundleDir, 'cordis.patch.yml'), [
  ...
  writeFileSync(join(bundleDir, 'package.json'), JSON.stringify({
    name: 'monolith-startup-bundle',
    version: '0.0.0',
    type: 'module',
    monolith: { bundle: { patch: './cordis.patch.yml' } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'monolith-profile-startup',
    private: true,
    dependencies: {},
    monolith: { profile: { bundles: ['monolith-startup-bundle'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
  return {
    home,
...
function startStartupProfile(fixture: StartupFixture, args: readonly string[]) {
  return execa(process.execPath, [monolithBin, '--profile', 'startup', ...args], {
    cwd: fixture.home,
    input: '',
    reject: false,
    timeout: SPAWN_TIMEOUT_MS,
...
describe.skipIf(!existsSync(monolithBin))('monolith BUILT bin (node lib/bin.js, no tsx)', () => {
  it('requires --profile and rejects removed commands', async () => {
    ...
    expect(help.stdout).toContain('monolith --profile web')
    expect(help.stdout).toContain('monolith plugin --profile')
  ...
  it('routes help and usage errors without activating startup-dependent rows', async () => {
    const home = mkdtempSync(join(tmpdir(), 'monolith-app-help-'))
    try {
      const web = await runBuiltBin(['--profile', 'web', '--help'], {
        MONOLITH_HOME: home,
        MONOLITH_TELEMETRY_DISABLED: '1',
      })
      expect(web.code).toBe(0)
      expect(web.stderr).toBe('')
      expect(web.stdout).toContain('Usage: monolith --profile web')
      expect(web.stdout).toContain('--port <port>')
      expect(web.stdout).not.toContain('monolith web: http://')
      ...
      expect(wildcardHost.stderr).not.toContain('monolith web: http://')
      ...
      expect(headlessHelp.stdout).toContain('Usage: monolith --profile headless')
      ...
      expect(sdkHelp.stdout).toContain('Usage: monolith --profile sdk')
      ...
      expect(acpHelp.stdout).toContain('Usage: monolith --profile acp')
  ...
  it('reports SDK startup failure when stdin reaches EOF first', async () => {
    const home = mkdtempSync(join(tmpdir(), 'monolith-built-sdk-startup-failure-'))
    const patch = join(home, 'broken-sdk.cordis.yml')
    writeFileSync(patch, [
      '- insert:',
      '    - id: missing-sdk-startup-plugin',
      '      name: "@monolith/missing-sdk-startup-plugin"',
      '',
    ].join('\n'))
    try {
      ...
      expect(result.stderr).toContain('@monolith/missing-sdk-startup-plugin')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  ...
  it('serves the SDK protocol through the sdk profile and exits after shutdown', async () => {
    const home = mkdtempSync(join(tmpdir(), 'monolith-built-sdk-'))
    const child = execa(process.execPath, [monolithBin, '--profile', 'sdk'], {
      cwd: home,
      reject: false,
      timeout: SPAWN_TIMEOUT_MS,
      killSignal: 'SIGKILL',
    ...
    try {
      ...
      expect(await response(1)).toMatchObject({
        jsonrpc: '2.0',
        id: 1,
        result: { serverInfo: { name: 'monolith-sdk-runtime' } },
      })
  ...
  it('runs a mock-backed ACP turn through the acp profile and exits on disconnect', async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), 'monolith-built-acp-'))
    const child = execa(process.execPath, [monolithBin, '--profile', 'acp'], {
      cwd: home,
      reject: false,
      timeout: SPAWN_TIMEOUT_MS,
      killSignal: 'SIGKILL',
      env: {
        ...
        MONOLITH_PERMISSION_MODE: 'danger-full-access',
      },
      extendEnv: false,
    })
    const rawOut: string[] = []
    ...
    const clientApp = createAcpClientApp({ name: 'monolith-built-acp-profile' })
    ...
    try {
      ...
      expect(initialized.agentInfo).toMatchObject({ name: 'monolith-acp' })
  ...
  it('runs the headless profile through its app-owned task positional', async () => {
    const apiKey = 'built-monolith-headless-key'
    const server = await startMockLlmServer({
    ...
    const home = mkdtempSync(join(tmpdir(), 'monolith-built-headless-'))
    try {
      ...
      expect(result.stderr).toBe('monolith: reasoning:\nInspecting the published entry.')
  ...
  it('does not load a project environment for --version', async () => {
    const project = mkdtempSync(join(tmpdir(), 'monolith-version-project-'))
  ...
  it('fails loud on a nonexistent profile with the plugin-command hint', async () => {
    const home = mkdtempSync(join(tmpdir(), 'monolith-missing-profile-'))
    try {
      const result = await runBuiltBin(['--profile', 'nope'], { MONOLITH_HOME: home })
      expect(result.code).toBe(1)
      expect(result.stderr).toContain('profile "nope" does not exist')
      expect(result.stderr).toContain('monolith plugin --profile nope add')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, SPAWN_TIMEOUT_MS + 30_000)
  ...
  it('uses the launching endpoint and managed credential through the published entry', async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), 'monolith-home-environment-'))
    const project = mkdtempSync(join(tmpdir(), 'monolith-home-project-'))
  ...
  it('reports a patch-overlay boot failure without hanging', async () => {
    ...
    // refresh drain: monolith exited 13 with no diagnostic instead of settling
    // ([Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-03-hmr-initial-scan-boot-deadlock.md)).
    const home = mkdtempSync(join(tmpdir(), 'monolith-invalid-patch-'))
    try {
  ...
  it('fully settles a custom profile, hot-reloads its patch layer with removal reverting, and disposes on a signal', async () => {
    ...
    try {
      ...
      // The home-level user layer ($MONOLITH_HOME/cordis.patch.yml) is live too
  ...
  it('anchors a relative add spec to the invoking directory, not the profile', async () => {
    // `monolith plugin --profile x add .` from a plugin checkout must install THAT
    // checkout — pnpm's cwd is the profile directory, so an un-anchored `.`
    // would self-link the profile.
    const home = mkdtempSync(join(tmpdir(), 'monolith-plugin-anchor-'))
    const checkout = mkdtempSync(join(tmpdir(), 'monolith-plugin-checkout-'))
    try {
      ...
      const result = await execa(process.execPath, [monolithBin, 'plugin', '--profile', 'anchor', 'add', '.'], {
        cwd: checkout,
        input: '',
        timeout: SPAWN_TIMEOUT_MS,
        killSignal: 'SIGKILL',
        reject: false,
        env: { MONOLITH_HOME: home },
      })
      expect(result.exitCode).toBe(0)
      const manifest = JSON.parse(readFileSync(join(home, 'profiles', 'anchor', 'package.json'), 'utf8')) as {
        dependencies: Record<string, string>
        monolith: { profile: { bundles: string[] } }
      }
      expect(Object.keys(manifest.dependencies)).toEqual(['anchored-bundle'])
      expect(manifest.monolith.profile.bundles).toContain('anchored-bundle')
      ...
      expect(afterRemove.monolith.profile.bundles).not.toContain('anchored-bundle')
    } finally {
      rmSync(home, { recursive: true, force: true })
  ...
  it('activates a dependency that gained monolith.bundle in a later update', async () => {
    ...
    // version declares monolith.bundle. Simulated without a registry: hand-place
    // the installed package, flip its manifest, and run a benign pnpm verb.
    const home = mkdtempSync(join(tmpdir(), 'monolith-plugin-update-'))
    try {
      const profileDir = join(home, 'profiles', 'up')
      const installed = join(profileDir, 'node_modules', 'late-bundle')
      mkdirSync(installed, { recursive: true })
      writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
        name: 'monolith-profile-up',
        private: true,
        dependencies: { 'late-bundle': 'file:./late-bundle' },
        monolith: { profile: { bundles: ['@monolith/base'] } },
      }))
      writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
      // v1: no monolith manifest — a plain dependency.
      writeFileSync(join(installed, 'package.json'), JSON.stringify({ name: 'late-bundle', version: '1.0.0' }))
      const first = await runBuiltBin(['plugin', '--profile', 'up', 'root'], { MONOLITH_HOME: home })
      expect(first.code).toBe(0)
      let manifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8')) as { monolith: { profile: { bundles: string[] } } }
      expect(manifest.monolith.profile.bundles).toEqual(['@monolith/base'])
      // v2: the installed package now declares monolith.bundle (an update landed).
      writeFileSync(join(installed, 'package.json'), JSON.stringify({
        name: 'late-bundle', version: '2.0.0', monolith: { bundle: { patch: './cordis.patch.yml' } },
      }))
      writeFileSync(join(installed, 'cordis.patch.yml'), '[]\n')
      const second = await runBuiltBin(['plugin', '--profile', 'up', 'root'], { MONOLITH_HOME: home })
      expect(second.code).toBe(0)
      manifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8')) as { monolith: { profile: { bundles: string[] } } }
      expect(manifest.monolith.profile.bundles).toEqual(['@monolith/base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, SPAWN_TIMEOUT_MS * 2 + 30_000)
  ...
  describe('config dump', () => {
    let home: string
    beforeEach(() => { home = mkdtempSync(join(tmpdir(), 'monolith-dump-bin-')) })
    afterEach(() => { rmSync(home, { recursive: true, force: true }) })

    it('prints the web profile bundle layers without a user layer', async () => {
      const { stdout, code, stderr } = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { MONOLITH_HOME: home })
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@monolith/agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @monolith/base')
      expect(stdout).toContain("name: '@monolith/host-webserver'")
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@monolith/headless'")
      expect(stdout).not.toMatch(/name: '@monolith\/host-/)
      expect(stdout).not.toContain("name: '@monolith/web-app'")
      expect(stdout).not.toMatch(/name: '@monolith\/client-/)
    }, SPAWN_TIMEOUT_MS + 30_000)

    it('prints the exact standalone sdk-minimal tree without monolith-base', async () => {
      const { stdout, code, stderr } = await runBuiltBin(
      ...
      expect(stdout).toContain('# == @monolith/sdk-minimal')
      expect(stdout).not.toContain('@monolith/base')
      expect(stdout).not.toContain('@monolith/web-app')
    }, SPAWN_TIMEOUT_MS * 2 + 30_000)

    it('composes the profile user layer and a --patch overlay in order', async () => {
      // Auto-init the web profile first, then write its user layer.
      const init = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { MONOLITH_HOME: home })
