...
export default async function open(url) {
  ...
  console.log(`monolith browser-open: ${JSON.stringify({
    url,
    status: response.status,
    bootManifest: html.includes('__MONOLITH_BOOT__'),
    apiKeyPresent: process.env.DEEPSEEK_API_KEY !== undefined,
    monolithHomePresent: process.env.MONOLITH_HOME !== undefined,
  })}`)
  ...
  const launcher = spawn(process.execPath, [
    '--eval', handoffProbe,
    '--', join(process.cwd(), `.monolith-browser-open-${process.ppid}`), String(process.pid),
  ], { stdio: 'ignore' })
  launcher.unref()
  return launcher
}
