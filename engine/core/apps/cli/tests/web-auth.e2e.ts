/** Real `monolith web` authentication against a temporary MONOLITH home. */
...
const MONOLITH_SOURCE_BIN = join(REPO_ROOT, 'apps/cli/src/bin.ts')
...
function cleanEnvironment(root: string, monolithHome: string): NodeJS.ProcessEnv {
...
async function startWeb(root: string, monolithHome: string, port: number): Promise<RunningWeb> {
  const child = spawn(process.execPath, [
    ...
    cwd: root,
    env: cleanEnvironment(root, monolithHome),
  ...
  const launchUrl = await new Promise<string>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      fail(new Error(`monolith web did not become ready:\n${redact(output)}`))
    }, 90_000)
    ...
    child.once('exit', (code) => {
      fail(new Error(`monolith web exited before readiness (${String(code)}):\n${redact(output)}`))
    })
  })
...
describe('monolith web authentication through the real CLI', () => {
  it('rejects a forged loopback Host and preserves the browser cookie across restart', { timeout: 180_000 }, async () => {
    const root = await mkdtemp(join(tmpdir(), 'monolith-web-auth-real-cli-'))
    const monolithHome = join(root, '.monolith')
    const port = await freePort()
    let first: RunningWeb | undefined
    let second: RunningWeb | undefined
    try {
      first = await startWeb(root, monolithHome, port)
      ...
      await stopWeb(first)
      first = undefined
      second = await startWeb(root, monolithHome, port)
      ...
      const credentialMode = (await stat(join(monolithHome, '.credentials.yaml'))).mode & 0o777
