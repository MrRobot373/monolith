...
import { parseMonolithArgs } from '../src/args.ts'

const parse = (argv: string[]) => parseMonolithArgs(argv, '1.2.3')
...
describe('parseMonolithArgs', () => {
  ...
  it('routes the plugin pnpm forwarder', () => {
    ...
    expect(parse(['plugin', '--profile', 'tui', 'why', '@monolith/cordis']))
      .toEqual({ mode: 'plugin', profile: 'tui', args: ['why', '@monolith/cordis'] })
