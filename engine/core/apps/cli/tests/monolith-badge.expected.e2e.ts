...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, runLoaderSmoke } from '@monolith/loader-smoke'
const binScript = fileURLToPath(new URL('./fixtures/monolith-badge/snapshot.ts', import.meta.url))
const configPath = fileURLToPath(new URL('./fixtures/monolith-badge/cordis.yml', import.meta.url))
const defaultConfigPath = fileURLToPath(new URL('./fixtures/monolith-badge/default.cordis.yml', import.meta.url))
const tsconfigPath = fileURLToPath(new URL('../../../tsconfig.json', import.meta.url))
const badgeAssetsPath = fileURLToPath(new URL('../../../packages/skill/skill-badge/assets/', import.meta.url))

describe('monolith badge assembled snapshot', () => {
  it('advertises and loads the opt-in bundled skill through the shipped app', async () => {
    const disabled = await runLoaderSmoke({
      label: 'disabled monolith badge skill snapshot',
      tempDirPrefix: 'headless-snapshot-monolith-badge-disabled-',
      binScript,
    ...
    const enabled = await runLoaderSmoke({
      label: 'monolith badge skill snapshot',
      tempDirPrefix: 'headless-snapshot-monolith-badge-',
      binScript,
      libBinScript: binScript,
      configPath,
      tsconfigPath,
    ...
    expect(disabledSnapshot).toMatchInlineSnapshot(`
      {
        ...
        "result": {
          "content": [
            {
              "text": "Error: skill "monolith-badge" is unknown or no longer available",
              "type": "text",
            },
          ],
          "error": {
            "message": "skill "monolith-badge" is unknown or no longer available",
          },
          "isError": true,
        },
        "summary": null,
    ...
    expect(enabledSnapshot).toMatchInlineSnapshot(`
      ...
      A user may also invoke a skill directly; its <skill_content> block then appears in this conversation. Follow it, and do not call the \`skill\` tool again for that skill.
        ...
        "result": {
          "content": [
            {
              "text": "<skill_content name="monolith-badge">
      <skill_resources>
      ...
      Add the official “powered by monolith” badge without recreating or restyling it.

      ## Assets

      - Shields.io image URL: \`https://img.shields.io/badge/powered_by-monolith-4D6BFE?style=flat-square\`
      - Project URL: \`https://github.com/MrRobot373/monolith\`
      ...
      \`\`\`markdown
      [![](https://img.shields.io/badge/powered_by-monolith-4D6BFE?style=flat-square)](https://github.com/MrRobot373/monolith)
      \`\`\`

      If attribution should not be linked, use:

      \`\`\`markdown
      ![](https://img.shields.io/badge/powered_by-monolith-4D6BFE?style=flat-square)
      \`\`\`
