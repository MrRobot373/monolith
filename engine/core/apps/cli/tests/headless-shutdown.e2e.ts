...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, resolveExampleLaunch } from '@monolith/loader-smoke'

const monolithBinScript = fileURLToPath(new URL('../src/bin.ts', import.meta.url))
...
markers = [b"monolith-test: never-dispose ready", b"monolith-test: never-dispose started"]
output = bytearray()
marker_index = 0
...
async function runHeadlessPtySmoke(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'monolith-headless-shutdown-'))
  try {
    const home = join(cwd, '.monolith')
    ...
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      name: 'monolith-profile-headless',
      private: true,
      dependencies: {},
      monolith: { profile: { bundles: ['@monolith/base', '@monolith/headless'] } },
    }, undefined, 2))
    ...
    const launch = resolveExampleLaunch({
      ...
      env: {
        ...
        MONOLITH_TEST_SHUTDOWN_ARM_FILE: join(cwd, 'shutdown-armed'),
      },
    })
    ...
    if (result.timedOut) {
      throw new Error(`monolith headless PTY driver did not exit. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    if (result.failed) {
      throw new Error(`monolith headless PTY driver exited ${String(result.exitCode)}. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
...
describe.skipIf(process.platform === 'win32')('headless process shutdown (real Loader tree in a PTY)', () => {
  it('lets a second Ctrl+C force exit while the first signal is draining', async () => {
    ...
    expect(output).not.toContain('monolith: observing at ')
    expect(output).toContain('monolith-test: never-dispose ready')
    expect(output).toContain('monolith-test: never-dispose started')
