 ...
 * the REAL shipped bundle layers (monolith-base + monolith-web-app resolved from the
...
import { SHIPPED_PRESET_ROOT } from '@monolith/agent-presets'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@monolith/app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  // resolve from the REAL monolith-base/monolith-web-app packages through it, so this
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    home = mkdtempSync(join(tmpdir(), 'monolith-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'web'), ['@monolith/base', '@monolith/web-app'])
    const profile = loadProfile('monolith', 'web', anchor, home)
    ...
    for (const name of ['@monolith/pwsh-sandbox', '@monolith/tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'monolith-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@monolith/base'])
    const profile = loadProfile('monolith', 'base-only', anchor, home)
    const warnings: string[] = []
    const rows = composeEntries(
      profile.layers.map(layer => layer.patches),
      message => warnings.push(message),
