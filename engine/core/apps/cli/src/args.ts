/**
 * Commander adapter for the `monolith` command line.
 *
 ...
 * `@monolith/cmdline`). Launcher flags therefore come first: the first
 * token this parser does not recognize starts the inner arguments, so
 * `monolith --profile tui --resume abc` boots the tui profile with `--resume abc`,
 * and `monolith --profile web -h` prints the web app's help, not this one's.
 *
 * `web` is a hardcoded alias for `--profile web`; `plugin` manages a profile's
 * plugin dependencies by forwarding to pnpm.
 * @module @monolith/cli/args
 */
...
/** The resolved `monolith` invocation. Help, version, and errors exit inside {@link parseMonolithArgs}. */
export type MonolithInvocation = ProfileInvocation | DumpConfigInvocation | PluginInvocation
...
const HELP_EXAMPLES = `
Examples:
  monolith --profile web                          boot the web profile (same as: monolith web)
  monolith --profile headless "run the tests"     answer one task, print the result, and exit
  monolith --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  monolith --profile tui --resume <session>       arguments after the launcher flags reach the app
  monolith --profile web --help                   the web app's own flags and help
  monolith plugin --profile tui add <package>     install a plugin into the tui profile
`
 ...
 */
function resolveBoot(program: Command, profile: string, options: BootOptions, args: string[]): MonolithInvocation {
 ...
 * @returns the resolved invocation.
 */
export function parseMonolithArgs(argv: readonly string[], version: string): MonolithInvocation {
  let resolved: MonolithInvocation | undefined
  ...
  program
    ...
    .description('monolith: boot a MONOLITH profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    .addHelpText('after', HELP_EXAMPLES)
    .exitOverride()
    // The launcher's flags come first and end at the first token it does not
    // know; everything from there on belongs to the booted app, including
    // its -h. `monolith -h` with no profile still prints this help, below.
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: monolith --profile <name> --help)')
    .option('--profile <name>', 'the profile under $MONOLITH_HOME/profiles to boot')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
    .option('--dump-config', 'print the composed profile tree and exit')
    .option('--dump-default-config', 'print the profile tree without its user layer or --patch overlays and exit')
    .action((args: string[], options: BootOptions & { profile?: string }) => {
      // With the app owning -h, the launcher's own help is what a bare
      // `monolith -h` (no profile to hand it to) must print.
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: monolith web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
  ...
  if (resolved === undefined) throw new Error('monolith: no invocation resolved')
  return resolved
}
