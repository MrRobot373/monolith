/**
 * `monolith plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `monolith.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `monolith.bundle` joins the layer stack; a
 ...
 * gained its `monolith.bundle` declaration in a newer version.
 * @module @monolith/cli/plugin
 */
 ...
 * @returns true when the package manifest declares `monolith.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.monolith?.bundle?.patch !== undefined
}

/**
 * Reconcile `monolith.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `monolith.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.monolith?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no monolith.bundle — installed as a plain dependency, not a profile layer `
  ...
  for (const packageName of [...plugins]) {
    ...
    // bundles (monolith-base and friends) are not dependencies.
  ...
  if (!changed) return
  after.monolith = { ...after.monolith, profile: { ...after.monolith?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `monolith` was invoked from.
...
/**
 * Run one `monolith plugin` invocation: init if needed, forward to pnpm, reconcile.
