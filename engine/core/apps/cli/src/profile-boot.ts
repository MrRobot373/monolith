/**
 * Shared profile boot for every `monolith` surface: resolve the profile, stack its
 * patch layers (bundle layers in `monolith.profile.bundles` order, the profile's
 ...
 * @module @monolith/cli/profile-boot
 */

import { writeFileSync } from 'node:fs'
import { join, resolve } from 'node:path'
import { fileURLToPath } from 'node:url'
import { FiberState, type Context } from '@monolith/cordis'
...
import { installProxyFromEnvironment } from '@monolith/http-proxy'
import { MONOLITH_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from '@monolith/launch-environment'
import { provideCmdline, type AppReady } from '@monolith/cmdline'
...
/**
 * The home-level user patch layer (`$MONOLITH_HOME/cordis.patch.yml`), applied
 * over every profile's own layer. Resolved per call, not at module load:
 * `$MONOLITH_HOME` may be set by the test or launcher after import.
 * @returns the absolute patch-file path.
 */
export function homePatchPath(): string {
  return join(resolveMonolithHome(), PROFILE_PATCH_FILENAME)
}

/** Absolute path of this monolith installation's package.json (both anchors: src/ and lib/ sit one level under apps/cli). */
export const INSTALL_ANCHOR = fileURLToPath(new URL('../package.json', import.meta.url))

/** The session-telemetry row id the MONOLITH_TELEMETRY_DISABLED switch targets. */
const TELEMETRY_ROW_ID = 'session-telemetry-otel'

/** The empty root entry list every profile tree patches over. */
const PROFILE_ROOT_CONFIG = `# monolith profile root — an empty entry list. The tree is composed as patches:
# each bundle in package.json's monolith.profile.bundles, then cordis.patch.yml, then any
 ...
 * switch set.
 * @param disabledEnv - the raw `MONOLITH_TELEMETRY_DISABLED` value (`undefined` when unset).
...
interface ComposedProfile {
  ...
  /** The home-level user layer (`$MONOLITH_HOME/cordis.patch.yml`), applied after the profile's own. */
 ...
 * `monolith.profile.bundles` order (a base-backed profile gets the base bundle's
 * platform-gated shell rows), the profile's user layer, the home-level user
 * layer (`$MONOLITH_HOME/cordis.patch.yml` — machine-local preferences that apply
...
async function composeProfile(
  ...
  const telemetryPatch = resolveTelemetryPatch(process.env.MONOLITH_TELEMETRY_DISABLED, rows.has(TELEMETRY_ROW_ID))
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  const ctx = await boot(NAME, rootConfig, structuredClone(allPatches(composed)), (hostCtx) => {
    ...
    hostCtx.provide(MONOLITH_LAUNCH_ENVIRONMENT_KEY, options.environment)
  ...
  if (composed.profile.patchReload === 'live'
    ...
    try {
      // Config-only HMR for the live profile patch layer: monolith-base disables
      ...
      if (ctx.get('hmr') === undefined) {
        if (ctx.get('timer') === undefined) {
          await ctx.loader.create({ name: '@monolith/cordis-plugin-timer' })
        }
        await ctx.loader.create({ name: '@monolith/cordis-plugin-hmr', config: { root: [] } })
      }
