...
Everything MONOLITH changes about DeepSeek Harness lives here. `engine/dsh` is otherwise
vendored upstream and unmodified, so a version bump is a merge of that directory alone.

| Package | Role |
|---|---|
| [`bundle/`](bundle/) | `@monolith/dsh-bundle` — the patch layer over `dsh-base` + `dsh-web-app`, and the dependency anchor that makes MONOLITH plugins resolvable to a profile |
| [`skills/`](skills/) | MONOLITH's skill library — 22 bundles across documents, data and engineering; see below |
| [`ui-brand/`](ui-brand/) | `@monolith/dsh-client-ui-brand` — MONOLITH occupants for the sidebar brand and conversation-hero slots |

## Why a bundle rather than `--patch`

A `--patch` overlay can *name* a plugin package, but it cannot make one resolvable. The Loader
imports plugin packages from the profile directory (`$DSH_HOME/profiles/<name>/`), and
`healProfileModuleFallback` links only the packages inside a selected **bundle's dependency
closure** into that directory. A `--patch` row naming `@monolith/dsh-client-ui-brand` therefore
fails with `ERR_MODULE_NOT_FOUND`. A deployment plugin has to ship as a dependency of a bundle
package — the same shape `dsh-web-app` uses for the `ui-*` rows it rosters.
...
1. **The profile** names the bundle layers in order. MONOLITH's layer comes last so its rows
   ...
   ```json
   { "dsh": { "profile": { "bundles": [
     "@deepseek-ai/dsh-base", "@deepseek-ai/dsh-web-app", "@monolith/dsh-bundle"
   ] } } }
   ```
...
2. **The installation** must be able to resolve that bundle. `resolveBundleDir` tries the
   ...
   `@monolith/dsh-bundle` is a dependency of `apps/cli` exactly as `@deepseek-ai/dsh-web-app` is.
   That one line in `apps/cli/package.json` is the only edit MONOLITH makes to a vendored file.

Then:

```
dsh --profile monolith --no-open --port 3080
```

## Verified behavior

Booted against this overlay, the served browser roster contains `@monolith/dsh-client-ui-brand`,
no longer contains `@deepseek-ai/dsh-client-ui-brand-official`, and carries
`@deepseek-ai/dsh-client-ui-schedule`. `--dump-config` shows the LiteLLM route on `llm-pi-ai`,
...
```
node packages/monolith/skills/install.mjs "$DSH_HOME/skills"
```

`$DSH_HOME/skills` is already a default skill root, so no `customSkillDirs`
...
name where that executable *is*. dsh does give the model a `Base directory for
...
**Documents** — dsh ships no document production at all, so this is the whole
