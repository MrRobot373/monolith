...
import { normalizeWorkspaceFolderPath } from "@/react-app/domains/workspace/workspace-folder-path";
...
import { seedLocalWorkspaceOpencodeConfig } from "./monolith-opencode-seed";
...
export function SessionRoute() {
  ...
  // MONOLITH: the hub launcher deep-links a preset domain agent via ?agent=<name>.
  useEffect(() => {
    if (typeof window === "undefined") return;
    const p = new URLSearchParams(window.location.search).get("agent");
    if (p && p.trim()) setSelectedAgent(p.trim());
  ...
  const handleCreateWorkspace = useCallback(async (preset: WorkspacePreset, folder: string | null) => {
    const folderPath = normalizeWorkspaceFolderPath(folder);
    if (!folderPath) return;
    setCreateWorkspaceBusy(true);
    setCreateWorkspaceError(null);
    try {
      const workspaceName = folderNameFromPath(folderPath);
      ...
      const workspacePath = targetWorkspace?.path?.trim() || folderPath;
      await seedLocalWorkspaceOpencodeConfig(workspacePath);
      if (createdId) {
  ...
  const createWorkspaceControlAction = useMemo<OpenworkControlAction>(() => ({
    ...
    execute: async (args) => {
      const folder = normalizeWorkspaceFolderPath((args as { path?: string } | undefined)?.path);
