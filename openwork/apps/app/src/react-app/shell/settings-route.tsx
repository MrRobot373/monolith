...
function SettingsRouteContent(props: SettingsSurfaceProps = {}) {
  ...
  const [customerMode] = useState(() => {
    ...
    // Customer mode is ON by default. Set localStorage "openwork.customerMode" to "0" to disable.
    return window.localStorage.getItem("openwork.customerMode") !== "0";
  });
  ...
  const handleCreateWorkspace = async (preset: WorkspacePreset, folder: string | null) => {
    ...
    try {
      ...
      await seedLocalWorkspaceOpencodeConfig(targetWorkspace?.path?.trim() || folderPath);
      if (createdId) {
