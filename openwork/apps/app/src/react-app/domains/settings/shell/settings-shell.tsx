        ...
        <header className="flex h-11 shrink-0 items-center justify-between gap-2 border-b border-dls-border px-3 mac:titlebar-drag">
          <div className="flex min-w-0 items-center gap-2 mac:titlebar-no-drag">
            <SettingsSectionMenu
              ...
              customerMode={props.customerMode}
            />
            <WorkspaceMenu
...
function SettingsSectionMenu(props: Pick<SettingsPageFrameProps, "activeTab" | "developerMode" | "onSelectTab"> & { customerMode?: boolean }) {
  ...
  const sections: Array<{ label: string | null; tabs: SettingsTab[] }> = [
    ...
    { label: t("settings.group_workspace"), tabs: getWorkspaceSettingsTabs(props.customerMode) },
    { label: t("settings.group_global"), tabs: getGlobalSettingsTabs(props.developerMode, props.customerMode) },
    { label: t("settings.group_cloud"), tabs: getCloudSettingsTabs(memoryEnabled, props.customerMode) },
  ];
