...
export function CreateWorkspaceLocalPanel(
  ...
  const selectedFolder = normalizeWorkspaceFolderPath(props.selectedFolder);
        ...
        <div className="space-y-4">
          <div className={surfaceCardClass}>
            ...
            <div className="mt-4 space-y-2">
              <label className="text-[12px] font-medium text-dls-secondary" htmlFor="workspace-folder-path">
                Folder path
              </label>
              <Input
                id="workspace-folder-path"
                value={props.selectedFolder ?? ""}
                onChange={(event) => props.onFolderInput(event.currentTarget.value)}
                disabled={props.submitting}
                spellCheck={false}
                placeholder="C:\\Users\\Yash\\Downloads\\MyProject"
                className="font-mono text-[12px]"
              />
              <div className="text-[11px] leading-relaxed text-dls-secondary">
                Paste a Windows folder path, or use the folder picker below.
              </div>
            </div>
            <div className="mt-4">
              <button
        ...
        <div className="flex justify-end gap-3">
          ...
          {props.onConfirmWorker ? (
            <Button
              ...
              variant="secondary"
              onClick={() =>
                props.onConfirmWorker?.(props.preset, selectedFolder || null)
              }
              disabled={
                !selectedFolder ||
                props.submitting ||
          ...
          <Button
            ...
            disabled={!selectedFolder || props.submitting}
            title={
              !selectedFolder
                ? t("dashboard.choose_folder_continue")
                : undefined
