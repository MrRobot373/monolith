...
import { normalizeWorkspaceFolderPath } from "./workspace-folder-path";
import type {
...
async function pickFolderThroughNativeWebBridge(): Promise<{ path: string | null; cancelled: boolean }> {
  ...
  try {
    ...
    const payload = (await response.json()) as { path?: unknown; cancelled?: unknown };
    const path = normalizeWorkspaceFolderPath(typeof payload.path === "string" ? payload.path : "");
    return { path: path || null, cancelled: payload.cancelled === true };
  } catch {
    return { path: null, cancelled: false };
  }
}

function promptForFolderPath(): string | null {
  if (typeof window === "undefined") return null;
  const value = window.prompt("Paste the full folder path for the workspace:", "C:\\Users\\Yash\\Downloads\\MyProject");
  const trimmed = normalizeWorkspaceFolderPath(value);
  return trimmed || null;
}
...
export function CreateWorkspaceModal(props: CreateWorkspaceModalProps) {
  ...
  const normalizedSelectedFolder = normalizeWorkspaceFolderPath(selectedFolder);
  const hasSelectedFolder = Boolean(normalizedSelectedFolder);
  ...
  const handlePickFolder = async () => {
    ...
    try {
      ...
      try {
        ...
        const normalized = normalizeWorkspaceFolderPath(next);
        if (normalized) {
          setSelectedFolder(normalized);
          return;
        }
      } catch (error) {
        console.warn("[create-workspace] desktop folder picker unavailable", error);
      }

      const bridged = await pickFolderThroughNativeWebBridge();
      if (bridged.path) {
        setSelectedFolder(bridged.path);
        return;
      }
      if (bridged.cancelled) return;

      const typed = promptForFolderPath();
      if (typed) setSelectedFolder(typed);
    } finally {
  ...
  const handleLocalSubmit = async () => {
    props.onConfirm(preset, normalizedSelectedFolder || null);
  };
