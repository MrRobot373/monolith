...
Run the helper script via the Bash tool:

### 1. Get structural metadata and sheet/outline info:
```bash
python3 .opencode/skills/read-office-file/read_office.py info "<FILE_PATH>"
```

### 2. Read contents of the file:
* **For Word documents (.docx):**
  ```bash
  python3 .opencode/skills/read-office-file/read_office.py read "<FILE_PATH>"
  ```
...
* **For Excel sheets (.xlsx) - read a specific sheet by name or index, limiting row count:**
  ```bash
  python3 .opencode/skills/read-office-file/read_office.py read "<FILE_PATH>" "<SHEET_NAME_OR_INDEX>" [MAX_ROWS]
  ```
