...
def handle_pdf_create(file_path, json_data):
    # json_data can contain title and list of content objects (Heading, Paragraph, Table)
    ...
    for el in elements_data:
        ...
        if t == "heading":
            ...
            # Pack headers into rows list for ReportLab Table
...
def handle_pptx_create(file_path, json_data):
    ...
    for s_data in json_data:
        # Use bullet/text slide layout (layout index 1 is Title + Content)
...
def main():
    ...
    # Convert relative to absolute for robustness
    file_path = os.path.abspath(file_path)

    # For create/edit commands, read JSON data from argument or file
    json_data = None
